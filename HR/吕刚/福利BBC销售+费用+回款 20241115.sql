select sum(sale_amt) from csx_analyse_tmp.csx_analyse_tmp_flbbs_sale_detail where customer_code='203926' and month='202409'
        ...
        (
          ...
          where
           ...
           -- and source_bill_no='R2408010410590817A'
            ...
            and regexp_replace(date(happen_date), '-', '')>='20240801'
        ) a
        left join 
        (select order_code ,
            business_type_code,
...
-- select month,sum(sale_amt) from  csx_analyse_tmp.csx_analyse_tmp_bbc_expense_detail  where customer_code='203926' group by month
  
-- 1、优先处理BBC运费
drop table csx_analyse_tmp.csx_analyse_tmp_bbc_expense_detail 
...
select group_flag,
    ...
    -- where merchant_order_number='2408110610642821'
    -- where a.customer_code='203926'
) a 
group by group_flag,
    a.type_flag,
    a.operation_mode_name,
...
(select
    ...
    from tmp_flbbc_sale a 
      left join 
       csx_analyse_tmp.csx_analyse_tmp_bbc_expense_detail  b on a.order_code=b.order_code and a.operation_mode_name=b.operation_mode_name and a.type_flag=b.type_flag and a.sdt=b.sdt
       left join 
       csx_analyse_tmp.csx_analyse_tmp_tms_entrucking_order c on a.order_code=c.order_code and a.operation_mode_name=c.operation_mode_name and a.type_flag=c.type_flag and a.sdt=b.sdt
    ...
    -- select * from tmp_sale_expense   where customer_code='203926'
    ,
   tmp_sss_detail as 
    (     
        select
    ...
    from csx_analyse_tmp.csx_tmp_sss_customer_receivable_amount a 
    join (
          select
            month_of_year,
...
from csx_analyse_tmp.csx_analyse_tmp_close_bill_amt_order
group by 
	 month,
	 group_flag,
	 type_flag,
	 ...
	 business_type_name,
	 customer_code	
) f on a.customer_code=f.customer_code and a.month=f.month and a.operation_mode_name=f.operation_mode_name 
	and a.business_type_code=f.business_type_code and a.group_flag=f.group_flag and a.type_flag=f.type_flag	 
    where a.customer_code='203926'
;
...
create table csx_tmp.csx_tmp_sss_customer_receivable_amount as with tmp_sss_customer_receivable_amount as (
  -- 计算应收金额和剩余金额
  select
    sdt,
    customer_code,
...
create table csx_analyse_tmp.csx_analyse_tmp_close_bill_amt_order as 
  ...
  from 
	...
	(
      ...
      where sdt = '20241031' -- 现在算截止到10月底的回款 下月算截止到11月底的回款
        and regexp_replace(date(happen_date), '-', '') between '20240801' and '20241031'
       -- and source_bill_no='R2408010410590817A'
        -- and customer_code='130536'
		group by 
         case when substr(source_bill_no,1,1) ='B' then substr(source_bill_no, 2,length(source_bill_no)-2)  -- 涉及 BBC单号有字母开头的处理
