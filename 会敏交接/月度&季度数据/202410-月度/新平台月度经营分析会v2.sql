...
SELECT 
	...
	,ab.xinqkh
	,count(distinct case when a.channel_code in (1,9) and substr(aa.first_sale_date,1,6)=a.smonth then a.customer_code end) xin_customer_cnt  
	...
	,avg(case when a.channel_code in (1,9) and substr(aa.first_sale_date,1,6)<a.smonth then a.sale_amt end) old_sales_cust_cnt
...
left join 
	...
	from 
		(select * from csx_dws.csx_dws_crm_customer_active_di 
		 where sdt='current' 
			and sign_date>='20250101' 
			and sign_date<='20250331'
		)a 
	left join 
...
left join 
	...
	left join 
		(select * from csx_dim.csx_dim_crm_customer_info where sdt='current')b on a.customer_code=b.customer_code
    where a.sdt='current' 
		and a.sign_date>='20250101' 
		and a.sign_date<='20250331'
	group by 
		b.performance_city_code,
		substr(a.sign_date,1,6)
...
from
	...
	where 
		sdt between '20240101' and '20250228'
