...
left join (select b.performance_province_code,
			...
			from csx_dws.csx_dws_crm_customer_active_di a 
			left join 
			(select customer_code,performance_province_code from csx_dim.csx_dim_crm_customer_info where sdt='current')b on a.customer_code=b.customer_code
              where sdt='current' and sign_date>='20250101' and sign_date<'20250401'
...
left join (select customer_code,if(first_sale_date>='20250101' and  first_sale_date<'20250401','202501',substr(first_sale_date,1,6)) first_sale_date
		  from csx_dws.csx_dws_crm_customer_active_di 
		  where sdt='current' and  first_sale_date<'20250401')aa ON a.customer_code = aa.customer_code
