...
from 
	...
	where 
		sdt>='20231001' and sdt<=date_format(date_sub(current_date,1),'yyyyMMdd')
...
--临时加单、紧急补货 客户+商品
drop table if exists csx_analyse_tmp.csx_analyse_tmp_zhisong_beijing_02;
create table csx_analyse_tmp.csx_analyse_tmp_zhisong_beijing_02
as
select
	*
from
	...
	from 
		...
		from
			csx_dws.csx_dws_sale_detail_di 
		where 
			sdt>='20231028' and sdt<=date_format(date_sub(current_date,1),'yyyyMMdd')
			and channel_code in('1','7','9')
			and business_type_code in (1) 
			and performance_province_name in ('北京市')
			and delivery_type_code=2
...
;
select * from csx_analyse_tmp.csx_analyse_tmp_zhisong_beijing_02
;
...
from
	...
	from 
		...
		from
			csx_dws.csx_dws_sale_detail_di 
		where 
			sdt>='20230901' and sdt<=date_format(date_sub(current_date,1),'yyyyMMdd')
			and channel_code in('1','7','9')
			and business_type_code in (1) 
			and performance_province_name in ('北京市')
			and delivery_type_code=2
...
-- @程晓林 @叶晓亮 @颜进祥 @蒋心炜 各位好，以下是北京各品类下直送TOP客户解决跟进表，请查收
