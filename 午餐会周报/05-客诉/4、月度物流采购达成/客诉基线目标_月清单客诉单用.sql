...
-- 客诉剔除清单表-导入 csx_analyse_tmp.complaint_code_delete_list;

-- drop table csx_analyse_tmp.complaint_code_delete_list;
...
from
	...
	where 
		...
		and performance_province_name not in ( '平台-B'	)  
...
(select 
	...
	/*case when e.week_of_year='202416' and a.customer_code in ('120327','129941') then '1'
		 when e.week_of_year='202417' and a.customer_code in ('121276','234161') then '1'
		 when e.week_of_year='202420' and a.customer_code in ('128454','246442') then '1'
	else '2' end as delete_type,
	*/
	case 
		when coalesce(c.direct_delivery_type,j.direct_delivery_type) is null then '其他'
		when coalesce(c.direct_delivery_type,j.direct_delivery_type) ='' then '其他'
...
from 
	...
	select 
		...
		customer_code,sale_order_code,goods_code,goods_name,
		-- if(first_level_department_name='采购',if(classify_middle_code<>'',classify_middle_code,second_level_department_name),'') as classify_middle_code,
		if(classify_middle_code<>'',classify_middle_code,second_level_department_name) as classify_middle_code,
		if(classify_middle_code<>'',classify_middle_name,second_level_department_name) as classify_middle_name,
		complaint_code,
	...
	where 
		...
		-- or (complaint_source=2 and complaint_amt !=0 and main_category_name <> '送货后调整数量')) 
		-- and customer_name not like '%XM%' 
		-- 增加版本：只含客诉等级1-2级
		-- and complaint_level not in(3,-1)
		and shipper_code='YHCSX'
...
-- 退货类型
select 
	substr(sdt,1, 6) smonth,    --	退货申请日期 客诉发生日期
	week_of_year,
	...
	first_level_department_name,  -- 一级责任部门名称
	if (first_level_department_name ='采购',classify_middle_name,'') as classify_middle_name , 
	count(goods_code) ths,
	count(case when main_category_name ='商品质量问题' then goods_code end ) as ths_spzl  -- 商品质量
from 
	(
	select 
		a.*,c.week_of_year,
		case when week_of_year='202416' and customer_code in ('120327','129941') then '1'
			 when week_of_year='202417' and customer_code in ('121276','234161') then '1'
			 when week_of_year='202420' and customer_code in ('128454','246442') then '1'
		else '2' end as delete_type	
	from 
		(select *
		from csx_dws.csx_dws_oms_complaint_detail_di
		...
		where  
			sdt>='${sdt_3m}' and sdt<='${sdt_tdm}'
			and main_category_name <> '送货后调整数量'  -- 剔除一级退货原因编码 001送货后调整数量
			-- and reason_detail <> '仓位调整'                  -- 剔除仓位调整
			and complaint_amt !=0
			and complaint_source =2  -- 客退单生成
			and customer_name not like '%XM%'	
			and performance_province_name not in ('东北','平台-B','null')
		)a
		left join
		(
		select
...
group by 
	...
	performance_city_name,     --  城市组名称(业绩划分)
	first_level_department_name,  -- 一级责任部门名称
	if (first_level_department_name ='采购',classify_middle_name,'') 
...
select 
	...
	count(goods_code) ths,
	count(case when first_level_reason_name ='商品质量问题' then goods_code end ) as ths_spzl  -- 商品质量
from 
	(
	select 
		a.*,c.week_of_year,
