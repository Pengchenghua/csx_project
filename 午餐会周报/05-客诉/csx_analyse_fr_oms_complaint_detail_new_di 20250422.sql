...
drop table csx_analyse_tmp.csx_analyse_sale_order_goods_flag; 
create table csx_analyse_tmp.csx_analyse_sale_order_goods_flag as 
insert overwrite table csx_analyse.csx_analyse_sale_order_goods_flag
...
--     order_code,
--     delivery_time,
--     regexp_replace(substr(delivery_time, 1, 10), '-', '') delivery_date,
--     rn
--   from
--     (
--       select
...
--             delivery_time
--           from
--             csx_report.csx_report_csms_yszx_order_hi
--           group by
--             order_no,
--             delivery_time
--         ) a
  ...
  from
    (
      select
        ...
        transfer_nos -- 调拨单集合
      from
        csx_dwd.csx_dwd_oms_refund_responsible_detail_di 
...
from 
  (
    select -- package_code,
      sale_order_code,
      goods_code,
	  max(delivery_type_code) as delivery_type_code, --	物流模式 1-配送 2-直送 3-自提
    ...
    where shipper_code='YHCSX'
    and sdt >= '20211101' -- 因客诉表的最早日期是20211206
...
select a.order_code,a.goods_code,a.direct_delivery_type,
  ...
  a.business_type_code,a.business_type_name,
  a2.extra as direct_delivery_large_type,
  concat(a2.extra,'-',a2.name) as new_direct_delivery_type  -- 详细履约模式
from 
(
  select order_code,goods_code,direct_delivery_type,
  delivery_type_code,delivery_type_name,
...
select
  ...
  complaint_source as complanit_source_code,
  -- 客户来源
  case
  ...
  coalesce(g.change_content_before,
    concat('原始原因、',a.main_category_name,'-',a.sub_category_name,'、',a.create_by)
  ) as reason_original,
  ...
  d.delivery_date,
  -- 出库日期
  coalesce(m.delivery_type_name,l.delivery_type_name,l.delivery_type_name,i.delivery_type_name) as delivery_type_name,
  ...
  end,'-') as submit_by_flag,   -- 确认编码来源:系统or人工
  l.carrier, -- 承运商编码
  l.carrier_name, -- 承运商
  l.driver_name, -- 司机姓名
...
-- 查数
  select sale_order_code,goods_code,delivery_type_code,*
  from csx_dwd.csx_dwd_oms_sale_refund_order_detail_di 
  where
    sdt >= '20250318' -- 因客诉表的最早日期是20211206
    and child_return_type_code in(1)
    and shipper_code='YHCSX'
    and parent_refund_code <> ''
    and refund_code='RH25042100000303'
    
 select responsible_code,refund_code,*
 from csx_dwd.csx_dwd_oms_refund_responsible_detail_di
 where sdt>='20250318'
 -- and refund_code in('RH25042100000303','RH25042100000302','RH25041700003751','RH25041700003750')
 and responsible_code in('PZ25042200000770','PZ25041800001637') 
 
 
select relation_order_code,*
from csx_dws.csx_dws_oms_complaint_detail_di
where complaint_code in('KS25041800002647','KS25042200001494')



select delivery_type_name,new_direct_delivery_type,
count(1) aa,
min(sdt) as min_sdt,
max(sdt) as max_sdt
from dev.csx_analyse_fr_oms_complaint_detail_new_di
group by delivery_type_name,new_direct_delivery_type

select sdt,complaint_code,complaint_time,sale_order_code,relation_order_code,complaint_source_name,goods_code,delivery_type_name,new_direct_delivery_type,*
from dev.csx_analyse_fr_oms_complaint_detail_new_di
where sdt>='20250301'
and delivery_type_name is null
