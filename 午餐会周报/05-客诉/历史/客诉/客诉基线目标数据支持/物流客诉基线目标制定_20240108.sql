...
select 
	...
	count(case when a.refund_order_flag=1  and  d.source_type !=1  and d.first_level_reason_code != '001'  then a.goods_code else null end ) as th_sku,	
	count(case when d.refund_order_type_code is null then g.goods_code else null end ) as bh_sku		
from
	( 
	select
	...
	(
	select -- 退货单数据
		sale_order_code,refund_code,
		goods_code,
		source_type, -- 订单来源(0-签收差异或退货 1-改单退货)
		first_level_reason_code, -- 一级退货原因编码 001送货后调整数量
		refund_order_type_code	-- 退货单类型(0:差异单 1:退货单）
	 from csx_dwd.csx_dwd_oms_sale_refund_order_detail_di
	 where sdt>='${sdt_4m}'  -- 往前卡一个月
		   and child_return_type_code=1
	       and parent_refund_code<>''
	)d on d.refund_code = a.order_code and a.goods_code= d.goods_code	
	left join -- 补货单数据
	(
	select e.replenishment_order_code,e.apply_code,e.goods_code,e.replace_goods_code,f.sale_order_code
	from 
		(
		select 
			replenishment_order_code,     -- 补货单号
			apply_code,    -- 申请补货订单
			scm_source_type -- 来源类型 0-缺货 1-客诉 2-签收差异 3-订单补货
			customer_code,
			goods_code,replace_goods_code
		from csx_dwd.csx_dwd_oms_emergency_replenishment_detail_df
		where item_create_time>='2023-07-01'  --无分区
			and coalesce(cancel_flag,0)<>1
			and scm_source_type in (0,3)
		)e 
		left join  -- 补货单关联销售单
		(
		select
		replenishment_order_code,sale_order_code
		from csx_dwd.csx_dwd_oms_emergency_sale_order_relation_df
		)f on e.replenishment_order_code =f.replenishment_order_code
	)g on g.apply_code =a.original_order_code  -- 补货最早单子
	      -- g.sale_order_code  = a.order_code -- 补货之后的单子	
	and g.replace_goods_code =a.goods_code
group by 
