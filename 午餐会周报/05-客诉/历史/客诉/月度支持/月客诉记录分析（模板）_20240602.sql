...
(select 
	...
	a.sub_category_name,
	case 
		when coalesce(f.direct_delivery_type,j.direct_delivery_type) is null then '普通'
		when coalesce(f.direct_delivery_type,j.direct_delivery_type) ='' then '普通'
