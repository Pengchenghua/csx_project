...
select 
	...
	case when performance_province_name in('广东深圳','广东广州') then '广东省'
		else performance_province_name end as performance_province_name,
	case when performance_city_name in('上海松江','上海宝山','江苏苏州') then performance_city_name
...
select 
	...
	coalesce(performance_province_name,'全国') as performance_province_name,
	coalesce(performance_city_name,'全国') as performance_city_name,
	sum(case when a.week=weekofyear(date_sub(current_date,3)) then sale_amt else 0 end)/10000 as bz_sale_amt,
	sum(case when a.week=weekofyear(date_sub(current_date,3)) then profit else 0 end)/10000 as bz_profit,
	sum(case when a.week=weekofyear(date_sub(current_date,3+7)) then sale_amt else 0 end)/10000 as sz_sale_amt,
	sum(case when a.week=weekofyear(date_sub(current_date,3+7)) then profit else 0 end)/10000 as sz_profit
from
	( 
	select 
		inventory_dc_code,
		...
		profit
	from csx_dws.csx_dws_sale_detail_di
	where sdt>=replace(date_sub(current_date,20),'-','')
	-- and sdt <='20231208'
	and performance_province_name not like '平台%'
