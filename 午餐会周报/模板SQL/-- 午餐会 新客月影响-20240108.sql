...
(
	...
	from
	(
		select  performance_province_code province_code
		       ,performance_province_name province_name
		       ,performance_city_code city_group_code
		       ,performance_city_name city_group_name
		       ,a.customer_code
		       ,d.customer_name
		       ,sum(case when a.sdt >= '20240101' and a.sdt <= '20240107' then a.sale_amt else 0 end) sales_value
		       ,sum(case when a.sdt >= '20231230' and a.sdt <= '20240105' then a.sale_amt end) sz_sales_value
		       ,sum(case when a.sdt >= '20231230' and a.sdt <= '20240105' then a.profit end ) sz_profit
		       ,sum(case when a.sdt >= '20231223' and a.sdt <= '20231229' then a.sale_amt end) bz_sales_value
		       ,sum(case when a.sdt >= '20231223' and a.sdt <= '20231229' then a.profit end ) bz_profit
		       ,sum(case when a.sdt >= '20231216' and a.sdt <= '20231222' then a.sale_amt end) xz_sales_value
		       ,sum(case when a.sdt >= '20231216' and a.sdt <= '20231222' then a.profit end ) xz_profit
		...
		(
			select  distinct customer_code
			       ,customer_name
			       ,substr(first_business_sale_date,1,6) first_sales_date
			from csx_dws.csx_dws_crm_customer_business_active_di
			where sdt = 'current'
			and business_type_code = 1
			--
			and substr(first_business_sale_date, 1, 6) in ('202312', '202401') 
 
		)d
		on d.customer_code = a.customer_code
		where c.shop_code is null
		and first_sales_date in ('202312', '202401')
...
(
	select  *
	       ,case when profit < -100 then '是'  else '否' end f_profit
	       ,if(sales_value < 0,'是','否') f_amt
	from csx_analyse.csx_analyse_fr_customer_new_old_di
) a
where sdt = '20240107'
      and f_profit = '是'
      and customer_name not like '%日用品%'                                -- 剔除监狱客户 
      and f_amt = '否'
order by profit asc
