...
(
 ...
 where c.shop_code is null 
 and d.first_sales_date<>substr(regexp_replace(trunc('${i_sdate}','MM'),'-',''),1,6)
 and    inventory_dc_code not in ('W0AJ','W0G6','WB71','W0J2') -- 3海军仓 和 监狱仓W0J2
        ...
        from (
                select performance_region_code region_code,
                    ...
                    sum(
                        case
                            when a.sdt >= '20231101'
                            and a.sdt <= '20231103' then a.sale_amt
                        end
                    ) sz_sales_value,
                    sum(
                        case
                            when a.sdt >= '20231101'
                            and a.sdt <= '20231103' then a.profit
                        end
                    ) sz_profit,
                    sum(
                        case
                            when a.sdt >= '20231104'
                            and a.sdt <= '20231110' then a.sale_amt
                        end
                    ) bz_sales_value,
                    sum(
                        case
                            when a.sdt >= '20231104'
                            and a.sdt <= '20231110' then a.profit
                        end
                    ) bz_profit,
                    sum(
                        case
                            when a.sdt >= '20231111'
                            and a.sdt <= '20231117' then a.sale_amt
                        end
                    ) xz_sales_value,
                    sum(
                        case
                            when a.sdt >= '20231111'
                            and a.sdt <= '20231117' then a.profit
                        end
                    ) xz_profit,
                    ...
                    sum(case when  a.sdt >= regexp_replace(trunc('2023-11-19','MM'),'-','') and a.sdt <= regexp_replace(add_months('2023-11-19',0),'-','') then a.sale_amt end) by_sales_value, 	
                    sum(case when a.sdt >= regexp_replace(trunc('2023-11-19','MM'),'-','') and a.sdt <= regexp_replace(add_months('2023-11-19',0),'-','') then a.profit end) by_profit,	 	
                    sum(case when a.sdt >= regexp_replace(add_months(trunc('2023-11-19','MM'),-1),'-','') and a.sdt <= regexp_replace(add_months('2023-11-19',-1),'-','') then a.sale_amt end) sy_sales_value, 	
                    sum(case when a.sdt >= regexp_replace(add_months(trunc('2023-11-19','MM'),-1),'-','') and a.sdt <= regexp_replace(add_months('2023-11-19',-1),'-','') then a.profit end) sy_profit	      
                    from   (     select *  	from csx_dws.csx_dws_sale_detail_di     
                    where  sdt >=regexp_replace(add_months(trunc('2023-11-19','MM'),-1),'-','')   	  
                    and sdt<= regexp_replace(add_months('2023-11-19',0),'-','')       
                    ...
                    and performance_city_name not in  	  ('南平市','三明市','宁德市','龙岩市','东北','黔江区','宁波市','台州市')   
                     ...
                     from csx_dws.csx_dws_crm_customer_business_active_di  
                      ...
                      where c.shop_code is null 
                      and d.first_sales_date<substr(regexp_replace(trunc('2023-11-19','MM'),'-',''),1,6)   
