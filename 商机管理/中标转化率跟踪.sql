...
-- select * from tmp_business_info where no='80'
 tmp_customer_info 
as (select no, 
    concat_ws('、',collect_set(business_number)) as list_business_number,
    concat_ws('、',collect_set(business_attribute_name)) business_attribute_name,
    sum(estimate_contract_amount) amt,
    min(business_sign_time) business_sign_time ,
    concat_ws('、',collect_set(business_type_name))  
...
(select business_type_name,
    ...
    sum(case when sdt>='20251201' then sale_amt end) months_sale_amt
