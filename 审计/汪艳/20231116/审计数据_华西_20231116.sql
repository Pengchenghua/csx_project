...
as
select
	'20221001-20231031' as qijian,a.performance_province_name,a.performance_city_name,a.business_type_name,a.delivery_type_name,
...
from
	...
	from
		...
		--	sdt>='20221001' and sdt<='20230630'
			 channel_code in('1','7','9') -- 渠道编码(1.大客户 2.商超 4.大宗 5.供应链(食百) 6.供应链(生鲜) 8.其他 9.业务代理)
		...
		join
			(
			select
				goods_code,sale_amt,performance_city_name,rn
			from
				(
				select
					goods_code,performance_city_name,sum(sale_amt) as sale_amt,row_number()over(partition by performance_city_name order by sum(sale_amt) desc) as rn
				from
					csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
				where 
				--	sdt>='20221001' and sdt<='20230630'
				...
				--	and performance_province_name='浙江省'
				group by 
					goods_code	,performance_city_name
				) a 
			where
				rn<=40
			) b on b.goods_code=a.goods_code and a.performance_city_name=b.performance_city_name
		left join
			(
			select
...
as
select
	'20230101-20231130' as qijian,a.performance_province_name,a.performance_city_name,a.business_type_name,a.delivery_type_name,
	a.order_code,a.order_sale_date,
	-- a.order_sale_amt/a.order_sale_qty as order_sale_price,
	-- a.order_sale_qty,a.sale_price,
	a.order_sale_amt,a.customer_code,b.customer_name,a.goods_code,c.goods_name,a.sale_amt,a.rn,a.unit_name,a.sale_qty
from
	...
	from
		...
		join
			(
			select
				performance_province_name,performance_city_name ,goods_code,sale_amt,rn
			from
				(
				select
					performance_province_name,performance_city_name,
					goods_code,sum(sale_amt) as sale_amt,row_number()over(partition by performance_city_name order by sum(sale_amt) desc) as rn
				from
					csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
				where 
				--	sdt>='20221001' and sdt<='20230630'
					...
					--and performance_province_name='浙江省'
				group by 
					performance_province_name,performance_city_name,goods_code	
				) a 
			where
				rn<=20
			) b on b.goods_code=a.goods_code and a.performance_city_name=b.performance_city_name
...
from
	...
	from
		...
		join
			(
			select
				order_code,business_type_name,sale_amt,days_cnt,rn,performance_city_name
			from
				(
				select
					order_code,
					...
					row_number()over(partition by performance_city_name, business_type_name order by sum(sale_amt)) as rn
				from
					csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
				where 
				--	sdt>='20221001' and sdt<='20231031'
				...
				--	and performance_province_name='浙江省'
					...
					and order_channel_code not in(4,6)
				group by 
					order_code,business_type_name,performance_city_name
				) a 
			where
				rn<=30
			) b on b.order_code=a.order_code and b.business_type_name=a.business_type_name and a.performance_city_name=b.performance_city_name
		left join
			(
			select
			...
			from
				csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
			where 
			--	sdt>='20201001' and sdt<='20231031' -- 多往前追1年
				 channel_code in('1','7','9') -- 渠道编码(1.大客户 2.商超 4.大宗 5.供应链(食百) 6.供应链(生鲜) 8.其他 9.业务代理)
...
from
	...
	from
		...
		join
			(
			select
				goods_code,business_type_name,sale_amt,days_cnt,rn,performance_city_name
			from
				(
				select
					goods_code,business_type_name,performance_city_name,sum(sale_amt) as sale_amt,count(distinct sdt) as days_cnt,row_number()over(partition by performance_city_name,business_type_name order by sum(sale_amt)) as rn
				from
					csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
				where 
				--	sdt>='20221001' and sdt<='20231031'
				...
				--	and performance_province_name='浙江省'
					...
					and order_channel_code not in(4,6)
				group by 
					goods_code,business_type_name,performance_city_name
				) a 
			where
				rn<=30
			) b on b.goods_code=a.goods_code and b.business_type_name=a.business_type_name and a.performance_city_name=b.performance_city_name
		left join
			(
			select
				-- if(business_type_name='BBC',if(order_code='B2211180606585688A',order_code,substr(order_code,2,16)),order_code) as order_code,
			...
			from
				csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
			where 
				sdt>='20211001' and sdt<='20231031' -- 多往前追1年
				and channel_code in('1','7','9') -- 渠道编码(1.大客户 2.商超 4.大宗 5.供应链(食百) 6.供应链(生鲜) 8.其他 9.业务代理)
				and original_order_code !=''
			--	and performance_province_name='浙江省'
				and refund_order_flag=0 -- 退货订单标识(0-正向单 1-逆向单)
...
from
	...
	from
		...
		join
			(
			select
				goods_code,sale_amt,days_cnt,rn,performance_city_name
			from
				(
				select
					goods_code,performance_city_name,sum(sale_amt) as sale_amt,count(distinct sdt) as days_cnt,row_number()over(partition by performance_city_name order by sum(sale_amt)) as rn
				from
					csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
				where 
				--	sdt>='20221001' and sdt<='20231031'
...
from
	...
	from
		...
		join
			(
			select
				customer_code,business_type_name,sale_amt,days_cnt,rn,performance_city_name
			from
				(
				select
					customer_code,business_type_name,performance_city_name,sum(sale_amt) as sale_amt,count(distinct sdt) as days_cnt,
					row_number()over(partition by performance_city_name,business_type_name order by count(distinct sdt) desc) as rn
				from
					csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
				where 
			--		sdt>='20221001' and sdt<='20231031'
				...
				--	and performance_province_name='浙江省'
					...
					and order_channel_code not in(4,6)
				group by 
					customer_code,business_type_name,performance_city_name
				) a 
			where
				rn<=30
			) b on b.customer_code=a.customer_code and b.business_type_name=a.business_type_name and a.performance_city_name=b.performance_city_name
		left join
			(
			select
				-- if(business_type_name='BBC',if(order_code='B2211180606585688A',order_code,substr(order_code,2,16)),order_code) as order_code,
	...
	from
		...
		join
			...
			from
				(
				select
					customer_code,performance_city_name,business_type_name,sum(sale_amt) as sale_amt,count(distinct sdt) as days_cnt,
					row_number()over(partition by performance_city_name,business_type_name order by sum(sale_amt)) as rn
				from
					csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
				where 
...
from
	...
	join
		(
		select
			customer_code,sale_amt,rn,days_cnt,performance_city_name
		from
			(
			select
				customer_code,performance_city_name,sum(sale_amt) as sale_amt,row_number()over(partition by performance_city_name order by sum(sale_amt) desc) as rn,count(distinct sdt) as days_cnt
			from
				csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
			where 
			--	sdt>='20221001' and sdt<='20231001'
			...
			--	and performance_province_name='浙江省'
				and business_type_code=4 -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.bbc 7.大宗一部 8.大宗二部 9.商超)
			group by 
				customer_code,performance_city_name
			) a 
		where
			rn<=30
		) c on c.customer_code=a.customer_code and a.performance_city_name=c.performance_city_name
	left join
		(
		select
		...
		from
			csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
		where 
		--	sdt>='20211001' and sdt<='20231001' -- 多往前追1年
			 channel_code in('1','7','9') -- 渠道编码(1.大客户 2.商超 4.大宗 5.供应链(食百) 6.供应链(生鲜) 8.其他 9.业务代理)
			and business_type_code=4 -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.bbc 7.大宗一部 8.大宗二部 9.商超)
			and refund_order_flag=1 -- 退货订单标识(0-正向单 1-逆向单)
		--	and performance_province_name='浙江省'
...
--		and performance_province_name='浙江省'
	...
	join
		(
		select
			customer_code,business_type_name,sale_amt,rn,performance_city_name
		from
			(
			select
				customer_code,business_type_name,performance_city_name,sum(sale_amt) as sale_amt,row_number()over(partition by business_type_name,performance_city_name order by sum(sale_amt) desc) as rn
			from
				csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
			where 
			--	sdt>='20221001' and sdt<='20231031'
...
;
select   qijian,
	a.performance_province_name,a.performance_city_name,a.order_code,a.sdt,a.customer_code,REGEXP_REPLACE(customer_name,'\r|\n|\r\n|\001|\002|\003|\t','') customer_name,a.business_type_name,zs_sale_amt,a.delivery_type_name,
	a.sale_amt,sale_amt_total,  rate,rn
	from csx_analyse_tmp.csx_analyse_tmp_finance_shenji_09 a  where performance_province_name in('重庆市','四川省','贵州省')
;
/*
-- 直送 占比大于等于80%
...
from
	...
	join
		...
		from
			(
			select
				customer_code,performance_city_name,business_type_name,sum(sale_amt) as sale_amt,row_number()over(partition by performance_city_name,business_type_name order by sum(sale_amt) desc) as rn
			from
				csx_analyse_tmp.csx_analyse_tmp_finance_shenji_sale_detail
			where 
			--	sdt>='20221001' and sdt<='20231031'
		...
		where
			rn<=30
		) c on c.customer_code=a.customer_code and c.business_type_name=a.business_type_name and a.performance_city_name=c.performance_city_name
	left join
		(
		select
			customer_code,customer_name,performance_region_name,performance_province_name,performance_city_name,first_category_name,second_category_name,third_category_name,sign_company_code,
...
as
select
	performance_region_name,b.performance_province_name,performance_city_name,a.tf_order_code,a.original_order_code,
...
from
	...
	where
		...
		and to_date(adjust_time) between '2023-01-01' and '2023-11-30'
	group by 
	...
	where
		sdt>='20200101'
		and audit_status_code='1' -- 审核状态编码:(0:待审核 1:审核通过 2:审核拒绝)
		and to_date(rebate_time) between '2023-01-01' and '2023-11-30'
...
select performance_region_name,	performance_province_name,	performance_city_name,	tf_order_code,	original_order_code,	business_type_name,	tf_adjust_type_name,	flag,	sign_company_name,	customer_code,REGEXP_REPLACE(customer_name,'\r|\n|\r\n|\001|\002|\003|\t','') 	customer_name ,
tf_date,	tf_sales_value,	sale_amt,	rate,	abs_tf_sales_value
from csx_analyse_tmp.csx_analyse_tmp_finance_shenji_12 where performance_region_name='华西大区';
...
as
select
	performance_region_name,b.performance_province_name,performance_city_name,
	a.tf_order_code,
...
from
	(select '调价单' as flag,
	adjust_price_order_code as tf_order_code,
	to_date(adjust_time) as tf_date,
	customer_code,
	customer_name,
	sign_company_code,
	sign_company_name,
	adjust_type_name as tf_adjust_type_name,
	business_type,
	original_order_code,
	-- goods_code,
	-- goods_name,
	sum(sales_value) as tf_sales_value
from csx_dwd.csx_dwd_sss_customer_adjust_price_detail_di
where sdt >= '20200101' --	and sdt<='20231031'
	and audit_status_code = 1 -- 审核状态编码:(0:待审核 1:审核通过 2:审核拒绝)
	and to_date(adjust_time) between '2023-01-01' and '2023-11-30'
group by adjust_price_order_code,
	to_date(adjust_time),
	customer_code,
	customer_name,
	sign_company_code,
...
select performance_region_name,
	...
	a.original_order_code,
	-- a.goods_code,
	-- REGEXP_REPLACE(goods_name, '\r|\n|\r\n|\001|\002|\003|\t', '') goods_name,
	business_type_name,
	a.tf_adjust_type_name,
	a.flag,
	a.sign_company_name,
	a.customer_code,
	REGEXP_REPLACE(customer_name, '\r|\n|\r\n|\001|\002|\003|\t', '') customer_name,
	a.tf_date,
	a.tf_sales_value
from csx_analyse_tmp.csx_analyse_tmp_finance_shenji_15 a
where performance_region_name = '华西大区'