...
-- A 商品销售数量、销售金额，销售成本、毛利额、毛利率
WITH tmp_customer_goods as (
select a.*,b.* from csx_analyse_tmp.csx_analyse_tmp_customer a 
full  join 
csx_analyse_tmp.csx_analyse_tmp_goods_link_01 b on 1=1 
),
tmp_A_sale_goods AS (
  SELECT 
    a.customer_code,
...
tmp_B_entry_goods AS (
  SELECT 
    ...
    SUM(a.receive_amt) AS receive_amt
  FROM csx_analyse.csx_analyse_scm_purchase_order_flow_di a
  JOIN csx_analyse_tmp.csx_analyse_tmp_goods_link_01 b 
    ON a.goods_code = b.goods_code_b
  WHERE a.sdt >= regexp_replace(date_add('2025-10-26', -30), '-', '')
    AND a.sdt <= regexp_replace('2025-10-26', '-', '')
    AND a.assign_type <> '1' -- 剔除客户指定数据
...
final_result AS (
  SELECT 
    ...
    t4.customer_price
  FROM tmp_A_sale_goods a
  JOIN tmp_customer_goods m 
    ON a.customer_code = m.customer_code 
    AND a.goods_code = m.goods_code
  LEFT JOIN (
    -- 主客户商品 B 报价
    SELECT 
