...
from
	...
	from
		...
		from
			...
			where 
				...
				and is_self_sale = 1 --自营，联营(非自营)，1自营，0联营(非自营)
			group by 
				goods_code
			) a
			left join
		...
		--here 
		--	c.division_code in (10,11) --10-加工部 11-生鲜部 12-食品类 13-用品类 14-服装 15-易耗品
		) a
	...
	from
		...
		select
			goods_code,
			case when sum(is_remark) > 0 then 1 else 0 end as is_remark
		from
			(
			select
				a.order_no,
				a.goods_code,
				case when b.order_no is null then 0 else 1 end as is_remark
			from
				(
				select
					order_no,
	...
	from
		...
		where 
			b.product_level <> '-1' --商品等级,1-特级,2-一级,3-正常,4-B,5-定制,6-小,-1-其它
			--and b.division_code in (10,11) --10-加工部 11-生鲜部 12-食品类 13-用品类 14-服装 15-易耗品
		) a
	...
	from
		...
		from
			...
			select
				a.order_no,
				a.goods_code,
				case when c.order_no is null then 0 else 1 end as is_remark
			from
				(
				select
					order_no,
	...
	where 
		...
		--and division_code in (10,11) --10-加工部 11-生鲜部 12-食品类 13-用品类 14-服装 15-易耗品
	) a
;
