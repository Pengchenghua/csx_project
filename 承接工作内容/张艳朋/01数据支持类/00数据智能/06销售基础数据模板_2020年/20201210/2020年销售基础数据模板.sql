...
insert overwrite directory '/tmp/zhangyanpeng/sales_base_b_1_20201209' row format delimited fields terminated by '\t' 
...
from
	...
	where 
		sdt between '20200101' and '20201208'
	...
	left join
		(
		select 
			customer_no,customer_name,attribute,sign_time,first_category,second_category,third_category
		from 
...
select
	a.province_name,a.smonth,a.dc_code,a.dc_name,e.sales_belong_flag,a.customer_no,b.customer_name,a.goods_code,a.goods_name,
...
insert overwrite directory '/tmp/zhangyanpeng/sales_base_new_sign_20201209' row format delimited fields terminated by '\t' 

select
	a.province_name,a.province_code,a.city_group_name,b.channel,b.channel_code,a.customer_no,b.customer_name,b.create_by,b.create_time,
...
from
	...
	where 
		--sdt between '20200101' and '20201208'
		--and channel in('2') --1-大客户 2-商超 4-大宗 5-供应链（食百） 6-供应链（生鲜） 7-企业购 9-业务代理
		sales_type in ('qyg','sapqyg','sapgc','sc','bbc')
		and province_name not like '平台%'
	group by 
	...
	left join   
		(
		select 
			customer_no,customer_name,channel,channel_code,create_by,create_time,sign_time,estimate_contract_amount,attribute_code,attribute,
...
where
	c.region_name='华西大区'
	and to_date(b.sign_time) between '2020-01-01' and '2020-12-08'
...
from
	...
	left join
		...
		where 
			sdt = '20201208' --可取最新分区
			and first_order_date between '20200101' and '20201208'
		group by
			customer_no,first_order_date	
		) d on a.customer_no=d.customer_no
where
