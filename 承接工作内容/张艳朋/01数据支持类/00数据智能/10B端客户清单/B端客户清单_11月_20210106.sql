	...
	select
		...
		a.return_flag,
		case when a.order_kind='NORMAL' then '正常单' when a.order_kind='WELFARE' then '福利单' else '其他' end as order_kind,
		a.goods_code,
		c.goods_name,
	...
	from 
		...
		select 
			...
			customer_no,order_no,order_kind,return_flag,goods_code,sales_value,profit,excluding_tax_sales,excluding_tax_profit
		from 
	...
	group by 
		...
		concat("'",a.order_no),
		a.return_flag,
		case when a.order_kind='NORMAL' then '正常单' when a.order_kind='WELFARE' then '福利单' else '其他' end,
		a.goods_code,
		c.goods_name