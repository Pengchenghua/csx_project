...
-- 目标表
set target_table=csx_dw.report_sss_r_m_receivable_overdue_amount;
...
from
	...
	left join -- 客户信息（每个城市都有虚拟主管，按省区汇总的时候把工号处理成相同的）
		(
		select 
			customer_no,customer_name,sales_region_code,sales_region_name,province_code,province_name,city_group_code,city_group_name,
...
INVALIDATE METADATA csx_dw.report_sss_r_m_receivable_overdue_amount;	



/*
--------------------------------- hive建表语句 -------------------------------
-- csx_dw.report_sss_r_m_receivable_overdue_amount  华北日报 应收逾期金额统计

drop table if exists csx_dw.report_sss_r_m_receivable_overdue_amount;
create table csx_dw.report_sss_r_m_receivable_overdue_amount(
