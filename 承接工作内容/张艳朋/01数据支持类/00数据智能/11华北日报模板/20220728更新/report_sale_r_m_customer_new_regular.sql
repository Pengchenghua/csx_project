...
-- 切换tez计算引擎
set mapred.job.name=report_sale_r_m_customer_new_regular;
...
-- 目标表
set target_table=csx_dw.report_sale_r_m_customer_new_regular;
...
(
	...
	from
		(
		select
			customer_no,channel_code,business_type_code,business_type_name,sales_value,profit,
		...
		--	(
		--	select
		--		customer_no,sales_region_code,sales_region_name,province_code,province_name,
...
INVALIDATE METADATA csx_dw.report_sale_r_m_customer_new_regular;	



/*
--------------------------------- hive建表语句 -------------------------------
-- csx_dw.report_sale_r_m_customer_new_regular  华北日报 回单统计

drop table if exists csx_dw.report_sale_r_m_customer_new_regular;
create table csx_dw.report_sale_r_m_customer_new_regular(
...
--alter table csx_dw.report_sale_r_m_customer_new_regular add columns(
--	city_group_code string comment '城市组编码',
--	city_group_name string comment '城市组名称');