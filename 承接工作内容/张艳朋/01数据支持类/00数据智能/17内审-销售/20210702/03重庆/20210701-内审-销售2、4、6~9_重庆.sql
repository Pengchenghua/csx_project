...
from
	(
	select 
		province_name,customer_no,customer_name,comp_code as company_code,comp_name as payment_name,
		ac_all as receivable_amount,ac_all-ac_wdq as over_amt,
		rank() over (partition by province_name order by ac_all-ac_wdq desc ) as rno1
	from 
		csx_tmp.ads_fr_account_receivables
	where 
		sdt=regexp_replace(date_sub(current_date, 1), '-', '')
		and province_name='重庆市' and channel_name in('大客户','业务代理','其他')
		--and smonth='小计'
	)a
	left join 
		(
...
from
	...
	select 
		...
		rank() over (partition by province_name order by abs(sum(sales_value)) desc ) as rno1 --调价返利金额排名
	from 
		csx_dw.dws_sale_r_d_detail
	where 
		sdt>=regexp_replace(date_sub(current_date, 91), '-', '')  --90天内
		and sales_type='fanli'
		and province_name in ('重庆市','四川省','贵州省')
	group by 
		province_name,customer_no,customer_name
	)a
	left join 
		...
		where 
			sdt='current'
			--and channel_code in('1','7','9')
			and sales_province_name in ('重庆市','四川省','贵州省')
		) b on b.customer_no=a.customer_no
where a.rno1<=5;
...
select 
	a.sales_province_name,a.city_group_name,a.customer_no,a.customer_name,a.sign_date,
	a.work_no,a.sales_name,a.first_supervisor_name,a.first_category_name,a.second_category_name,
	a.third_category_name,a.attribute_desc,a.cooperation_mode_name,
	a.dev_source_name,a.estimate_contract_amount*10000 as estimate_contract_amount
from 
