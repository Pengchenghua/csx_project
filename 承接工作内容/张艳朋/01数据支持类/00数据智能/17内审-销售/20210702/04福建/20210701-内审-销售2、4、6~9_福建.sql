...
select
	a.sales_province_name,a.city_group_name,a.work_no,a.sales_name,b.position,d.begin_date,
	floor(months_between('2021-07-01',from_unixtime(unix_timestamp(d.begin_date,'yyyyMMdd'),'yyyy-MM-dd'))) as date_dif,
	b.sales_supervisor_name,b.org_name,c.channel_name
from
	(
	select
		a.sales_id,a.work_no,a.sales_name,a.sales_province_name,a.city_group_name,max(first_order_date) as max_date
	from
		(	
		select
			sales_id,work_no,sales_name,customer_no,customer_name,sales_province_code,sales_province_name,city_group_code,city_group_name
		from
			csx_dw.dws_crm_w_a_customer
		where
			sdt='20210630'
	...
	group by 
		a.sales_id,a.work_no,a.sales_name,a.sales_province_name,a.city_group_name
	having
		max(first_order_date)<'20210401'
	) as a
	join
		(	
		select
...
where
	floor(months_between('2021-07-01',from_unixtime(unix_timestamp(d.begin_date,'yyyyMMdd'),'yyyy-MM-dd')))>=3 --在岗时长大于等于3个月
...
select
	regexp_replace(substr(sign_time,1,7),'-','') as s_month,
	sales_province_name,city_group_name,customer_no,customer_name,regexp_replace(substr(sign_time,1,10),'-','') as sign_date,
	work_no,sales_name,first_supervisor_name,
	first_category_name,second_category_name,third_category_name,attribute_desc,cooperation_mode_name,dev_source_name,estimate_contract_amount
from  
	csx_dw.dws_crm_w_a_customer 
...
where 
	...
	and regexp_replace(split(sign_time, ' ')[0], '-', '')>='20210630';
...
select
	substr(b.first_order_date,1,6) as s_month,
	a.sales_province_name,a.city_group_name,a.customer_no,a.customer_name,a.sign_date,b.first_order_date,
	a.work_no,a.sales_name,a.first_supervisor_name,a.first_category_name,a.second_category_name,
	a.third_category_name,a.attribute_desc,a.cooperation_mode_name,
	a.dev_source_name,a.estimate_contract_amount*10000 as estimate_contract_amount
from 
	(
	select
		customer_no,customer_name,regexp_replace(substr(sign_time,1,10),'-','') as sign_date,sales_province_name,city_group_name,
...
from
	...
	where 
		...
		and province_name='重庆市' and channel_code in('1','7','9')
		and smonth='小计'
	)a
	left join 
		(
...
--==============================================================================================	
--6.2 逾期金额前5客户名称 SAP系统
select
  a.province_name,b.city_group_name,a.customer_no,b.customer_name,a.company_code,a.payment_name,b.attribute_desc,b.work_no,b.sales_name,
  a.receivable_amount,a.over_amt,
  b.estimate_contract_amount,b.sign_date,a.rno1
from
	(
	select 
		province_name,sales_city,customer_no,customer_name,comp_code as company_code,comp_name as payment_name,
		ac_all as receivable_amount,ac_all-ac_wdq as over_amt,
		rank() over (partition by province_name,sales_city order by ac_all-ac_wdq desc ) as rno1
	from 
		csx_tmp.ads_fr_account_receivables
	where 
		sdt=regexp_replace(date_sub(current_date, 1), '-', '')
		and province_name='福建省' and channel_name in('大客户','业务代理','其他')
		--and smonth='小计'
	)a
	left join 
		(
		select 
			sales_province_name,channel_name,customer_no,customer_name,city_group_name,
			attribute_desc,dev_source_name,cooperation_mode_name,
			work_no,sales_name,first_supervisor_work_no,first_supervisor_name,
			regexp_replace(split(first_sign_time, ' ')[0], '-', '') as first_sign_date,
			regexp_replace(split(sign_time, ' ')[0], '-', '') as sign_date,
...
--逾期客户 认领口径=逾期金额-认领未核销金额
	(
	select 
		province_name,customer_no,customer_name,comp_code as company_code,diff as payment_name,
		ac_all as receivable_amount,ac_all-ac_wdq as over_amt
	from 
		csx_tmp.ads_fr_account_receivables
	where 
...
from
	...
	select 
		...
		rank() over (partition by province_name order by sum(sales_value) asc ) as rno1 --调价返利金额排名
	from 
		csx_dw.dws_sale_r_d_detail
	where 
...
--==============================================================================================
--9、返利客户稽核-客户调价返利情况 按绝对值

select 
	a.province_name,a.city_group_name,a.customer_no,b.customer_name,b.channel_name,b.attribute_desc,b.work_no,b.sales_name,b.sign_date,
	a.sales_value,a.rno1
from
	(
	select 
		province_name,city_group_name,customer_no,customer_name,
		sum(sales_value) sales_value,   --含税销售额
		-- abs(sum(sales_value)) as abs_sales_value,
		rank() over (partition by province_name,city_group_name order by abs(sum(sales_value)) desc ) as rno1 --调价返利金额排名
	from 
		csx_dw.dws_sale_r_d_detail
	where 
		sdt>=regexp_replace(date_sub(current_date, 91), '-', '')  --90天内
...
-- 拜访统计
select
	count(distinct a.visit_person_id) as sales_cnt,
	count(a.visit_person_id) as visit_cnt,
	count(a.visit_person_id)/count(distinct a.visit_person_id) as avg_visit_cnt,
	count(distinct a.customer_id) as customer_cnt,
	count(distinct case when a.is_cooperation='是' then a.customer_id else null end) as cooperation_customer_cnt,
	count(distinct case when a.is_cooperation='是' then a.customer_id else null end)/count(distinct a.customer_id) as cooperation_customer_rate,
	count(distinct case when a.is_cooperation='否' then a.customer_id else null end) as n_cooperation_customer_cnt,
	count(distinct case when a.is_cooperation='否' then a.customer_id else null end)/count(distinct a.customer_id) as n_cooperation_customer_rate
from
	( 
	select 
		*
	...
	join
		...
		where 
			...
			and position = 'SALES'
			and org_name !='商超'
		) b on b.sales_id=a.visit_person_id
	join
		(
		select
			employee_code,employee_name,begin_date,end_date,emp_status
