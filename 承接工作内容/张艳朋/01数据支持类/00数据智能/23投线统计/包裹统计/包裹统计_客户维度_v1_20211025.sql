...
from 
	(
	select 
		a.plan_date,a.warehouse_code,c.split_group_code,c.split_group_name,a.`status` as zhuangtai,a.picking_sort_number as fahuoquyu,a.sub_custom_name as kehumingcheng,
	...
	union all
	select 
		d.plan_date,a.warehouse_code,c.split_group_code,c.split_group_name,a.`status` as zhuangtai,d.picking_sort_number as fahuoquyu,a.sub_custom_name as kehumingcheng,
		(case when container_id is null then '0'  else '1' end) as touxianzhuangtai
	from 
...
group by 
	replace(xx.plan_date, '-', ''),xx.fahuoquyu,xx.kehumingcheng
order by 
	replace(xx.plan_date, '-', ''),xx.fahuoquyu,xx.kehumingcheng;