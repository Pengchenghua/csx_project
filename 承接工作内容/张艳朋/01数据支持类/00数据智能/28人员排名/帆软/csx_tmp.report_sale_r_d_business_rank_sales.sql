...
-- 切换tez计算引擎
set mapred.job.name=report_sale_r_d_business_rank_sales;
...
-- 目标表
set target_table=csx_tmp.report_sale_r_d_business_rank_sales;
...
select
	...
	case when b.user_position='SALES' and (d.user_position is null or d.user_position !='CUSTOMER_SERVICE_MANAGER') then '销售员'
		when b.user_position='CUSTOMER_SERVICE_MANAGER' or (b.user_position='SALES' and d.user_position='CUSTOMER_SERVICE_MANAGER') then '服务管家'
		when b.user_position='SALES_SUPPORT' then '销售支持'
		else '其他'
	end as person_type,
	'' as sales_target,
	...
	row_number()over(partition by c.province_name,c.city_group_name order by f.sales_value desc) as rank_number
from
	(
	select --销售员信息
...
INVALIDATE METADATA csx_tmp.report_sale_r_d_business_rank_sales;	



/*
--------------------------------- hive建表语句 -------------------------------
-- csx_tmp.report_sale_r_d_business_rank_sales  销售员指标

drop table if exists csx_tmp.report_sale_r_d_business_rank_sales;
create table csx_tmp.report_sale_r_d_business_rank_sales(
...
`person_type`                    string              COMMENT    '人员类别',
