...
-- 切换tez计算引擎
set mapred.job.name=report_sale_r_d_business_rank_sales_supervisor;
...
-- 目标表
set target_table=csx_tmp.report_sale_r_d_business_rank_sales_supervisor;
...
select
	...
	c.city_group_code,
	c.city_group_name,
	j.sales_manager_work_no,
	j.sales_manager_name,
	...
	row_number()over(partition by c.province_name,c.city_group_name,a.first_supervisor_work_no order by f.sales_value desc) as rank_number
from
	(
	select --人员信息
	...
	left join
		( 
		select
			sales_name,work_no,position,sales_manager_id,sales_manager_work_no,sales_manager_name
		from
			(
			select
				name as sales_name,
				...
				-- 销售经理
				first_value(case when leader_user_position = 'SALES_CITY_MANAGER' then leader_id end, true) over(partition by user_number order by distance) as sales_manager_id,
				first_value(case when leader_user_position = 'SALES_CITY_MANAGER' then leader_name end, true) over(partition by user_number order by distance) as sales_manager_name,
				first_value(case when leader_user_position = 'SALES_CITY_MANAGER' then leader_user_number end, true) over(partition by user_number order by distance) as sales_manager_work_no,
		...
		where 
			tmp.rank = 1
			and sales_manager_work_no is not null
		group by 
			sales_name,work_no,position,sales_manager_id,sales_manager_work_no,sales_manager_name
...
select
	concat_ws('&',city_group_code,first_supervisor_work_no,business_type,${hiveconf:current_end_day}) as biz_id,
	region_code,
	region_name,
	province_code,
	province_name,
...
INVALIDATE METADATA csx_tmp.report_sale_r_d_business_rank_sales_supervisor;	



/*
--------------------------------- hive建表语句 -------------------------------
-- csx_tmp.report_sale_r_d_business_rank_sales_supervisor  销售员指标

drop table if exists csx_tmp.report_sale_r_d_business_rank_sales_supervisor;
create table csx_tmp.report_sale_r_d_business_rank_sales_supervisor(
