...
-- 切换tez计算引擎
set mapred.job.name=tmp_r_m_rank_sales_manager;
...
-- 目标表
set target_table=csx_tmp.tmp_r_m_rank_sales_manager;
	
with current_tmp_r_m_rank_sales_manager as 	
(
select
	b.province_code,
	b.province_name,
...
from
	current_tmp_r_m_rank_sales_manager
;



INVALIDATE METADATA csx_tmp.tmp_r_m_rank_sales_manager;	



/*
--------------------------------- hive建表语句 -------------------------------
-- csx_tmp.tmp_r_m_rank_sales_manager  销售员指标

drop table if exists csx_tmp.tmp_r_m_rank_sales_manager;
create table csx_tmp.tmp_r_m_rank_sales_manager(
`biz_id`                         string              COMMENT    '业务主键',
`province_code`                  string              COMMENT    '省区编码',
`province_name`                  string              COMMENT    '省区名称',
`city_group_code`                string              COMMENT    '城市编码',
`city_group_name`                string              COMMENT    '城市',
`third_supervisor_work_no`       string              COMMENT    '主管工号',
