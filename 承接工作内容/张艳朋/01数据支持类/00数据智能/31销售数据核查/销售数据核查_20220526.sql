...
select 
	customer_no,customer_name,substr(sdt,1,6) as syear,
	shop_company_name,
	sum(sales_value) as sales_value,
	sum(excluding_tax_sales) as excluding_tax_sales,
	sum(excluding_tax_profit) as excluding_tax_profit
...
where 
	channel_code in('1','7','9')
	and sdt>='20190101' and sdt<='20220525'
	and customer_no in ('107592','122422')
group by 
	customer_no,customer_name,substr(sdt,1,6),shop_company_name;

	
insert overwrite directory '/tmp/zhangyanpeng/20220512_01_02' row format delimited fields terminated by '\t'	
select
	regexp_replace(substr(posting_time,1,7),'-','') syear,
    customer_code, -- 客户编码
    company_code, -- 公司代码
	opp_acc_name,
    sum(paid_amount) as paid_amount
from  
	csx_dw.dwd_sss_r_d_money_back
where
	substr(posting_time,1,7)>='2019-01'
	and customer_code in ('107592','122422')
group by 
	regexp_replace(substr(posting_time,1,7),'-',''),customer_code,company_code,opp_acc_name
;
