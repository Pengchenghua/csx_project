...
from
	...
	from
		...
		select
			...
			if(substr(move_type,1,3) in ('115','116') and amt_no_tax<>'0' and substr(purchase_group_code,1,1) not in('A'),if(direction='-',-1*amt_no_tax,amt_no_tax),0) as post_amt_no_tax,   --财务盘点过账不含税金额 3月份财务盘点不含食百
