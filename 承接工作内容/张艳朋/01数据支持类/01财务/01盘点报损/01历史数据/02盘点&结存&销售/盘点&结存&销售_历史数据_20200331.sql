...
from
	...
	from
		...
		select
			...
			--财务盘点-盘盈
			if(substr(move_type,1,3) in ('115') and amt_no_tax<>'0' and substr(purchase_group_code,1,1) not in('A'),if(direction='-',-1*qty,qty),0) as post_surplus_qty,   --财务盘点过账不含税数量	
			if(substr(move_type,1,3) in ('115') and amt_no_tax<>'0' and substr(purchase_group_code,1,1) not in('A'),if(direction='-',-1*amt_no_tax,amt_no_tax),0) as post_surplus_amt_no_tax,   --财务盘点过账不含税金额
			--财务盘点-盘亏
			if(substr(move_type,1,3) in ('116') and amt_no_tax<>'0' and substr(purchase_group_code,1,1) not in('A'),if(direction='-',-1*qty,qty),0) as post_loss_qty,   --财务盘点过账不含税数量	
			if(substr(move_type,1,3) in ('116') and amt_no_tax<>'0' and substr(purchase_group_code,1,1) not in('A'),if(direction='-',-1*amt_no_tax,amt_no_tax),0) as post_loss_amt_no_tax  --财务盘点过账不含税金额	
		from 
	...
	full join 
		...
		where 
			sdt between '20200301' and '20200331'
