...
insert overwrite table csx_tmp.report_finance_r_m_inventory_stock_sale partition(sdt)

select
	coalesce(b.province_name,c.province_name,a.province_name) as province_name,
	coalesce(b.city_name,c.city_name,a.city_name) as city_name,
	...
	coalesce((a.surplus_amt+a.loss_amt)/c.sales_value,'0') as loss_rate,
...
from
	...
	from
		...
		left join
			...
			group by
				shop_id,shop_name,province_code,province_name,city_code,city_name
			) tmp1 on tmp1.shop_id=t1.location_code
	group by
		case when location_code in ('W0G1','W0H1') then '大宗一'
			when location_code in ('W0H4','W0S1') then '大宗二'
			else tmp1.province_name
	...
	full join 
		...
		from
			...
			left join
				...
				group by
					shop_id,shop_name,province_code,province_name,city_code,city_name
				) tmp2 on tmp2.shop_id=t2.dc_code
		) b on b.dc_code=a.location_code and b.division_name=a.division_name and b.province_name=a.province_name and b.city_name=a.city_name
	-- 销售信息
	full join 
		(
		select
			t3.province_name,
			tmp3.city_name as city_name,
			dc_code,
			division_name,
			sales_value,
			sales_cost,
		...
		from
			(
			select
				case when province_name like '%大宗%' then '大宗一'
					when province_name like '%采购%' then '大宗二'
					else province_name
				end as province_name,
				dc_code,
				if(substr(department_code,1,1) in('U','H'),'生鲜',if(substr(department_code,1,1)='A','食百','易耗品')) division_name,   --部类
				sum(excluding_tax_sales) as sales_value, --不含税
				sum(excluding_tax_cost) as sales_cost, --不含税
				sum(excluding_tax_profit) as profit --不含税
			from 
			...
			left join
				...
				group by
					shop_id,shop_name,province_code,province_name,city_code,city_name
				) tmp3 on tmp3.shop_id=t3.dc_code
		) c on c.dc_code=b.dc_code and c.division_name=b.division_name and c.province_name=b.province_name and c.city_name=b.city_name
;


INVALIDATE METADATA csx_tmp.report_finance_r_m_inventory_stock_sale;		
	


/*
--------------------------------- hive建表语句 -------------------------------
-- csx_tmp.report_finance_r_m_inventory_stock_sale  盘点报损-计算

drop table if exists csx_tmp.report_finance_r_m_inventory_stock_sale;
create table csx_tmp.report_finance_r_m_inventory_stock_sale(
