--==============================================================================================================
--月累计
insert overwrite directory '/tmp/zhangyanpeng/_w46_m' row format delimited fields terminated by '\t' 
...
from	
	...
	where 
		(sdt between '20201101' and '20201113' or sdt between '20201001' and '20201013')
...
insert overwrite directory '/tmp/zhangyanpeng/_w46_w' row format delimited fields terminated by '\t' 					
...
select
	...
	a.profit,
	if(a.department_code in ('U01','H03','H05','H01','H02','H04','104'),a.department_name,'其他') as department_type,--熟食课,蔬菜课,猪肉课,干货课,水果课,家禽课,易耗品采购组,其他
	regexp_replace(b.region_name,'大区','') as region_name
from	
	(	
	select
		sdt,
	...
	from 
		csx_dw.dws_sale_r_d_customer_sale
	where 
		sdt between '20201031' and '20201113'
		and channel = '2' --1-大客户 2-商超 4-大宗 5-供应链（食百） 6-供应链（生鲜） 7-企业购 9-业务代理
		and (dc_code not in ('W0R1','W0T6','W0M4','W0T3','W0T7','W0M6','W0S8','W0X4','W0T5') or (dc_code in ('W0M4') and department_code not in ('H03','H01')))--数据不含代加工DC 
		--and province_name='福建省'
	group by
		sdt,province_code,province_name,department_code,department_name
	) as a	
	left join
		(
		select 
