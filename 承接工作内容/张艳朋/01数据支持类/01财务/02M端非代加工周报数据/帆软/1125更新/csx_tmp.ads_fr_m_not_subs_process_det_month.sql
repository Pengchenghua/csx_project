...
insert overwrite table csx_tmp.ads_fr_m_not_subs_process_det_month partition(sdt)
...
select
	...
	c.region_name,
	case when a.sdt between ${hiveconf:i_sdate_12} and ${hiveconf:i_sdate_11} then '本月'
		when a.sdt between ${hiveconf:i_sdate_22} and ${hiveconf:i_sdate_21} then '上月'
		else '其他'
	end as month_type,
...
INVALIDATE METADATA csx_tmp.ads_fr_m_not_subs_process_det_month;


/*
--------------------------------- hive建表语句 -------------------------------
-- csx_tmp.ads_fr_m_not_subs_process_det_month  M端非代加工月环比数据明细

drop table if exists csx_tmp.ads_fr_m_not_subs_process_det_month;
create table csx_tmp.ads_fr_m_not_subs_process_det_month(
...
`month_type`               string              COMMENT    '月度',
`update_time`              string              COMMENT    '更新时间'
) COMMENT 'zhangyanpeng:M端非代加工月环比数据明细'
