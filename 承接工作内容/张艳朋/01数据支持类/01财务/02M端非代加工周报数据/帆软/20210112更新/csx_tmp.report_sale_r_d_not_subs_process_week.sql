...
--上周六日期
set i_sdate_2 =regexp_replace(date_sub(current_date,14),'-','');
...
insert overwrite table csx_tmp.report_sale_r_d_not_subs_process_week partition(sdt)
...
select
	...
	if(sdt>=${hiveconf:i_sdate_3},'本周','上周') as week_type,
...
where 
	sdt between ${hiveconf:i_sdate_2} and ${hiveconf:i_sdate_1}
...
group by
	...
	if(sdt>=${hiveconf:i_sdate_3},'本周','上周')
...
select
	...
	coalesce(sum(a.profit)/abs(sum(a.sales_value)),0) as profit_rate,
	${hiveconf:i_sdate_1} as update_time,
	${hiveconf:i_sdate_1} as sdt
from	
	(	
...
INVALIDATE METADATA csx_tmp.report_sale_r_d_not_subs_process_week;	


	
/*
--------------------------------- hive建表语句 -------------------------------
-- csx_tmp.report_sale_r_d_not_subs_process_week  M端非代加工周报数据

drop table if exists csx_tmp.report_sale_r_d_not_subs_process_week;
create table csx_tmp.report_sale_r_d_not_subs_process_week(
