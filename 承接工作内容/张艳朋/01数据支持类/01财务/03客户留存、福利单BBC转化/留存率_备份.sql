...
from 
(
  select customer_no,sdt,sales_value from csx_dw.sale_item_m 
  where sdt>='20180101' and sdt<'20190101' and sales_type in('qyg','sapqyg','sapgc','sc','bbc','gc','anhui') and (order_kind='NORMAL' or order_kind is null)
union all 
select customer_no,sdt,sales_value from csx_dw.dws_sale_r_d_detail 
where sdt>='20190101' and sdt<'20210201' 
...
select a.new_smonth,
	...
	concat(substr(a.new_smonth,1,4),'-',substr(a.new_smonth,5,2),'-','01'))) diff_month,
	count(distinct b.customer_no) counts,
	sum(b.sales_value) sales_value,
	avg(b.sales_value) avg_sales_value
from
	(select customer_no,substr(min_sdt,1,6) new_smonth
	from csx_tmp.tmp_cust_sale_1
	where substr(min_sdt,1,6)>='201901'
