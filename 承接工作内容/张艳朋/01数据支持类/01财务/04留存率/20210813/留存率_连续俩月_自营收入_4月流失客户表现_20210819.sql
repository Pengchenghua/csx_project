...
insert overwrite directory '/tmp/zhangyanpeng/20210819_01_03' row format delimited fields terminated by '\t' 
--结果表  
...
from
	...
	left join
		...
		group by 
			customer_no,substr(sdt,1,6)
		)d on d.customer_no=a.customer_no and d.smonth=a.four_months_after
where
...
select
	customer_no,next_month,
	substr(regexp_replace(add_months(from_unixtime(unix_timestamp(concat(next_month,'01'),'yyyyMMdd')),4),'-',''),1,6) as four_months_after
from
	(
	select
		customer_no,
