...
insert overwrite directory '/tmp/zhangyanpeng/20210819_01_01' row format delimited fields terminated by '\t' 
--结果表  
...
from
	...
	join
		(
		select 
			customer_no,substr(max(sdt),1,6) as last_order_month
		from 
			csx_dw.dws_sale_r_d_detail
		where 
			sdt>='20190101' and sdt<='20210630'
...
insert overwrite directory '/tmp/zhangyanpeng/20210819_01_02' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select 
	c.sales_province_name,
