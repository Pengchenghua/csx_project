...
drop table csx_tmp.tmp_self_one_consecutive_months;
create table csx_tmp.tmp_self_one_consecutive_months
as 
select
	customer_no,
	min(sdt) as min_sdt,
...
insert overwrite directory '/tmp/zhangyanpeng/20211011_01_03' row format delimited fields terminated by '\t' 
--结果表  
...
from
	...
	from
		csx_tmp.tmp_self_one_consecutive_months
	where 
...
-- ====================================================================================================================================================
insert overwrite directory '/tmp/zhangyanpeng/20211011_01_04' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select 
	c.sales_province_name,
...
insert overwrite directory '/tmp/zhangyanpeng/20211011_01_05' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select 
	c.sales_province_name,
...
from
	...
	join
		...
		group by 
			customer_no
		having
			sum(sales_value)/(months_between(from_unixtime(unix_timestamp(max(sdt),'yyyyMMdd')),from_unixtime(unix_timestamp(min(sdt),'yyyyMMdd')))+1)>=150000
...
-- ====================================================================================================================================================
insert overwrite directory '/tmp/zhangyanpeng/20211011_01_06' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select 
	c.sales_province_name,
...
insert overwrite directory '/tmp/zhangyanpeng/20211011_01_07' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select 
	c.sales_province_name,
...
from
	...
	join
		...
		group by 
			customer_no
		having
			sum(sales_value)/(months_between(from_unixtime(unix_timestamp(max(sdt),'yyyyMMdd')),from_unixtime(unix_timestamp(min(sdt),'yyyyMMdd')))+1)>=60000
		) d on d.customer_no=a.customer_no
group by
	c.sales_province_name, 
	a.new_smonth,
...
-- ====================================================================================================================================================
insert overwrite directory '/tmp/zhangyanpeng/20211011_01_08' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select 
	c.sales_province_name,
