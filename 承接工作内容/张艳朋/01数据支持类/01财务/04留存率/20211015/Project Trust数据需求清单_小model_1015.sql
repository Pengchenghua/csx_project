...
insert overwrite directory '/tmp/zhangyanpeng/20211015_01_normal' row format delimited fields terminated by '\t' 
--结果表  
	...
	from
		...
		left join
			...
			select 
				...
				sales_region_code,sales_region_name,sales_province_code,province_name,city_group_code,city_group_name,cooperation_mode_name
			from 
...
insert overwrite directory '/tmp/zhangyanpeng/20211015_02_fuli' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select
	province_name,
...
insert overwrite directory '/tmp/zhangyanpeng/20211015_03_bbc' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select
	province_name,
