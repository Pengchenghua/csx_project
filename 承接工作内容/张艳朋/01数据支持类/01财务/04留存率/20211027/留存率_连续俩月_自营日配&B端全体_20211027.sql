...
drop table csx_tmp.tmp_self_two_consecutive_months_normal;
create table csx_tmp.tmp_self_two_consecutive_months_normal
as 
select
...
insert overwrite directory '/tmp/zhangyanpeng/20211027_01_01' row format delimited fields terminated by '\t' 
--结果表  
...
from
	...
	from
		csx_tmp.tmp_self_two_consecutive_months_normal
	where 
...
drop table csx_tmp.tmp_self_two_consecutive_months_b;
create table csx_tmp.tmp_self_two_consecutive_months_b
as 
select
	customer_no,next_month,
	substr(regexp_replace(add_months(from_unixtime(unix_timestamp(concat(next_month,'01'),'yyyyMMdd')),5),'-',''),1,6) as four_months_after
...
insert overwrite directory '/tmp/zhangyanpeng/20211027_01_02' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select 
	c.sales_province_name,
...
from
	...
	select 
		customer_no,substr(next_month,1,6) new_smonth
	from
		csx_tmp.tmp_self_two_consecutive_months_b
	where 
		substr(next_month,1,6)>='201901'
	)a
	join 
