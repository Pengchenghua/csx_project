...
drop table csx_tmp.tmp_trust_customer_normal;
create table csx_tmp.tmp_trust_customer_normal
as
select
...
insert overwrite directory '/tmp/zhangyanpeng/20211118_01_01' row format delimited fields terminated by '\t' 
--结果表  
...
from
	...
	from
		...
		from
			csx_tmp.tmp_trust_customer_normal
		)a
		right join 
			(
			select 
...
drop table csx_tmp.tmp_trust_customer_fuli;
create table csx_tmp.tmp_trust_customer_fuli
as
select
...
insert overwrite directory '/tmp/zhangyanpeng/20211118_01_02' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select
	sales_region_name,
...
from
	...
	from
		...
		from
			csx_tmp.tmp_trust_customer_fuli
		)a
...
drop table csx_tmp.tmp_trust_customer_bbc;
create table csx_tmp.tmp_trust_customer_bbc
as
select
	customer_no,substr(min(sdt),1,6) as first_month
from
...
insert overwrite directory '/tmp/zhangyanpeng/20211118_01_03' row format delimited fields terminated by '\t' 
--结果表  
--判断各月新客，计算每月新客在之后各月是否有销售
select
	sales_region_name,
...
from
	...
	from
		...
		select 
			customer_no,substr(first_month,1,6) new_smonth
		from
			csx_tmp.tmp_trust_customer_bbc
		)a
		right join -- 每月日配业务履约金额大于0	
			(
			select 
...
select 
	region_name,province_name,substr(sdt,1,6) smonth,
	sum(excluding_tax_sales) excluding_tax_sales,
	sum(excluding_tax_profit) as excluding_tax_profit
from 
	csx_dw.dws_sale_r_d_detail
...
--城市服务商数量
select 
	a.region_name,a.province_name,a.s_month,
...
select
	...
	a.province_name,
	a.smonth,
	case when a.smonth>=d.first_month then 'B端转化' else '非B端转化' end as type_2,
	count(distinct case when a.smonth=b.first_order_date then a.customer_no else null end) cnt1,
	count(distinct case when a.smonth !=b.first_order_date then a.customer_no else null end) cnt2
from
	(
	select
		region_name,province_name,substr(sdt,1,6) smonth,customer_no
	from 
		csx_dw.dws_sale_r_d_detail
	where 
		sdt>='20190101' and sdt<='20220430'
 	...
 	group by 
		region_name,province_name,substr(sdt,1,6),customer_no
	) a 
	left join
		(
		select
			customer_no,substr(min(sdt),1,6) as first_order_date
		from 
			csx_dw.dws_sale_r_d_detail
		where 
			sdt>='20190101' and sdt<='20220430'
...
group by 
	...
	a.province_name,
	a.smonth,
	case when a.smonth>=d.first_month then 'B端转化' else '非B端转化' end
;

--=============================================================================================================================================================================
-- 查数 城市服务商业务 缺少供应商

insert overwrite directory '/tmp/zhangyanpeng/20211118_02_01' row format delimited fields terminated by '\t' 
