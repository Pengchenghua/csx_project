...
from
	...
	select 
		customer_no,substr(next_month,1,6) new_smonth,
		case when substr(next_month,1,6)<='202109' then substr(regexp_replace(add_months(from_unixtime(unix_timestamp(concat(next_month,'01'),'yyyyMMdd')),12),'-',''),1,6)
			else substr(regexp_replace(add_months(from_unixtime(unix_timestamp(concat(next_month,'01'),'yyyyMMdd')),cast(months_between('2022-09-01',to_date(from_unixtime(unix_timestamp(concat(next_month,'01'),'yyyyMMdd'))))as int)),'-',''),1,6)
		end as 12_months
	...
	join 	
		(
		select 
			customer_no,substr(max(sdt),1,6) max_month
		from 
			csx_dw.dws_sale_r_d_detail
		where 
			sdt>='20190101' and sdt<='20220930'
