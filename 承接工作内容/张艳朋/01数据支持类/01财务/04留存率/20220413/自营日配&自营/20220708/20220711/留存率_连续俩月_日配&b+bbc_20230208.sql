...
-- 日配_至少A、B类有两个月的客户
drop table if exists csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230113;
create table csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230113
as
select 
...
;	
select * from csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230113;

-- ====================================================================================================================================================

-- 日配_至少A、B类有两个月的客户
drop table if exists csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208;
create table csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208
as
select 
	c.performance_province_name,
	a.new_smonth,
...
group by
	...
	concat(substr(a.new_smonth,1,4),'-',substr(a.new_smonth,5,2),'-','01')))
;	
select * from csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208;

-- ====================================================================================================================================================

-- 日配_至少A、B类有两个月的客户
drop table if exists csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v2;
create table csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v2
as
select 
	c.performance_province_name,
	a.customer_code,
...
from
	...
	from
		csx_analyse_tmp.csx_analyse_tmp_ripei_lianxu_months
	where 
		substr(next_month,1,6)>='202201' and substr(next_month,1,6)<='202207'
	)a
	left join
		(
		select 
			customer_code,substr(max(sdt),1,6) smonth
		from 
			-- csx_dw.dws_sale_r_d_detail
			csx_dws.csx_dws_sale_detail_di
		where 
...
where
	b.smonth=a.new_smonth
;	
select * from csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v2;


1、202201至202212新客 在202301没下单的客户清单
2、202201至202210新客 从202301月开始大于等于3个月未履约客户清单

-- ====================================================================================================================================================

-- 日配_至少A、B类有两个月的客户
drop table if exists csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v3;
create table csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v3
as
select 
	c.performance_province_name,
	a.customer_code,
...
from
	...
	from
		csx_analyse_tmp.csx_analyse_tmp_ripei_lianxu_months
	where 
		substr(next_month,1,6)>='202201' and substr(next_month,1,6)<='202212'
	)a
	left join
		(
		select 
...
;	
select * from csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v3;

-- ====================================================================================================================================================

-- 日配_至少A、B类有两个月的客户
drop table if exists csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v4;
create table csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v4
as
select 
	c.performance_province_name,
	a.customer_code,
...
from
	...
	from
		csx_analyse_tmp.csx_analyse_tmp_ripei_lianxu_months
	where 
		substr(next_month,1,6)>='202201' and substr(next_month,1,6)<='202210'
	)a
	left join
		(
		select 
			customer_code,substr(max(sdt),1,6) as max_month
		from 
			-- csx_dw.dws_sale_r_d_detail
			csx_dws.csx_dws_sale_detail_di
		where 
...
where
	b.max_month <='202210'
;	
select * from csx_analyse_tmp.csx_analyse_tmp_ripei_liucun_20230208_v4;
