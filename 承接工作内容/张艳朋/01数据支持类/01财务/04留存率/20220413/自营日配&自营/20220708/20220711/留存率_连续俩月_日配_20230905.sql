...
from
	...
	select 
		customer_code,substr(next_month,1,6) new_smonth,
		case when substr(next_month,1,6)<='202207' then substr(regexp_replace(add_months(from_unixtime(unix_timestamp(concat(next_month,'01'),'yyyyMMdd')),12),'-',''),1,6)
			else substr(regexp_replace(add_months(from_unixtime(unix_timestamp(concat(next_month,'01'),'yyyyMMdd')),cast(months_between('2023-07-01',to_date(from_unixtime(unix_timestamp(concat(next_month,'01'),'yyyyMMdd'))))as int)),'-',''),1,6)
		end as 12_months
