...
insert overwrite directory '/tmp/zhangyanpeng/20220705_01_01' row format delimited fields terminated by '\t' 
--结果表  
...
from
	(
	select distinct 
		customer_no,substr(normal_first_order_date,1,6) as new_smonth
	from 
		csx_dw.dws_crm_w_a_customer_active
	where 
		sdt = 'current' 
		and substr(normal_first_order_date,1,6) between '201901' and '202211'
	)a
	join
		(
		select 
