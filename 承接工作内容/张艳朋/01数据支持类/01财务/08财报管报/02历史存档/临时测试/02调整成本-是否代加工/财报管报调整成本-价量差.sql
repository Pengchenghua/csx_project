...
from
	...
	from 
		...
		where 
			...
			and period in(substr(${hiveconf:i_sdate_12},1,7))
		)a  --'2020-05'
		left join 
			(
			select 
		...
		left join 
			...
			where 
				sdt = 'current' and owner_ship = 'CITY'
			) city_company on a.company_code = city_company.code
	where city_company.code is null
	) a
left join 
	(
	select 
...
drop table csx_tmp.cbgb_tz_m_cbgb_jlc_djg;
create table csx_tmp.cbgb_tz_m_cbgb_jlc_djg
as 
select 
	...
	cast(amount as decimal(21,6)) as amount,
