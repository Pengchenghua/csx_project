select region_name,case when substr(sdt,1,6)<='202107' and province_name like '江苏%' then '江苏省' else province_name end province_name,
...
from csx_data_market.report_sale_r_d_achievement
where substr(sdt,1,6)='${EDATE}'
and (region_name not like '%大宗%' and region_name not like '%供应链%')
group by region_name,case when substr(sdt,1,6)<='202107' and province_name like '江苏%' then '江苏省' else province_name end,
