...
where sdt = '19990101'
and posting_time >= '2020-01-01 00:00:00' 
and  posting_time < '2020-02-01 00:00:00') a
left join 
...
and a.posting_time < '2020-02-01 00:00:00')a
left join 
(select if(shop_id like '9%',concat('E',substr(shop_id,2,3)),shop_id)shop_id,shop_name,province_name
from csx_dw.shop_m where sdt = 'current') b on b.shop_id=a.location_code
group by case when a.location_code='W0H4' then '供应链' else b.province_name end;
...
and attribution_date < '2020-02-01' )a
...
and commit_time<'2020-02-01 00:00:00')a
left join 
(select if(shop_id like '9%',concat('E',substr(shop_id,2,3)),shop_id)shop_id,shop_name,province_name
...
------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------------------
--第二部分 调整项对应的各明细附表 到课组 或到课组商品
--其中前两项对应附表6-附表7有王威出的4张表是否还需要出
--1、工厂月末分摊-调整销售订单--课组
insert overwrite directory '/tmp/raoyanhua/linshi01' row format delimited fields terminated by '\t' 
select  
case when a.location_code='W0H4' then '供应链' else b.province_name end province_name,
...
--4、附表9 盘盈 盘亏 amt_no_tax,amt 到商品课组
...
left join b2b_tmp.tmp_sale_order_flag c on a.wms_order_no=c.order_no and a.product_code=c.goods_code
...
from csx_dw.shop_m where sdt = 'current') b on b.shop_id=a.inventory_dc_code
where a.type in ('0','1')
and a.commit_time>='2020-01-01 00:00:00'
and a.commit_time<'2020-02-01 00:00:00')a
left join b2b_tmp.tmp_sale_order_flag c on a.original_order_no=c.origin_order_no and a.product_code=c.goods_code --渠道
left join --是否工厂商品
(select
    workshop_code, province_code, goods_code
  from csx_dw.dws_mms_w_a_factory_setting_craft_once_all
...
left join --课组
(select goods_id,goods_name,department_id,department_name
	from csx_dw.goods_m where sdt = 'current' )e on a.product_code=e.goods_id
