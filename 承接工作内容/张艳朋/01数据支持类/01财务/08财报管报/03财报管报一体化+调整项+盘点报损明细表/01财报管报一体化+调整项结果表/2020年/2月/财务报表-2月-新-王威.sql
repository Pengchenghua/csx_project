...
--数据源，处理省区、渠道信息
drop table b2b_tmp.cust_sales_m_v2;
create temporary table b2b_tmp.cust_sales_m_v2
as
select 
  ...
  b.attribute,b.first_category,b.second_category,b.third_category,b.sales_name,b.work_no,dc_company_code,dc_company_name,
  a.credential_no,a.goods_code,a.goods_name,a.order_no,a.department_code,a.department_name
from
(
  select 
    AA,credential_no,a.goods_code,goods_name,order_no,department_code,department_name,dc_company_code,dc_company_name,sdt,
  ...
  (
    select 
      ...
      if(customer_no not like '1%', concat('S',customer_no), customer_no) as customer_no,
  ...
  (
    select shop_id,province_code, company_code as dc_company_code,company_name as dc_company_name
    from csx_dw.shop_m where sdt = 'current'
  ...
  union all
  select 
    '销售'AA,''credential_no,goods_code,goods_name,order_no,department_code,department_name,dc_company_code,dc_company_name,
    sdt,
...
drop table b2b_tmp.cust_sales_m1_v2;
create temporary table b2b_tmp.cust_sales_m1_v2
as
select
  ...
  category_code,category_name,dc_company_code,dc_company_name,
...
(
  select
    ...
    a.category_code,c.category_name,dc_company_code,dc_company_name,
    ...
    case when a.shop_id in ('W0M1','W0M4','W0J6','W0M6') then '商超平台' 
      ...
      when customer_no='SW055'  then '上海市'
  ...
  (
    select 
      ...
      credential_no,goods_code,goods_name,order_no,department_code,department_name,sign_company_code,new_or_old,dc_company_code,dc_company_name,
      case when dc_code like 'E%' then concat('9',substr(dc_code,2,3)) else dc_code end shop_no 
    from b2b_tmp.cust_sales_m_v2 
  ) a left outer join
  (
    select
      shop_id,
...
drop table b2b_tmp.cust_sales_m2_v2;
create table b2b_tmp.cust_sales_m2_v2
as
select 
  sign_company_code,
  ...
  sales_value,excluding_tax_sales,sales_cost,excluding_tax_cost,profit,excluding_tax_profit,dc_company_code,dc_company_name,
...
(
  select
    ...
    attribute,first_category,second_category,third_category,sales_name,work_no,supervisor_id,is_factory_goods_name,dc_company_code,dc_company_name,
...
select sign_company_code,sum(channel_value) as channel_value,sum(caiwu_sales_value) as caiwu_sales_value,
  ...
  new_or_old,a.sdt,a.customer_no,a.customer_name,a.channel_name,a.province_code,a.province_name,city_real,a.shop_id,dc_company_code,dc_company_name,
...
from b2b_tmp.cust_sales_m2_v2 a 
where channel_name <> '商超'
group by sign_company_code,new_or_old,a.AA,a.sdt,a.customer_no,a.customer_name,a.channel_name,a.province_code,dc_company_code,dc_company_name,
  a.province_name,city_real,a.shop_id,a.attribute,a.first_category,a.second_category,third_category,a.sales_name,a.work_no,
  category_code,category_name,a.department_code,a.department_name,is_factory_goods_name

-- 按课组汇总（商超）
select
  sign_company_code,channel_value,caiwu_sales_value,excluding_tax_channel_value,excluding_tax_caiwu_sales_value,
  new_or_old,a.sdt,sales_belong_flag,a.customer_no,a.customer_name,a.channel_name,a.province_code,a.province_name,
  city_real,a.shop_id,dc_company_code,dc_company_name,a.attribute,a.first_category,a.second_category,third_category,
  a.sales_name,a.work_no,category_code,category_name,a.department_code,a.department_name,sales_value,excluding_tax_sales,
  sales_cost,excluding_tax_cost,profit,excluding_tax_profit,is_factory_goods_name
from
(
  select sign_company_code,sum(channel_value) as channel_value,sum(caiwu_sales_value) as caiwu_sales_value,
    sum(excluding_tax_channel_value) as excluding_tax_channel_value, sum(excluding_tax_caiwu_sales_value) as excluding_tax_caiwu_sales_value,
    new_or_old,a.sdt, customer_no,
    a.customer_name,a.channel_name,a.province_code,a.province_name,city_real,a.shop_id,dc_company_code,dc_company_name,
    a.attribute,a.first_category,a.second_category,third_category,a.sales_name,a.work_no,category_code,category_name,a.department_code,
    a.department_name,
    sum(a.sales_value) sales_value,sum(excluding_tax_sales) as excluding_tax_sales,
    sum(a.sales_cost) sales_cost,sum(a.excluding_tax_cost) excluding_tax_cost,
...
-- 1、就北京的发货地点，客户名称，客户编码，猪肉课，商品名称，销售额，成本。含税和不含税都要
-- (就是用周六我们抽的那个数据，和艳华一起对的财务数据，加一个渠道，M端和B端)
select
  b.province_code, b.province_name, channel_name, a.customer_no, a.customer_name, department_code, department_name, goods_code, goods_name,
  sales_value, excluding_tax_sales, sales_cost, excluding_tax_cost, profit, excluding_tax_profit
from
(
  select shop_id, channel_name, customer_no, customer_name, department_code, department_name, goods_code, goods_name,
    sum(sales_value) sales_value,sum(excluding_tax_sales) as excluding_tax_sales,
    sum(sales_cost) sales_cost,sum(excluding_tax_cost) excluding_tax_cost,
    sum(profit) profit, sum(excluding_tax_profit) excluding_tax_profit
  from b2b_tmp.cust_sales_m2_v2
  where department_code = 'H05'
  group by shop_id, channel_name, customer_no, customer_name, department_code, department_name, goods_code, goods_name
) a join
(
  select shop_id, province_code, province_name from csx_dw.shop_m where sdt = 'current' and province_code = '110000'
...
from
(
  select if(shop_id like 'E%', concat('9',substr(shop_id,2,3)), shop_id) as shop_id, 
    channel_name, customer_no, customer_name, department_code, department_name, goods_code, goods_name,
    sum(sales_value) sales_value,sum(excluding_tax_sales) as excluding_tax_sales,
    sum(sales_cost) sales_cost,sum(excluding_tax_cost) excluding_tax_cost,
    sum(profit) profit, sum(excluding_tax_profit) excluding_tax_profit
  from b2b_tmp.cust_sales_m2_v2
  group by if(shop_id like 'E%', concat('9',substr(shop_id,2,3)), shop_id), 
    channel_name, customer_no, customer_name, department_code, department_name, goods_code, goods_name
) a left join
(
  select shop_id, province_code, province_name from csx_dw.shop_m where sdt = 'current'
) b on a.shop_id = b.shop_id;
...
/*
select 'cust_sales_m_v2' aa,count(1) from b2b_tmp.cust_sales_m_v2
union all
select 'cust_sales_m1_v2' aa,count(1) from b2b_tmp.cust_sales_m1_v2
union all
select 'cust_sales_m2_v2' aa,count(1) from b2b_tmp.cust_sales_m2_v2;
*/
...
select * from b2b_tmp.region_dist_result02;
...
select * from b2b_tmp.region_result02;
