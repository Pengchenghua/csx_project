...
--csx_ods.source_sync_r_d_data_sync_sale_order  --data_sync_sale_order  wangwei
--19990101 分区 

--csx_ods.source_cas_r_d_accounting_credential_item   --csx_dw.dwd_cas_r_d_accounting_credential_detail
--csx_ods.source_sync_r_d_data_sync_sale_order_item  --data_sync_sale_order_item wangwei

-- 北京6月明细
drop table csx_tmp.cust_sign_detail_1;
create temporary table csx_tmp.cust_sign_detail_1
as
select
 b.province_code,
 b.province_name,d.channel,e.sales_belong_flag,c.received_cust_code,c.sale_cust_name,
 ...
 item.credential_no,item.move_type,item.direction,item.product_code product_code1,item.product_name product_name1,
 ...
 item.reservoir_area_code,item.reservoir_area_name,item.link_wms_order_no,
 item.company_code,item.company_name,item.wms_batch_no,item.move_name,item.source_order_no,item.link_wms_batch_no,
 ...
 orders.order_no,orders.product_code,orders.product_name,orders.spec,orders.pieces_num,orders.unit,
...
from
 ...
 and posting_time < '2020-07-01 00:00:00' 
 and (source_order_no not in ('OC200529000043','OC200529000044','OC200529000045','OC200529000046') or source_order_no is null)
) item  
left join (select * from csx_ods.source_sync_r_d_data_sync_sale_order_item 
where sdt='19990101') orders 
 on  orders.order_no = item.source_order_no and item.product_code = orders.product_code
left join 
(select shop_id,shop_name,province_code,province_name
from csx_dw.dws_basic_w_a_csx_shop_m where sdt = 'current') b on b.shop_id=item.location_code
left join 
(select if(sale_cust_code like '9%' or sale_cust_code like 'W%',concat('S',substr(received_cust_code,1,4)),sale_cust_code)received_cust_code,
...
select
    ...
    where sdt >= '20200630' and customer_no <> '' and customer_no is not null
)d on c.received_cust_code=d.customer_no
left join 
(select shop_id,shop_name,sales_belong_flag
from csx_dw.dws_basic_w_a_csx_shop_m where sdt = 'current') e on concat('S',e.shop_id)=c.received_cust_code
