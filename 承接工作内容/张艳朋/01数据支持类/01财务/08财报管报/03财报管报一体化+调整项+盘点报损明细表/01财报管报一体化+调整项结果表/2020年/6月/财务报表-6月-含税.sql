...
--省区大区对应关系
drop table csx_tmp.dist_region;
create table csx_tmp.dist_region
as
...
-- 新建表 顺序
--drop table csx_tmp.caibao_yue;
CREATE TABLE IF NOT EXISTS `csx_tmp.caibao_yue` (
...
--load data inpath '/tmp/raoyanhua/aa.csv' overwrite into table csx_tmp.caibao_yue;
--select * from csx_tmp.caibao_yue;


-- 销售额
drop table csx_tmp.y_region_dist_sales;
create temporary table csx_tmp.y_region_dist_sales
as
...
province_code,province_name,channel_name,sum(sales_value)/10000 sales_value
...
left join csx_tmp.dist_region b on b.dist_no=a.province_code
group by b.region,
case when a.channel_name like '大客户%' then 'B端' 
	when a.channel_name like '商超%' then 'M端' 
	when a.channel_name like '企业购%' then 'BBC' 
...
--销售额占比
drop table csx_tmp.y_region_dist_sales_rate01;
create temporary table csx_tmp.y_region_dist_sales_rate01
as
...
--定价毛利 (订单毛利)
drop table csx_tmp.y_region_dist_profit01;
create temporary table csx_tmp.y_region_dist_profit01
as
...
from (
select substr(sdt,1,6) smonth,
province_code,province_name,channel_name,sum(profit)/10000 profit
from csx_dw.dws_sale_r_d_customer_sale
where sdt>='20200501'
and sdt<'20200701'
and sales_type in ('sapqyg','sapgc','qyg','sc','bbc')
...
--定价毛利率
drop table csx_tmp.y_region_dist_prorate01;
create temporary table csx_tmp.y_region_dist_prorate01
as
...
from csx_tmp.y_region_dist_sales a
left join csx_tmp.y_region_dist_profit01 b on b.region=a.region and b.province_code=a.province_code and b.lable01=a.lable01 and b.smonth=a.smonth;



--定价成本 
drop table csx_tmp.y_region_dist_cost01;
create temporary table csx_tmp.y_region_dist_cost01
as
...
-- 总客户
-- 客户属性-人数（成交客户）
drop table csx_tmp.y_region_dist_cust_counts02;
create temporary table csx_tmp.y_region_dist_cust_counts02
as
...
-- 客户属性-销售额（成交客户）
drop table csx_tmp.y_region_dist_cust_sales;
create temporary table csx_tmp.y_region_dist_cust_sales
as
...
from 
(select substr(sdt,1,6) smonth,
province_code,province_name,customer_no,sum(sales_value)/10000 sales_value
from csx_dw.dws_sale_r_d_customer_sale
where sdt>='20200501'
and sdt<'20200701'
and sales_type in ('sapqyg','sapgc','qyg','sc','bbc')
...
drop table csx_tmp.tmp_cust_min_sale;
create table csx_tmp.tmp_cust_min_sale
as 
select
...
--CRM签约客户数
drop table csx_tmp.tmp_cust_count_info;
create table csx_tmp.tmp_cust_count_info
as 
...
left join csx_tmp.tmp_cust_min_sale b on a.customer_no=b.customer_no
left join csx_tmp.dist_region c on c.dist=a.province_name;
*/


 
-- 客户状态-人数
drop table csx_tmp.y_region_dist_cust_counts01;
create temporary table csx_tmp.y_region_dist_cust_counts01
as
select '05'NO_1,'总客户-客户数' lable,region,'成交客户'lable01,
province_code,province_name,smonth,sum(lable_value)lable_value 
from csx_tmp.y_region_dist_cust_counts02
group by region,province_code,province_name,smonth
union all
...
from csx_tmp.tmp_cust_count_info
...
from csx_tmp.tmp_cust_count_info
where sign_time>='20200501'
and sign_time<'20200701'
...
from csx_tmp.tmp_cust_count_info
where min_sales_date>='20200501'
and min_sales_date<'20200701'
group by substr(min_sales_date,1,6),
province_code,province_name
)a
left join csx_tmp.dist_region b on b.dist=a.province_name
group by b.region,a.lable01,a.province_code,a.province_name,a.smonth;


--总客户-客户属性-人数占比
drop table csx_tmp.y_region_dist_cust_counts02_rate;
create temporary table csx_tmp.y_region_dist_cust_counts02_rate
as
...
from csx_tmp.y_region_dist_cust_counts02 a
left join 
(select '05'NO_1,'成交客户' lable,region,'z小计'lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_cust_counts01  -- 客户状态-人数
where lable01='成交客户'
) b on b.region=a.region and b.province_code=a.province_code and b.smonth=a.smonth;


--总客户-客户属性-金额占比
drop table csx_tmp.y_region_dist_cust_sales_rate;
create temporary table csx_tmp.y_region_dist_cust_sales_rate
as
...
-- 结果表-拼接
drop table csx_tmp.y_region_dist_result01;
create table csx_tmp.y_region_dist_result01
as
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_sales  --销售额
union all
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_sales_rate01  --销售额占比
union all
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_cost01  --定价成本
union all
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_profit01  --定价毛利
union all
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_prorate01  --定价毛利率
union all
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_cust_counts01  --总客户-客户数
union all
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_cust_counts02  --总客户-成交客户人数
union all                    
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_cust_sales      --总客户-成交客户销售额
union all
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_cust_counts02_rate  --总客户-成交客户-客户属性-人数占比
union all                    
select NO_1,lable,region,lable01,province_code,province_name,smonth,lable_value
from csx_tmp.y_region_dist_cust_sales_rate      --总客户-成交客户-客户属性-金额占比
union all     
select '01'NO_1,'销售额' lable,region,'z小计'lable01,province_code,province_name,smonth,sum(lable_value)lable_value
from csx_tmp.y_region_dist_sales --销售额-z小计
group by region,province_code,province_name,smonth
union all
select '02'NO_1,'定价成本'lable,region,'z小计'lable01,province_code,province_name,smonth,sum(lable_value)lable_value
from csx_tmp.y_region_dist_cost01  --定价成本-z小计
group by region,province_code,province_name,smonth
union all
select '02'NO_1,'定价毛利'lable,region,'z小计'lable01,province_code,province_name,smonth,sum(lable_value)lable_value
from csx_tmp.y_region_dist_profit01  --定价毛利-z小计
group by region,province_code,province_name,smonth
union all                           --定价毛利率-z小计
...
-- 结果表-客户类型 
drop table csx_tmp.y_region_dist_result02;
create temporary table csx_tmp.y_region_dist_result02
as
...
a.smonth,sum(a.lable_value)lable_value
from csx_tmp.y_region_dist_result01 a
left join csx_tmp.caibao_yue b on a.lable=b.lable1 and a.lable01=b.lable_detail
...
-- 结果表-省区 
drop table csx_tmp.y_region_dist_result03;
create table csx_tmp.y_region_dist_result03
as
...
from csx_tmp.y_region_dist_result02 a
full join 
...
lable_value
from csx_tmp.y_region_dist_result02
where smonth<'202006' 
...
select * from csx_tmp.y_region_dist_result03;

------------------------------------------------------------------------------------------------
--------------- 大区
/*
--销售额占比
drop table csx_tmp.y_region_sales_rate01;
create temporary table csx_tmp.y_region_sales_rate01
as
...
from csx_tmp.y_region_dist_sales
...
--定价毛利率
drop table csx_tmp.y_region_prorate01;
create temporary table csx_tmp.y_region_prorate01
as
...
left join 
(select region,lable01,smonth,sum(lable_value)lable_value 
from csx_tmp.y_region_dist_profit01
group by region,lable01,smonth) b on b.region=a.region and b.lable01=a.lable01 and b.smonth=a.smonth;


--总客户-客户属性-人数占比
drop table csx_tmp.y_region_cust_counts02_rate;
create temporary table csx_tmp.y_region_cust_counts02_rate
as
...
--总客户-客户属性-金额占比
drop table csx_tmp.y_region_cust_sales_rate;
create temporary table csx_tmp.y_region_cust_sales_rate
as
select '06'NO_1,'总客户-成交客户销售额占比' lable,a.region,a.lable01,a.smonth,a.lable_value/b.lable_value lable_value
from 
(select region,lable01,smonth,sum(lable_value)lable_value 
from csx_tmp.y_region_dist_cust_sales
group by region,lable01,smonth ) a
left join 
...
-- 结果表-拼接
drop table csx_tmp.y_region_result01;
create table csx_tmp.y_region_result01
as
select NO_1,lable,region,lable01,smonth,sum(lable_value) lable_value
from csx_tmp.y_region_dist_result01
where NO_1<>'4' 
...
from csx_tmp.y_region_sales_rate01  --销售额占比
union all  
select NO_1,lable,region,lable01,smonth,lable_value
from csx_tmp.y_region_prorate01  --定价毛利率
union all  
select NO_1,lable,region,lable01,smonth,lable_value
from csx_tmp.y_region_cust_counts02_rate  --总客户-客户属性-人数占比
union all  
select NO_1,lable,region,lable01,smonth,lable_value
from csx_tmp.y_region_cust_sales_rate  --总客户-客户属性-金额占比
union all                           --定价毛利率-z小计
...
-- 结果表-客户类型 
drop table csx_tmp.y_region_result02;
create temporary table csx_tmp.y_region_result02
as
select b.no_1,b.no2_lable_detail,b.lable1,b.lable_detail,a.NO_1 NO_01,a.lable,a.region,a.lable01,
a.smonth,sum(a.lable_value)lable_value
from csx_tmp.y_region_result01 a
left join csx_tmp.caibao_yue b on a.lable=b.lable1 and a.lable01=b.lable_detail
group by b.no_1,b.no2_lable_detail,b.lable1,b.lable_detail,a.NO_1,a.lable,a.region,a.lable01,a.smonth;


-- 结果表-大区 
drop table csx_tmp.y_region_result03;
create table csx_tmp.y_region_result03
as
select coalesce(a.no_1,b.no_1),coalesce(a.no2_lable_detail,b.no2_lable_detail),coalesce(a.lable1,b.lable1),
coalesce(a.lable_detail,b.lable_detail),coalesce(a.no_01,b.no_01),coalesce(a.lable,b.lable),coalesce(a.region,b.region),coalesce(a.lable01,b.lable01),
...
from csx_tmp.y_region_result02 a
full join 
(select no_1,no2_lable_detail,lable1,lable_detail,no_01,lable,region,lable01,
substr(regexp_replace(add_months(to_date(from_unixtime(unix_timestamp(concat(smonth,'01'),'yyyyMMdd'))),1),'-',''),1,6) smonth,
lable_value
from csx_tmp.y_region_result02
where smonth<'202006' 
)b on a.no_1=b.no_1 and a.no2_lable_detail=b.no2_lable_detail 
and a.region=b.region and a.smonth=b.smonth;
...
select * from csx_tmp.y_region_result03;
*/
...
--销售额占比
drop table csx_tmp.y_all_sales_rate01;
create temporary table csx_tmp.y_all_sales_rate01
as
...
from csx_tmp.y_all_sales_rate01  --销售额占比
union all  
select NO_1,lable,lable01,smonth,lable_value
from csx_tmp.y_region_prorate01  --定价毛利率
union all  
