...
left join (select shop_id,shop_name,province_name,city_group_name as city_name
...
as 
select a.province_name,a.city_name,a.channel_name,
d.department_id dept_id,d.department_name dept_name,
sum(a.untax_sale)untax_sale,sum(a.sale)sale 
from 
	(select province_name,city_group_name city_name,channel_name,goods_code,sum(excluding_tax_sales)untax_sale,sum(sales_value)sale
	from csx_dw.dws_sale_r_d_detail
	where sdt>=${hiveconf:i_sdate_22}
	and sdt<${hiveconf:i_sdate_21}
	group by province_name,city_group_name,channel_name,goods_code)a 
left join (select * from csx_dw.dws_basic_w_a_csx_product_m where sdt='current' 
			) d on d.goods_id=a.goods_code			
group by a.province_name,a.city_name,a.channel_name,
...
from
	...
	group by province_name,city_name,dept_id,dept_name)a
left join 
	(select province_name,city_name,dept_id,
	sum(net_value) net_value_sr
	from csx_tmp.tmp_cbgb_tz_sr_1 
	group by province_name,city_name,dept_id 
