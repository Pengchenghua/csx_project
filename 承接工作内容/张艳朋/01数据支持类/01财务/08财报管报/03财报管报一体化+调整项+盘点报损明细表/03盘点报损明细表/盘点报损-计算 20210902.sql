...
drop table csx_tmp.pandian_post_no; 
create temporary table csx_tmp.pandian_post_no
as
select
	a.credential_no,
	b.province_name,   --省区
...
left join (select *  from  csx_dw.dws_basic_w_a_csx_product_m where sdt = 'current')c on c.goods_id = a.product_code;
	

insert overwrite directory '/tmp/raoyanhua/pandian01' row format delimited fields terminated by '\t'
select * from csx_tmp.pandian_post_no; 

	
--/*财务盘点*/	
--盘点过账	+省区字段
--insert overwrite directory '/tmp/raoyanhua/pandian02' row format delimited fields terminated by '\t'
drop table csx_tmp.pandian_post_yes; 
create temporary table csx_tmp.pandian_post_yes
as
select
	a.credential_no,
	b.province_name,   --省区
...
insert overwrite directory '/tmp/raoyanhua/pandian02' row format delimited fields terminated by '\t'
select * from csx_tmp.pandian_post_yes; 
...
select '业务盘点' pandian_group,*,amt amount
from csx_tmp.pandian_post_no
where reservoir_area_code='PD01'
and division_name in('食百','易耗品')
union all
select '盘点过账' pandian_group,*,amt_no_tax amount
from csx_tmp.pandian_post_yes
; 
...
--报损
insert overwrite directory '/tmp/raoyanhua/baosun01' row format delimited fields terminated by '\t'
 select
	e.province_name,   --省区
	e.city_name,   --城市 
	--a.wms_order_no,
...
left join (select * from csx_ods.source_cas_r_d_accounting_credential_header where sdt='19990101')header on a.credential_no=header.credential_no
left join 
  (
    select
    	a.*,b.reservoir_area_name,b.reservoir_area_attribute reservoir_area_prop
    from
    (
     select
    ...
    left join
    (select * from csx_ods.source_wms_w_a_wms_reservoir_area
    )b on a.location_code=b.warehouse_code and a.reservoir_area_code=b.reservoir_area_code
    where (b.reservoir_area_attribute='C' or b.reservoir_area_attribute='Y')
  ) c on c.credential_no=a.credential_no and c.product_code=a.product_code		
left join 
	(
...
left join (select *  from  csx_dw.dws_basic_w_a_csx_product_m where sdt = 'current')f on f.goods_id = a.product_code;
