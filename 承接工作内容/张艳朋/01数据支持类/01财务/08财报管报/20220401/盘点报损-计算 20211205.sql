	...
	left join 
		...
		from
			...
			select
				...
				sum(if(move_type in ('117B','118B'),-1*qty,qty)) qty,
				sum(if(move_type in ('117B','118B'),-1*amt_no_tax,amt_no_tax)) amt_no_tax,
				sum(if(move_type in ('117B','118B'),-1*amt,amt)) amt
			from 
			...
			group by 
				location_code,location_name,company_code,company_name,goods_code,goods_name,unit,price_no_tax,
				credential_no,posting_time,purchase_group_code,purchase_group_name,move_type,reservoir_area_code,wms_biz_type_code,
				wms_order_no,wms_biz_type_code,wms_biz_type_name,cost_center_code,cost_center_name				
			)a
			left join
				(
				select 
		...
		where 
			(b.reservoir_area_attribute='C' or b.reservoir_area_attribute='Y')
