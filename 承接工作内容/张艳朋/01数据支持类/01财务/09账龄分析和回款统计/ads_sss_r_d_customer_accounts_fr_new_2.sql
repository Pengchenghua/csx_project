...
insert overwrite table  csx_tmp.ads_sss_r_d_customer_accounts_fr_new partition(sdt)
select     
	x.customer_no,
	customer_name,
	channel_code,
	...
	temp_begin_time,
	temp_end_time,
	if(overdue_amount<0,0,overdue_amount) as overdue_amount,
	overdue_amount1,
	overdue_amount15,
	overdue_amount30,
	overdue_amount60,
...
from   
    ...
    select 
		...
		a.sdt
	from 
		--csx_dw.dws_sss_r_a_customer_accounts a 
        ...
        left outer join
			...
			where 
				sdt =regexp_replace(${hiveconf:e_date},'-','')
	...
	left outer join 
		...
		where 
			sdt='current' 
		) d on x.customer_no=d.customer_no
	left join 
		(
		select 
			* 
		...
		where 
			sdt='current' 
		) g on x.sales_id=g.sales_id and x.province_code=g.province_code
where
    1 = 1 
;
