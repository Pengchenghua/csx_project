...
select
	...
	a.performance_province_name,a.order_code,a.order_sale_date,a.order_sale_amt,a.customer_code,b.customer_name,a.goods_code,c.goods_name,a.sale_amt,a.rn
from
	(
	select
		a.performance_province_name,a.customer_code,a.order_code,a.goods_code,a.sale_amt,b.rn,c.order_sale_amt,c.order_sale_date
	from
		(
		select
			performance_province_name,customer_code,order_code,goods_code,sum(sale_amt) as sale_amt
		from
...
from
	(
	select
		a.performance_province_name,a.customer_code,a.order_code,a.goods_code,a.business_type_name,a.sale_amt,b.rn,c.order_sale_amt,c.order_sale_date
	from
		(
		select
			performance_province_name,customer_code,order_code,goods_code,business_type_name,sum(sale_amt) as sale_amt
		from
			csx_dws.csx_dws_sale_detail_di
		where 
			sdt>='20221001' and sdt<='20230228'
			...
			and performance_province_name='陕西省'
		group by 
			performance_province_name,customer_code,order_code,goods_code,business_type_name
		) a
		join
			(
			select
...
from
	...
	from
		...
		left join
			(
			select
				if(business_type_name='BBC',if(order_code='B2211180606585688A',order_code,substr(order_code,2,16)),order_code) as order_code,business_type_name,
			...
			where 
				...
				and performance_province_name='陕西省'
			group by 
				if(business_type_name='BBC',if(order_code='B2211180606585688A',order_code,substr(order_code,2,16)),order_code),business_type_name
			) c on c.order_code=a.original_order_code and c.business_type_name=a.business_type_name
	) a 
	left join
		(
...
select
	'20221001-20230228' as qijian,
	a.performance_province_name,a.order_code,a.sdt,a.customer_code,d.customer_name,a.business_type_name,a.sale_amt,a.delivery_type_name,
...
from
	(
	select
		performance_province_name,customer_code,order_code,sdt,'自营' as business_type_name,delivery_type_name,goods_code,sum(sale_amt) as sale_amt
	from
		csx_dws.csx_dws_sale_detail_di
	where 
		sdt>='20221001' and sdt<='20230228'
