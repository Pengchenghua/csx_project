...
select
	...
	case when a.sdt_month=b.first_sale_month then '当月新客'
		...
		when (a.next_1_month !='9999-12-31' and months_between(a.next_1_month,a.add_3_month)>=1) 
			or (a.next_1_month ='9999-12-31' and months_between('2023-06-01',a.add_3_month)>=1) then '预断约'
...
select * from csx_analyse_tmp.csx_analyse_tmp_oms_complaint_report_01;	
		...
		select 
			performance_province_name,substr(sdt,1,6) as smonth,customer_code,customer_name,
			sum(sale_amt) as sale_amt
		from 	
			csx_dws.csx_dws_sale_detail_di
		where 
			sdt>='20230401' and sdt<='20230630'
			and channel_code in('1','7','9')
			and business_type_code in(1)
		group by 
			performance_province_name,substr(sdt,1,6),customer_code,customer_name
