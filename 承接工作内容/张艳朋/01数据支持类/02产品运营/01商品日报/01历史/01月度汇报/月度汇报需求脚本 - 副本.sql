...
from
    ...
    from 
		csx_dw.dws_sale_r_m_customer_sale_archives
    where 
		month>='202004'
...
from
	(
  	select 
		...
		sum(sales_value) sales_value
	from 
		csx_dw.dws_sale_r_m_customer_sale_archives a
		left join
			( 
			select 
				shop_id,
...
from
	...
	select 
        ...
        sum(a.profit) profit
	from 
		csx_dw.dws_sale_r_m_customer_sale_archives a ---fengcun
		left join
			(
			select
				* 
...
from
    ...
    where 
		month in ('202007') 
		and month=substr(sales_date,1,6)
		and sales_type IN ('qyg','sapqyg','sapgc','sc','bbc')
		and channel IN (1,9,2,7) 
