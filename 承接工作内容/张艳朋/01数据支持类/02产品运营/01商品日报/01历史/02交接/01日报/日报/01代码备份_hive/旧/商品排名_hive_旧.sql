...
select
    concat(t1.sales_city,cast(row_number() over(partition by t1.sales_city order by t1.sales_value desc) as string)) as `辅助列`, 
	...
	t1.profit as`毛利额`,
	t1.AAA as `客户数`,
	row_number() over(partition by t1.sales_city order by t1.sales_value desc) as `排名`
from 
	(
	select 
		a.province_name,
