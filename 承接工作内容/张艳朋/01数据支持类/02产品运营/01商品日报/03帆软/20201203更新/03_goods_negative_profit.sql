...
insert overwrite table csx_tmp.ads_fr_goods_negative_profit_day partition(sdt)


select 
	province_name,sales_city,division_type,department_name,goods_code,goods_name,
	sales_value,profit,profit_rate,sales_qty,customer_cnt,days_cnt,rn,
...
from 
	...
	select
		...
		b.days_cnt,
		row_number()over(partition by a.sales_city order by a.profit asc) as rn
	from
		(
		select 
		...
		left join
			(
			select
				province_name,sales_city,goods_code,
			...
			group by 
				province_name,sales_city,goods_code
			) b on b.province_name=a.province_name and b.sales_city=a.sales_city and b.goods_code=a.goods_code
	)tmp1
where 
	rn<=10
;

INVALIDATE METADATA csx_tmp.ads_fr_goods_negative_profit_day;	


/*
--------------------------------- hive建表语句 -------------------------------
-- csx_tmp.ads_fr_goods_negative_profit_day  盘点报损-计算

drop table if exists csx_tmp.ads_fr_goods_negative_profit_day;
create table csx_tmp.ads_fr_goods_negative_profit_day(
...
`days_cnt`                 decimal(26,6)       COMMENT    '天数',
`rn`                       decimal(26,6)       COMMENT    '排名',
`update_time`              string              COMMENT    '更新时间'
) COMMENT 'zhangyanpeng:商品日报-负毛利'
