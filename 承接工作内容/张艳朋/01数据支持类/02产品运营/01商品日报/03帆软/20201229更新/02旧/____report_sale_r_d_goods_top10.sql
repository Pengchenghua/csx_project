...
(
  select 
    concat_ws('-', city_name, goods_code, substr(${hiveconf:current_day}, 1, 6)) as id,
    province_name,
    city_name,
    goods_code,
    goods_name,
    ...
    sum(profit) / sum(sales_value) as profit_prorate,
	sum(sales_qty) as sales_qty,
    count(distinct customer_no) as customer_amount,
    row_number() over(partition by city_name order by sum(sales_value) desc) as goods_sale_no
  ...
  group by province_name, city_name, goods_code, goods_name
)
select 
  *,
