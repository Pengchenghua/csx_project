...
CREATE temporary table csx_tmp.temp_hsale_bom
as
select 
	a.id,
	province_name,
...
CREATE temporary table csx_tmp.temp_hsale_bom_yc
as
select distinct
	a.shop_id_in,
...
CREATE temporary table csx_tmp.temp_hsale
as
select 
	'sale' gtype,
...
CREATE temporary table csx_tmp.temp_hsale1   --这一段是为了做销售占比的处理逻辑、
as
select 
	a.province_name,
	a.goods_code,
...
CREATE temporary table csx_tmp.temp_purprice
as
select DISTINCT
	stype,
	d.dept_id_channel,
...
(
	select distinct
		...
		case when shop_channel ='csx' then '彩食鲜' else '云超' end stype
	from 
		csx_dw.ads_sale_r_d_purprice_globaleye_shop		--自己建的码表
	where 
		sdt='current' 
		and
		shop_id <> 'W0B1'		--运营 ： 剔除四川W0B1仓的数据  
...
CREATE temporary table csx_tmp.temp_purprice_dept
as
select distinct
		stype,
...
CREATE temporary table csx_tmp.temp_purprice_dept1
as
select
	a.stype,
	a.province_name ,
...
--	if(b.goodsid is null or b.goodsid ='',a.sdt_c,b.sdt_c) as sdt_c,
	if(b.goodsid is null or b.goodsid ='',a.amount,b.amount) as amount,
	if(b.goodsid is null or b.goodsid ='',a.qty,b.qty) as qty,
	if(b.goodsid is null or b.goodsid ='',a.pur_price,b.pur_price) as pur_price  --周/月平均价格计算加权平均价 即总入库金额/总入库数量
from 
(
	select
		stype,
...
(
	select
		...
		sum(pur_price*pur_qty_in) as amount,
		sum(pur_qty_in) as qty,
		sum(pur_price*pur_qty_in)/sum(pur_qty_in) as pur_price  	--周平均价格
	from 
		csx_dw.ads_wms_r_d_fineReport_city_dept_purprice_globaleye_detail
	where 
		sdt=regexp_replace(date_sub(current_date,1),'-','') 
		and 
		sdt_c >=regexp_replace(date_sub(current_date,7),'-','')
	group by 
		stype,
		province_name ,
		city_name,
		...
		shop_id_in,
		shop_name
) as b	on a.stype=b.stype and a.province_name=b.province_name and a.city_name=b.city_name and a.shop_id_in=b.shop_id_in   and a.goodsid=b.goodsid
;
...
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao 
as 
select distinct
	a.province_name ,
	a.city_name,
...
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_amt 
as 
select 
	a.stype,
	a.province_name,
...
--获取 不同品类下的 商品金额和数量  
--所有省区 算全国、 按科组划分生鲜食百、 所有汇总算小计
--占比 分子  

--省区城市下 品类 商品维度
--作为分子

--下面段可以抽取出一个临时表、因为是后来改的逻辑没有优化 、复用代码比较多、、
--下面两段临时表是为了求  商品维度下 在  中类维度下 的占比

drop table csx_tmp.temp_purprice_dept_duibiao_amt_middle;
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_amt_middle 
as 
select
	x.stype,
	x.province_name ,
...
--获取 彩食鲜 省区下 指定课组 大类 中类 汇总金额  汇总数量
--所有省区算全国、  按课组划分  生鲜 、食百、 所有品类算小计 
...
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_amt_all 
as 
select
	x.stype,
	x.province_name ,
...
--权重 ： 以彩食鲜为基准 权重计算  云超也使用 彩食鲜占比

drop table csx_tmp.temp_purprice_dept_duibiao_amt_rno_goods;
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_amt_rno_goods
as 
select
	a.stype,
	a.province_name ,
...
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_amt_rno_pur_price 
as 
select	
	a.stype,
	a.province_name ,
...
CREATE temporary table csx_tmp.temp_purprice_dept_sku_avg_price
as
select
	a.province_name ,
	a.city_name,
...
CREATE temporary table csx_tmp.temp_purprice_dept_sku_tar
as
select
...
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_hight_fm 
as 
select
	'全国' as province_name,
	'全国' as city_name,
...
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_hight_fz_city
as
select 
	province_name,
	city_name,
...
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_hight_fz
as	
select
	province_name,
	city_name,	
...
CREATE temporary table csx_tmp.temp_purprice_dept_duibiao_hight_tar
as
select 
	province_name,
	city_name,
