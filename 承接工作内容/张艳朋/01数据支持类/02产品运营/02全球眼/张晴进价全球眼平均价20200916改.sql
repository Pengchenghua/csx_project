...
(
	...
	where 
		...
		sdt_c >=regexp_replace(date_sub(current_date,30),'-','')
		 and (catg_m_id not in ('110404') or dept_id not in ('H05'))
	group by 
		stype,
		province_name ,
		city_name,
...
(
	...
	group by 
		...
		--shop_id_in,
		--shop_name
) as b	on a.stype=b.stype and a.province_name=b.province_name and a.city_name=b.city_name   and a.goodsid=b.goodsid
union all
	...
	where 
		...
		and 
		sdt_c >=regexp_replace(date_sub(current_date,7),'-','')
		and   (catg_m_id  in ('110404') or dept_id='H05')
	group by 
		stype,
		province_name ,
		city_name,
