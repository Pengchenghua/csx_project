...
insert overwrite table csx_analyse.csx_analyse_fr_product_pool_indicator_di partition(sdt)
...
select
	...
	coalesce(h.create_date,'-') as create_date, -- 换品开始时间
	coalesce(h.base_goods_cnt,0) as base_goods_cnt, -- 基础商品池总数
	coalesce(h.base_purchase_price_goods_cnt,0) as base_purchase_price_goods_cnt, -- 有采购报价的基础商品数
	coalesce(h.base_guide_price_goods_cnt,0) as base_guide_price_goods_cnt, -- 有建议售价的基础商品数
	coalesce(h.base_sale_goods_cnt,0) as base_sale_goods_cnt, -- 有动销的基础商品数
	coalesce(h.base_goods_sale_amt,0) as base_goods_sale_amt, -- 有动销的基础商品日配销售额
...
from
	(
	select
		shop_code as inventory_dc_code,performance_region_code,performance_region_name,performance_province_code,performance_province_name,performance_city_code,performance_city_name
	from
		csx_dim.csx_dim_shop
	where
		sdt='current'
		and shop_code in  ('W0A3','W0P8','W0Q9','W0A2','W0BR','W0BH','W0N0','W0AS','W0A5','W0R9','W0A7','W0A6','W0Q2','W0A8','W0BK')
	) a 
	left join -- DC总日配销售额
		(
		select
				...
				left join
					...
					from
						...
						select
							...
							regexp_replace(to_date(lead(update_time,1,'9999-12-31')over(partition by inventory_dc_code,change_product_code,sap_cus_code order by update_time,id)),'-','') as next_finish_date
						from
							csx_ods.csx_ods_b2b_mall_prod_yszx_change_product_task_df
						where
							sdt='${ytd}'	
	...
	left join -- 通过下单添加至客户商品池的商品
		...
		from
			...
			where
				...
				and regexp_replace(to_date(updated_time),'-','') between concat(substr('${ytd}',1,6),'01') and '${ytd}'
			)a 
			left join
				(
				select
	...
	group by 
		...
		) h on h.inventory_dc_code=a.inventory_dc_code
;

/*

create table csx_analyse.csx_analyse_fr_product_pool_indicator_di(
...
`create_date`                           string              COMMENT    '换品开始时间',
`base_goods_cnt`                        decimal(26,6)       COMMENT    '基础商品池总数',
`base_purchase_price_goods_cnt`         decimal(26,6)       COMMENT    '有采购报价的基础商品数',
`base_guide_price_goods_cnt`            decimal(26,6)       COMMENT    '有建议售价的基础商品数',
`base_sale_goods_cnt`                   decimal(26,6)       COMMENT    '有动销的基础商品数',
`base_goods_sale_amt`                   decimal(26,6)       COMMENT    '有动销的基础商品日配销售额',
`dc_sale_amt`                           decimal(26,6)       COMMENT    'dc日配销售额',
`bind_common_product_customer_cnt`      decimal(26,6)       COMMENT    '绑定基础商品池的客户数',
`on_rule_customer_cnt`                  decimal(26,6)       COMMENT    '配置开启的客户数',
`need_on_rule_customer_cnt`             decimal(26,6)       COMMENT    '需开启总数',
`bind_common_product_customer_sale_amt` decimal(26,6)       COMMENT    '落地客户日配销售额',
`category_cnt`                          decimal(26,6)       COMMENT    '品类总数',
`config_th_rule_category_cnt`           decimal(26,6)       COMMENT    '配置汰换规则的品类数',
`base_dth_product_cnt`                  decimal(26,6)       COMMENT    '待替换商品为基础商品的数量',
`no_base_dth_product_cnt`               decimal(26,6)       COMMENT    '待替换商品不是基础商品的数量',
...
`total_sale_amt`                        decimal(26,6)       COMMENT    '总销售额',
`finish_sale_amt`                       decimal(26,6)       COMMENT    '完成商品销售额',
`refuse_sale_amt`                       decimal(26,6)       COMMENT    '拒绝商品销售额',
`xd_goods_cnt`                          decimal(26,6)       COMMENT    '通过下单添加至客户商品池的商品总数',
`base_xd_goods_cnt`                     decimal(26,6)       COMMENT    '基础商品数',
