...
insert overwrite table csx_analyse.csx_analyse_fr_product_pool_indicator_wi partition(sdt)
...
from
	(
	select
		sdt,shop_code as inventory_dc_code,performance_region_code,performance_region_name,performance_province_code,performance_province_name,performance_city_code,performance_city_name
	from
	...
	left join -- 通过下单添加至客户商品池的商品
		...
		from
			...
			where
				...
				and regexp_replace(to_date(updated_time),'-','') between regexp_replace(date_sub('${td_date}',if(pmod(datediff('${td_date}','2012-01-06'),7)=0,7,pmod(datediff('${td_date}','2012-01-06'), 7))),'-','') and '${ytd}'
			)a 
			left join
				(
				select
...
create table csx_analyse.csx_analyse_fr_product_pool_indicator_wi(
