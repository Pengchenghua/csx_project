...
-- 换品明细
drop table if exists csx_analyse_tmp.csx_analyse_tmp_yszx_change_product_task_02;
...
select
	...
	a.reject_reason,c.business_division_name,c.classify_large_name,c.classify_middle_name,c.classify_small_name
from
	(
	select
	...
	from
		...
		select
			...
			a.update_by,a.change_product_code,a.change_product_name,a.change_unit,a.create_time,a.update_time,a.create_date,a.month_ago_1,a.month_ago_2,a.month_ago_3,a.reject_reason,
		...
		from	
			...
			where
				...
				and to_date(create_time)>='2023-03-16'
				and inventory_dc_code in ('W0A3','W0A7','W0A8') -- 北京、重庆、福州
				and change_product_source=0 -- 换品来源  0-销售换品 1-运营换品
			) a 
			left join
				(
				select
	...
	join
		...
		where
			sdt='current'
			and business_division_code='12' -- 只看食百
		) c on c.goods_code=a.main_product_code
;
