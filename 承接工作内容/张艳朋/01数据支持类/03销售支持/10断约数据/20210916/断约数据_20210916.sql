--====================================================================================================================================
--断约
insert overwrite directory '/tmp/zhangyanpeng/20210916_break_contract' row format delimited fields terminated by '\t'
select
	a.province_name,
	a.customer_no,
	c.customer_name,
...
from 
	...
	left join   
		...
		from 
			csx_dw.dws_sale_r_d_detail 
		where 
			sdt between '20210701' and '20210915'
			and business_type_code in ('1') -- 业务类型编码(1.日配单 2.福利单 3.批发内购 4.城市服务商 5.省区大宗 6.BBC)
			and channel_code in('1','7','9') -- 渠道编码(1:大客户 2:商超 4:大宗 5:供应链(食百) 6:供应链(生鲜) 7:bbc 8:其他 9:业务代理)
			and province_name='北京市'
			and sales_type !='fanli'
	...
	left join
		(
		select
			customer_no,customer_name,work_no,sales_name,first_supervisor_work_no,first_supervisor_name,attribute_desc,cooperation_mode_name
		from
	...
	left join
		(
		select
			customer_no,customer_name,normal_last_order_date
		from 
