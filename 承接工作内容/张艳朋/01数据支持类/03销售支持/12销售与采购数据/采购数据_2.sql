--====================================================================================================================================
-- 采购
insert overwrite directory '/tmp/zhangyanpeng/20210113_wms_entry_2' row format delimited fields terminated by '\t'
...
from
	...
	from
		(
		select
			substr(sdt,1,6)mon,order_type_code,business_type,receive_location_code,goods_code,supplier_code,order_code,
			receive_qty,price*receive_qty as amount
		from
			csx_dw.dws_wms_r_d_entry_detail
		where
			sdt>='20200101' and sdt<='20201231'
			and (order_type_code like 'P%' or business_type in ('ZN01','ZN02','ZN03','ZC01'))
			and receive_location_code like 'W%'
		) as a
		left join --商品信息
			(
			select
