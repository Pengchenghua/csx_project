...
select	
	...
	a.s_sdt as `0301至0702日配业务下单次数`,
	a.sales_value as `0301至0702日配业务销售额`,
	a.profit as `0301至0702日配业务定价毛利额`,
	a.profit_rate as `0301至0702日配业务定价毛利率`,
	case when d.customer_no is not null then '是' else '否' end as `0703至0708是否下日配业务`,
	coalesce(d.sales_value,0) as `0703至0708日配业务销售额`,
	coalesce(d.profit,0) as `0703至0708日配业务定价毛利额`,
	coalesce(d.profit_rate,0) as `0703至0708日配业务定价毛利率`,
	case when e.customer_no is not null then '是' else '否' end as `0703至0708是否下其他业务`,
	coalesce(e.sales_value,0) as `0703至0708其他业务销售额`,
	coalesce(e.profit,0) as `0703至0708其他业务定价毛利额`,
	coalesce(e.profit_rate,0) as `0703至0708其他业务定价毛利率`
from
	(
	select
