...
--月度新客数
	...
	from
		...
		select 
			customer_no,
			business_type_name,
			substr(min(sdt),1,6) as first_month
		from 
			csx_dw.dws_sale_r_d_detail 
		where 
			sdt between '20190101' and '20211231'
...
from
	...
	join
		...
		group by 
			customer_no,
			business_type_name
		) b on b.customer_no=a.customer_no and b.business_type_name=a.business_type_name
	left join
		(
		select
			customer_no,customer_name,sales_id,work_no,sales_name,province_code,province_name,sales_region_code,sales_region_name,city_group_code,city_group_name
