...
set i_sdate_11 =regexp_replace('2020-12-31','-','');

--昨日月1日
set i_sdate_12 =regexp_replace(trunc('2020-12-31','MM'),'-','');

select
	concat_ws('-', substr(${hiveconf:i_sdate_12},5,4),substr(${hiveconf:i_sdate_11},5,4)) as statistic_month,
	province_name,city_group_name,first_supervisor_work_no,first_supervisor_name,work_no,sales_name,
...
from	
	(	
	select	
		first_date,customer_no,province_name,city_group_name,first_supervisor_work_no,first_supervisor_name,work_no,sales_name,
	...
	from	
		(
		select
			b.first_date,a.customer_no,a.province_name,a.city_group_name,c.first_supervisor_work_no,c.first_supervisor_name,a.work_no,
			a.sales_name,a.order_no,a.sales_value,a.profit,a.front_profit,a.profit_rate
		from
			(
			select
				customer_no,province_name,city_group_name,work_no,sales_name,order_no,
			...
			where 
				...
				and first_category_code in ('21', '23') -- 客户一级分类
				and business_type_code in ('1','2','6') -- 业务类型编码(1.日配单 2.福利单 3.批发内购 4.城市服务商 5.省区大宗 6.BBC)
			group by
				customer_no,province_name,city_group_name,work_no,sales_name,order_no
			) a 
			left join
			...
			left join 
				(
				select 
					customer_no,customer_name,first_category_code,first_category,attribute_code,attribute,first_supervisor_work_no,first_supervisor_name
				from 
...
--上月1日、上月最后1日
set i_sdate_22 =regexp_replace(add_months(trunc('2020-12-31','MM'),-1),'-','');
set i_sdate_23 =regexp_replace(last_day(add_months('2020-12-31',-1)),'-','');
...
select
	concat_ws('-', substr(${hiveconf:i_sdate_12},5,4),substr(${hiveconf:i_sdate_11},5,4)) as statistic_month,
	t1.province_name,t1.city_group_name,t1.first_supervisor_work_no,t1.first_supervisor_name,t1.work_no,t1.sales_name,
...
from
	...
	from
		(
		select
			customer_no,province_name,city_group_name,first_supervisor_work_no,first_supervisor_name,work_no,sales_name,order_no,count_days,
			sales_value,profit,front_profit,profit_rate
		from
			(
			select
				a.customer_no,a.province_name,a.city_group_name,b.first_supervisor_work_no,b.first_supervisor_name,a.work_no,a.sales_name,a.order_no,d.count_days,
			...
			from
				...
				from 
					csx_dw.dws_sale_r_d_detail
				where 
					sdt between '20201201' and '20201231'
					and channel_code in ('1', '7','9') --1大客户2商超4大宗5供应链食百6供应链生鲜7企业购9业务代理
				...
				left join 
					(
					select 
						customer_no,customer_name,first_category_code,first_category,attribute_code,attribute,work_no,first_supervisor_work_no,first_supervisor_name
					from 
						csx_dw.dws_crm_w_a_customer_m_v1
					where 
						sdt = '20201231'
...
select
	concat_ws('-', substr(${hiveconf:i_sdate_12},5,4),substr(${hiveconf:i_sdate_11},5,4)) as statistic_month,
	province_name,city_group_name,supervisor_work_no,supervisor_name,work_no,sales_name,
	sum(sales_value) as sales_value,
	sum(front_profit) as front_profit,
	sum(front_profit)/sum(sales_value) as front_profit_rate,
	count(distinct customer_no) as cust_count,
...
from	
	(
	select
		b.first_date,a.customer_no,a.province_name,a.city_group_name,a.supervisor_work_no,a.supervisor_name,a.work_no,a.sales_name,
		a.sales_value,a.front_profit,a.sales_value/(${hiveconf:i_sdate_11}-b.first_date+1) as avg_sales_value
	from
		(
		select
			customer_no,province_name,city_group_name,supervisor_work_no,supervisor_name,work_no,sales_name,
			sum(sales_value) as sales_value, --含税销售额
			sum(profit) as profit, --含税毛利额
			sum(front_profit) as front_profit, --前端含税毛利
			sum(profit)/sum(sales_value) as profit_rate
		...
		where 
			sdt between ${hiveconf:i_sdate_12} and ${hiveconf:i_sdate_11}
			and channel_code in ('7') --1大客户2商超4大宗5供应链食百6供应链生鲜7企业购9业务代理
			and first_category_code in ('21', '23') --21企事业单位 22商贸批发 23餐饮企业 24食品加工企业 25个人及其他
			and business_type_code in ('1','2','6') -- 业务类型编码(1.日配单 2.福利单 3.批发内购 4.城市服务商 5.省区大宗 6.BBC)
		group by
			customer_no,province_name,city_group_name,supervisor_work_no,supervisor_name,work_no,sales_name
...
select
	concat_ws('-', substr(${hiveconf:i_sdate_12},5,4),substr(${hiveconf:i_sdate_11},5,4)) as statistic_month,
	province_name,city_group_name,supervisor_work_no,supervisor_name,
	sum(sales_value) as sales_value,
	sum(front_profit) as front_profit,
	sum(front_profit) / sum(sales_value) as front_profit_rate,
	count(distinct customer_no) as cust_count,
...
from	
	(	
	select	
		first_date,customer_no,province_name,city_group_name,supervisor_work_no,supervisor_name,
		sum(sales_value) as sales_value,sum(front_profit) as front_profit,
		sum(sales_value) / (${hiveconf:i_sdate_11}-first_date+1) as avg_sales_value
	from	
		(
		select
			b.first_date,a.customer_no,a.province_name,a.city_group_name,a.supervisor_work_no,a.supervisor_name,
			a.order_no,a.sales_value,a.profit,a.front_profit,a.profit_rate
		from
			(
			select
				customer_no,province_name,city_group_name,supervisor_work_no,supervisor_name,order_no,
				sum(sales_value) as sales_value, --含税销售额
				sum(profit) as profit, --含税毛利额
				sum(front_profit) as front_profit, --前端含税毛利
				sum(profit)/sum(sales_value) as profit_rate
...
from	
	...
	from
		(
		select
			customer_no,province_name,city_group_name,supervisor_work_no,supervisor_name,
			sum(sales_value) as sales_value, --含税销售额
			sum(profit) as profit, --含税毛利额
			sum(front_profit) as front_profit, --前端含税毛利
			sum(profit)/sum(sales_value) as profit_rate
