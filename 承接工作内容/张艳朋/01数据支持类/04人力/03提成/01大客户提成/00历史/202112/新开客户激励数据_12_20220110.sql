...
-- 开始日期
set start_day = '20211001';

-- 结束日期
set end_day = '20211231';

select
	a.province_name,b.work_no,b.sales_name,a.customer_no,b.customer_name,c.normal_first_order_date,a.sales_value,
...
from
	...
	where 
		sdt between ${hiveconf:start_day} and ${hiveconf:end_day}
	...
	join
		...
		where 
			sdt = ${hiveconf:end_day}
			and customer_no<>''	
		) b on b.customer_no = a.customer_no
	join
		(
		select 
			customer_no,customer_name,normal_first_order_date,normal_last_order_date
		from 
			csx_dw.dws_crm_w_a_customer_active
		where 
			sdt = ${hiveconf:end_day}
			and substr(normal_first_order_date,1,6) =substr(${hiveconf:start_day},1,6) --月首次成交
...
-- 开始日期
set start_day = '20210601';

-- 结束日期
set end_day = '20210831';
...
from
	...
	where 
		...
		and business_type_code in ('1') -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
		and channel_code in('1','7','9') -- 渠道编码(1:大客户 2:商超 4:大宗 5:供应链(食百) 6:供应链(生鲜) 7:bbc 8:其他 9:业务代理)
		and customer_no in ('103145','103207')
	group by 
		province_name,customer_no
	) a
	join
