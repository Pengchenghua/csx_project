...
select 
	...
	sum(customer_fuli_bbc_sales_value) as customer_fuli_bbc_sales_value
from 
	(
	select 
		...
		-- 福利+BBC销售额
		sum(case when dc_code !='W0K4' and business_type_code in('2','6') then sales_value else 0 end) as customer_fuli_bbc_sales_value		
	from 
		csx_dw.dws_sale_r_d_detail
	where 
...
from
	...
	from
		csx_tmp.tc_customer_sales_value_profit_00
	group by 
		work_no,sales_name
	) a 
;
...
select 
	...
	a.customer_fuli_bbc_sales_value
from
