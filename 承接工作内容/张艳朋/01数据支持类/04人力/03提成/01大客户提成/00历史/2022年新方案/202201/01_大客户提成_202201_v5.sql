...
from 
	...
	select 
		...
		-- BBC销售额、定价毛利额
		sum(case when dc_code !='W0K4' and business_type_code in('6') then sales_value else 0 end) as customer_bbc_sales_value,
		sum(case when dc_code !='W0K4' and business_type_code in('6') then profit else 0 end) as customer_bbc_profit
	from 
		csx_dw.dws_sale_r_d_detail
	where 
