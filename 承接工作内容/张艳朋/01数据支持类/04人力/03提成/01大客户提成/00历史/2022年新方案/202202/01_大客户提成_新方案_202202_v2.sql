...
select 
	...
	coalesce(tc_bbc_sales_value_salesperson,0)+coalesce(tc_bbc_profit_salesperson,0) as total_salesperson,
	coalesce(tc_sales_value_service,0)+coalesce(tc_profit_service,0) as total_service
from
...
select
	...
	coalesce(sum(tc_bbc_sales_value_salesperson),0)+coalesce(sum(tc_bbc_profit_salesperson),0) tc_salesperson
from 
	csx_tmp.tc_new_cust_salary
group by 
	smonth,sales_province_name,work_no,sales_name,
...
select
	...
	coalesce(sum(tc_sales_value_service),0)+coalesce(sum(tc_profit_service),0) tc_service
from 
	csx_tmp.tc_new_cust_salary
where
