...
as
select 
	b.sales_province_name,b.city_group_name,a.customer_no,b.customer_name,a.smonth,
	-- 销售额
...
from 
	...
	where 
		sdt>=${hiveconf:month_start_day} and sdt<=${hiveconf:month_end_day}
			...
			and customer_no in ('114834','111832','123685','120723','124367','124387','124416','119760','121425','121841','123553')
...
as
select 
	a.sales_province_name,a.city_group_name,a.customer_no,a.customer_name,a.smonth,
...
select
	...
	0.02 as salesperson_ripei_bbc_profit_tc_rate,
	-- 福利定价毛利额提成比例
	...
	0.02 as salesperson_fuli_profit_tc_rate
from
	(
	select 	
		work_no,sales_name,
...
as
select 
	a.smonth,a.sales_province_name,a.city_group_name,a.customer_no,a.customer_name,a.work_no,a.sales_name,a.is_part_time_service_manager,a.service_user_work_no,a.service_user_name,
...
from  
	(
	select 
		sales_province_name,city_group_name,customer_no,customer_name,work_no,sales_name,is_part_time_service_manager,service_user_work_no,service_user_name,
		salesperson_sales_value_fp_rate, --销售员_销售额_分配比例
...
select
	...
	if(a.service_fee_2 is not null,service_fee_2,if(a.service_fee is not null,a.service_fee,
...
select
	...
	if(customer_no in ('124524'),tc_salesperson*0.5,tc_salesperson) as tc_salesperson,
	if(customer_no in ('124524'),tc_service*0.5,tc_service) as tc_service
from
...
select
	smonth,b.province_name,b.city_group_name,work_no,sales_name,
...
from 
	...
	left join csx_tmp.tc_person_info b on b.user_number=a.work_no
group by 
	smonth,b.province_name,b.city_group_name,work_no,sales_name,salesperson_sales_value_ytd,
...
select
	smonth,b.province_name,b.city_group_name,service_user_work_no,service_user_name,
	sum(customer_sales_value) as customer_sales_value,
	sum(customer_ripei_sales_value) as customer_ripei_sales_value,
	sum(customer_bbc_sales_value) as customer_bbc_sales_value,
	sum(customer_fuli_sales_value) as customer_fuli_sales_value,	
...
from 
	csx_tmp.tc_new_cust_salary a 
	left join csx_tmp.tc_person_info b on b.user_number=a.service_user_work_no
where
	service_user_work_no !=''
group by 
	smonth,b.province_name,b.city_group_name,service_user_work_no,service_user_name,service_user_over_rate
;
