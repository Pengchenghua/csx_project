...
select 
	...
	a.salary_bbc_sales_value*(1-coalesce(if(a.salesperson_over_rate<=0.5,a.salesperson_over_rate,1),0)) as salary_bbc_sales_value_salesperson,
	a.salary_bbc_profit*(1-coalesce(if(a.salesperson_over_rate<=0.5,a.salesperson_over_rate,1),0)) as salary_bbc_profit_salesperson,
	--提成_销售额_销售员
	...
	a.salary_ripei_sales_value*(1-coalesce(if(a.salesperson_over_rate<=0.5,a.salesperson_over_rate,1),0))*coalesce(a.salesperson_sales_value_fp_rate,0) as salary_sales_value_salesperson, --提成_销售额_销售员
	--提成_销售额_服务管家
	a.salary_ripei_sales_value*(1-coalesce(if(a.service_user_over_rate<=0.5,a.service_user_over_rate,1),0))*coalesce(a.service_user_sales_value_fp_rate,0) salary_sales_value_service,--提成_销售额_服务管家
	--提成_定价毛利额_销售员
	...
	a.salary_ripei_profit*(1-coalesce(if(a.salesperson_over_rate<=0.5,a.salesperson_over_rate,1),0))*coalesce(a.salesperson_profit_fp_rate,0) salary_profit_salesperson,--提成_定价毛利额_销售员
	--提成_定价毛利额_服务管家
	a.salary_ripei_profit*(1-coalesce(if(a.service_user_over_rate<=0.5,a.service_user_over_rate,1),0))*coalesce(a.service_user_profit_fp_rate,0) salary_profit_service, --提成_定价毛利额_服务管家
...
select 
	...
	coalesce(salary_fuli_sales_value_salesperson,0)+coalesce(salary_fuli_profit_salesperson,0)+
	coalesce(salary_bbc_sales_value_salesperson,0)+coalesce(salary_bbc_profit_salesperson,0) as total_salesperson,
...
select
	...
	coalesce(sum(salary_fuli_sales_value_salesperson),0)+coalesce(sum(salary_fuli_profit_salesperson),0)+
	coalesce(sum(salary_bbc_sales_value_salesperson),0)+coalesce(sum(salary_bbc_profit_salesperson),0) salary_sale
from 
	csx_tmp.tc_new_cust_salary
group by 
	smonth,sales_province_name,work_no,sales_name,
