...
from 
	...
	--河南省 当月
	union all select '127298' as customer_no,'202205' as smonth,0 as sales_value, 0 as rp_sales_value,0 as bbc_sales_value,0 as fl_sales_value,
	-3380.00 as profit,0.00 as rp_profit, 0.00 as bbc_profit, -3380.00 as fl_profit
...
select 
	...
	--'126259','127312','127336' 毛利单独核算 每月
	a.rp_sales_value*coalesce(f.sales_rp_bbc_sales_value_tc_rate,0.002) as salary_rp_sales_value, -- 奖金包_日配销售额
	a.rp_profit*(case when a.customer_no in ('116566','120723','112906','118509','117817','121472','121994','124356','124367','124387','124401','124416','119760','121841','115221',
		'126084') then 0.02
		when a.customer_no in ('126259') then 0.03
		when a.customer_no in ('127312') then 0
		when a.customer_no in ('127336') then 0.03
...
from
	...
	select 
		...
		--'126259','127312','127336' 单独核算毛利提成 每月
		...
		a.salary_rp_profit*(1-coalesce(if(a.sales_over_rate<=0.5,a.sales_over_rate,1),0))*coalesce(a.rp_sales_profit_fp_rate,0)*
			if(a.customer_no in ('123311','124033','118376','124652','126069','126113'),0,if(a.customer_no in ('124524'),0.5,1)) as tc_rp_profit_sales,		
		...
		a.salary_fl_profit*(1-coalesce(if(a.sales_over_rate<=0.5,a.sales_over_rate,1),0))*coalesce(a.fl_sales_profit_fp_rate,0)*
			if(a.customer_no in ('123311','124033','118376','124652','126069','126113'),0,if(a.customer_no in ('124524'),0.5,1)) as tc_fl_profit_sales,
		...
		a.salary_bbc_profit*(1-coalesce(if(a.sales_over_rate<=0.5,a.sales_over_rate,1),0))*coalesce(a.bbc_sales_profit_fp_rate,0)*
			if(a.customer_no in ('123311','124033','118376','124652','126069','126113'),0,if(a.customer_no in ('124524'),0.5,1)) as tc_bbc_profit_sales,		
		...
		a.salary_rp_profit*(1-coalesce(if(a.rp_service_user_over_rate<=0.5,a.rp_service_user_over_rate,1),0))*coalesce(a.rp_service_user_profit_fp_rate,0)*
			if(a.customer_no in ('123311','124033','118376','124652','126069','126113'),0,if(a.customer_no in ('124524'),0.5,1)) as tc_rp_profit_service,	
		...
		a.salary_fl_profit*(1-coalesce(if(a.fl_service_user_over_rate<=0.5,a.fl_service_user_over_rate,1),0))*coalesce(a.fl_service_user_profit_fp_rate,0)*
			if(a.customer_no in ('123311','124033','118376','124652','126069','126113'),0,if(a.customer_no in ('124524'),0.5,1)) as tc_fl_profit_service,
		...
		a.salary_bbc_profit*(1-coalesce(if(a.bbc_service_user_over_rate<=0.5,a.bbc_service_user_over_rate,1),0))*coalesce(a.bbc_service_user_profit_fp_rate,0)*
			if(a.customer_no in ('123311','124033','118376','124652','126069','126113'),0,if(a.customer_no in ('124524'),0.5,1)) as tc_bbc_profit_service,			
