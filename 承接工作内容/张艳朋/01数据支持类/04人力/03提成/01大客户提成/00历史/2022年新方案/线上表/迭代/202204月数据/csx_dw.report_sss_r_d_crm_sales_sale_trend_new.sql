...
drop table if exists  csx_tmp.tmp_report_sss_r_d_crm_sales_sale_trend_1;
create table csx_tmp.tmp_report_sss_r_d_crm_sales_sale_trend_1
as
select
	a.*,
	a2.customer_name,
...
from
	...
	left join
		(
		select 
			substr(sdt,1,6) as smonth,customer_no,customer_name,work_no,sales_name,sales_id
		from 
...
--刷新数据
insert overwrite table csx_tmp.report_sss_r_d_crm_sales_sale_trend_new partition(sdt)
select
	concat_ws('&',cast(sales_id as string),sdt) as biz_id,
	sales_id,
	work_no,
	sales_name,
	sdt as sales_date,
	...
	sum(customer_refund_ripei_sales_value) as ripei_refund_sales_value,
	sum(customer_refund_bbc_sales_value) as bbc_refund_sales_value,
	sum(customer_refund_ripei_bbc_sales_value) as ripei_bbc_refund_sales_value,
	sum(customer_refund_fuli_sales_value) as fuli_refund_sales_value,
	substr(sdt,1,6) as smonth,
	from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as updated_time,
	sdt
from
	csx_tmp.tmp_report_sss_r_d_crm_sales_sale_trend_1
where 
    work_no <>''
