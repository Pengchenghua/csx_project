...
--昨天所在年第一天
set last_1day_year_first_day=regexp_replace(trunc(date_sub(current_date,1),'YEAR'),'-','');
--昨天所在月第一天
...
set target_table = csx_tmp.report_sss_r_m_crm_customer_commission_detail;
	

-- 创建人员信息表，获取销售员和服务管家的城市，因为存在一个业务员名下客户跨城市的情况
drop table if exists csx_tmp.tc_employee_org;
create table csx_tmp.tc_employee_org
as
select 
...
;
-- 创建客户对应销售员和服务管家
drop table if exists csx_tmp.tc_customer_sales_service;
create table csx_tmp.tc_customer_sales_service
as
select 
	...
	rp_sales_sale_rate as rp_sales_sale_fp_rate,
	rp_sales_profit_rate as rp_sales_profit_fp_rate,			
	fl_sales_sale_rate as fl_sales_sale_fp_rate,
	fl_sales_profit_rate as fl_sales_profit_fp_rate,
	bbc_sales_sale_rate as bbc_sales_sale_fp_rate,
	bbc_sales_profit_rate as bbc_sales_profit_fp_rate,
	rp_service_user_sale_rate as rp_service_user_sale_fp_rate,
	rp_service_user_profit_rate as rp_service_user_profit_fp_rate,
	fl_service_user_sale_rate as fl_service_user_sale_fp_rate,
	fl_service_user_profit_rate as fl_service_user_profit_fp_rate,
	bbc_service_user_sale_rate as bbc_service_user_sale_fp_rate,     
	bbc_service_user_profit_rate as bbc_service_user_profit_fp_rate,
	is_sale,is_overdue
from 
	csx_tmp.report_crm_w_a_customer_service_manager_info_business_new
where 
	month=substr(${hiveconf:last_1day},1,6)
;			
-- 销售员年度累计销额提成比例
...
from
	csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_2 a 
	left join
		(
		--202201月签呈，重庆市 每月
	...
	left join		
		...
		select 
			*
		from 
			csx_tmp.tc_customer_sales_service
...
drop table if exists csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_4;
...
from
	...
	from
		csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_3
	where
...
--逾期处理
drop table if exists csx_tmp.tc_customer_commission_over_rate_0;
create table csx_tmp.tc_customer_commission_over_rate_0 
as 
select
	a.sdt,
...
from	
	...
	where
		sdt=${hiveconf:last_1day}
		--202202月签呈，剔除逾期，每月
		and customer_code not in ('104192','123395','117927','119925')
		--202202月签呈，客户已转代理人，剔除逾期，每月
		and customer_code not in ('122221','123086')
		--202202月签呈，剔除逾期，每月
		and customer_code not in ('116661','103369','104817','105601','104381','105304','105714','118794','120595','120830','122837','116932','119209','119022','119214','119257',
		'113425','112129')
		--202202月签呈，公司BBC客户，不算提成，每月
		and customer_code not in ('123623')
		--202202月签呈，每月
		and customer_code not in ('106626','112779','113122','124976','119977','102754')
		--202202月签呈，剔除逾期，每月
		and customer_code not in ('106526','120317','119906','108127','105838','105806')
		--202203月签 剔除逾期 每月
		and customer_code not in ('120314','120426','111120','105235','105557')
		--202203月签呈，剔除逾期 每月
		and customer_code not in ('120314','120426','104192','123395','117927','115393','119925','122495','115393','105696')
		--202203月签呈 不算提成和逾期 每月 
		and customer_code not in ('104192','123395','117927','126154')
		--202204月签呈 剔除逾期 每月
		and customer_code not in ('104596','121244','121248','121259','121274','121286','121305')
		--202204月签呈 剔除逾期 每月
		and customer_code not in ('112718','124486','120417')	
		--202204月签呈 不算提成和逾期 每月 
		and customer_code not in ('120459','121206','102524','111204','109377')	
	) a 
	--剔除业务代理与内购客户
	join		
		(
...
--销售员 粒度 的逾期系数
drop table if exists csx_tmp.tc_customer_commission_over_rate_1;
create table csx_tmp.tc_customer_commission_over_rate_1 
as 
select 
	a.channel_name,	-- 渠道
	d.work_no,	-- 销售员工号
...
from
	...
	select
		...
		overdue_coefficient_denominator -- 应收金额*账期天数 计算因子，用于计算逾期系数分母
	from 
		csx_tmp.tc_customer_commission_over_rate_0  
	where 
		channel_name = '大客户' 
		and sdt = ${hiveconf:last_1day} 	
	)a
...
;
--服务管家 粒度 的逾期系数
drop table if exists csx_tmp.tc_customer_commission_over_rate_2;
create table csx_tmp.tc_customer_commission_over_rate_2
as 
select 
...
;
--客户 粒度 的逾期系数
drop table if exists csx_tmp.tc_customer_commission_over_rate_3;
create table csx_tmp.tc_customer_commission_over_rate_3
as 
select 
	channel_name,	-- 渠道
	customer_no,	-- 客户编码
	...
	coalesce(round(
		...
		/(sum(case when overdue_coefficient_denominator>=0  and receivable_amount>0 then overdue_coefficient_denominator else 0 end)) end, 6),0) as over_rate -- 逾期系数 
from 
	csx_tmp.tc_customer_commission_over_rate_0 a 
where 
	channel_name = '大客户' 
	and sdt = ${hiveconf:last_1day} 
group by 
...
;
--客户本月提成，未乘分配比例
drop table if exists csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_5;
create table csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_5
as
select 
...
from  
	...
	left join csx_tmp.tc_customer_commission_over_rate_3 b on b.customer_no=a.customer_no
	left join csx_tmp.tc_customer_commission_over_rate_1 c on c.sales_name=a.sales_name and coalesce(c.work_no,0)=coalesce(a.work_no,0)
	left join csx_tmp.tc_customer_commission_over_rate_2 d on d.service_user_name=a.rp_service_user_name and coalesce(d.service_user_work_no,0)=coalesce(a.rp_service_user_work_no,0)
	left join csx_tmp.tc_customer_commission_over_rate_2 d2 on d2.service_user_name=a.fl_service_user_name and coalesce(d2.service_user_work_no,0)=coalesce(a.fl_service_user_work_no,0)
	left join csx_tmp.tc_customer_commission_over_rate_2 d3 on d3.service_user_name=a.bbc_service_user_name and coalesce(d3.service_user_work_no,0)=coalesce(a.bbc_service_user_work_no,0)
	left join csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_4 e on e.work_no=a.work_no and e.sales_name=a.sales_name
	left join csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_1 f on f.work_no=a.work_no and f.sales_name=a.sales_name
;

--客户本月提成，乘分配比例
drop table if exists csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_6;
create table csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_6
as
select
	a.smonth,a.sales_region_code,a.sales_region_name,a.province_code,a.province_name,a.city_group_code,a.city_group_name,a.customer_id,a.customer_no,a.customer_name,
...
from
	...
	select 
		...
		refund_rp_bbc_sales_value,refund_fl_sales_value
	from
		csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_5 a 
...
--客户提成详情
drop table if exists csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_7; 
create table csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_7
as
select
	a.smonth,
	--客户地区信息
...
from
	csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_6 a 
	left join csx_tmp.tc_employee_org c on c.id=a.sales_id
	left join csx_tmp.tc_employee_org d on d.user_number=split(a.rp_service_user_work_no,';')[0]
	left join csx_tmp.tc_employee_org d2 on d2.user_number=split(a.fl_service_user_work_no,';')[0]
	left join csx_tmp.tc_employee_org d3 on d3.user_number=split(a.bbc_service_user_work_no,';')[0]
where
	a.customer_id <>''
...
from  
	csx_tmp.report_sss_r_m_crm_sales_customer_commission_new_7
;
