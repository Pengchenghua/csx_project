...
set month_start_day =regexp_replace(add_months(trunc(date_sub(current_date,1),'MM'),0),'-','');
...
select
	...
	coalesce(d.region_code,'') as region_code,
	coalesce(d.region_name,'') as region_name,
	coalesce(d.province_code,'') as province_code,
	coalesce(d.province_name,'') as province_name,
	coalesce(d.city_group_code,'') as city_group_code,
	coalesce(d.city_group_name,'') as city_group_name,
	substr(${hiveconf:last_1day},1,6) as yearmonth,
	...
	coalesce(d.sales_id_new,'') as sales_id,
	coalesce(d.work_no_new,'') as work_no,	-- 销售员工号
	coalesce(d.sales_name_new,'') as sales_name,	-- 销售员
	
	coalesce(d.rp_service_user_id_new,'') as rp_service_user_id,
	coalesce(d.rp_service_user_work_no_new,'') as rp_service_user_work_no,
	coalesce(d.rp_service_user_name_new,'') as rp_service_user_name,
	
	coalesce(d.fl_service_user_id_new,'') as fl_service_user_id,
	coalesce(d.fl_service_user_work_no_new,'') as fl_service_user_work_no,
	coalesce(d.fl_service_user_name_new,'') as fl_service_user_name,	
	
	coalesce(d.bbc_service_user_id_new,'') as bbc_service_user_id,
	coalesce(d.bbc_service_user_work_no_new,'') as bbc_service_user_work_no,
