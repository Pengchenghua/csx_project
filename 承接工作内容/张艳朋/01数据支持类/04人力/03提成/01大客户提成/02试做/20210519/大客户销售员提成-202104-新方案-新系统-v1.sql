...
--★★★合并返利新增  		
	...
	left join 
		...
		select distinct customer_no,customer_name,work_no,sales_name,sales_province_name,
			...
			--case when dev_source_code='4' then '内购' end as ng_cust
		from 
			csx_dw.dws_crm_w_a_customer where sdt=${hiveconf:i_sdate_11}   --上月最后1日
	...
	left join 
		...
		from 
			csx_tmp.tmp_salesname_rate_ytd where sdt>=${hiveconf:i_sdate_12} and sdt<=${hiveconf:i_sdate_11}  --上月1日，昨日月1日
		group by 
...
where 
	...
	and b.ng_cust is null
group by 
	b.sales_province_name,a.customer_no,b.customer_name,b.work_no,b.sales_name,d.service_user_work_no,d.service_user_name,c.sale_rate,c.profit_rate,a.smonth;
