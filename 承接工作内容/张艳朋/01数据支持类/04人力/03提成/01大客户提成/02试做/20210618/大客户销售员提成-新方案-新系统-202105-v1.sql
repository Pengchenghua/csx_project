...
-- insert overwrite directory '/tmp/zhangyanpeng/20210618_01' row format delimited fields terminated by '\t'
-- select * from csx_dw.report_crm_w_a_customer_service_manager_info where sdt='20210617';


-- load data inpath '/tmp/zhangyanpeng/sales_income_info_05.csv' overwrite into table csx_tmp.sales_income_info partition (sdt='20210531');
...
from
	...
	left join (select distinct work_no,income_type from csx_tmp.sales_income_info where sdt='20210531' and cust_type='大客户') c on c.work_no=b.work_no
where 
...
insert overwrite directory '/tmp/zhangyanpeng/tc_customer_202105' row format delimited fields terminated by '\t'
...
insert overwrite directory '/tmp/zhangyanpeng/tc_customer_overdue_202105' row format delimited fields terminated by '\t'
