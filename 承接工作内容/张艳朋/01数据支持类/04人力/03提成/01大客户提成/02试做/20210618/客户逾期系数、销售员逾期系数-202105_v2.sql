...
--订单应收金额、逾期日期、逾期天数
drop table csx_tmp.tmp_cust_order_overdue_dtl_v2;
create table csx_tmp.tmp_cust_order_overdue_dtl_v2
as
select
	c.channel_name,
	c.channel_code,	
...
from
	...
	from csx_tmp.tmp_cust_order_overdue_dtl_v2
...
join		 ----渠道编号-1.大客户 2.商超 4.大宗 5.供应链(食百) 6.供应链(生鲜) 7.企业购 8.其他 9.业务代理
  (
    ...
    where sdt=${hiveconf:i_sdate_11} and channel_code in('1','7','8') and (customer_name not like '%内%购%' and customer_name not like '%临保%')
...
--客户逾期系数
drop table csx_tmp.temp_cust_over_rate_v2;
create table csx_tmp.temp_cust_over_rate_v2
as
select 
	channel_name,	-- 渠道
	customer_no,	-- 客户编码
...
from
	(select
		...
		sum(case when over_days>=1 then receivable_amount*over_days else 0 end) as over_amt_s,
		sum(case when receivable_amount>=0 then receivable_amount else 0 end* if(COALESCE(account_period_val,0)=0,1,acc_val_calculation_factor)) as receivable_amount_s
	from csx_tmp.tmp_cust_order_overdue_dtl_v2 a 
	where channel_name = '大客户' and sdt = ${hiveconf:i_sdate_11}
	--签呈客户不考核，不算提成,因此不算逾期 2021年3月签呈取消剔除103717
	and customer_no not in('111118','102755','104023','105673','104402')
	and customer_no not in('107338','104123','102629','104526','106375','106380','106335','107268','104296','108391','108390','108072','108503')
...
--销售员逾期系数
drop table csx_tmp.temp_salesname_over_rate_v2;
create table csx_tmp.temp_salesname_over_rate_v2
as
select 
	a.channel_name,	-- 渠道
	b.work_no,	-- 销售员工号
...
left join
  (
    select customer_no,
	--  if(customer_no in('113873','113918','113935','113936','113940'),'80886296',
	--	 work_no) as work_no,
	--  if(customer_no in('113873','113918','113935','113936','113940'),'蒋玲',
	--	 sales_name) as sales_name
	work_no,sales_name
	from csx_dw.dws_crm_w_a_customer 
    where sdt=${hiveconf:i_sdate_11} 
	and channel_code in('1','7','8') and (customer_name not like '%内%购%' and customer_name not like '%临保%')
  )b on b.customer_no=a.customer_no  
...
--服务管家逾期率
drop table csx_tmp.temp_service_user_over_rate_v2;
create table csx_tmp.temp_service_user_over_rate_v2
as
select 
	a.channel_name,	-- 渠道
...
--截至某天的订单应收明细
insert overwrite directory '/tmp/zhangyanpeng/ysmx' row format delimited fields terminated by '\t'
select 
	b.sales_province,	-- 省区
	b.work_no,	-- 销售员工号
	b.sales_name,	-- 销售员
