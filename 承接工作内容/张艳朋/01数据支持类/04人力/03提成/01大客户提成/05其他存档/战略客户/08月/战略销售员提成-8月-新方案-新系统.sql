...
--load data inpath '/tmp/zhangyanpeng/strategic_cust_info_08.csv' overwrite into table csx_tmp.strategic_cust_info_new partition (sdt='202108');
--select * from csx_tmp.strategic_cust_info_new where sdt='202108';
...
drop table csx_tmp.tmp_tc_strategic_salesname_rate_ytd;
create table csx_tmp.tmp_tc_strategic_salesname_rate_ytd
as
...
from 
	(
	select 
		a.sdt,c.work_no,c.sales_name,coalesce(c.income_type,'Q1')income_type,
	...
	from 
		...
		where 
			sdt>='20210101' and sdt<=${hiveconf:i_sdate_11} 
		...
		join 
			(
			select 
				distinct a.province_name,a.customer_no,a.sales_name_strat sales_name,a.sdt smonth,b.work_no,b.income_type
			from csx_tmp.strategic_cust_info_new a
				left join 
					(
					select 
						distinct work_no,sales_name,income_type 
					from 
...
drop table csx_tmp.temp_tc_strategic_new_cust_01;
create table csx_tmp.temp_tc_strategic_new_cust_01
as
select 
...
from 
	...
	join 
		(
		select 
			distinct province_name,customer_no,sales_name_strat 
		from 
			csx_tmp.strategic_cust_info_new 
		where 
			sdt=substr(${hiveconf:i_sdate_11},1,6)
	...
	left join 
		...
		where 
			sdt=${hiveconf:i_sdate_11} 
			and cust_type='战略客户'
		) c on c.sales_name=d.sales_name_strat
	left join 
		(
		select  
			work_no,sales_name,sdt,max(sale_rate) sale_rate,max(profit_rate) profit_rate
		from 
			csx_tmp.tmp_tc_strategic_salesname_rate_ytd 
		where 
...
group by 
	a.province_name,a.customer_no,b.customer_name,c.work_no,c.sales_name,a.smonth;


--逾期系数
--战略客户销售员逾期率
drop table csx_tmp.temp_tc_strategic_salesname_over_rate;
create table csx_tmp.temp_tc_strategic_salesname_over_rate
as
select 
	a.channel_name,	-- 渠道
	b.work_no,	-- 销售员工号
...
from
	...
	left join 
		...
		where 
			sdt=${hiveconf:i_sdate_11} 
			and cust_type='战略客户'
		) b on b.sales_name=d.sales_name_strat
group by 
...
drop table csx_tmp.temp_tc_strategic_new_cust_salary;
create table csx_tmp.temp_tc_strategic_new_cust_salary
as
select 
...
from  
	csx_tmp.temp_tc_strategic_new_cust_01 a
	left join csx_tmp.temp_tc_cust_over_rate b on b.customer_no=a.cust_id
	left join csx_tmp.temp_tc_strategic_salesname_over_rate c on c.sales_name=a.sales_name
; 

insert overwrite directory '/tmp/zhangyanpeng/zl_kehu' row format delimited fields terminated by '\t'
select * from csx_tmp.temp_tc_strategic_new_cust_salary;

--销售员当月提成
insert overwrite directory '/tmp/zhangyanpeng/zl_xiaoshou' row format delimited fields terminated by '\t'
select 
...
from 
	csx_tmp.temp_tc_strategic_new_cust_salary
group by 
	smonth,dist,work_no,sales_name,sale_over_rate ;


--计算逾期率
insert overwrite directory '/tmp/zhangyanpeng/zl_yqxs' row format delimited fields terminated by '\t'
select 
	a.channel_name,	-- 渠道
...
from
	...
	left join 
		...
		where 
			sdt=${hiveconf:i_sdate_11} 
			and cust_type='战略客户'
		) e on e.sales_name=d.sales_name_strat
	left join
		(
		select
			customer_no,
