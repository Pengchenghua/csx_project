...
--load data inpath '/tmp/raoyanhua/strategic_cust_info_12.csv' overwrite into table csx_tmp.strategic_cust_info_new partition (sdt='202012');
--select * from csx_tmp.strategic_cust_info_new where sdt='202012';
...
from 
(
select '销售' s_type,sdt,substr(sdt,1,6) smonth,province_name,customer_no,
sum(sales_value)sales_value,
...
--★★★合并返利新增  		
  union all
  select '返利' s_type,a.sdt,substr(a.sdt,1,6) smonth,a.province_name,a.customer_no,
    sum(a.sales_value_fanli)sales_value,
    sum(a.sales_value_fanli) profit,
	1 prorate,
    sum(a.sales_value_fanli) as front_profit,
    1 as fnl_prorate
  from
    (
  	select customer_no,province_name,
  	${hiveconf:i_sdate_22} as sdt,
  	sum(sales_value) as sales_value_fanli
  	from csx_dw.dwd_csms_r_d_rebate_order
  	where sdt >=${hiveconf:i_sdate_22}
  	and sdt <=${hiveconf:i_sdate_12}  --昨日月1日
  	and order_type_code in (0, 1) and order_status = 1
  	group by customer_no,province_name
    )a
    group by a.sdt,substr(a.sdt,1,6),a.province_name,a.customer_no
)a
left join 
