...
create temporary table b2b_tmp.temp_strategic_cust_order
as
...
where sdt>='20180101' and sdt<'20200201'
...
case when ptype=1 then zt_prorate-rate else sap_prorate-pro_std-rate end as fnl_prorate,
(case when ptype=1 then zt_prorate-rate else sap_prorate-pro_std-rate end)*sales_qty as fnl_profit
...
and sdt=regexp_replace(current_date,'-','') and a.budat>='20180101' and a.budat<'20200201' 
and (belnr like '01%'or belnr like '1%' or belnr like '009%')  and mandt='800' 
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961')
group by kunnr
union all 
select kunnr,0 amount,sum(ac_all) ac_all from csx_dw.account_age_dtl_fct where sdt='20171231' and hkont='1122010000'
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961') group by kunnr)a
where a.amount<0 or a.ac_all<0
group by kunnr)x;
*/

--106566客户 何玫本月离职，她的提成需要算2月回款
drop table b2b_tmp.temp_strategic_current02;
CREATE table b2b_tmp.temp_strategic_current02
as
select x.kunnr,x.in_value,x.in_amt
...
union all 
select kunnr,sum(a.dmbtr)amount,0 ac_all from ods_ecc.ecc_ytbcustomer  a where hkont='1122010000' 
and sdt=regexp_replace(current_date,'-','') and a.budat>='20200201' 
and (belnr like '01%'or belnr like '1%' or belnr like '009%')  and mandt='800' 
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961')
and kunnr like'%106566%'
group by kunnr
union all 
select kunnr,0 amount,sum(ac_all) ac_all from csx_dw.account_age_dtl_fct where sdt='20171231' and hkont='1122010000'
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961') group by kunnr)a
...
as
select a.dist,a.jobno,a.salename,a.cust_id,a.cust_name,
c.in_value,c.in_amt,c.in_amt+c.tax_value left_amt,--left_amt 扣除特殊福利单覆盖后，剩余回款金额
a.smonth,
a.sap_sales-coalesce(b.tax_value,0) tax_sale,
a.sap_profit_mf*(1-coalesce(b.tax_value,0)/coalesce(b.tax_sale,1)) sap_profit_mf,
a.zt_profit_mf*(1-coalesce(b.tax_value,0)/coalesce(b.tax_sale,1)) zt_profit_mf,
a.salery_cal*(1-coalesce(b.tax_value,0)/coalesce(b.tax_sale,1))salary,
a.salery_zt_cal*(1-coalesce(b.tax_value,0)/coalesce(b.tax_sale,1))salery_zt,
a.zterm,a.diff
from (select dist,jobno,salename,cust_id,cust_name,
sum(sap_sales)sap_sales,
sum(sap_profit_mf)sap_profit_mf,sum(zt_profit_mf)zt_profit_mf,sum(salery_cal)salery_cal,
sum(salery_zt_cal)salery_zt_cal,smonth,zterm,diff
from b2b_tmp.temp_res02 where (orderflag<>'福利' and smonth='201909')or smonth<>'201909'
group by dist,jobno,salename,cust_id,cust_name,smonth,zterm,diff) a 
...
from  csx_dw.sale_salary_cal_new1_1 where smm<'202001' group by regexp_extract(cust_id, '(0|^)([^0].*)',2),smonth) b 
...
as
select dist,jobno,salename,cust_id,cust_name,in_value,in_amt,smonth,tax_sale,
sap_profit_mf,zt_profit_mf,salary,salery_zt,
zterm,diff,
case when left_amt+sum_bq<=0 then tax_sale 
when left_amt+coalesce(sum_sq,0)<0  and left_amt+sum_bq>0 then -1*(left_amt+coalesce(sum_sq,0)) else 0 end tax_value
from 
(select a.*,
...
sum(tax_sale)over(PARTITION BY cust_id order by smonth ROWS BETWEEN UNBOUNDED PRECEDING AND 0 PRECEDING )sum_bq
 from b2b_tmp.temp_strategic_res003 a where smonth<='202001' and tax_sale<>0)m;
*/
...
as
select 
smonth,dist,cust_id,customer_name,attribute,channel,sales_name,sign_time,
sales_name_strat,present_date,confirmation,remarks,count_flag,cooperate_type,proportion,
...
case when in_amt+sum_bq<=0 then sales_value 
when in_amt+coalesce(sum_sq,0)<0  and in_amt+sum_bq>0 then -1*(in_amt+coalesce(sum_sq,0)) else 0 end tax_value
from 
(select a.*,
row_number() OVER(PARTITION BY cust_id ORDER BY smonth)rno,
sum(sales_value)over(PARTITION BY cust_id order by smonth ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING ) sum_sq,
sum(sales_value)over(PARTITION BY cust_id order by smonth ROWS BETWEEN UNBOUNDED PRECEDING AND 0 PRECEDING )sum_bq
 from (
 select a.*,x.kunnr,x.in_value,x.in_amt from b2b_tmp.temp_strategic_cust_01 a
 left join b2b_tmp.temp_strategic_current02 x on lpad(x.kunnr,10,'0')=lpad(a.cust_id,10,'0')
) a where smonth<='202001' and sales_value<>0)m;
...
as
select a.smonth,a.dist,a.cust_id,a.customer_name,a.attribute,a.channel,a.sales_name,a.sign_time,
a.sales_name_strat,a.present_date,a.confirmation,a.remarks,a.count_flag,a.cooperate_type,a.proportion,
a.sales_value,a.sap_profit,a.sap_prorate,a.fnl_profit,a.fnl_prorate,
a.salary_dl,a.salary,a.tax_value,
...
sales_name_strat,present_date,confirmation,remarks,count_flag,cooperate_type,proportion,
sales_value,sap_profit,sap_prorate,fnl_profit,fnl_prorate,
salary_dl,salary,salery_cal,
salery_cal*(tax_value/sales_value) salery_cal_1,
tax_value
