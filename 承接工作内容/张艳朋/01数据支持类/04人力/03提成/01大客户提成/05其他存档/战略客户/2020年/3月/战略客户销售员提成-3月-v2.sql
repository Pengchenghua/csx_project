...
where sdt>='20200301' and sdt<'20200401'
...
--5、109575 客户3月计算，但2月有销售额，无回款，需补算2月销售额
/*
drop table b2b_tmp.temp_strategic_cust_99;
create temporary table b2b_tmp.temp_strategic_cust_99
as
...
where smonth='202002'
union all
select '福建' dist,'109575' cust_id,'中石化森美（福建）石油有限公司' customer_name,'企业购' attribute,'-' channel,
'-' sales_name,'万长安' sales_name_strat,'-' confirmation,'' remarks,'是' count_flag,'' cooperate_type,'NULL' proportion,
7662.29 sales_value,6292.41 sap_profit,0.821217939 sap_prorate,5602.8039 fnl_profit,0.731217939 fnl_prorate,'202002' smonth
;

drop table b2b_tmp.temp_strategic_cust_98;
create temporary table b2b_tmp.temp_strategic_cust_98
as
select * from csx_dw.sale_strategic_income_res;
...
sales_name_strat,confirmation,remarks,count_flag,cooperate_type,proportion,
sales_value,sap_profit,sap_prorate,fnl_profit,fnl_prorate,smonth
 from b2b_tmp.temp_strategic_cust_99;
*/

--6、下列7位客户3月计算，但部分3月前有销售额，需补算销售额
...
and sdt=regexp_replace(current_date,'-','') and a.budat>='20200301' and a.budat<'20200401' 
and (belnr like '01%'or belnr like '1%' or belnr like '009%')  and mandt='800' 
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961')
group by kunnr
union all 
select kunnr,sum(a.dmbtr)amount,0 ac_all from ods_ecc.ecc_ytbcustomer  a where hkont='1122010000' 
and sdt=regexp_replace(current_date,'-','') and a.budat>='20180101' and a.budat<'20200301' 
and (belnr like '01%'or belnr like '1%' or belnr like '009%')  and mandt='800' 
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961')
and (kunnr like'%109575%' or kunnr like'%103141%' or kunnr like'%104501%' or kunnr like'%109401%' or kunnr like'%110602%' 
		or kunnr like'%110807%' or kunnr like'%110881%' or kunnr like'%PF0458%')
group by kunnr
union all 
select kunnr,0 amount,sum(ac_all) ac_all from csx_dw.account_age_dtl_fct where sdt='20200229' and hkont='1122010000'
...
left join 
	...
	where smm<'202003' group by regexp_extract(cust_id, '(0|^)([^0].*)',2),smonth) c 
...
max(tax_sale)tax_sale,sum(tax_value)tax_value --最大回款覆盖金额，销售额
from  csx_dw.sale_strategic_salary_cal where smm<'202003' group by regexp_extract(cust_id, '(0|^)([^0].*)',2),smonth) b 
...
sum(tax_sale)over(PARTITION BY cust_id order by smonth ROWS BETWEEN UNBOUNDED PRECEDING AND 0 PRECEDING )sum_bq
 from b2b_tmp.temp_strategic_res003 a where smonth<='202003' and tax_sale<>0)m;
...
sum(x.tax_value)tax_value,'202003' smm
...
--alter table csx_dw.sale_strategic_salary_cal drop partition(smm='202003');
...
tax_sale,in_value,in_amt,sap_profit,fnl_profit,salery_cal,tax_value,'202003' smm
