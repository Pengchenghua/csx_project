...
and sdt=regexp_replace(current_date,'-','') and a.budat>='20200401' and a.budat<'20200501' 
and (belnr like '01%'or belnr like '1%' or belnr like '009%')  and mandt='800' 
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961')
group by kunnr
union all 
select kunnr,0 amount,sum(ac_all) ac_all from csx_dw.account_age_dtl_fct where sdt='20200331' and hkont='1122010000'
...
left join 
	...
	where smm<'202004' group by regexp_extract(cust_id, '(0|^)([^0].*)',2),smonth) c 
...
max(tax_sale)tax_sale,sum(tax_value)tax_value --最大回款覆盖金额，销售额
from  csx_dw.sale_strategic_salary_cal where smm<'202004' group by regexp_extract(cust_id, '(0|^)([^0].*)',2),smonth) b 
...
sum(x.tax_value)tax_value,'202004' smm
...
--alter table csx_dw.sale_strategic_salary_cal drop partition(smm='202004');
...
tax_sale,in_value,in_amt,sap_profit,fnl_profit,salery_cal,tax_value,'202004' smm
