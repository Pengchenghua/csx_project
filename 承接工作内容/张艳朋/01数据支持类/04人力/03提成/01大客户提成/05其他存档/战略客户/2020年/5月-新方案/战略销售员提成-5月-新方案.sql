...
--load data inpath '/tmp/raoyanhua/strategic_cust_info.csv' overwrite into table csx_dw.strategic_cust_info_new partition (sdt='202005');
--select * from csx_dw.strategic_cust_info_new where sdt='202005';
...
--load data inpath '/tmp/raoyanhua/sales_income_info.csv' overwrite into table csx_dw.sales_income_info partition (sdt='20200531');
--select * from csx_dw.sales_income_info where sdt='20200531';
...
where sdt>='20200101' and sdt<'20200601' 
and sales_type in('qyg','sapqyg','sapgc','sc','bbc')  
and item_channel_code in('1','7')
group by sdt,customer_no,substr(sdt,1,6))a 
join (select distinct province_name,customer_no,sales_name_strat from csx_dw.strategic_cust_info_new where sdt='202005') d on d.customer_no=a.customer_no
--left join (select * from csx_dw.dws_crm_w_a_customer_m where sdt='20200531') b on b.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_dw.sales_income_info where sdt='20200531' and cust_type='战略客户') c on c.sales_name=d.sales_name_strat
)a;
...
left join 
	...
	from csx_dw.dws_crm_w_a_customer_m where sdt='20200531') b on b.customer_no=a.customer_no
join (select distinct province_name,customer_no,sales_name_strat from csx_dw.strategic_cust_info_new where sdt='202005') d on d.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_dw.sales_income_info where sdt='20200531' and cust_type='战略客户') c on c.sales_name=d.sales_name_strat
left join 
	(select  work_no,sales_name,sdt,max(sale_rate) sale_rate,max(profit_rate) profit_rate
	from csx_tmp.strategic_salesname_rate_ytd where sdt>='20200501' and sdt<'20200601' 
...
from
	...
	where channel = '大客户' and sdt = '20200531' 
...
join (select distinct province_name,customer_no,sales_name_strat from csx_dw.strategic_cust_info_new where sdt='202005') d on d.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_dw.sales_income_info where sdt='20200531' and cust_type='战略客户') b on b.sales_name=d.sales_name_strat
...
from
	...
	and subject_code='1122010000'
	group by channel, customer_no, customer_name, zterm, payment_days, payment_terms, comp_code, comp_name )a
left join (select * from csx_dw.dws_crm_w_a_customer_m where sdt='20200531') c on c.customer_no=a.customer_no
join (select distinct province_name,customer_no,sales_name_strat from csx_dw.strategic_cust_info_new where sdt='202005') d on d.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_dw.sales_income_info where sdt='20200531' and cust_type='战略客户') b on b.sales_name=d.sales_name_strat;
