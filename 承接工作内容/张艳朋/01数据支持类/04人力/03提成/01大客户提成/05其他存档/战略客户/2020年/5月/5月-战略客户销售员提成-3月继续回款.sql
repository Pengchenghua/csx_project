...
--20200413 3月注意事项
--1、两个客户3月前计算，3月销售员离职不计算 2个：108149	中国铁路北京局集团（北京动车段），106566	中国邮政集团公司福建省分公司
--2、客户3月前说是19年9月前客户，3月信息表中有，经确认依然不计算： 107125	中国人民武警警察部队北京市总队执勤第七支队
--3、198804	西城烟草 以前客户编号为108804，本月有误，依然为108804
--4、3月客户对应的销售员变更：107428	恒久物业管理有限公司（北京）聂丹华（以前张晨）
--5、109575 客户3月计算，但2月有销售额，无回款，需补算2月销售额
...
/*
-----00、订单明细
drop table csx_tmp.temp_strategic_cust_order;
create table csx_tmp.temp_strategic_cust_order
as
...
from (select * from csx_dw.dws_sale_r_d_customer_sale
where sdt>='20200301' and sdt<'20200501'
and sales_type in ('qyg','sapqyg','sapgc','sc','bbc') and customer_no not like 'S%'
)a
...
--合伙人的前端毛利取记账毛利
drop table csx_tmp.temp_strategic_cust_01;
create table csx_tmp.temp_strategic_cust_01
as
select 
...
case when customer_no in('102755','107856','103887') then sap_prorate-rate
		when ptype=1 then zt_prorate-rate 
		else sap_prorate-pro_std-rate end as fnl_prorate,

(case when customer_no in('102755','107856','103887') then sap_prorate-rate
		when ptype=1 then zt_prorate-rate 
		else sap_prorate-pro_std-rate end)*sales_value as fnl_profit
from csx_tmp.temp_strategic_cust_order )a 
...
select * from csx_tmp.temp_strategic_cust_01;





--alter table csx_dw.sale_strategic_income_res drop partition(smonth='202001');
--SELECT smonth,count(1) from csx_dw.sale_strategic_income_res group by smonth;
...
sales_value,sap_profit,sap_prorate,fnl_profit,fnl_prorate,smonth
 from csx_tmp.temp_strategic_cust_01;
...
-- 计算薪酬
drop table csx_tmp.temp_strategic_res031;
create table csx_tmp.temp_strategic_res031
as
select 
...
drop table csx_tmp.temp_strategic_res032;
CREATE temporary table csx_tmp.temp_strategic_res032
as
...
a.sales_value*if(b.salary>0,b.salary,0)/b.sales_value salery_cal
 from csx_tmp.temp_strategic_res031 a 
join (select dist,sales_name_strat,smonth,sum(sales_value)sales_value,sum(salary)salary
 from csx_tmp.temp_strategic_res031
group by dist,sales_name_strat,smonth)b on (a.dist=b.dist and a.sales_name_strat=b.sales_name_strat and a.smonth=b.smonth);




--('103141','104501','109401','110602','110807','110881','PF0458')
--02 回款
drop table csx_tmp.temp_strategic_current02;
CREATE table csx_tmp.temp_strategic_current02
as
...
and sdt=regexp_replace(current_date,'-','') and a.budat>='20200501' and a.budat<'20200601' 
and (belnr like '01%'or belnr like '1%' or belnr like '009%')  and mandt='800' 
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961')
group by kunnr
union all 
select kunnr,0 amount,sum(ac_all) ac_all from csx_dw.account_age_dtl_fct where sdt='20200430' and hkont='1122010000'
and (substr(kunnr,1,1) not in ('G','L','V','S') or kunnr='S9961') group by kunnr)a
where a.amount<0 or a.ac_all<0
--and (kunnr not like'%109575%' and kunnr not like'%103141%' and kunnr not like'%104501%' and kunnr not like'%109401%' and kunnr not like'%110602%' 
--		and kunnr not like'%110807%' and kunnr not like'%110881%' and kunnr not like'%PF0458%')
group by kunnr)x;
...
select a.smonth,d.province_name dist,a.cust_id,d.customer_name,d.attribute,d.channel,d.sales_name,
d.sales_name_strat,d.confirmation,d.remarks,d.count_flag,d.cooperate_type,d.proportion,
a.sales_value,a.sap_profit,a.sap_prorate,a.fnl_profit,a.fnl_prorate,
a.salary_dl,a.salary,a.salery_cal,
b.in_value,b.in_amt,c.tax_value
from csx_tmp.temp_strategic_res032 a
left join csx_tmp.temp_strategic_current02 b on lpad(a.cust_id,10,'0')=lpad(b.kunnr,10,'0')
left join 
	(select regexp_extract(cust_id, '(0|^)([^0].*)',2) cust_id,smonth,
	max(tax_sale)tax_sale,sum(tax_value)tax_value --最大回款覆盖金额，销售额
	from  csx_dw.sale_strategic_salary_cal 
	where smm<'202005' group by regexp_extract(cust_id, '(0|^)([^0].*)',2),smonth) c 
	on (lpad(a.cust_id,10,'0')=lpad(c.cust_id,10,'0') and a.smonth=c.smonth)
join csx_dw.strategic_cust_info d on d.customer_no=a.cust_id
order by a.smonth; 
...
-- b表客户历史月销售额、回款覆盖金额
-- c表回款额
drop table csx_tmp.temp_strategic_res003;
create table csx_tmp.temp_strategic_res003
as
...
from csx_tmp.temp_strategic_res032  a 
left join csx_tmp.temp_strategic_current02 c on lpad(a.cust_id,10,'0')=lpad(c.kunnr,10,'0')
left join (select regexp_extract(cust_id, '(0|^)([^0].*)',2) cust_id,smonth,
max(tax_sale)tax_sale,sum(tax_value)tax_value --最大回款覆盖金额，销售额
from  csx_dw.sale_strategic_salary_cal where smm<'202005' group by regexp_extract(cust_id, '(0|^)([^0].*)',2),smonth) b 
...
sum(tax_sale)over(PARTITION BY cust_id order by smonth ROWS BETWEEN UNBOUNDED PRECEDING AND 0 PRECEDING )sum_bq
 from csx_tmp.temp_strategic_res003 a where smonth<='202004' and tax_sale<>0)m;




-- 各月未消销额、奖金包 当月回款覆盖金额
drop table csx_tmp.temp_strategic_res03;
CREATE table csx_tmp.temp_strategic_res03
as
select d.province_name dist,x.cust_id,d.customer_name cust_name,d.sales_name,d.sales_name_strat,
...
sum(x.tax_value)tax_value,'202005' smm
from csx_tmp.temp_strategic_res031 x
join csx_dw.strategic_cust_info d on d.customer_no=x.cust_id
group by d.province_name,x.cust_id,d.customer_name,d.sales_name,d.sales_name_strat,
...
--alter table csx_dw.sale_strategic_salary_cal drop partition(smm='202005');
...
tax_sale,in_value,in_amt,sap_profit,fnl_profit,salery_cal,tax_value,'202005' smm
 from csx_tmp.temp_strategic_res03;
...
sales_value,''in_value,''in_amt,sap_profit,fnl_profit,salery_cal,tax_value,'202001' smm
 from csx_tmp.temp_strategic_res032;
*/
...
/*
 drop table csx_tmp.temp_111;
CREATE table csx_tmp.temp_111
as
 select *
 from csx_tmp.temp_strategic_res032; 
*/
