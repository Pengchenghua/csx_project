...
--load data inpath '/tmp/raoyanhua/strategic_cust_info_6.csv' overwrite into table csx_dw.strategic_cust_info_new partition (sdt='202006');
--select * from csx_dw.strategic_cust_info_new where sdt='202006';
...
--load data inpath '/tmp/raoyanhua/sales_income_info_6.csv' overwrite into table csx_dw.sales_income_info partition (sdt='20200630');
--select * from csx_dw.sales_income_info where sdt=${hiveconf:i_sdate_23};
...
join (select distinct province_name,customer_no,sales_name_strat from csx_dw.strategic_cust_info_new where sdt=substr(${hiveconf:i_sdate_23},1,6)) d on d.customer_no=a.customer_no
--left join (select * from csx_dw.dws_crm_w_a_customer_m where sdt=${hiveconf:i_sdate_23}) b on b.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_dw.sales_income_info where sdt=${hiveconf:i_sdate_23} and cust_type='战略客户') c on c.sales_name=d.sales_name_strat
)a;
...
left join 
	...
	from csx_dw.dws_crm_w_a_customer_m where sdt=${hiveconf:i_sdate_23}) b on b.customer_no=a.customer_no
join (select distinct province_name,customer_no,sales_name_strat from csx_dw.strategic_cust_info_new where sdt=substr(${hiveconf:i_sdate_23},1,6)) d on d.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_dw.sales_income_info where sdt=${hiveconf:i_sdate_23} and cust_type='战略客户') c on c.sales_name=d.sales_name_strat
left join 
...
join (select distinct province_name,customer_no,sales_name_strat from csx_dw.strategic_cust_info_new where sdt=substr(${hiveconf:i_sdate_23},1,6)) d on d.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_dw.sales_income_info where sdt=${hiveconf:i_sdate_23} and cust_type='战略客户') b on b.sales_name=d.sales_name_strat
