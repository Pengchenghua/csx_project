...
--load data inpath '/tmp/raoyanhua/strategic_cust_info_9.csv' overwrite into table csx_tmp.strategic_cust_info_new partition (sdt='202009');
--select * from csx_tmp.strategic_cust_info_new where sdt='202009';
...
set i_sdate_11 ='20200930';
set i_sdate_12 ='20201001';
	
set i_sdate_22 ='20200901';				
set i_sdate_23 ='20200930';
...
as
select 
a.sales_province dist,a.customer_no cust_id,b.customer_name cust_name,c.work_no,c.sales_name,
...
left join 
	...
	from csx_dw.dws_crm_w_a_customer_m_v1 where sdt=${hiveconf:i_sdate_23}) b on b.customer_no=a.customer_no
join (select distinct province_name,customer_no,sales_name_strat from csx_tmp.strategic_cust_info_new where sdt=substr(${hiveconf:i_sdate_23},1,6)) d on d.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_tmp.sales_income_info where sdt=${hiveconf:i_sdate_23} and cust_type='战略客户') c on c.sales_name=d.sales_name_strat
left join 
...
group by a.sales_province,a.customer_no,b.customer_name,c.work_no,c.sales_name,a.smonth;
...
select 
    ...
    coalesce(round(case  when coalesce(sum(case when receivable_amount>=0 then receivable_amount else 0 end), 0) <= 1 then 0  
				else coalesce(sum(case when over_amt_s>=0 then over_amt_s else 0 end), 0)/(sum(case when receivable_amount>=0 then receivable_amount_s else 0 end)) end
			, 6),0) over_rate 	-- 逾期系数
from
	(select
		channel,
...
select 
	c.sales_province,a.channel,b.work_no,b.sales_name,a.customer_no,a.customer_name,
	e.account_period_code,	-- 账期编码
	if(e.account_period_code like 'Y%',if(e.account_period_val=31,45,e.account_period_val+15),e.account_period_val) account_period_val,	-- 帐期天数
	e.account_period_name,	-- 账期名称
    ...
    coalesce(round(case when coalesce(case when a.receivable_amount>=0 then a.receivable_amount else 0 end, 0) <= 1 then 0  
						else (coalesce(case when a.over_amt_s>=0 then a.over_amt_s else 0 end,0)/case when a.receivable_amount>=0 then a.receivable_amount_s else 0 end) end
		  , 6),0) over_rate 	-- 逾期系数
from
	(select
		channel,
	...
	where channel = '大客户' and sdt = ${hiveconf:i_sdate_23} 
	group by channel, customer_no, customer_name,company_code, company_name )a
left join (select * from csx_dw.dws_crm_w_a_customer_m_v1 where sdt=${hiveconf:i_sdate_23}) c on c.customer_no=a.customer_no
join (select distinct province_name,customer_no,sales_name_strat from csx_tmp.strategic_cust_info_new where sdt=substr(${hiveconf:i_sdate_23},1,6)) d on d.customer_no=a.customer_no
left join (select distinct work_no,sales_name,income_type from csx_tmp.sales_income_info where sdt=${hiveconf:i_sdate_23} and cust_type='战略客户') b on b.sales_name=d.sales_name_strat
left join
	(select
	...
	where sdt='current'
	and customer_number<>''
	)e on (a.customer_no=e.customer_number and a.company_code=e.company_code)
;
