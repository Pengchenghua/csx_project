...
round(sum(coalesce(a.front_profit-a.sales_value*coalesce(z.rate,0),0)*coalesce(a.profit_rate,0.1)),2) salary_fnl_profit
...
round(sum(coalesce(a.front_profit,0)*0.06),2) salary_fnl_profit
from csx_tmp.temp_new_cust_00 a
where a.cust_id in(
...
--每月核销金额
drop table csx_tmp.temp_payment_cust_01;
create temporary table csx_tmp.temp_payment_cust_01
as
select a.smonth,
	a.province_name dist,a.customer_no,b.customer_name cust_name,b.work_no,b.sales_name,  --b.dev_source_name
	sum(a.sales_value)sales_value,
	sum(a.profit) profit,
	sum(a.front_profit) as fnl_profit,
	sum(e.payment_amount_s0) payment_amount_s0,
	sum(e.payment_amount_s1) payment_amount_s1,
	sum(e.payment_amount_s2) payment_amount_s2,
	sum(e.payment_amount_s3) payment_amount_s3,
	sum(e.payment_amount_s4) payment_amount_s4	
from 
...
left join 
	...
	sum(case when regexp_replace(substr(paid_time,1,10),'-','')>='20210401' 
			  and regexp_replace(substr(paid_time,1,10),'-','')< '20210501'
			 then payment_amount end ) payment_amount_s0,
	sum(case when regexp_replace(substr(paid_time,1,10),'-','')>='20210501'
			  and regexp_replace(substr(paid_time,1,10),'-','')< '20210601'
			 then payment_amount end ) payment_amount_s1,
	sum(case when regexp_replace(substr(paid_time,1,10),'-','')>='20210601'
			  and regexp_replace(substr(paid_time,1,10),'-','')< '20210701'
			 then payment_amount end ) payment_amount_s2,
	sum(case when regexp_replace(substr(paid_time,1,10),'-','')>='20210701'
			  and regexp_replace(substr(paid_time,1,10),'-','')< '20210801'
			 then payment_amount end ) payment_amount_s3,
	sum(case when regexp_replace(substr(paid_time,1,10),'-','')>='20210801'
			  and regexp_replace(substr(paid_time,1,10),'-','')< '20210901'
			 then payment_amount end ) payment_amount_s4,			 
		sum(payment_amount) payment_amount
	from
		csx_dw.dwd_sss_r_d_close_bill_account_record_20200908
	--where (regexp_replace(substr(happen_date,1,10),'-','')<=${hiveconf:i_sdate_11} or happen_date='' or happen_date is NULL)
...
group by a.smonth,a.province_name,a.customer_no,b.customer_name,b.work_no,b.sales_name;
...
a.salary_fnl_profit,
a.salary_sales_value+a.salary_fnl_profit salary,
b.receivable_amount,b.over_amt,
b.over_rate,
--if(z.salary_sales_value is null,if(a.salary_sales_value<0 or b.over_rate is null,
--    a.salary_sales_value,a.salary_sales_value*(1-coalesce(if(b.over_rate<=0.5,b.over_rate,1),0)) )
--	,z.salary_sales_value) salary_sales_value_sale,	
--	
--if(z.salary_fnl_profit is null,if(b.over_rate is null,
--    a.salary_fnl_profit,a.salary_fnl_profit*(1-coalesce(if(b.over_rate<=0.5,b.over_rate,1),0)) )
--	,z.salary_fnl_profit) salary_fnl_profit_sale,
coalesce(if(payment_amount_s0<0,0,payment_amount_s0),0) payment_amount_s0,
coalesce(if(payment_amount_s1<0,0,payment_amount_s1),0) payment_amount_s1,
coalesce(if(payment_amount_s2<0,0,payment_amount_s2),0) payment_amount_s2,
coalesce(if(payment_amount_s3<0,0,payment_amount_s3),0) payment_amount_s3,
coalesce(if(payment_amount_s4<0,0,payment_amount_s4),0) payment_amount_s4	
from  
...
left join csx_tmp.temp_payment_cust_01 d on d.customer_no=a.cust_id
; 
...
if(payment_amount_s0<=0,0,if(sales_value>=payment_amount_s0,round(payment_amount_s0/sales_value,6),1))*salary salary_s0,

if(payment_amount_s1<=0 or (sales_value-payment_amount_s0)<=0,0,
   if((sales_value-payment_amount_s0)>=payment_amount_s1,round(payment_amount_s1/sales_value,6),
     (sales_value-payment_amount_s0)/sales_value))*salary salary_s1,
	 
if(payment_amount_s2<=0 or (sales_value-payment_amount_s0-payment_amount_s1)<=0,0,
   if((sales_value-payment_amount_s0-payment_amount_s1)>=payment_amount_s2,round(payment_amount_s2/sales_value,6),
     (sales_value-payment_amount_s0-payment_amount_s1)/sales_value))*salary*0.9 salary_s2,
	 
if(payment_amount_s3<=0 or (sales_value-payment_amount_s0-payment_amount_s1-payment_amount_s2)<=0,0,
   if((sales_value-payment_amount_s0-payment_amount_s1-payment_amount_s2)>=payment_amount_s3,round(payment_amount_s3/sales_value,6),
     (sales_value-payment_amount_s0-payment_amount_s1-payment_amount_s2)/sales_value))*salary*0.8 salary_s3,
	 
if(payment_amount_s4<=0 or (sales_value-payment_amount_s0-payment_amount_s1-payment_amount_s2-payment_amount_s3)<=0,0,
   if((sales_value-payment_amount_s0-payment_amount_s1-payment_amount_s2-payment_amount_s3)>=payment_amount_s4,round(payment_amount_s4/sales_value,6),
     (sales_value-payment_amount_s0-payment_amount_s1-payment_amount_s2-payment_amount_s3)/sales_value))*salary*0.7 salary_s4
