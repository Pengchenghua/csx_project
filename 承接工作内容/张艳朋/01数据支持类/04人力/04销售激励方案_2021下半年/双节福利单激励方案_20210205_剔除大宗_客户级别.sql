...
select
	concat('20201201-','20210131') as sdt_s,
...
from 
	...
	where 
		sdt between '20201201' and '20210131'
...
insert overwrite directory '/tmp/zhangyanpeng/20210223_encourage_data' row format delimited fields terminated by '\t'
select
	concat('20201201-','20210131') as sdt_s,
	...
	a.customer_no,e.customer_name,a.attribute_name,concat("'",order_no) as order_no,a.order_category_name,
	case when a.channel_code in ('1','9') then '大客户' when a.channel_code='7' then 'BBC' else '其他' end as channel,
	a.sales_value,
	a.profit,
	a.front_profit,
	e.first_category_name,e.second_category_name,e.third_category_name
from 
	(
	select 
		region_name,channel_code,province_code,province_name,city_group_name,work_no,sales_name,customer_no,attribute_name,order_no,order_category_name,
		sum(sales_value)as sales_value,
		sum(profit)as profit,
		sum(front_profit) as front_profit
	from 
	...
	left join -- 客户表
		(
		select 
			customer_no,customer_name,attribute,work_no,sales_name,first_category_name,second_category_name,third_category_name
		from 
			csx_dw.dws_crm_w_a_customer
		where 
			sdt = 'current' 
