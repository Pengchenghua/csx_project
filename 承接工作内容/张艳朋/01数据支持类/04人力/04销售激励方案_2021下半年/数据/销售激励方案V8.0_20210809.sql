...
insert overwrite directory '/tmp/zhangyanpeng/20210809_linshi_1' row format delimited fields terminated by '\t' 
...
from 
	csx_dw.dws_sale_r_d_detail 
where 
	sdt between '20210701' and '20210808'
	and channel_code in('1','7','9') -- 渠道编码(1:大客户 2:商超 4:大宗 5:供应链(食百) 6:供应链(生鲜) 7:bbc 8:其他 9:业务代理)	
	and business_type_code in ('2','6') -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
	and sales_name not rlike 'A|B|C|M' --虚拟销售
group by 
...
--==========================================================================================
--销售员激励案_销售之星_日配激励案_拓客之星

--set current_day =regexp_replace(date_sub(current_date,1),'-','');
--set last_month_day1 ='20210701';

set current_start_day ='20210701';

set current_end_day ='20210731';

select
	concat_ws('-', ${hiveconf:current_start_day},${hiveconf:current_end_day}) as period,
...
set last_month_start_day ='20210601';

set last_month_end_day ='20210630';
...
select
	...
	a.sales_value,a.front_profit,a.front_profit_rate,coalesce(if(b.sales_value<0,0,b.sales_value),0) as last_month_sales_value,
	a.sales_value-coalesce(if(b.sales_value<0,0,b.sales_value),0) as growth,
	row_number()over(order by a.sales_value-coalesce(if(b.sales_value<0,0,b.sales_value),0) desc) as rn
from	
	(
	select
		work_no,
...
where
	a.sales_value-coalesce(if(b.sales_value<0,0,b.sales_value),0)>=50000 --月环比增长销售额≥5万元
; 


--==========================================================================================
--销售员激励案_销售之星_日配激励案_毛利之星
...
select
	concat_ws('-', ${hiveconf:current_start_day},${hiveconf:current_end_day}) as period,
	c.sales_region_name,c.sales_province_name,c.sales_city_name,c.first_supervisor_work_no,c.first_supervisor_name,c.work_no,c.sales_name,d.begin_date,
	a.sales_value,a.front_profit,a.front_profit_rate,coalesce(if(b.front_profit<0,0,b.front_profit),0) as last_month_front_profit,
	a.front_profit-coalesce(if(b.front_profit<0,0,b.front_profit),0) as growth,
	row_number()over(order by a.front_profit-coalesce(if(b.front_profit<0,0,b.front_profit),0) desc) as rn
from	
	(
	select
		work_no,
...
where
	a.front_profit-coalesce(if(b.front_profit<0,0,b.front_profit),0)>=30000 --月环比增长销售额≥3万元
; 


--==========================================================================================
--销售主管激励案_绩优主管竞赛_日配&福利&BBC_拓客绩优主管
...
--==========================================================================================
--销售主管激励案_绩优主管竞赛_日配&福利&BBC_业绩绩优主管
...
--==========================================================================================
--销售主管激励案_绩优主管竞赛_日配&福利&BBC_人效绩优主管
...
-- 明细
	select
		province_name,customer_no,customer_name,work_no,sales_name,supervisor_work_no,supervisor_name,
		sum(sales_value) as sales_value, --含税销售额
		sum(profit) as profit, --含税毛利额
		sum(front_profit) as front_profit, --前端含税毛利
		sum(profit)/abs(sum(sales_value)) as profit_rate,
