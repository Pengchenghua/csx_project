...
set money_back_s_day='20221101';

set money_back_e_day='20221130';
	...
	from 
		...
		left join
			...
			where
				1=1
				--sdt>='20220801'
				and regexp_replace(to_date(paid_time),'-','') >=${hiveconf:money_back_s_day}
				and regexp_replace(to_date(paid_time),'-','') <=${hiveconf:money_back_e_day}
				and is_deleted =0
...
;
			...
			where 
				...
				and channel_code in('1','7','9') -- 渠道编码(1:大客户 2:商超 4:大宗 5:供应链(食百) 6:供应链(生鲜) 7:bbc 8:其他 9:业务代理)	
				and business_type_code in ('2','6') -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
				and goods_code in ('8718','8708','8649')
				and customer_no='125263'
			group by 
				customer_no,order_no,business_type_name
...
insert overwrite directory '/tmp/zhangyanpeng/20221108_01' row format delimited fields terminated by '\t'
				...
				where
					1=1
					-- and sdt>='20220801'
					and regexp_replace(to_date(paid_time),'-','') >='20221101'
					and regexp_replace(to_date(paid_time),'-','') <='20221130'
					and is_deleted =0
					and to_date(happen_date) between '2022-08-01' and '2022-09-30'
					and customer_code='125263'
				-- group by 
				...
				where
					...
					and is_deleted =0
					and to_date(happen_date) between '2022-08-01' and '2022-09-30'
					-- and customer_code='120250'		
					and close_bill_no='2209210606295738'
			...
			group by 
				...
				where
					...
					and to_date(happen_date) between '2022-08-01' and '2022-09-30'
					-- and customer_code='120250'		
					and close_bill_no like '%2209300606355317%'
...
insert overwrite directory '/tmp/zhangyanpeng/20221108_01' row format delimited fields terminated by '\t'
			...
			where
				1=1
				--sdt>='20220801'
				and regexp_replace(to_date(paid_time),'-','') >='2022-08-01'
				and regexp_replace(to_date(paid_time),'-','') <='2022-10-31'
				and is_deleted ='0'
				and to_date(happen_date)>='2022-08-01'
				and to_date(happen_date)<='2022-09-30'
				-- and (invoice_no is not null or id='6375456')
				and customer_code='114991'
			group by 
				case when substr(close_bill_no,1,2)='B2' then substr(close_bill_no,2,16)
					when substr(close_bill_no,1,2)='R2' then substr(close_bill_no,2,16)
					when substr(close_bill_no,1,2)='OC' then substr(close_bill_no,1,13)
					else close_bill_no end,customer_code,close_bill_no
...
;
	...
	from 
		...
		select
			customer_no,
			if(business_type_code in ('2'),order_no,origin_order_no) as order_no2,order_no,
			business_type_code,
			sum(sales_value)as sales_value,
			sum(profit) as profit	
		from
		...
		where 
			...
			and business_type_code in ('2','6') -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
			and goods_code not in ('8718','8708','8649')
			and customer_no='126991'
		group by 
			customer_no,if(business_type_code in ('2'),order_no,origin_order_no),business_type_code,order_no
		) a 
		left join
			(
			select
			...
			) b on b.close_bill_no=a.order_no2 and b.customer_code=a.customer_no
