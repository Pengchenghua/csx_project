	...
	from
		...
		from
			...
			select
				...
				if(business_sign_time=first_business_sign_time,'新客','老客') as business_flag
			...
			where
				...
				and to_date(business_sign_time) between '2022-10-01' and date_sub(current_date(),1)
			) a 
		where
...
from
	...
	where
		...
		and first_business_sale_date between '20221001' and regexp_replace(to_date(date_sub(current_date(),1)),'-','')
	) a 
	join
		(
		select 
