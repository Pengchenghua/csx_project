...
select
	...
	sum(a.profit_no_tax)/abs(sum(a.sale_amt)) as profit_no_tax_rate,
...
from 
	...
	from 
		...
		left join
			...
			where
				...
				and to_date(paid_time) <='2023-01-31'
	...
	having
		sum(sale_amt)>=10000
		and sum(profit_no_tax)/abs(sum(sale_amt))>0.03
	) a  
	left join   
		(
		select 
...
select 
	sdt_s,performance_region_name,performance_province_name,performance_city_name,customer_code,customer_name,sales_user_number,sales_user_name,a.sale_amt,a.profit_no_tax,profit_no_tax_rate,pay_amt
from
	(
	select
		concat('20221001-','20221231') as sdt_s,
		...
		a.sale_amt,
		a.profit_no_tax,
		a.profit_no_tax/abs(a.sale_amt) as profit_no_tax_rate,
