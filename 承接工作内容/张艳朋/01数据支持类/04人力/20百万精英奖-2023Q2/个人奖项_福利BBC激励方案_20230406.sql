...
select
	concat('20230401-',regexp_replace(to_date(date_sub(current_date(),1)),'-','')) as sdt_s,
...
from 
	...
	where 
		sdt between '20230401' and regexp_replace(to_date(date_sub(current_date(),1)),'-','') -- '20221231' -- '20230331' -- regexp_replace(to_date(date_sub(current_date(),1)),'-','') -- '20221231'
		...
		left join
			...
			from
				...
				where 
					...
					and regexp_replace(to_date(business_sign_time),'-','') between '20190101' and '20230426'
				) a 
				left join
					(
					select 
