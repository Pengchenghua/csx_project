...
select
	concat('20220501-','20220630') as sdt_s,
...
from 
	...
	where 
		sdt between '20220501' and '20220630' -- '20230331' -- regexp_replace(to_date(date_sub(current_date(),1)),'-','') -- '20221231'
...
from
	...
	select
		...
		a.sale_amt,
		a.profit,
		if(a.sale_amt=0,0,a.profit/abs(a.sale_amt)) as profit_rate,
		...
		left join
			...
			from
				...
				where 
					...
					and regexp_replace(to_date(business_sign_time),'-','') between '20190101' and '20220630'
				) a 
				left join
					(
					select 
