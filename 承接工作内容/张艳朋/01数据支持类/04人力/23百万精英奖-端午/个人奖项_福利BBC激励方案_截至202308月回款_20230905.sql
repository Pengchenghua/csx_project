	...
	from 
		...
		left join
			...
			where
				-- sdt>='20220801'
				delete_flag ='0'
				and to_date(paid_time) >='2023-05-01'
				and to_date(paid_time) <='2023-08-31'
				and to_date(happen_date)>='2023-05-01'
				and to_date(happen_date)<='2023-06-30'
			group by 
				case when substr(close_bill_code,1,2)='B2' then substr(close_bill_code,2,16)
					when substr(close_bill_code,1,2)='R2' then substr(close_bill_code,2,16)
					when substr(close_bill_code,1,2)='OC' then substr(close_bill_code,1,13)
		...
		left join
			...
			from
				...
				where 
					...
					and regexp_replace(to_date(business_sign_time),'-','') between '20190101' and '20230630'
				) a 
				left join
					(
					select 
...
-- 查数
			select
				customer_code,order_code,original_order_code,business_type_name,
				sum(sale_amt)as sale_amt,
				sum(profit) as profit
			from
				csx_dws.csx_dws_sale_detail_di
	...
	from 
		...
		from 
			...
			where 
				...
				and business_type_code in (2,6) -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
				and goods_code not in ('8718','8708','8649')
				and customer_code in ('128734','129222')
			group by 
				customer_code,
				if(business_type_code in (2),order_code,original_order_code)				
			) a 
			left join
				...
				where
					-- sdt>='20220801'
					delete_flag ='0'
					and to_date(paid_time) >='2023-01-01'
					and to_date(paid_time) <='2023-04-30'
					and to_date(happen_date)>='2023-01-01'
					and to_date(happen_date)<='2023-03-31'
				group by 
					case when substr(close_bill_code,1,2)='B2' then substr(close_bill_code,2,16)
						when substr(close_bill_code,1,2)='R2' then substr(close_bill_code,2,16)
						when substr(close_bill_code,1,2)='OC' then substr(close_bill_code,1,13)
		...
		left join
			...
			from
				...
				where 
					...
					and status=1  -- 是否有效 0.无效 1.有效 (status=0,'停止跟进')
					and business_stage=5
					and regexp_replace(to_date(business_sign_time),'-','') between '20190101' and '20230331'
				) a 
				left join
					(
					select 
...
;
			...
			where 
				...
				and business_type_code in (2,6) -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
				and goods_code not in ('8718','8708','8649')
				and customer_code in ('110696')
			group by 
				customer_code,order_code,original_order_code
