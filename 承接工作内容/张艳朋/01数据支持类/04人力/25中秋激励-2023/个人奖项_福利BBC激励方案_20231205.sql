...
select
	concat('20230901-','20231031') as sdt_s,
...
from 
	...
	where 
		sdt between '20230901' and '20231031' -- regexp_replace(to_date(date_sub(current_date(),1)),'-','') -- '20221231'
		and channel_code in('1','7','9') -- 渠道编码(1:大客户 2:商超 4:大宗 5:供应链(食百) 6:供应链(生鲜) 7:bbc 8:其他 9:业务代理)	
		and business_type_code in (2,6) -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
		and goods_code not in ('8718','8708','8649') --剔除茅台
	group by 
		customer_code
	having
		sum(sale_amt)>=10000
		...
		left join
			...
			from
				...
				where 
					...
					and regexp_replace(to_date(business_sign_time),'-','') between '20190101' and '20231031'
				) a 
				left join
					(
					select 
...
select 
	sdt_s,performance_region_name,performance_province_name,performance_city_name,customer_code,customer_name,sales_user_number,sales_user_name,a.sale_amt,a.profit,profit_rate,
	user_number,guide_user_name,pay_amt
from
	(
	select
		concat('20230901-','20231031') as sdt_s,
...
-- 		b.customer_name,
		...
		row_number() over(order by a.sale_amt desc) as rn,
		sum(a.sale_amt)over(partition by b.sales_user_number) as sales_total_sale_amt,
		c.user_number,c.guide_user_name,pay_amt
	from 
		(
	select 
		customer_code,
	...
	from 
		...
		from
			csx_dws.csx_dws_sale_detail_di
		where 
			sdt between '20230901' and '20231031'
			and channel_code in('1','7','9') -- 渠道编码(1:大客户 2:商超 4:大宗 5:供应链(食百) 6:供应链(生鲜) 7:bbc 8:其他 9:业务代理)	
			and business_type_code in (2,6) -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
			and goods_code not in ('8718','8708','8649')
		group by 
		...
		left join
			...
			where
				-- sdt>='20220801'
				delete_flag ='0'
				and to_date(paid_time) >='2023-09-01'
				and to_date(paid_time) <='2023-11-30'
				and to_date(happen_date)>='2023-09-01'
				and to_date(happen_date)<='2023-10-31'
			group by 
				case when substr(close_bill_code,1,2)='B2' then substr(close_bill_code,2,16)
					when substr(close_bill_code,1,2)='R2' then substr(close_bill_code,2,16)
					when substr(close_bill_code,1,2)='OC' then substr(close_bill_code,1,13)
...
-- 			csx_dws.csx_dws_sale_detail_di
-- 		where 
-- 			sdt between '20230901' and '20231007' -- regexp_replace(to_date(date_sub(current_date(),1)),'-','') -- '20221231'
-- 			and channel_code in('1','7','9') -- 渠道编码(1:大客户 2:商超 4:大宗 5:供应链(食百) 6:供应链(生鲜) 7:bbc 8:其他 9:业务代理)	
-- 			and business_type_code in (2,6) -- 业务类型编码(1.日配业务 2.福利业务 3.批发内购 4.城市服务商 5.省区大宗 6.BBC 7.大宗一部 8.大宗二部 9.商超)
-- 			and goods_code not in ('8718','8708','8649')
-- 		group by 
	...
	from 
		...
		left join
			...
			where
				...
				delete_flag ='0'
				and to_date(paid_time) >='2023-09-01'
				and to_date(paid_time) <='2023-12-31'
				and to_date(happen_date)>='2023-09-01'
				and to_date(happen_date)<='2023-10-31'
			group by 
				case when substr(close_bill_code,1,2)='B2' then substr(close_bill_code,2,16)
...
-- 			and sum(profit)/abs(sum(sale_amt))>0.03
		...
		left join
		 ...
		 group by 	b.performance_region_name,
		b.performance_province_name,
		b.performance_city_name,
-- 		a.customer_code,
-- 		b.customer_name,
...
from 
	...
	join
		(
		select
			customer_code,min(first_business_sale_date) as fl_bbc_first_business_sale_date
		from
		...
		group by 
			customer_code
		having
			min(first_business_sale_date)>='20230901'
		) c on c.customer_code=a.customer_code
group by 
	b.performance_region_name,
	b.performance_province_name,
...
--===================================================================================================================================================================
-- 个人奖项_福利BBC激励方案_开客之星奖明细

select 
	sdt_s,performance_region_name,performance_province_name,performance_city_name,customer_code,customer_name,sales_user_number,sales_user_name,a.sale_amt,a.profit,profit_rate,
	fl_bbc_first_business_sale_date
from
	(
	select
		concat('20230901-','20231031') as sdt_s,
		...
		row_number() over(order by a.sale_amt desc) as rn,
		sum(a.sale_amt)over(partition by b.sales_user_number) as sales_total_sale_amt,
		c.fl_bbc_first_business_sale_date
	from 
		(
		select 
			customer_code,
