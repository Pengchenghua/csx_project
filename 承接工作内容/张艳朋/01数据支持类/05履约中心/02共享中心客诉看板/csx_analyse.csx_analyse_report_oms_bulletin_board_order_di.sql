...
select
  ...
  a.recep_user_number,
  d.employee_name as recep_order_by,
  d.cost_center_code,
  d.cost_center_name,
  d.employee_org_code,
  d.employee_org_name,
...
from
	...
	select
		...
		order_date,order_code,goods_code,inventory_dc_code 
	from 
	...
	left join
		(
		select
			employee_code,employee_name,begin_date, cost_center_code,cost_center_name,employee_org_code,employee_org_name
		from
			(
			select
				row_number() over(partition by employee_code order by begin_date desc) as rank,
				employee_code,employee_name,begin_date,cost_center_code,cost_center_name,employee_org_code,employee_org_name
			from
