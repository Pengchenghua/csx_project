...
from 
	...
	from 
	    ...
	    where 
			...
			sdt >= '20230401' and sdt <= regexp_replace(date_sub(current_date,1),'-','') 
...
;
		...
		select 
			substr(sdt,1,6) as smonth,customer_code,customer_name,sum(sale_amt) as sale_amt,sum(profit) as profit,sum(profit)/sum(sale_amt)
	    from 
			csx_dws.csx_dws_sale_detail_di 
	    where 
			-- sdt >= regexp_replace(add_months(trunc(date_sub(current_date,1),'MM'),-1),'-','') and sdt <= regexp_replace(add_months(date_sub(current_date,1),0),'-','') 
			...
			and performance_region_name in ('华北大区')
			and customer_code='130236'
		group by 
			substr(sdt,1,6),customer_code,customer_name