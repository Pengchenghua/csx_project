...
drop table if exists csx_analyse_tmp.csx_analyse_tmp_profit_week_report_yc_huabei;
create table csx_analyse_tmp.csx_analyse_tmp_profit_week_report_yc_huabei
as 
select
...
from 
	...
	from 
		(
		select 
			performance_province_name,performance_city_name,sdt,substr(sdt,1,6) smonth,business_type_code,business_type_name,customer_code,
			if(order_channel_code=6 ,'是','否') tiaojia_type,if(order_channel_code=4 ,'是','否') fanli_type,delivery_type_name,inventory_dc_code,
			sale_amt,profit,if(order_channel_detail_code=26,0,sale_qty) as sale_qty
	    from 
			csx_dws.csx_dws_sale_detail_di 
	    where 
			sdt >= regexp_replace(add_months(trunc(date_sub(current_date,1),'MM'),-1),'-','') 
			and sdt <= regexp_replace(date_sub(current_date,1),'-','') 
			and channel_code in('1','9')
			and business_type_code in ('1','2') 
			and performance_region_name in ('华北大区')
		) a
		left join(select distinct shop_code from csx_dim.csx_dim_shop where sdt='current' and shop_low_profit_flag=1)c on a.inventory_dc_code = c.shop_code
		left join(select calday,csx_week,csx_week_begin,csx_week_end from csx_dim.csx_dim_basic_date) d on d.calday=a.sdt
    group by 
...
;
select * from csx_analyse_tmp.csx_analyse_tmp_profit_week_report_yc_huabei
