...
-- 明细
drop table if exists csx_analyse_tmp.csx_analyse_tmp_profit_month_detail;
create table csx_analyse_tmp.csx_analyse_tmp_profit_month_detail
as
select 
	substr(a.sdt,1,6) as month,
	f.csx_week,
...
from 
	...
	from 
		csx_dws.csx_dws_sale_detail_di 
	where 
		((sdt>='20231001' and sdt<='20231031') or (sdt>='20230901' and sdt<='20230930'))
		and channel_code in('1','7','9')
		and business_type_code in (1) 
		and performance_region_name in ('华北大区')
		and performance_province_name not in ('河北省')
...
group by 
	...
	d.first_business_sale_date 
;
select * from csx_analyse_tmp.csx_analyse_tmp_profit_month_detail;
