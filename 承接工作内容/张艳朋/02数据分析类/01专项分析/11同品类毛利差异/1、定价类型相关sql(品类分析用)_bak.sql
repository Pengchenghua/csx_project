...
(select 
 ...
 from 
      ...
      from 
          (select 
           ...
           from 
            ...
            where sdt>=regexp_replace(substr(date_add(from_unixtime(unix_timestamp('${sdt_start}','yyyyMMdd'),'yyyy-MM-dd'),-30),1,10),'-','') and sdt<='${sdt_end}'
  ...
  and calday<=regexp_replace(substr(date_add(from_unixtime(unix_timestamp('${sdt_end}','yyyyMMdd'),'yyyy-MM-dd'),30),1,10),'-','') -- 结束日期往后延1个月
  ) t2 
  ...
  (select 
   ...
   from 
    ...
    from 
      ...
      from csx_dim.csx_dim_basic_date 
      where calday>=regexp_replace(substr(date_add(from_unixtime(unix_timestamp('${sdt_start}','yyyyMMdd'),'yyyy-MM-dd'),-30),1,10),'-','')  
      and calday<=regexp_replace(substr(date_add(from_unixtime(unix_timestamp('${sdt_end}','yyyyMMdd'),'yyyy-MM-dd'),30),1,10),'-','')  -- 结束日期往后延1个月
      ) d2 
   ...
   from 
    ...
    (select 
      *,
      ...
      regexp_replace(split(update_time, ' ')[0], '-', '') as update_date   
     from csx_dwd.csx_dwd_price_customer_price_guide_invalid_di 
     where sdt>=regexp_replace(substr(date_add(from_unixtime(unix_timestamp('${sdt_start}','yyyyMMdd'),'yyyy-MM-dd'),-30),1,10),'-','')
    ) b4 
    on b1.associate_no=b4.associate_no
    -- and b1.customer_no=b4.customer_code and b1.dc_code=b4.warehouse_code and b1.goods_code=b4.product_code and b1.price_begin_date=b4.price_begin_date and b1.price_end_date=b4.price_end_date 
    where b4.update_date>='${sdt_start}' or b4.update_date is null 
...
select 
  ...
  a.sale_price,
  if(a.order_channel_detail_code=26,0,a.sale_qty) as sale_qty,
  a.sale_amt,
  a.cost_price,
  a.profit,
 ...
 (case -- 子客户多次报价 
  ...
  (case when order_channel_code=4 then '调价' when order_channel_code=6 then '返利' end) as if_tjfl,
  (case when refund_order_flag=0 then '是' else '否' end) as if_td  
from 
  (select 
      a1.*,
  ...
  where (sdt >= regexp_replace(add_months(trunc(from_unixtime(unix_timestamp('${sdt_start}','yyyyMMdd'),'yyyy-MM-dd'),'MM'),-2), '-', '') OR sdt = '19990101')
  ) f 
...
select 
  ...
  (case when a.if_tmp_guide is null then '未匹配报价' else a.if_tmp_guide end) as if_tmp_guide,
  (case when a.price_type is null then '未匹配报价' else a.price_type end) as price_type,
  a.price_source,
  a.if_sg,
...
drop table if exists csx_analyse_tmp.csx_analyse_tmp_cus_price_guide_order_final_supple_last; 
create table if not exists csx_analyse_tmp.csx_analyse_tmp_cus_price_guide_order_final_supple_last as 
select 
    a.*,
    (case when sec_price_type='全对标' then '对标对象' 
          ...
          when price_source='指定成交价' and price_type like '%建议售价%' and b.product_code is not null then '建议售价-手工导入' 
          when price_source='指定成交价' then price_type 
          when price_source='库存地点售价' then '建议售价' 
     else '其他报价方式' end) as price_type_new,
    (case when sec_price_type='全对标' then '对标对象' 
          when price_source='对标市调' and shop_price=sale_price then '建议售价' 
          when price_source like '对标%' then '对标对象' 
          when price_source='指定成交价' and price_type like '%建议售价%' and b.product_code is not null then '建议售价' 
          when price_source='指定成交价' then substring_index(price_type,'(', 1)  
          when price_source='库存地点售价' then '建议售价' 
          when price_source like '%毛利事中%' then '毛利事中改价' 
     else price_source end) as price_type_new1,
    (case when b.product_code is not null then '是' else '否' end) as if_xx_dr,
    c.fact_bottom_ai_price 
from csx_analyse_tmp.csx_analyse_tmp_cus_price_guide_order_final_supple a 
left join 
-- 手工导入的建议售价不做智能建议售价赋值
(select distinct 
     warehouse_code,
     product_code
from csx_ods.csx_ods_csx_price_prod_goods_price_guide_df 
where sdt='20230613' 
and is_expired='false' 
and base_product_status='0' 
and suggest_price_type='3') b 
on a.inventory_dc_code=b.warehouse_code and a.goods_code=b.product_code 
left join 
-- 智能建议售价关联
(select 
  c2.calday,
  ...
  c1.customer_no,
  c1.goods_code,
  max(c1.fact_bottom_ai_price) as fact_bottom_ai_price 
from 
  (select * 
  from csx_analyse.csx_analyse_report_ai_test_customer_prices_di 
  where sdt>=regexp_replace(substr(date_add('2023-06-01',-30),1,10),'-','')
  ) c1 
  cross join 
  (select distinct calday 
  from csx_dim.csx_dim_basic_date 
  where calday>=regexp_replace(substr(date_add('2023-06-01',-30),1,10),'-','') 
  and calday<='20230613' -- 结束日期往后延1个月
  ) c2 
  where c1.price_begin_date<=c2.calday and c1.price_end_date>=c2.calday 
group by 
  c2.calday,
  c1.dc_code,
  c1.customer_no,
...
on a.final_qj_date=c.calday and a.inventory_dc_code=c.dc_code and a.customer_code=c.customer_no and a.goods_code=c.goods_code  
...
select 
  ...
  customer_no as `客户编码`,
  goods_code as `商品编码`,
  price_type_new1 as `定价类型` 
from csx_analyse_tmp.csx_analyse_tmp_cus_price_guide_order_final_supple_last