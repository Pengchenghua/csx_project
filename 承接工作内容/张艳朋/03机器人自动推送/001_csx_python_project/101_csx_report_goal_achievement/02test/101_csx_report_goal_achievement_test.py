...
Sub TheMacro()
    ...
    With ActiveSheet.ChartObjects.Add(0, 0, selection.Width, selection.Height).Chart  '通过在当前工作表添加相同大小的图表的方式转存成图片
               ...
               .Paste  '粘贴复制后的图片
               .Export thisworkbook.path & "/" & "101_csx_report_goal_achievement" & ".JPG"
               .Parent.Delete  '删除该图表
    End With
End Sub
'''
...
def main():
    login_down_file('WEB-INF/schedule/彩食鲜业绩战报-目标达成_mysql/')
    ...
    #res=fei.send_msg("测试")
    res=fei.send_pic(os.path.join(os.path.dirname(os.path.abspath(__file__)),'101_csx_report_goal_achievement.JPG'))
