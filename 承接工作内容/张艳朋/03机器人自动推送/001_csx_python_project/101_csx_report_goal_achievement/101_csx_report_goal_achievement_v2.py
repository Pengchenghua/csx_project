...
def excel_catch_screen(vba_code):
    ...
    local_file_name='彩食鲜战报报表-目标达成_'+ (datetime.date.today()-datetime.timedelta(days=1)).strftime("%Y%m%d") +'.xlsx' #改
...
Sub TheMacro()
    ...
    With ActiveSheet.ChartObjects.Add(0, 0, selection.Width, selection.Height).Chart  '通过在当前工作表添加相同大小的图表的方式转存成图片
               ...
               .Paste  '粘贴复制后的图片
               .Export thisworkbook.path & "/" & "101_csx_report_goal_achievement" & ".JPG" '改
               .Parent.Delete  '删除该图表
    End With
End Sub
'''
...
def main():
    login_down_file('WEB-INF/schedule/彩食鲜业绩战报-目标达成_mysql/') #改
    ...
    res=fei.send_pic(os.path.join(os.path.dirname(os.path.abspath(__file__)),'101_csx_report_goal_achievement.JPG')) #改
