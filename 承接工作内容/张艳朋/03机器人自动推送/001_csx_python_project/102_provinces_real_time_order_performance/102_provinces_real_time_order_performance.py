...
def login_down_file():
    ...
    #拼接目录
    remote_directory='WEB-INF/schedule/省区实时下单业绩/' + today_str + '/' #改
...
def excel_catch_screen(vba_code):
    ...
    local_file_name='省区实时下单业绩.xlsx' #改
...
Sub TheMacro()
    ...
    With ActiveSheet.ChartObjects.Add(0, 0, selection.Width, selection.Height).Chart  '通过在当前工作表添加相同大小的图表的方式转存成图片
               ...
               .Paste  '粘贴复制后的图片
               .Export thisworkbook.path & "/" & "102_provinces_real_time_order_performance" & ".JPG" '改
               .Parent.Delete  '删除该图表
    End With
End Sub
'''
...
def main():
    ...
    res=fei.send_pic(os.path.join(os.path.dirname(os.path.abspath(__file__)),'102_provinces_real_time_order_performance.JPG'))

if __name__ == '__main__':
	scheduler=BlockingScheduler()
	scheduler.add_job(main,'cron',hour='17',minute='32',coalesce=True, misfire_grace_time=300)
	scheduler.start()
