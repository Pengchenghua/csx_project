...
def login_down_file():
    ...
    #拼接目录
    remote_directory='WEB-INF/schedule/永辉彩食鲜业绩日报-实时/' + today_str + '/'

    # 进入目标目录
    ftp.cwd(remote_directory)
    ftp.nlst()
    #print(ftp.nlst()[-1])
    dir_name = ftp.nlst()[-1]+'/通用结果/'
    ftp.cwd('./'+dir_name)
...
def excel_catch_screen(vba_code):
    ...
    local_file_name='永辉彩食鲜业绩日报-实时.xlsx'
...
Sub TheMacro()
    ...
    With ActiveSheet.ChartObjects.Add(0, 0, selection.Width, selection.Height).Chart  '通过在当前工作表添加相同大小的图表的方式转存成图片
               ...
               .Paste  '粘贴复制后的图片
               .Export thisworkbook.path & "/" & "103_csx_performance_daily_real_time" & ".JPG"
               .Parent.Delete  '删除该图表
    End With
End Sub
'''
...
def main():
    ...
    res=fei.send_pic(os.path.join(os.path.dirname(os.path.abspath(__file__)),'103_csx_performance_daily_real_time.JPG'))

if __name__ == '__main__':
	scheduler=BlockingScheduler()
	#scheduler.add_job(main,'cron',day_of_week='mon-fri',hour='10,18',minute='10',coalesce=True, misfire_grace_time=300)
	#scheduler.start()
