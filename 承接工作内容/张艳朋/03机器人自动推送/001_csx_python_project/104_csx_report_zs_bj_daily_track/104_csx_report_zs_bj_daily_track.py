...
def login_down_file():
    ...
    #拼接目录
    remote_directory='WEB-INF/schedule/北京直送日情况跟踪/' #改
    # 进入目标目录
    ...
    # 下载Excel文件
    try:
        with open(os.path.join(local_file_path,remote_file_name) , "wb") as f:
...
def OpenWorkbookRunVba():
    local_file_path=os.path.dirname(os.path.abspath(__file__))
    local_file_name='北京直送日情况跟踪_模板.xlsm' #改
    pythoncom.CoInitialize()  #excel多线程相关
    excel = win32.DispatchEx('Excel.Application')
    excel.Visible = True
    excel.DisplayAlerts = False
    wb = excel.Workbooks.Open(os.path.join(local_file_path,local_file_name))
...
def main():
    ...
    res=fei.send_pic('bj_zs_daily.JPG')

if __name__ == '__main__':
	scheduler=BlockingScheduler()
	#scheduler.add_job(main,'cron',hour='9',minute='30',coalesce=True, misfire_grace_time=300)
	#scheduler.start()
