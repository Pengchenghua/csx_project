...
-- 设置任务名
set mapred.job.name=report_sale_r_w_customer_new_regular;
-- 动态分区
...
-- 目标表
set target_table=csx_dw.report_sale_r_w_customer_new_regular;
...
(
  ...
  (
    select
      sdt,customer_no,region_code,region_name,province_code,province_name,city_group_code,city_group_name,channel_code,business_type_code,business_type_name,sales_value,profit
    from csx_dw.dws_sale_r_d_detail
    where sdt>=${hiveconf:current_week_start_day} and sdt<=${hiveconf:current_day} and channel_code in ('1','2','7','9')
  ) a left join
  (
    select
...
select
  concat_ws('&', week_of_year, province_code, channel_type, channel_type_detail) as biz_id,
  region_code, -- 大区编码
  region_name, -- 大区名称
  province_code, -- 省区编码
  province_name, -- 省区名称
...
(
  ...
  group by region_code, region_name, province_code, province_name,city_group_code,city_group_name, channel_type, channel_type_detail, week_of_year
  union all
  select
    region_code, -- 大区编码
    region_name, -- 大区名称
    ...
    sum(profit) as total_profit
  from current_customer_sales
  where channel_type = 'B端'
  group by region_code, region_name, province_code, province_name,city_group_code,city_group_name, channel_type, week_of_year
) as t1;
