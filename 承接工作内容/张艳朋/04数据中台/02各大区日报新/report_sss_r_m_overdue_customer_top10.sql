...
from
	...
	select
		...
		a.overdue_amount,
		row_number() over(partition by city_group_name order by overdue_amount desc) as ranking
	from
		( 
		select --应收逾期
			customer_no,
		...
		left join -- 客户信息
			(
			select 
				customer_no,customer_name,sales_region_code,sales_region_name,province_code,province_name,city_group_code,city_group_name,work_no,sales_name,first_supervisor_work_no,first_supervisor_name
			from 
