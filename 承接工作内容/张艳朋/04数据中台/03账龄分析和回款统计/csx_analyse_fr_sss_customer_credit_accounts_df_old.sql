...
from 
	...
	left join
		(
		select 
			customer_code,credit_code,sign_company_code,last_sale_date,last_to_today_days,customer_sign_company_active_status_code,customer_sign_company_active_status_name
		from 
			csx_dws.csx_dws_crm_credit_sign_company_active
		where 
			sdt ='current'
		) b on a.customer_code= b.customer_code and a.company_code = b.sign_company_code and a.credit_code=b.credit_code
	left join
		(
		select
