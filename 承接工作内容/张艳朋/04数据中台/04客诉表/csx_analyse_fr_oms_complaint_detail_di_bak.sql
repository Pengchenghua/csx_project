...
select
	...
	unit_name,purchase_qty,purchase_unit_name,purchase_unit_rate,sale_price,complaint_amt,complaint_describe,evidence_imgs,channel_type_code,responsible_user_id,
	...
	reason,result,plan,complaint_deal_status,disagree_reason,replay_report,reject_reason,disagreement_reason,complaint_reject_status,task_sync_time,complaint_date,
	basic_performance_province_code,basic_performance_province_name,basic_performance_city_code,basic_performance_city_name,complaint_date_time,complaint_time_de,deal_date,
	processing_time,complaint_status_name,sdt
from 
	(
	select 
		*,
		...
		case when complaint_status_code=10 then '待处理'
			when complaint_status_code=20 then '已处理待确认'
			when complaint_status_code=21 then '驳回待确认'
			when complaint_status_code=30 then '已处理'
...
`complaint_deal_status`          int                 COMMENT    '是否认同 1-认同 2-不认同',
...
`complaint_status_name`          string              COMMENT    '客诉状态'
