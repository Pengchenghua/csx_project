...
select
	concat_ws('&',cast(d.customer_id as string),coalesce(a.credit_code,''),a.company_code,'${ytd}') as biz_id,
	...
	coalesce(a.credit_code,'') as credit_code,
	coalesce(a.credit_business_attribute_code,'') as credit_business_attribute_code,
	coalesce(a.credit_business_attribute_name,'') as credit_business_attribute_name,
...
from
	...
	select
		...
		overdue_coefficient_numerator, -- 逾期金额*逾期天数 计算因子，用于计算逾期系数分子
		overdue_coefficient_denominator, -- 应收金额*账期天数 计算因子，用于计算逾期系数分母
		credit_code,credit_business_attribute_code,credit_business_attribute_name
	from
