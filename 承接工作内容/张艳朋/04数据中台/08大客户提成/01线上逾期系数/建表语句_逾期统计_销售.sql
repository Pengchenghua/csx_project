...
alter table report_sss_r_m_crm_customer_sale_detail add (
`credit_code` varchar(16) DEFAULT NULL COMMENT '信控号',
`credit_business_attribute_code` varchar(16) DEFAULT NULL COMMENT '信控业务属性编码',
`credit_business_attribute_name` varchar(16) DEFAULT NULL COMMENT '信控业务属性名称',
`sign_company_code` varchar(16) DEFAULT NULL COMMENT '签约公司编码'
)

delete from csx_crm_report.report_sss_r_m_crm_customer_sale_detail where month>=date_format(date_sub(date_sub(curdate(),interval 1 day ), interval 1 month), '%Y%m');
delete from csx_crm_report.report_sss_r_m_crm_customer_sale_detail where month>=date_format(date_sub(now(), interval 1 month), '%Y%m');
