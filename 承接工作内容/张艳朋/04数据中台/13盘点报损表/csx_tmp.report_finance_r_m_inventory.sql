...
from
	...
	from
		...
		select
			...
			if(substr(move_type,1,3) in ('117'),if(direction='-',-1*amt_no_tax,amt_no_tax),0) as bs_amt_no_tax   --报损不含税金额
		from 
	...
	left join 
		(
		select 
			shop_id,shop_name,province_code,province_name,city_code,city_name,warehouse_purpose_name
		from 
...
INVALIDATE METADATA csx_tmp.report_finance_r_m_inventory;


/*
--------------------------------- hive建表语句 -------------------------------
-- csx_tmp.report_finance_r_m_inventory  盘点报损-计算

drop table if exists csx_tmp.report_finance_r_m_inventory;
create table csx_tmp.report_finance_r_m_inventory(
