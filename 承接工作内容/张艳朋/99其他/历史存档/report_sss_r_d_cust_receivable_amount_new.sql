...
from
		   ...
		   case when  account_period_code = 'Z007'  and unpaid_amounts>=residual_amount and  unpaid_amounts-residual_amount<unpaid_amount then unpaid_amounts-residual_amount
			...
			from
				...
				select
					...
					t2.cus_type,t2.settle_cycle,--t2.project_end,
					case when date_format(t1.happen_date,'yyyy-MM-dd')>=date_format(t2.project_begin,'yyyy-MM-dd') 
							  and date_format(t1.happen_date,'yyyy-MM-dd')<=date_format(t2.project_end,'yyyy-MM-dd')
						  then t2.project_end 
					...
					COALESCE(t4.diff, 0)account_period_val,t3.residual_amount,
					t1.unpaid_amounts
				from
					(
					select 
						a.source_bill_no,a.customer_code,a.company_code,a.source_statement_amount,a.money_back_status,a.unpaid_amount,a.bad_debt_amount,a.residue_total_amount,a.source_sys,
						to_date(a.happen_date) as happen_date,
						b.unpaid_amount_hour,
						nvl(b.unpaid_amount_hour,0)+sum(a.unpaid_amount)
										over (partition by a.customer_code, a.company_code,a.hour order by concat_ws(',',a.happen_date,cast(a.id as string ))) unpaid_amounts
					from
						( 
						select 
							*,
							date_format(happen_date, 'yyyy-mm-dd hh') hour
						from 
						...
						left join
							...
							select 
								...
								company_code,
								hour,
								sum(unpaid_amount_hour)over (partition by customer_code, company_code order by hour rows between unbounded preceding and 1 preceding) unpaid_amount_hour
							from 
								(
								select 
									customer_code,
									company_code,
									date_format(happen_date, 'yyyy-mm-dd hh') hour,
									sum(unpaid_amount) unpaid_amount_hour
								from 
									csx_ods.source_sss_r_a_source_bill
								where 
									sdt=${hiveconf:current_day}
								...
								group by 
									customer_code,
									company_code,
									date_format(happen_date, 'yyyy-mm-dd hh')
								) bb
							) b on a.customer_code=b.customer_code and a.company_code =b.company_code and a.hour=b.hour
					) t1
					left join 
						(
						select 
	...
	from
		(
		select
			order_no,customer_no,company_code,happen_date,source_statement_amount,money_back_status,
			...
			--unpaid_amount,
			account_period_code,account_period_name,account_period_val,
			beginning_mark,bad_debt_amount,
			case when  account_period_code in ('Z001','Z002','Z003','Z004','Z005','Z006') and ( project_end is null or  project_end='')
						then date_add(happen_date, COALESCE( account_period_val, 0))
					when account_period_code like 'Y%' and (project_end is null or project_end='') and (settle_cycle is  null or settle_cycle='')
					...
					when project_end is not null and project_end<>'' then date_add(substr(project_end,1,10),int(account_period_val-1)) 
					---预付款客户 逾期 按照大于客户的信控额度的上期逾期日期+1天
					when  account_period_code = 'Z007' and unpaid_amount>residual_amount then date_add(happen_date,1) -- and unpaid_amount>residue_total_amount 
					when (project_end is null or project_end='')  then date_format(current_date,'yyyy-MM-dd')
					when (account_period_code is null or account_period_code ='')  
					  and  (settle_cycle is  null or settle_cycle='') 
					  and ( project_end is null or  project_end='') 
		...
		from
			...
			select 
				id as order_no,	-- 来源单号
				t1.customer_code as customer_no,	-- 客户编码
				t1.company_code,	-- 签约公司编码		
				happen_date,	-- 发生时间		
				--overdue_date,	-- 逾期时间	
				...
				bad_debt_amount,	--坏账金额
				--if((money_back_status<>'ALL' or (datediff(${hiveconf:current_day1}, overdue_date)+1)>=1),datediff(${hiveconf:current_day1}, overdue_date)+1,0) as over_days	-- 逾期天数	
				project_end,settle_cycle,project_begin,residual_amount
			from 
				(
				select
...
select
  ...
  a.bad_debt_amount,	--坏账金额
  a.receivable_amount,	-- 应收金额
  if(a.over_amt<0,0,over_amt) as over_amt ,	-- 逾期金额
  a.max_over_days,		--最大逾期天数
  a.claim_amount,	--认领回款金额（含核销与未核销的，含补救单）
  a.payment_amount_1,		--认领回款中已核销金额  
