...
from
	...
	from
		...
		select 
			...
			from 
				csx_ods.source_sss_r_a_source_bill as t1 --客户对账来源单---全量未处理
				left join 
					(
					select 
						customer_number,
			...
			where 
				t1.sdt=${hiveconf:current_day}
				and t1.beginning_mark='1'  	-- 期初标识 0-是 1-否
				--and money_back_status<>'ALL'	
				and date(t1.happen_date)<=${hiveconf:current_day1}
		) t1
	union all
	select
	...
	from
		...
		from
			...
			from 
				csx_ods.source_sss_r_a_beginning_receivable as t1  --期初应收账款表---全量未处理 
				left join 
					(
					select 
						customer_number,
