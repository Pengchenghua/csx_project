...
select
  ...
  a.receivable_amount,a.overdue_amount,a.bad_debt_amount,a.max_overdue_day,
  b.estimate_contract_amount,b.sign_date,a.rno1
from
(
  select province_name,customer_no,customer_name,company_code,payment_name,receivable_amount,overdue_amount,bad_debt_amount,max_overdue_day,
    rank() over (partition by province_name order by overdue_amount desc ) as rno1
...
select
  a.province_name,a.customer_no,b.customer_name,a.company_code,a.payment_name,b.attribute_desc,b.work_no,b.sales_name,
  a.receivable_amount,a.overdue_amount,a.bad_debt_amount,a.max_overdue_day,
  b.estimate_contract_amount,b.sign_date,
  a.rno1,a.rno2,a.rno3
from
(
  select province_name,customer_no,customer_name,company_code,payment_name,receivable_amount,overdue_amount,bad_debt_amount,max_overdue_day,
    rank() over (partition by province_name order by overdue_amount desc ) as rno1, --逾期金额排名
	rank() over (partition by province_name order by overdue_amount/receivable_amount desc ) as rno2, --逾期率排名
	rank() over (partition by province_name order by max_overdue_day desc ) as rno3 --逾期天数排名
  from csx_dw.dws_sss_r_a_customer_accounts
  where sdt=regexp_replace(date_sub(current_date, 1), '-', '')
  and province_name='四川省'
