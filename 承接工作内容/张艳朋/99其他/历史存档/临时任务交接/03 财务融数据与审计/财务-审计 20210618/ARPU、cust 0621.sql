...
(
  select a.smonth,a.province_name,
    ...
    sum(case when a.ripei_excluding_tax_sales is not null and (substr(b.normal_first_order_date,1,6)<>a.smonth or
    ...
    sum(case when a.ripei_excluding_tax_sales is not null and substr(b.first_order_date,1,6)=a.smonth and substr(b.normal_first_order_date,1,6)=a.smonth then a.ripei_excluding_tax_sales end) ripei_excluding_tax_sales_new,
    count(distinct case when a.ripei_excluding_tax_sales is not null then a.customer_no end) ripei_counts,
    count(distinct case when a.ripei_excluding_tax_sales is not null and (substr(b.normal_first_order_date,1,6)<>a.smonth or
            (substr(b.first_order_date,1,6)<>a.smonth and substr(b.normal_first_order_date,1,6)=a.smonth) or substr(b.normal_first_order_date,1,6) is null)
          then a.customer_no end) ripei_counts_old,
    count(distinct case when a.ripei_excluding_tax_sales is not null and substr(b.first_order_date,1,6)=a.smonth and substr(b.normal_first_order_date,1,6)=a.smonth then a.customer_no end) ripei_counts_new,
    
    sum(a.B_nof_excluding_tax_sales) B_nof_excluding_tax_sales,
    sum(case when a.B_nof_excluding_tax_sales is not null and (substr(b.B_nof_first_order_date,1,6)<>a.smonth or substr(b.B_nof_first_order_date,1,6) is null) then a.B_nof_excluding_tax_sales end) B_nof_excluding_tax_sales_old,
    sum(case when a.B_nof_excluding_tax_sales is not null and substr(b.B_nof_first_order_date,1,6)=a.smonth then a.B_nof_excluding_tax_sales end) B_nof_excluding_tax_sales_new,
    count(distinct case when a.B_nof_excluding_tax_sales is not null then a.customer_no end) B_nof_counts,
    count(distinct case when a.B_nof_excluding_tax_sales is not null and (substr(b.B_nof_first_order_date,1,6)<>a.smonth or substr(b.B_nof_first_order_date,1,6) is null) then a.customer_no end) B_nof_counts_old,
    count(distinct case when a.B_nof_excluding_tax_sales is not null and substr(b.B_nof_first_order_date,1,6)=a.smonth then a.customer_no end) B_nof_counts_new,
    
    sum(a.B_excluding_tax_sales) B_excluding_tax_sales,
    sum(case when a.B_excluding_tax_sales is not null and (substr(b.B_first_order_date,1,6)<>a.smonth or substr(b.B_first_order_date,1,6) is null) then a.B_excluding_tax_sales end) B_excluding_tax_sales_old,
    sum(case when a.B_excluding_tax_sales is not null and substr(b.B_first_order_date,1,6)=a.smonth then a.B_excluding_tax_sales end) B_excluding_tax_sales_new,
    count(distinct case when a.B_excluding_tax_sales is not null then a.customer_no end) B_counts,
    count(distinct case when a.B_excluding_tax_sales is not null and (substr(b.B_first_order_date,1,6)<>a.smonth or substr(b.B_first_order_date,1,6) is null) then a.customer_no end) B_counts_old,
    count(distinct case when a.B_excluding_tax_sales is not null and substr(b.B_first_order_date,1,6)=a.smonth then a.customer_no end) B_counts_new,
    ...
    count(distinct case when a.BBBC_excluding_tax_sales is not null then a.customer_no end) BBBC_counts,
    count(distinct case when a.BBBC_excluding_tax_sales is not null and (substr(b.first_order_date,1,6)<>a.smonth or substr(b.first_order_date,1,6) is null) then a.customer_no end) BBBC_counts_old,
    count(distinct case when a.BBBC_excluding_tax_sales is not null and substr(b.first_order_date,1,6)=a.smonth then a.customer_no end) BBBC_counts_new
  from
  (
...
(  
  select a.smonth,a.province_name,
  --客户数
    count(distinct case when ripei_excluding_tax_sales is not null then a.customer_no end) ripei_counts,
	count(distinct case when fuli_excluding_tax_sales is not null then a.customer_no end) fuli_counts,
	count(distinct case when ripei_excluding_tax_sales is not null and fuli_excluding_tax_sales is not null then a.customer_no end) c_rpfl_counts,
	
	count(distinct case when B_nof_excluding_tax_sales is not null then a.customer_no end) B_nof_counts,
	count(distinct case when BBC_excluding_tax_sales is not null then a.customer_no end) BBC_counts,
	count(distinct case when B_nof_excluding_tax_sales is not null and BBC_excluding_tax_sales is not null then a.customer_no end) c_B_nof_BBC_counts,
	
	count(distinct case when B_nof_BBC_excluding_tax_sales is not null then a.customer_no end) B_nof_BBC_counts,
	count(distinct case when fws_excluding_tax_sales is not null then a.customer_no end) fws_counts,
	count(distinct case when B_nof_BBC_excluding_tax_sales is not null and fws_excluding_tax_sales is not null then a.customer_no end) c_B_nof_BBC_fws_counts,
		
	count(distinct case when BBBC_excluding_tax_sales is not null then a.customer_no end) BBBC_counts,
	count(distinct case when dazong_excluding_tax_sales is not null then a.customer_no end) dazong_counts,
	count(distinct case when all_excluding_tax_sales is not null then a.customer_no end) all_counts,
	count(distinct case when B_excluding_tax_sales is not null then a.customer_no end) B_counts, --B端含合伙人

  --首次履约客户数
    count(distinct case when substr(b.normal_first_order_date,1,6)=a.smonth and ripei_excluding_tax_sales is not null then a.customer_no end) ripei_counts_new,
	count(distinct case when substr(b.welfare_first_order_date,1,6)=a.smonth and fuli_excluding_tax_sales is not null then a.customer_no end) fuli_counts_new,
	count(distinct case when substr(b.normal_first_order_date,1,6)=a.smonth and substr(b.welfare_first_order_date,1,6)=a.smonth and 
	                  ripei_excluding_tax_sales is not null and fuli_excluding_tax_sales is not null then a.customer_no end) c_rpfl_counts_new,
	
	count(distinct case when substr(b.b_nof_first_order_date,1,6)=a.smonth and B_nof_excluding_tax_sales is not null then a.customer_no end) B_nof_counts_new,
	count(distinct case when substr(b.bbc_first_order_date,1,6)=a.smonth and BBC_excluding_tax_sales is not null then a.customer_no end) BBC_counts_new,
	count(distinct case when substr(b.b_nof_first_order_date,1,6)=a.smonth and substr(b.bbc_first_order_date,1,6)=a.smonth and 
	                  B_nof_excluding_tax_sales is not null and BBC_excluding_tax_sales is not null then a.customer_no end) c_B_nof_BBC_counts_new,
	
	count(distinct case when substr(b.B_nof_BBC_first_order_date,1,6)=a.smonth and B_nof_BBC_excluding_tax_sales is not null then a.customer_no end) B_nof_BBC_counts_new,
	count(distinct case when substr(b.fws_first_order_date,1,6)=a.smonth and fws_excluding_tax_sales is not null then a.customer_no end) fws_counts_new,
	count(distinct case when substr(b.B_nof_BBC_first_order_date,1,6)=a.smonth and substr(b.fws_first_order_date,1,6)=a.smonth and 
	                  B_nof_BBC_excluding_tax_sales is not null and fws_excluding_tax_sales is not null then a.customer_no end) c_B_nof_BBC_fws_counts_new,
		
	count(distinct case when substr(b.first_order_date,1,6)=a.smonth and BBBC_excluding_tax_sales is not null then a.customer_no end) BBBC_counts_new,
	count(distinct case when substr(b.dazong_first_order_date,1,6)=a.smonth and dazong_excluding_tax_sales is not null then a.customer_no end) dazong_counts_new,
	count(distinct case when substr(b.all_first_order_date,1,6)=a.smonth and all_excluding_tax_sales is not null then a.customer_no end) all_counts_new,
	count(distinct case when substr(b.B_first_order_date,1,6)=a.smonth and B_excluding_tax_sales is not null then a.customer_no end) B_counts_new --B端含合伙人	
...
select a.smonth,a.province_name,a.customer_no,b.first_order_date,
    ...
    sum(case when substr(b.first_order_date,1,6)=a.smonth then a.BBBC_excluding_tax_sales end) BBBC_excluding_tax_sales_new,
    count(distinct case when a.BBBC_excluding_tax_sales is not null then a.customer_no end) BBBC_counts,
    count(distinct case when a.BBBC_excluding_tax_sales is not null and substr(b.first_order_date,1,6)<>a.smonth then a.customer_no end) BBBC_counts_old,
    count(distinct case when a.BBBC_excluding_tax_sales is not null and substr(b.first_order_date,1,6)=a.smonth then a.customer_no end) BBBC_counts_new
  from
  (
    select substr(sdt,1,6) smonth,province_name,customer_no,
