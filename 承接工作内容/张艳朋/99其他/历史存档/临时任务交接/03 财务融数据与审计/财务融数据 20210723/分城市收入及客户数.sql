...
select province_name,substr(sdt,1,6) smonth,
  --B端自营、bbc、城市服务商、M端--销售额
  sum(case when channel_code in('1','9') and business_type_code <>'4' then excluding_tax_sales end) excluding_tax_sales_B,--销售额
  sum(case when business_type_code ='6' then excluding_tax_sales end) excluding_tax_sales_bbc,
  sum(case when business_type_code ='4' then excluding_tax_sales end) excluding_tax_sales_csfws,	
  sum(case when channel_code ='2' then excluding_tax_sales end) excluding_tax_sales_M,
  
  --B端自营、bbc、B+BBC、城市服务商、M端--客户数
