...
select 
  ...
  a.region_name,a.province_name,
  count(distinct a.customer_no) count_all,
  sum(a.sales_value) sales_value_all,
  sum(a.profit) profit_all,  
  count(distinct case when substr(b.min_sdt,1,6)=a.smonth then a.customer_no end) count_new,
  sum(case when substr(b.min_sdt,1,6)=a.smonth then a.sales_value end) sales_value_new,
  sum(case when substr(b.min_sdt,1,6)=a.smonth then a.profit end) profit_new
from 
(
select 
...
region_name,province_name,customer_no,substr(sdt,1,6) smonth,
sales_value,profit
from csx_dw.dws_sale_r_d_detail
where sdt>='20200101'
and channel_code in('1','7','9')
...
group by a.smonth,a.business_type_code,a.business_type_name,a.region_name,a.province_name
union all
select 
  a.smonth,
  ...
  sum(case when substr(b.first_order_date,1,6)=a.smonth then a.sales_value end) sales_value_new,
  sum(case when substr(b.first_order_date,1,6)=a.smonth then a.profit end) profit_new
from 
(
select 
case when (business_type_code='1' and dc_code in('W0K4')) then '9' else business_type_code end business_type_code,
...
group by a.smonth,a.region_name,a.province_name;


--2、新签约客户数 省区月度
select 
  a.sales_region_name,a.sales_province_name,a.first_sign_month,
  count(distinct a.customer_no) count_sign
from 
--CRM客户信息昨日
	(select *,
...
group by a.sales_region_name,a.sales_province_name,a.first_sign_month;
...
left join 
  (
	...
	from csx_dw.dws_crm_w_a_customer
	where sdt in('20210430','20210531')
...
third_supervisor_work_no,third_supervisor_name,first_supervisor_work_no,first_supervisor_name,
business_type_name,
sum(case when smonth='202104' then count_work_no end) count_cust_4,
sum(case when smonth='202105' then count_work_no end) count_cust_5,

sum(case when smonth='202104' then count_all end) count_all_4,
sum(case when smonth='202105' then count_all end) count_all_5,
sum(case when smonth='202104' then sales_value_all end) sales_value_all_4,
sum(case when smonth='202105' then sales_value_all end) sales_value_all_5,
sum(case when smonth='202104' then count_sign end) count_sign_4,
sum(case when smonth='202105' then count_sign end) count_sign_5,
...
third_supervisor_work_no,third_supervisor_name,first_supervisor_work_no,first_supervisor_name,
'' count_work_no,count_all,sales_value_all,'' count_sign,'' count_75,'' count_50
...
union all
select
'202105' smonth,'0' business_type_code,'总体' business_type_name,
sales_region_name as region_name,sales_province_name as province_name,city_group_name,
third_supervisor_work_no,third_supervisor_name,first_supervisor_work_no,first_supervisor_name,
'' count_work_no,'' count_all,'' sales_value_all,'' count_sign,count_75,count_50
