...
left join 
  (
	...
	where sdt in('20210630','20210731')
...
a.work_no,a.sales_name,if(b.user_id is not null, '是', '否') as is_part_time_service_manager,
business_type_name,
...
left join
(
select distinct sales_id,work_no,sales_name from csx_dw.dws_crm_w_a_customer where sdt='20210731'
)b1 on b1.work_no=a.work_no
--销售员是否兼职服务管家
left join 
(
  select user_id
  from csx_ods.source_uc_w_a_user_position
  where sdt = '20210731' and user_position = 'CUSTOMER_SERVICE_MANAGER'
)b on b1.sales_id = b.user_id
group by 
region_name,province_name,city_group_name,
--third_supervisor_work_no,third_supervisor_name,first_supervisor_work_no,first_supervisor_name,
a.work_no,a.sales_name,if(b.user_id is not null, '是', '否'),
business_type_name;
