...
set date_current =regexp_replace(date_sub(current_date,1+12),'-','');  --昨日
set date_current_1 =date_sub(current_date,1+12);  --昨日
set date_14before =regexp_replace(date_sub(current_date,1+14+12),'-','');
set date_14before_1 =date_sub(current_date,1+14+12);  
set date_7before =regexp_replace(date_sub(current_date,1+7+12),'-','');
set date_7before_1 =date_sub(current_date,1+7+12); 
set date_21before =regexp_replace(date_sub(current_date,1+21+12),'-','');
set date_21before_1 =date_sub(current_date,1+21+12); 
...
where sdt='20210710'
--where sdt>=${hiveconf:date_7before} and sdt<=${hiveconf:date_current} 
and dc_code in('W0A6','W0A8','W0A2','W0A3','W0A7'))a
--商品维表
left outer join 
  (
    select *
