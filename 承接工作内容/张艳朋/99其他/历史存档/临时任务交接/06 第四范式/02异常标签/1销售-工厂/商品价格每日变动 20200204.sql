...
left join (select distinct substr(sdt,1,6)smonth,customer_no from csx_dw.csx_partner_list1 where sdt>=regexp_replace(${hiveconf:i_sdate_l35},'-','')) b 
...
group by case when a.order_no like 'RH%' then c.sdt else a.sdt end,goods_code,dc_code;
