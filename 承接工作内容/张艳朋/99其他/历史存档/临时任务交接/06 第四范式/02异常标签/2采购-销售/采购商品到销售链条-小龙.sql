...
(
  ...
  where ((sdt >= '20210401' and sdt <= '20210409')
    or (sdt = '19990101' and substr(order_time,1,10) >= '20210401' 
	  and substr(order_time,1,10) <= '20210409'))
  group by order_code,goods_code
)a 
--第一步结果表 工厂工厂-凭证明细-入库批次明细
left join csx_tmp.tmp_factory_order_to_scm_11 b 
...
select 
  ...
  goods_reality_receive_qty,
  fact_qty,
  fact_values/fact_qty as fact_price,
  is_fact, --是否有原料价
  order_code, --批次入库单号或工单号
  a.goods_code,
  b.credential_no,
  c.is_purchase,  --是否有采购报价
  cost_price,  
  purchase_price,
  middle_office_price,
  sales_price,
...
(
  ...
  where sdt >= '20210401'
  group by split(id,'&')[0],goods_code,case when purchase_price_flag='1' then '是' end
)c on b.goods_code = c.goods_code and b.credential_no = c.credential_no
;


采购入库近10日采购价 原料价 成品价 采购报价
