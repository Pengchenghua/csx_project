...
set last_14day="20201001";

drop table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_0;
create temporary table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_0
  as
select 
  factory_order_code,
  a.goods_code,
...
(
  ...
  (
    ...
    where sdt >= ${hiveconf:last_14day} and mrp_prop_key in('3061','3010') 
...
drop table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_1;
create temporary table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_1
  as
select 
  a.scm_sdt,
...
(
  ...
  and ((sdt >= ${hiveconf:last_14day} and sdt <= ${hiveconf:current_day})
    or (sdt = '19990101' and substr(order_time,1,10) >= ${hiveconf:last_14day} 
    and substr(order_time,1,10) <= ${hiveconf:current_day}))
  and super_class in (1,3)   --1 供应商订单 
  --and source_type<>4 --剔除项目合伙人
  and source_type not in (2,3,4,11,15,16)   --剔除直送
...
left join 
csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_0 b on a.order_code = b.scm_order_code and a.goods_code = b.product_code
left join 
(
  select * from csx_dw.dws_basic_w_a_csx_shop_m where sdt = 'current'
)c on c.shop_id = a.target_location_code
...
drop table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_2;
create temporary table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_2
  as
select 
  a.scm_sdt,
  ...
  f.goods_name scm_goods_name,  --采购入库商品或原料名称
  a.order_qty,
  a.received_qty,
  a.received_price,
  a.received_amount,
...
from csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_1 a 
--入库批次明细表
left join 
(
  select
...
left join 
(
  select * from csx_dw.dws_dsfs_r_d_goods_received_d where scm_sdt>='20201001'
)d on d.order_code = a.scm_order_code and d.goods_code = a.scm_goods_code
--商品维表
left join 
(
  select * from csx_dw.dws_basic_w_a_csx_product_m where sdt='current'
)e on e.goods_id = a.product_code 
--商品维表
left join 
(
  select * from csx_dw.dws_basic_w_a_csx_product_m where sdt='current'
)f on f.goods_id = a.scm_goods_code ;


--临时表4：采购入库到最终入库批次明细+关联采购报价
drop table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_3;
create table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_3
  as
select 
  a.*,b.purchase_price
from csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_2 a
left join 
(
  ...
  (
    ...
    (
      ...
      where calday >=${hiveconf:last_14day}
...
--临时表5：历史采购报价 原料价
drop table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_4;
create table csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_4
  as
select distinct
  goods_code,
  DC_DC_code,
...
(
  ...
  from csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_3 
)a;
...
insert overwrite table csx_dw.dws_dsfs_r_d_received_batch_detail_abnormal_label partition(sdt)
select 
  ...
  batch_price, --批次成本价
  purchase_price,
  received_price_ls,received_price_last,received_price_yc,  --历史入库价、上一次入库价、永辉入库价
  fact_price_ls,purchase_price_ls,   --历史原料价、历史采购价  
  ...
  if(received_price_hight='1' or received_price_low='1',1,0) as received_price_abnormal,  --采购入库价异常 去掉价格突涨突降
  ...
  sdt_end as sdt --加工后或采购单到批次日期
from 
(
  select a.*,b.fact_price_ls,b.purchase_price_ls,
    --工厂加价异常高:工厂加价率大于30%
  ...
  from 
  csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_3 a 
  left join csx_tmp.tmp_dws_dsfs_r_d_received_batch_detail_abnormal_label_4 b on b.goods_code=a.goods_code and b.DC_DC_code=a.DC_DC_code and b.sdt_end=a.sdt_end
)a;
...
---------------------------------------------------------------------------------------------------------
---------------------------------------------hive 建表语句-----------------------------------------------
--采购入库到工厂到批次各环节异常标签 csx_dw.dws_dsfs_r_d_received_batch_detail_abnormal_label

drop table if exists csx_dw.dws_dsfs_r_d_received_batch_detail_abnormal_label;
CREATE TABLE `csx_dw.dws_dsfs_r_d_received_batch_detail_abnormal_label`(
  ...
  `scm_goods_name` string COMMENT '采购商品或原料名称',
  ...
  `received_price_ls` decimal(20,6) COMMENT '历史入库价',
  `received_price_last` decimal(20,6) COMMENT '上一次入库价',
  `received_price_yc` decimal(20,6) COMMENT '永辉入库价',
  `fact_price_ls` decimal(20,6) COMMENT '历史原料价',
  `purchase_price_ls` decimal(20,6) COMMENT '历史采购价',  
...
select DC_province_name,
  count(distinct concat_ws('-',scm_order_code,scm_goods_code)) scm_count,
  count(distinct case when received_price_abnormal='1' then concat_ws('-',scm_order_code,scm_goods_code) end) scm_count_yc,
  count(distinct id) count,
  count(distinct case when fact_price_abnormal='1' then id end) fact_price_count_yc,
  count(distinct case when gc_price_abnormal='1' then id end) gc_price_count_yc,
  count(distinct case when purchase_price_abnormal='1' then id end) purchase_price_count_yc
from csx_tmp.tmp_received_batch_detail_abnormal_label 
where sdt>='20210801'
group by DC_province_name;
