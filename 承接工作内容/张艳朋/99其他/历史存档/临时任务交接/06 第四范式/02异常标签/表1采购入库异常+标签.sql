...
-- 统计日期
set current_day = '20210220';
set current_day1 = '2021-02-20';
--前3天
set current_day_3bf ='20210217';
-- 14天前
set current_start_day ='20210206';
set current_start_day1 ='2021-02-06';
-- 库存操作起始日期
set wms_start_day ='20200701';




--临时表1：明细数据 采购入库表中取到采购单商品维度数据+采购量 采购单价，关联凭证号获取销售相关数据，为最细粒度明细数据
drop table csx_tmp.tmp_goods_received1;
create temporary table csx_tmp.tmp_goods_received1
as
select 
  aa.source_type,
  super_class,
...
--结果表1标签1
drop table csx_tmp.tmp_goods_received1_d1;
create table csx_tmp.tmp_goods_received1_d1
as
select a.*,
...
from csx_tmp.tmp_goods_received1 
...
(
  ...
  from 
	...
	where a.cn1=1
  group by goods_code,DC_DC_code
)c on c.goods_code=a.goods_code and c.DC_DC_code=a.DC_DC_code
--近3天永辉入库价
left join 
  (
  select
    a.goods_code,d.province_name,
  ...
  (
    ...
    where sdt='current' 
	and table_type ='2'
    and  sales_province_name in('重庆市','安徽省')
  ) as d on a.shop_id_in=d.shop_id
  group by a.goods_code,d.province_name
 )d on d.goods_code=a.goods_code and d.province_name=a.DC_province_name;
