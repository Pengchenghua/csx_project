...
select province_name,
  ...
  sum(capital_takes_up) as capital_takes_up      -- 资金占用费     	
...
(
  ...
  --regexp_replace(send_date,'-','') as sdt,
    ...
    sum(transport_amount)/1.09 as transport_amount      -- 运费
  from csx_dw.dws_tms_r_d_entrucking_order_detail a
  --where customer_code in('118582','117454','121125')
  left semi
  join 
  (
    select * 
  ...
  where sdt>='20210801'
  and regexp_replace(send_date,'-','')>='20210801' and regexp_replace(send_date,'-','')<='20210831'
