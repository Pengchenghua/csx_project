...
set mapred.job.name=report_sale_r_d_month_negative_customer;
...
-- 计算日期
-- 统计起始日期, 当月第一天
set start_day = regexp_replace(trunc(date_sub(current_date,1),'MM'),'-','');

-- 终止日期（昨天）
set end_day = regexp_replace(date_sub(current_date, 1), '-', '');

-- 当前月
set current_month = substr(${hiveconf:end_day}, 1, 6);

-- 销售明细表
set source_sale_detail=csx_dw.dws_sale_r_d_detail;

-- 目标表
set target_table=csx_dw.report_sale_r_d_month_negative_customer;

with current_month_2b_sale as
(
  select
    region_code,
    region_name,
...
(
  ...
  (
    ...
    from current_month_2b_sale
    where customer_goods_profit_no = 1 and region_code = '0'
  )t2 on t1.customer_no = t2.customer_no
)
insert overwrite table ${hiveconf:target_table} partition(sdt)
-- 全国top20客户
select
  concat_ws('&', '0', customer_no, ${hiveconf:end_day}) as id,
  '0' as statistic_area,
  region_code,
  region_name,
  province_code,
...
-- 省区top20客户
select
  concat_ws('&', '2', customer_no, ${hiveconf:end_day}) as id,
  '2' as statistic_area,
  region_code,
  region_name,
  province_code,
...
-- 城市top20客户
select
  concat_ws('&', '3', customer_no, ${hiveconf:end_day}) as id,
  '3' as statistic_area,
  region_code,
  region_name,
  province_code,
