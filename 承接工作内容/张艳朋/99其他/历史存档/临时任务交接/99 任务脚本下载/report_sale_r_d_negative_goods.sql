...
-- 切换tez计算引擎
set mapred.job.name=report_sale_r_d_negative_goods;
...
-- 库存操作起始日期
set wms_start_day = regexp_replace(date_sub(current_date, 180), '-', '');
...
-- 目标表
set target_table = csx_dw.report_sale_r_d_negative_goods;
...
select
  ...
  t1.goods_code,
  t1.goods_name,
  cast(t1.sales_qty as decimal(26,6)) as sales_qty,
  t1.sales_value,
  t1.sales_cost,
  t1.profit,
  cast(t1.cost_price as decimal(26,6)) as cost_price,
  cast(t1.purchase_price as decimal(26,6)) as purchase_price,
  cast(t1.middle_office_price as decimal(26,6)) as middle_office_price,
  cast(t1.sales_price as decimal(26,6)) as sales_price,
  cast(t3.fact_price as decimal(26,6)) as fact_price,
...
select
  -- 唯一主键
  concat_ws('&', '0', region_code, goods_code, ${hiveconf:current_day}) as id,
  -- 全国范围标识
...
-- 大区范围
 select
  -- 唯一主键
  concat_ws('&', '1', region_code, goods_code, ${hiveconf:current_day}) as id,
  -- 大区范围标识
...
select
  -- 唯一主键
  concat_ws('&', '2', region_code, goods_code, ${hiveconf:current_day}) as id,
  -- 省区范围标识
...
select
  -- 唯一主键
  concat_ws('&', '3', city_group_code, goods_code, ${hiveconf:current_day}) as id,
  -- 城市范围标识
