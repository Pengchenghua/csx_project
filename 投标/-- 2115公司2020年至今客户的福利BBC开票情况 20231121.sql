-- 2115公司2020年至今客户的福利BBC\日配开票情况 20231121
with temp_bbc_kp_year as (        
select
        a.customer_code,
        a.company_code,
...
-- 销售客户
select substr(sdt,1,4) years,performance_province_name,
customer_code,
customer_name,
sum(sale_amt)/10000 sale_amt,
sum(sum(sale_amt))over() total_amt
from csx_dws.csx_dws_sale_detail_di
where sdt>='20220101'
and business_type_code in ('2','6')
and company_code='2115'
...
customer_name,
 substr(sdt,1,4) 
order by sum(sum(sale_amt))over() desc 
...
where
 ...
 -- 更新日期
  (
    sdt >= regexp_replace(
      to_date(trunc(date_sub(current_timestamp(), 8), 'MM')),
      '-',
      ''
    )
    and sdt <= regexp_replace(to_date(date_sub(current_timestamp(), 1)), '-', '')
  )
  or sdt < regexp_replace(
    to_date(
      trunc(
        months_sub(date_sub(current_timestamp(), 1), 24),
        'YYYY'
      )
    ),
    '-',
