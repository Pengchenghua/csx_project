...
(
	...
	from
	(
		select d.order_no,e.classify_middle_name,sum(d.total_amount) total_amount
		from 	
			(
				select *,
				--  id排名取最新一条
			...
			(
				...
				where sdt = 'current'
			)e on d.product_code=e.goods_code
			where d.id_rank=1
			group by d.order_no,e.classify_middle_name
	)a
	group by order_no
)d on d.order_no=b.order_code
-- and b.num1=1
;
