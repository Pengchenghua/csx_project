...
-- 发票商品-大类
apply_classify_large_name as
(
	select order_no,
	-- concat_ws('、',collect_list(concat(classify_large_name,cast(total_amount as decimal(20,2))))) as classify_large_name_group
	concat_ws('、',collect_list(classify_large_name)) as classify_large_name_group
	from
	(
		select order_no,classify_large_name,sum(total_amount) total_amount
		from apply_goods_group
		group by order_no,classify_large_name
	)a
	group by order_no
),

-- 发票商品-中类
apply_classify_middle_group as
(
	select order_no,
	-- concat_ws('、',collect_list(concat(classify_middle_name,cast(total_amount as decimal(20,2))))) as classify_middle_name_group
	concat_ws('、',collect_list(classify_middle_name)) as classify_middle_name_group
	from
	(
		select order_no,classify_middle_name,sum(total_amount) total_amount
		from apply_goods_group
		group by order_no,classify_middle_name
	)a
	group by order_no
),
...
(
	select order_no,
	-- concat_ws('、',collect_list(concat(classify_small_name,cast(total_amount as decimal(20,2))))) as classify_small_name_group
	concat_ws('、',collect_list(classify_small_name)) as classify_small_name_group
	from
	(
		select order_no,classify_small_name,sum(total_amount) total_amount
		from apply_goods_group
		group by order_no,classify_small_name
	)a
	group by order_no
),

-- 发票商品-课组
apply_purchase_group_name as
(
	select order_no,
	-- concat_ws('、',collect_list(concat(purchase_group_name,cast(total_amount as decimal(20,2))))) as purchase_name_group
	concat_ws('、',collect_list(purchase_group_name)) as purchase_name_group
	from
	(
		select order_no,purchase_group_name,sum(total_amount) total_amount
		from apply_goods_group
		group by order_no,purchase_group_name
	)a
	group by order_no
)
...
select 
	...
	d2.classify_middle_name_group,		-- 发票品类明细	
	d1.classify_large_name_group,		-- 发票大类明细	
	d3.classify_small_name_group,		-- 发票小类明细	
	d4.purchase_name_group,		-- 发票课组明细	
	cast('${sdt_yes}' as string) as sdt -- 统计日期 	
from weaver_contract a 
-- 泛微合并信息关联发票表、关联客户信息、关联发票商品对应的课组及大中小类
left join invoice b on a.customer_no=b.customer_code -- and a.company_code=b.company_code
left join customer_info c on c.customer_code=a.customer_no
left join apply_classify_large_name d1 on d1.order_no=b.order_code
left join apply_classify_middle_group d2 on d2.order_no=b.order_code
left join apply_classify_small_name d3 on d3.order_no=b.order_code
left join apply_purchase_group_name d4 on d4.order_no=b.order_code
;
...
select 
	...
	e.bloc_code,     --  集团编码
	e.bloc_name,     --  集团名称	
	c1.invoice_customer_name,		-- 客户开发票名称
	'${sdt_yes}' as sdt -- 统计日期 		
from 
(
select *
...
(
	...
	from
		...
		select 
			...
			invoice_customer_name,		-- 客户开发票名称
			sdt,
			row_number() over(partition by customer_code order by create_time desc) as customer_row_no
		from csx_dwd.csx_dwd_sss_invoice_di
		where sdt>='20200101'
		and delete_flag='0'
		and sync_status=1
		)a 
	where customer_row_no=1
)c1 on a.customer_no=c1.customer_code 
-- 客户下单情况动态信息表
left join
(
