...
--hive 标讯中标转化
-- csx_ods.csx_ods_csx_data_config_bid_list_inversion_df;
...
left join csx_ods.csx_ods_csx_data_config_bid_list_inversion_df b on a.id=b.no;
...
left join 
	(select idd as id,business_number
	from csx_ods.csx_ods_csx_data_config_bid_list_business_customer_df
	where flag='商机') g on g.business_number=a.business_number
...
a.remark,	 -- 备注
	...
	f.cust_new_old,
	c.smonth,
	sum(case when a.win_bid_date <= c.sdt then c.sale_amt end) sale_amt,
	sum(case when a.win_bid_date <= c.sdt then c.rp_sale_amt end) rp_sale_amt,   --日配含税销售额
	sum(case when a.win_bid_date <= c.sdt then c.fl_sale_amt end) fl_sale_amt,   --福利含税销售额
	sum(case when a.win_bid_date <= c.sdt then c.bbc_sale_amt end) bbc_sale_amt,   --BBC含税销售额
	sum(case when a.win_bid_date <= c.sdt then c.bbc_sale_amt end) csfws_sale_amt,   --城市服务商含税销售额
	sum(case when a.win_bid_date <= c.sdt then c.bbc_sale_amt end) ngdz_sale_amt,   --内购大宗含税销售额
	sum(case when a.win_bid_date <= c.sdt then c.profit end) profit,
	sum(case when a.win_bid_date <= c.sdt then c.count_day end) count_day
from
(
select 
a.no as id,	 -- 序号
regexp_replace(substr(a.bid_date, 1, 10), '-', '') as bid_date,	 -- 投标日期
a.business_director,	 -- 商机指导人
a.business_principal,	 -- 商机负责人
a.bid_principal,	 -- 投标负责人
a.region_name,	 -- 大区
...
a.bid_company,	 -- 投标主体
a.bid_result,	 -- 项目结果
regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') as win_bid_date,	 -- 结果公布日期
a.win_bid_amount,	 -- 中标金额（万）
a.bid_result_company,	 -- 项目结果
a.bid_result_analysis,	 -- 开标情况
a.bid_result_remark,	 -- 备注
...
from csx_ods.csx_ods_csx_data_config_bid_list_inversion_df a
...
a.remark,	 -- 备注
	...
	count(distinct case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.sdt end) count_days,
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.sale_amt end) sale_amt,
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.rp_sale_amt end) rp_sale_amt,   --日配含税销售额
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.fl_sale_amt end) fl_sale_amt,   --福利含税销售额
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.bbc_sale_amt end) bbc_sale_amt,   --BBC含税销售额	
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.csfws_sale_amt end) csfws_sale_amt,   --城市服务商含税销售额
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.ngdz_sale_amt end) ngdz_sale_amt,   --内购大宗含税销售额
	
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.sale_amt_old end) sale_amt_old,
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.rp_sale_amt_old end) rp_sale_amt_old,   --老客日配含税销售额
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.fl_sale_amt_old end) fl_sale_amt_old,   --老客福利含税销售额
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.bbc_sale_amt_old end) bbc_sale_amt_old,   --老客BBC含税销售额	
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.csfws_sale_amt_old end) csfws_sale_amt_old,   --老客城市服务商含税销售额
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.ngdz_sale_amt_old end) ngdz_sale_amt_old,   --老客内购大宗含税销售额	
	sum(case when regexp_replace(substr(a.win_bid_date, 1, 10), '-', '') <= c.sdt then c.profit end) profit
from csx_ods.csx_ods_csx_data_config_bid_list_inversion_df a
--投标对应客户的销售
left join 
(
