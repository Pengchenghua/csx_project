...
union all  select '211' as id,'SJ22080900074' as business_number
...
select b.id,a.customer_no,a.first_order_date,a.first_sign_date,b.month,
	...
	case when a.first_order_date<regexp_replace(from_unixtime(unix_timestamp(b.bid_date,'yyyy/mm/dd'),'yyyy-mm-dd'), '-', '') 
			or a.first_sign_date<regexp_replace(from_unixtime(unix_timestamp(b.bid_date,'yyyy/mm/dd'),'yyyy-mm-dd'), '-', '')
		then '老客户' 
...
select 
	...
	count(distinct case when regexp_replace(from_unixtime(unix_timestamp(a.bid_date,'yyyy/mm/dd'),'yyyy-mm-dd'), '-', '') <= c.sdt then c.sdt end) count_days,
