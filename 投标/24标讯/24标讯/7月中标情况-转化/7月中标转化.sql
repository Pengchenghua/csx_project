...
as
select a.id,a.customer_no,a.first_order_date,a.first_sign_date,b.month,
	...
	then '老客' else '新客' end cust_new_old
...
select 
	...
	sum(case when regexp_replace(from_unixtime(unix_timestamp(a.bid_date,'yyyy/mm/dd'),'yyyy-mm-dd'), '-', '') <= c.sdt then c.rp_sales_value end) rp_sales_value,   --老客日配含税销售额
	sum(case when regexp_replace(from_unixtime(unix_timestamp(a.bid_date,'yyyy/mm/dd'),'yyyy-mm-dd'), '-', '') <= c.sdt then c.fl_sales_value end) fl_sales_value,   --老客福利含税销售额
	sum(case when regexp_replace(from_unixtime(unix_timestamp(a.bid_date,'yyyy/mm/dd'),'yyyy-mm-dd'), '-', '') <= c.sdt then c.bbc_sales_value end) bbc_sales_value,   --老客BBC含税销售额		
...
(
	select
		id,sdt,smonth,
			...
			sum(case when cust_new_old='老客' then rp_sales_value end) rp_sales_value_old,   --老客日配含税销售额
			sum(case when cust_new_old='老客' then fl_sales_value end) fl_sales_value_old,   --老客福利含税销售额
			sum(case when cust_new_old='老客' then bbc_sales_value end) bbc_sales_value_old,   --老客BBC含税销售额				
			sum(profit) profit   --含税毛利	
	from
	(
		select 
			distinct b.id,b.month,b.first_order_date,b.cust_new_old,
			c.customer_no,c.sdt,c.smonth,c.sales_value,c.rp_sales_value,c.fl_sales_value,c.bbc_sales_value,c.profit,c.count_day
