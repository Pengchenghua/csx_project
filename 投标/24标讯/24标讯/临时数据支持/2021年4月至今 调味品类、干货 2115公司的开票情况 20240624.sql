...
-- and e.classify_middle_name in('调味品类','干货')
),
...
apply_customer_classify_total as
(
select 
...
customer_code,
company_code,
row_number() over(partition by company_code order by sum(total_amount_all) desc) as rno,
sum(total_amount_all) as total_amount_all
from apply_customer_classify_total
...
select 
	...
	a.classify_middle_name,-- 管理中类	
	a.total_amount_all,
	a.total_amount_all/b.total_amount_all as total_amount_zb
from apply_customer_classify_total a
join apply_customer_total_top b on a.customer_code=b.customer_code



		
		
-- 3、2021年4月至今 调味品类、干货 2115公司的开票情况 20240624
with apply_goods_group as
-- 发票商品明细
(
select d.order_code,d.total_amt as total_amount,
