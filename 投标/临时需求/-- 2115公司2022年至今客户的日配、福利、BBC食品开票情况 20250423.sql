...
select 
        ...
        rp_invoice_amount_2024,
        rp_invoice_amount_2025 ,
        fl_invoice_amount,
        fl_invoice_amount_2022,
        fl_invoice_amount_2023,
        fl_invoice_amount_2024,
        fl_invoice_amount_2025
from
(        
select
        a.customer_code,
       ...
       sum(a.invoice_amount) invoice_amount,
        ...
        sum(case when b.business_type_code in ('2','6') or a.source_sys='BBC' then a.invoice_amount end) fl_invoice_amount,
        sum(case when b.business_type_code in ('2','6') or a.source_sys='BBC' and substr(a.happen_date,1,4)=2022 then a.invoice_amount end) fl_invoice_amount_2022,
        sum(case when b.business_type_code in ('2','6') or a.source_sys='BBC' and substr(a.happen_date,1,4)=2023 then a.invoice_amount end) fl_invoice_amount_2023,
        sum(case when b.business_type_code in ('2','6') or a.source_sys='BBC' and substr(a.happen_date,1,4)=2024 then a.invoice_amount end) fl_invoice_amount_2024,
        sum(case when b.business_type_code in ('2','6') or a.source_sys='BBC' and substr(a.happen_date,1,4)=2025 then a.invoice_amount end) fl_invoice_amount_2025,
        ...
        sum(case when (b.business_type_code='1' ) and substr(a.happen_date,1,4)=2025 then a.invoice_amount end) rp_invoice_amount_2025 
from 
(
        select 
...
(
        ...
        and classify_large_code in ('B04','B05','B06','B07','B01','B02','B03')
        group by 
        case when business_type_code='6' and substr(split(order_code,'-')[0],1,1)='B' and substr(split(order_code,'-')[0],-1,1) in ('A','B','C','D','E') then substr(split(order_code,'-')[0],2,length(split(order_code,'-')[0])-2)
                 when business_type_code='6' and substr(split(order_code,'-')[0],1,1)='B' and substr(split(order_code,'-')[0],-1,1) not in ('A','B','C','D','E')then substr(split(order_code,'-')[0],2,length(split(order_code,'-')[0])-1)
                 else split(order_code,'-')[0]
