...
from
    ...
    where
     -- sdt >= '20230901'
     -- and sdt <= '20240411'
      sdt>= regexp_replace(date_add(from_unixtime(unix_timestamp('${yes_date}','yyyyMMdd'),'yyyy-MM-dd'),-21-dayofweek(from_unixtime(unix_timestamp('${yes_date}','yyyyMMdd'),'yyyy-MM-dd'))),'-','')
      and sdt<='${yes_date}'    
      ...
      and basic_performance_region_name like '%大区%'
group by 
      -- substr(sdt,1,6) ,
      -- performance_region_name,
      -- performance_province_code,
...
from
(
	select  target_location_code as dc_code,
	        super_class,
	        order_code,
	        a.goods_code,
	        if( price2_include_tax != 0,price2_include_tax,price_include_tax ) as received_price1,
	        ...
	        ( case when super_class = 1 then '供应商订单' when super_class = 2 then '供应商退货订单' when super_class = 3 then '配送订单' when super_class = 4 then '返配订单' end ) as super_class_name,
	        ( case when header_status = 1 then '已创建' when header_status = 2 then '已发货' when header_status = 3 then '部分入库' when header_status = 4 then '已完成' when header_status = 5 then '已取消' end ) as status,
	        supplier_code,
	        supplier_name,
	        last_delivery_date,
	...
	on a.source_type = b.config_key
	left join
	(
		select  calday,
		        csx_week,
		        dense_rank()over( order by  csx_week DESC) as csx_week_rn
 ...
 sdt >= regexp_replace(date_add(from_unixtime(unix_timestamp('${yes_date}', 'yyyyMMdd'), 'yyyy-MM-dd'), -28-dayofweek(from_unixtime(unix_timestamp('${yes_date}', 'yyyyMMdd'), 'yyyy-MM-dd'))), '-', '')
	and sdt <= '${yes_date}' -- and sdt >= '20230101'  
-- and sdt <= '20240320' 
	and super_class in (1, 2, 3, 4)
	and ( source_type in ('1', '3', '9', '10', '13', '17', '19', '22', '23') and header_status in (1, 2, 3, 4) ) -- 头表状态(1-已创建、2-已发货、3-部分入库、4-已完成、5-已取消) 
	and price_remedy_flag <> 1 -- 不包含补救单据
) a 
-- 关联补救单，剔除补救数据 
left join
(
	select  original_order_code,
	        goods_code
...
select  c2.location_code,
        ...
        c1.shop_code,
        c1.shop_name,
        (case when c1.market_source_type_code = 1 then '永辉' when c1.market_source_type_code = 4 then '一批' when c1.market_source_type_code = 5 then '二批' when c1.market_source_type_code = 6 then '终端' end) as market_source_type_name,
...
from
(-- 目前失效数据数据 
	select  a1.product_id                   as market_goods_id,
	...
	where substr((case when status = 0 and price_end_time > date_add(date(update_time), -1) then date_add(date(update_time), -1) else price_end_time end), 1, 10) >= date_add(from_unixtime(unix_timestamp('${yes_date}', 'yyyyMMdd'), 'yyyy-MM-dd'), -365)
	...
	union all
	-- 目前生效市调数据 
	select  b1.product_id                   as market_goods_id,
