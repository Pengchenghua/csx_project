/*
 * @Author: Pengchenghua 
 * @Date: 2024-04-17 19:34:16
 * @LastEditors: Pengchenghua
 * @LastEditTime: 2024-04-19 09:12:21
 * @FilePath: \SQLd:\彩食鲜工作内容\输出文件\数据分析\成本进价预警\-- 全国TOP商品成本监控预警-20240418.sql
...
select  csx_week,
        ...
        supplier_code,
        supplier_name,
        case when a.goods_code in ('620','846778') then '620'
             ...
             when a.goods_code in ('262352','1325230') then '262352'  else a.goods_code end goods_code,
...
(
	select  calday,
	        ...
	        csx_week_begin,
	        csx_week_end,
	        dense_rank() over ( order by  csx_week desc ) as week_rn
	from csx_dim.csx_dim_basic_date
	where calday >=   regexp_replace(date_add(from_unixtime(unix_timestamp('${yes_date}','yyyyMMdd'),'yyyy-MM-dd'),-7-dayofweek(from_unixtime(unix_timestamp('${yes_date}','yyyyMMdd'),'yyyy-MM-dd'))),'-','')
        and calday <='${yes_date}'
) b
on a.sdt = b.calday
where  sdt>= regexp_replace(date_add(from_unixtime(unix_timestamp('${yes_date}','yyyyMMdd'),'yyyy-MM-dd'),-7-dayofweek(from_unixtime(unix_timestamp('${yes_date}','yyyyMMdd'),'yyyy-MM-dd'))),'-','')
  and sdt<='${yes_date}'
and remedy_flag <> '1'
...
and ( a.goods_code in ( '620', '1267377', '537', '3695', '441', '618', '2112', '621', '644', '607', '474', '471', '631', '472', '531', '438', '608', '619', '262352', '566', '846778', '846807', '1065627', '576', '1140572', '1065514', '1161767', '846746', '317208', '846732', '846501', '846759', '450', '1236384', '1065529', '1065502', '846570', '1325230', '1350237', '1359002', '1588624', '1532629', '1533133', '1359914', '1359004', '1588624', '1493716', '1728536', '1359006', '1533153', '1358979', '1358981', '1359886', '1532629', '1532629', '1601165', '1533153', '1359000', '1456612', '8778', '1456631', '1554919', '1747107', '1015421', '1', '1186656', '266', '260832', '6', '1182675', '1005762', '2624', '264135', '1015423', '2567', '5' ) or ( spu_goods_code in ( '1', '24', '56', '12', '26', '34', '36', '21', '88', '4', '48', '1074', '5', '250', '1106', '166', '171', '1158', '177', '1195', '184', '1179', '186', '212', '189', '198', '183', '1200', '1204', '301', '1222', '293', '537', '1411', '1321', '326', '1299', '516', '494', '1251', '1375', '1629', '625', '578', '640', '652', '615', '579', '646', '138', '577', '616', '611' ) and classify_large_code = 'B03' )
); 
...
with entry as (select  
  ...
  sum(sum(receive_amt)) over (partition by dc_code,spu_goods_code,csx_week ) spu_receive_amt,
  sum(sum(receive_qty)) over (partition by dc_code,spu_goods_code,csx_week ) spu_receive_qty,
  sum(avg(receive_qty)) over (partition by dc_code,spu_goods_code,csx_week ) spu_avg_receive_qty,
  SUM(SUM(receive_amt)) over (partition by performance_region_name, csx_week,goods_code ) area_sku_receive_amt,
  SUM(SUM(receive_qty)) over (partition by performance_region_name, csx_week,goods_code ) area_sku_receive_qty,
  SUM(SUM(receive_amt)) over (partition by spu_goods_code,csx_week ) all_sku_receive_amt,
  SUM(SUM(receive_qty)) over (partition by spu_goods_code,csx_week ) all_sku_receive_qty,
  SUM(SUM(receive_amt)) over (partition by performance_region_name, csx_week,spu_goods_code ) area_spu_receive_amt,
  SUM(SUM(receive_qty)) over (partition by performance_region_name, csx_week,spu_goods_code ) area_spu_receive_qty,
  SUM(SUM(receive_amt)) over (partition by spu_goods_code,csx_week ) all_spu_receive_amt,
  SUM(SUM(receive_qty)) over (partition by spu_goods_code,csx_week ) all_spu_receive_qty
from
  csx_analyse_tmp.csx_analyse_tmp_entry_top_cost a
  join 
  (select goods_code,goods_name,spu_goods_code,spu_goods_name from csx_dim.csx_dim_basic_goods where sdt='current') b on a.goods_code=b.goods_code
group by   
  csx_week,
  performance_region_code,
  performance_region_name,
  ...
  spu_receive_qty,
  spu_avg_receive_qty,
  spu_receive_amt/spu_receive_qty spu_avg_cost,
  area_sku_receive_amt,
  area_sku_receive_qty,
  all_sku_receive_amt,
  all_sku_receive_qty,
  area_spu_receive_amt,
  area_spu_receive_qty,
  all_spu_receive_amt,
  all_spu_receive_qty,
  area_sku_cost,  --sku大区均价
  all_sku_cost,     --sku全国均价
  area_spu_cost,  --spu大区均价
  all_spu_cost,     --sku全国均价
  top_area_sku_cost_flag,  --sku 大于大区标识
  top_sku_cost_flag,     -- sKu入库均价大于全国0.5%
  top_area_spu_cost_flag,  
  top_spu_cost_flag  -- spu入库均价大于全国0.5  
  from
  (
  select  csx_week,
  week_rn,
...
--   supplier_name,
  ...
  all_spu_receive_amt,
  all_spu_receive_qty,
  coalesce(area_sku_receive_amt/area_sku_receive_qty,0) area_sku_cost,  --sku大区均价
  coalesce(all_sku_receive_amt/all_sku_receive_qty,0) all_sku_cost,     --sku全国均价
  coalesce(area_spu_receive_amt/area_spu_receive_qty,0) area_spu_cost,  --spu大区均价
  coalesce(all_spu_receive_amt/all_spu_receive_qty,0) all_spu_cost,     --sku全国均价
  if(avg_cost/(coalesce(all_spu_receive_amt/all_spu_receive_qty,0))-1>0.05,1,0) top_spu_cost_flag,   -- spu入库均价大于全国0.5%
  if(avg_cost/(coalesce(all_sku_receive_amt/all_sku_receive_qty,0))-1>0.05,1,0) top_sku_cost_flag ,  -- sku入库均价大于全国0.5%
  if(avg_cost/(coalesce(area_sku_receive_amt/area_sku_receive_qty,0))-1>0.05,1,0) top_area_sku_cost_flag,   -- spu入库均价大于大区0.5%
  if(avg_cost/(coalesce(area_spu_receive_amt/area_spu_receive_qty,0))-1>0.05,1,0) top_area_spu_cost_flag   -- spu入库均价大于大区0.5%
  from entry a 
   where csx_week='202416'
  ) a 
 -- where (top_spu_cost_flag=1 or top_sku_cost_flag=1)
...
--   supplier_name,
  ...
  sum(receive_qty) receive_qty,
  avg(receive_qty) avg_receive_qty,
  SUM(SUM(receive_amt)) over (partition by performance_region_name, csx_week,goods_code ) area_skureceive_amt,
  SUM(SUM(receive_qty)) over (partition by performance_region_name, csx_week,goods_code ) area_skureceive_qty,
  SUM(SUM(receive_amt)) over (partition by spu_goods_code,csx_week ) all_sku_receive_amt,
  SUM(SUM(receive_qty)) over (partition by spu_goods_code,csx_week ) all_sku_receive_qty
  SUM(SUM(receive_amt)) over (partition by performance_region_name, csx_week,spu_goods_code ) area_spu_receive_amt,
  SUM(SUM(receive_qty)) over (partition by performance_region_name, csx_week,spu_goods_code ) area_spu_receive_qty,
...
--   supplier_name,
  ...
  all_receive_amt,
  all_receive_qty,
  coalesce(area_sku_receive_amt/area_sku_receive_qty,0) area_sku_cost,
  coalesce(all_sku_receive_amt/all_sku_receive_qty,0) all_sku_cost,
  coalesce(area_spu_receive_amt/area_spu_receive_qty,0) area_spu_cost,
  coalesce(all_spu_receive_amt/all_spu_receive_qty,0) all_spu_cost,
  if(avg_cost/(coalesce(all_receive_amt/all_receive_qty,0)-1)>0.05,1,0) top_cost_flag
  from entry a 
  ) a 
  where top_cost_flag=1
