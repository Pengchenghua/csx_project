...
from 
	...
	where 
		sdt between '20230530' and '20230829'
...
from 
	...
	from 
		csx_dws.csx_dws_sale_detail_di
	where 
		sdt between '20230911' and '20231010'
		and channel_code in ('1','7','9')
		and business_type_code =1 -- 仅日配 业务类型编码(1.日配单 2.福利单 3.批发内购 4.城市服务商 5.省区大宗 6.BBC)
		and delivery_type_code in (1) -- 剔除直送和自提 配送类型编码：1-配送 2-直送 3-自提 4-直通 11-同城配送 12-快递配送 13-一件代发
		--and order_channel_code not in (5) -- 剔除调价返利和价格补救 订单来源渠道: 1-b端 2-m端 3-bbc 4-客户返利管理 5-价格补救 6-客户调价管理 -1-sap
...
drop table csx_analyse_tmp.csx_analyse_tmp_shidiao_cover_rate;
create table csx_analyse_tmp.csx_analyse_tmp_shidiao_cover_rate
as
select 	
	...
	b.product_code,
	d.goods_name,
	sum(case when a.market_source_type_code='1' then 1 else 0 end) as yunchao,
	sum(case when a.shop_name like '%网%' or a.shop_name like '%平台' then 1 else 0 end) as wangzhan,
	sum(case when a.shop_name like '%京东自营%' then 1 else 0 end) as jingdong,
	sum(case when a.market_source_type_code not in ('1','2') and a.shop_name not like '%网%' and a.shop_name not like '%平台' then 1 else 0 end) as xianxia
from 
	(
	select 
		* 
...
group by 
	...
	d.goods_name
;
select * from csx_analyse_tmp.csx_analyse_tmp_shidiao_cover_rate;
