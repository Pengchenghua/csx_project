...
select
	...
	a.one_product_category_code,a.one_product_category_name,a.two_product_category_code,a.two_product_category_name,a.update_time,a.price_date
from
	(
	select 
	...
	from 
		...
		from 
			...
			select 
				...
				estimated_pricing_gross_margin,product_id,update_time,regexp_replace(substr(price_date,1,10),'-','') as price_date
			from csx_ods.csx_ods_csx_price_prod_market_research_not_yh_price_effective_df  -- 非永辉 生效
			where source_type_code!=1
			-- 本周市调价 本周一开始
			and regexp_replace(substr(price_date,1,10),'-','')>=regexp_replace(date_sub(current_date,dayofweek(current_date)-2),'-','')
...
where b.market_research_price is not null
;
