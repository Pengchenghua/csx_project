...
select
	...
	a.update_by,  -- 更新人
	a.update_time,  -- 更新时间
	case a.do_price when 0 then '否' when 1 then '是' end as do_price,  -- 是否已报价:0否;1:是
	case a.source when 0 then '市调导入' when 1 then '小程序' when 2 then '小程序-pc端' 
				  when 3 then '线上网站市调' when 4 then '通用市调系统生成' else a.source end as source,  -- 来源: 0:市调导入；1:小程序;2:小程序-pc端 3: 线上网站市调 4 通用市调系统生成
	case a.product_status when 0 then '正常' when 1 then '促销' else a.product_status end as product_status,  -- 商品状态:1促销 0正常
	a.bom,  -- 工厂bom配置
	case a.bom_type when 1 then '人工bom' when 2 then '报价策略人工bom' when 3 then '工厂bom' 
					when 4 then '报价策略工厂bom' else a.bom_type end as bom_type,  -- bom类型(1:人工bom;2:报价策略人工bom;3:工厂bom;4:报价策略工厂bom)
...
from 
	...
	from 
		(
		select *,
			row_number()over(partition by customer_code,market_code,goods_id,price_date order by create_time desc) as rn
