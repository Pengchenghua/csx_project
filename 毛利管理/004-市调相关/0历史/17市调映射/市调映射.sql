...
from 
	...
	select *from 
        ...
        where sdt >='${sdt_bf60}'	
            and regexp_replace(substr(price_date,1,10),'-','') >='${sdt_bf30}'
...
from 
	...
	select *from 
        ...
        where -- regexp_replace(split(create_time,' ')[0],'-','')>='${sdt_bf1}'
        	regexp_replace(substr(price_date,1,10),'-','')>='${sdt_bf30}'	 -- 市调日期近7天
...
from 
	...
	)b on a.location_code=b.location_code and a.market_code=b.market_code and a.product_code=b.product_code 
