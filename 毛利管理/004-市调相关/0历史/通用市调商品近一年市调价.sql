CREATE TABLE `csx_analyse.csx_analyse_fr_goods_days_not_yh_price_df`(
  ...
  `market_source_type_name` string COMMENT '市调来源一级', 
  `source_type_two_level_name` string COMMENT '市调来源二级',   
  `shop_code` string COMMENT '市调地点编码', 
  `shop_name` string COMMENT '市调地点名称', 
  `product_code` string COMMENT '商品编码', 
  `product_name` string COMMENT '商品名称',    
  `one_product_category_code` string COMMENT '一级管理编码', 
  `one_product_category_name` string COMMENT '一级管理名称', 
  `two_product_category_code` string COMMENT '二级管理编码', 
  `two_product_category_name` string COMMENT '二级管理名称',     
  `price` decimal(20,6) COMMENT '市调价',  
  `price_date` string COMMENT '市调日期', 
  `price_begin_date` string COMMENT '市调生效日期', 
  `price_end_date` string COMMENT '市调失效日期'      
 ) COMMENT '通用市调商品近一年市调价';
