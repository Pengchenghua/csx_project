	...
	from
		...
		from 
			...
			from 
				...
				-- 临时跑数
				and regexp_replace(split(create_time,' ')[0],'-','')<'20241111'
...
-- 市调清单与覆盖率-非清单
drop table csx_analyse_tmp.csx_analyse_price_market_research_cover_rate_nqd_wi;
create table csx_analyse_tmp.csx_analyse_price_market_research_cover_rate_nqd_wi as
with
-- 通用市调价
...
(
	...
	-- 临时跑数
	where (substr(price_end_time,1,10)>=cast(date_sub(to_date(from_unixtime(unix_timestamp('${sdt_yes}','yyyyMMdd'))),2) as string)
	or (shop_code='RW7' and substr(price_end_time,1,10)>=cast(date_sub(to_date(from_unixtime(unix_timestamp('${sdt_yes}','yyyyMMdd'))),3) as string))
	)
