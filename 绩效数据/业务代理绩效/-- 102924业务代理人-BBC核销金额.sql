...
-- 	sum(b_pay_amt) b_pay_amt,
	...
	sum(unpay_amt) unpay_amt,
	max(pay_amt_bill) pay_amt_bill,
	sum(case when regexp_replace(substr(a.paid_date,1,10),'-','')>= regexp_replace(to_date(add_months(trunc('${yesterdate}','MM'),-1)),'-','') 
			  and regexp_replace(substr(a.paid_date,1,10),'-','')<= regexp_replace(to_date(last_day(add_months('${yesterdate}',-1))),'-','')
			 then a.new_pay_amt end ) payment_amount_by,   -- 本月核销
	sum(case when regexp_replace(substr(a.paid_date,1,10),'-','')>=regexp_replace(to_date(add_months(trunc('${yesterdate}','MM'),-2)),'-','') 
		  and regexp_replace(substr(a.paid_date,1,10),'-','')<= regexp_replace(to_date(last_day(add_months('${yesterdate}',-2))),'-','')
			 then coalesce(last_pay_amt,0) end ) payment_amount_sy
	from (
            SELECT 
                a.customer_code,
                ...
                SUM(pay_amt ) AS new_pay_amt
            FROM csx_analyse.csx_analyse_customer_verification_detail_mf a
                where smt = substr(regexp_replace(to_date(last_day(add_months('${yesterdate}',-1))),'-',''),1,6)
            GROUP BY 
                a.customer_code,
                a.smonth,
                a.source_bill_no_new,
            ...
            ( SELECT 
                ...
                SUM(pay_amt ) AS last_pay_amt
            FROM csx_analyse.csx_analyse_customer_verification_detail_mf a
                where smt = substr(regexp_replace(to_date(last_day(add_months('${yesterdate}',-2))),'-',''),1,6)
            GROUP BY 
                a.source_bill_no_new,
                a.customer_code,
                paid_date
...
select 
	...
	sum(coalesce(e.payment_amount_sy,0)) payment_amount_sy,	    -- 上月核销
	sum(coalesce(e.payment_amount_by,0)) *0.03 as push_money,
	regexp_replace(to_date('${yesterdate}'),'-','') as sdt
from 
	 tmp_sale_detail  a
left join 
	tmp_bill_account_record_snapshot e on   a.sign_company_code=e.company_code and a.customer_code=e.customer_code
