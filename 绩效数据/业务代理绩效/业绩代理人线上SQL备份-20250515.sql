...
tmp_crm_info b on b.customer_code=a.customer_code  
	...
	where (
	        (sdt>='20210901' and sdt<regexp_replace(add_months(trunc('${yesterdate}','MM'),0),'-','') 
	       ...
	       and ((a.customer_code not in (select  customer_code from filtered_customers)   -- 上上月1日 -- 昨日月1日
	            and a.customer_code not in ('245148') )
                or  ( a.customer_code in ('245148') 
                and  a.sub_customer_code in ('Z040012','245148','Z084554','Z084555','Z084556','Z084557','Z084560','Z092946','Z084558','Z084559') 
                )
            )
        )
        or -- 以下客户包含直送仓
        (sdt>='20210901' and sdt<regexp_replace(add_months(trunc('${yesterdate}','MM'),0),'-','') 
		    and a.customer_code in (select  customer_code from filtered_customers)
        )
        )
	and status<>0
	and shipper_code='YHCSX'
...
tmp_crm_info b on b.customer_code=a.customer_code  
--  and a.business_type_name=b.business_type_name
	where ((sdt>='20210901' and sdt<regexp_replace(add_months(trunc('${yesterdate}','MM'),0),'-','') 
		   and  inventory_dc_code not in ('WB57','WB54','WB53','W0K4','WB44')  -- 福建剔除以下客户K4仓
		   -- 剔除子客户
		   AND  sub_customer_code NOT IN ('Z079454','Z099560','Z086058','102524','Z100070','Z099923','Z064006','Z065730','Z074477','Z074473','Z069386','Z074236',
		                                    'Z074762','Z068958','Z089538','Z063242','Z084424','Z102597','Z100485','Z102656','Z084002','Z083277','Z096203','Z102658','Z062118')
...
with temp_bill_settle_detail as
 (
	 select 
...
-- 	source_bill_no,
 ...
 flag_check AS (
    -- 精确标记同一claim_bill_code + close_bill_code组合在本月存在多状态
    SELECT 
        claim_bill_code,
        close_bill_code,
        COUNT(DISTINCT delete_flag) AS flag_count
    FROM 
        csx_dwd.csx_dwd_sss_close_bill_account_record_di
    WHERE 
        sdt BETWEEN regexp_replace(add_months(trunc('${yesterdate}', 'MM'), -1), '-', '') 
                AND regexp_replace(last_day(add_months('${yesterdate}', -1)), '-', '')
    GROUP BY 
        claim_bill_code, close_bill_code  -- 按组合分组
    HAVING 
        COUNT(DISTINCT delete_flag) >= 2  -- 存在0和1两种状态
),

filtered_invalid_records AS (
    -- 筛选需排除的delete_flag=1记录（仅限非多状态）
    SELECT 
        a.customer_code,
        a.claim_bill_code,
        a.close_bill_code
    ...
    LEFT JOIN flag_check c 
        ON a.claim_bill_code = c.claim_bill_code 
        AND a.close_bill_code = c.close_bill_code
    WHERE 
        a.sdt BETWEEN regexp_replace(add_months(trunc('${yesterdate}', 'MM'), -6), '-', '') 
                AND regexp_replace(last_day(add_months('${yesterdate}', -1)), '-', '')
        AND a.delete_flag = '1'
        AND c.claim_bill_code IS NULL  -- 仅排除非多状态的delete_flag=1记录
        -- AND a.delete_reason LIKE '%发票红冲%'  -- 按需恢复此条件
    GROUP BY 
        a.customer_code, a.claim_bill_code, a.close_bill_code
)
,
tmp_bill_account_record_snapshot as (
SELECT 
    a.customer_code,
    regexp_replace(substr(happen_date,1,7),'-','') AS smonth,
    CASE 
        WHEN substr(split(a.close_bill_code,'-')[0],1,1)='B' AND substr(split(a.close_bill_code,'-')[0],-1,1) IN ('A','B','C','D','E') 
            THEN substr(split(a.close_bill_code,'-')[0],2,length(split(a.close_bill_code,'-')[0])-2)
        WHEN substr(split(a.close_bill_code,'-')[0],1,1)='B' AND substr(split(a.close_bill_code,'-')[0],-1,1) NOT IN ('A','B','C','D','E') 
            THEN substr(split(a.close_bill_code,'-')[0],2,length(split(a.close_bill_code,'-')[0])-1)
        ELSE split(a.close_bill_code,'-')[0]
    END AS source_bill_no_new,
    a.company_code,
    SUM(CASE 
            WHEN regexp_replace(substr(paid_time,1,10),'-','') BETWEEN regexp_replace(add_months(trunc('${yesterdate}','MM'),-1),'-','') 
                AND regexp_replace(last_day(add_months('${yesterdate}',-1)),'-','') 
                THEN pay_amt 
        END) AS payment_amount_by,
    SUM(CASE 
            WHEN regexp_replace(substr(paid_time,1,10),'-','') BETWEEN regexp_replace(add_months(trunc('${yesterdate}','MM'),-2),'-','') 
                AND regexp_replace(last_day(add_months('${yesterdate}',-2)),'-','') 
                THEN pay_amt 
        END) AS payment_amount_sy,
    SUM(pay_amt) AS paid_amount
FROM 
    csx_ads.csx_ads_sss_close_bill_account_record_snapshot_mf a
LEFT JOIN filtered_invalid_records b 
    ON a.claim_bill_code = b.claim_bill_code 
    AND a.close_bill_code = b.close_bill_code 
    AND a.customer_code = b.customer_code
WHERE 
    a.smt = substr(regexp_replace(last_day(add_months('${yesterdate}',-1)),'-',''),1,6)
    AND (regexp_replace(substr(happen_date,1,10),'-','') <= regexp_replace(last_day(add_months('${yesterdate}',-1)),'-','') 
         OR happen_date = '' 
         OR happen_date IS NULL)
    AND regexp_replace(substr(paid_time,1,10),'-','') <= regexp_replace(last_day(add_months('${yesterdate}',-1)),'-','')
    AND a.delete_flag = '0'
    AND b.claim_bill_code IS NULL  -- 最终排除无效记录
    -- and a.close_bill_code='OM25011500002913'
GROUP BY 
    a.customer_code,
...
select 
	...
	sum(coalesce(e.payment_amount_by,0)) payment_amount_by,
	sum(coalesce(e.payment_amount_sy,0)) payment_amount_sy,
	end_date,
	fixed_amt,
	guarantee_profit_rate,
...
left join 
	tmp_bill_account_record_snapshot e on  e.smonth=a.smonth and a.sign_company_code=e.company_code and a.customer_code=e.customer_code
