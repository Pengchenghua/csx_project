...
-- 跨月红冲的剔除
	(select
		...
		where  smt=substr(regexp_replace(last_day(add_months('${yesterdate}',-1)),'-',''),1,6)
	    	...
	    	and regexp_replace(substr(paid_time,1,10),'-','') >=     regexp_replace(last_day(add_months('${yesterdate}',-7)),'-','')
...
tmp_bill_account_record_snapshot as (
	...
	sum(case when regexp_replace(substr(paid_time,1,10),'-','')>= regexp_replace(add_months(trunc('${yesterdate}','MM'),-1),'-','') 
			  ...
			  and  b.close_bill_code is null 
			 then a.pay_amt end ) payment_amount_by,   -- 本月核销
	sum(case when regexp_replace(substr(a.paid_time,1,10),'-','')>=regexp_replace(add_months(trunc('${yesterdate}','MM'),-2),'-','') 
		  and regexp_replace(substr(a.paid_time,1,10),'-','')<= regexp_replace(last_day(add_months('${yesterdate}',-2)),'-','')
			 then coalesce(c.pay_amt,0) end ) payment_amount_sy	    -- 上月核销		 
	...
	left join 
	 (select * 
	  from tmp_last_bill_account_record_snapshot 
	    where delete_flag=1) b
	    on a.claim_bill_code=b.claim_bill_code and a.close_bill_code=b.close_bill_code
	 left join 
	 (select * 
	 from tmp_last_bill_account_record_snapshot_01 
	    where delete_flag=0) c
