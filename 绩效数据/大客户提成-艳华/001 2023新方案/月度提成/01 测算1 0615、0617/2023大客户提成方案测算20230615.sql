...
from 
	(
		select region_code,region_name,province_code,province_name,
			...
			work_no,
			sales_name,
			sum(case when business_type_code in(1,4,5) then sale_amt end) as rp_sale_amt,
			sum(case when business_type_code in(1,4,5) then profit end) as rp_profit,
			sum(case when business_type_code in(2,6) then sale_amt end) as fl_bbc_sale_amt,
			sum(case when business_type_code in(2,6) then profit end) as fl_bbc_profit			
		from csx_analyse_tmp.tmp_tc_cust_credit_bill_xianjin_bujiu_1
		where work_no<>''
		...
		select region_code,region_name,province_code,province_name,
			...
			rp_service_user_work_no as work_no,
			rp_service_user_name as sales_name,
			sum(sale_amt) as rp_sale_amt,
			sum(profit) as rp_profit,
		...
		select region_code,region_name,province_code,province_name,
			...
			null as rp_sale_amt,
			null as rp_profit,
			sum(sale_amt) as fl_bbc_sale_amt,
			sum(profit) as fl_bbc_profit
		from csx_analyse_tmp.tmp_tc_cust_credit_bill_xianjin_bujiu_1
		where fl_service_user_work_no<>''
...
-- as
insert overwrite table csx_analyse.csx_analyse_fr_tc_customer_order_detail partition(smt)
select 
...
group by concat_ws('-',smt_ct,region_code,province_code,city_group_code,customer_code),
	...
	bbc_service_target_rate,
	bbc_service_target_rate_tc,
	from_utc_timestamp(current_timestamp(),'GMT'),smt_ct,smt;
