...
and (business_type_code in('1','2','6')
	...
	or customer_code in ('130733','128865','130078','114872','124484','227054','228705','225582','123415','113260'))
...
select distinct
	...
	a.unpay_amt,
	a.pay_amt,	-- 核销金额
	if(a.source_sys='BEGIN',1,coalesce(b.business_type_code,c.business_type_code)) business_type_code,
	if(a.source_sys='BEGIN','日配业务',coalesce(b.business_type_name,c.business_type_name)) business_type_name,
	coalesce(b.status,c.status) as status,  -- 是否有效 0.无效 1.有效
	coalesce(b.sale_amt,c.sale_amt) as sale_amt,
	coalesce(b.profit,c.profit) as profit,
	coalesce(b.sale_amt_jiushui,c.sale_amt_jiushui) as sale_amt_jiushui,
	coalesce(b.profit_jiushui,c.profit_jiushui) as profit_jiushui
from 
(
	select 
		a.bill_type,  -- 单据类型 10正常单 11福利单  20退货单 30返利单 40尾差调整单 -1 期初单
	...
	join
	(
		select a.close_bill_code,coalesce(b.trade_date,a.paid_date) paid_date,
		...
		left join
			(
			select  
				claim_bill_code,regexp_replace(substr(trade_time,1,10),'-','') trade_date,
				count(1) aa
			...
			group by claim_bill_code,regexp_replace(substr(trade_time,1,10),'-','')
			)b on a.claim_bill_code=b.claim_bill_code
		group by a.close_bill_code,coalesce(b.trade_date,a.paid_date)
...
left join   -- CRM客户信息取月最后一天
	(
		select 
			...
			case when channel_code='9' and customer_code not in ('130733','128865','130078','114872','124484','227054','228705','225582','123415','113260') then '业务代理' end as ywdl_cust,
...
as
select *,
	case when b.performance_province_name in('湖北省') and regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')<='202308' then profit_target_rate
		when b.performance_province_name not in('湖北省') and regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')<='202310' then profit_target_rate
