--大客户提成最后结果表-销售员提成拆分业务类型-测算

-- 表7 csx_analyse_fr_tc_customer_bill_month_dff_rate_detail_chai 大客户提成-客户结算月回款时间系数明细--------------------------------------
...
select a.*,
	case 
		 ...
		 and (e2.adjust_business_type=substr(a.business_type_name,1,2) or e2.adjust_business_type='全业务') then '是' 
		 -- 指定回款时间系数 打款日期
		 when (regexp_replace(substr(a.paid_date,1,10),'-','') between f2.date_star and f2.date_end) and f2.category_second is not null 
		 and (f2.adjust_business_type=substr(a.business_type_name,1,2) or f2.adjust_business_type='全业务') then '是' 		 
		 else '否' end yufu_flag,

	case 
		 -- 指定回款时间系数 发生日期
		 when (regexp_replace(substr(a.happen_date,1,10),'-','') between e2.date_star and e2.date_end) and e2.category_second is not null 
		 and (e2.adjust_business_type=substr(a.business_type_name,1,2) or e2.adjust_business_type='全业务') then e2.hk_dff_rate 
		 -- 指定回款时间系数 打款日期
		 when (regexp_replace(substr(a.paid_date,1,10),'-','') between f2.date_star and f2.date_end) and f2.category_second is not null 
		 and (f2.adjust_business_type=substr(a.business_type_name,1,2) or f2.adjust_business_type='全业务') then f2.hk_dff_rate 		 
...
(
	...
	split(remark,'：')[1] as happen_month_pay_amt,  -- 销售月_回款金额
...
)b on a.customer_code=b.customer_code and a.happen_month_pay_amt=b.happen_month_pay_amt 
;
...
select 
	...
	if(a.service_falg in('服务费','销售员按服务费'),0,
		if(d.category_second in('不算提成','销售员不算提成'),0,
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and c.paid_date_new is null and coalesce(e1.account_period_name,'0')<>'预付货款' and b2.category_second is null,0,a.tc_sales_bbc_zy)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and c.paid_date_new is null and coalesce(e1.account_period_name,'0')<>'预付货款' and b2.category_second is null,0,a.tc_sales_bbc_ly)
				))
...
-- drop table if exists csx_analyse_tmp.csx_analyse_fr_tc_customer_bill_month_dff_rate_detail_chai;
-- create temporary table csx_analyse_tmp.csx_analyse_fr_tc_customer_bill_month_dff_rate_detail_chai
-- as
insert overwrite table csx_analyse.csx_analyse_fr_tc_customer_bill_month_dff_rate_detail_chai partition(smt)
select 
	...
	-- 销售员提成拆分业务类型-测算，若按服务费发提成无法拆分则全都给到日配
...
drop table if exists csx_analyse.csx_analyse_fr_tc_customer_bill_month_dff_rate_detail_chai;
create table csx_analyse.csx_analyse_fr_tc_customer_bill_month_dff_rate_detail_chai(
...
`new_cust_rate` DECIMAL (20,6) COMMENT '日配新客系数',
`bill_date` STRING  COMMENT '结算日期',
`paid_date` STRING  COMMENT '核销日期（打款日期）',
`happen_month` STRING  COMMENT '销售月',
`sale_amt_real` DECIMAL (20,6) COMMENT '此月总销售额',
`rp_sale_amt_real` DECIMAL (20,6) COMMENT '此月日配销售额',
`bbc_sale_amt_zy_real` DECIMAL (20,6) COMMENT '此月BBC自营销售额',
`bbc_sale_amt_ly_real` DECIMAL (20,6) COMMENT '此月BBC联营销售额',
`fl_sale_amt_real` DECIMAL (20,6) COMMENT '此月福利销售额',
`service_falg` STRING  COMMENT '服务费标签',
`service_fee` DECIMAL (20,6) COMMENT '服务费',
`by_sale_amt` DECIMAL (20,6) COMMENT '本月销售额',
`by_rp_sale_amt` DECIMAL (20,6) COMMENT '本月日配销售额',
`by_bbc_sale_amt_zy` DECIMAL (20,6) COMMENT '本月BBC自营销售额',
`by_bbc_sale_amt_ly` DECIMAL (20,6) COMMENT '本月BBC联营销售额',
`by_fl_sale_amt` DECIMAL (20,6) COMMENT '本月福利销售额',
`by_profit` DECIMAL (20,6) COMMENT '本月毛利额',
`by_rp_profit` DECIMAL (20,6) COMMENT '本月日配毛利额',
`by_bbc_profit_zy` DECIMAL (20,6) COMMENT '本月BBC自营毛利额',
`by_bbc_profit_ly` DECIMAL (20,6) COMMENT '本月BBC联营毛利额',
`by_fl_profit` DECIMAL (20,6) COMMENT '本月福利毛利额',
`original_tc_sales` DECIMAL (20,6) COMMENT '无毛利目标_销售员提成',
`original_tc_rp_service` DECIMAL (20,6) COMMENT '无毛利目标_日配客服经理提成',
`original_tc_fl_service` DECIMAL (20,6) COMMENT '无毛利目标_福利客服经理提成',
`original_tc_bbc_service` DECIMAL (20,6) COMMENT '无毛利目标_BBC客服经理提成', 
`tc_sales_rp`	decimal(20,6)	COMMENT	'日配销售员提成',
`tc_sales_bbc`	decimal(20,6)	COMMENT	'BBC销售员提成',
`tc_sales_fl`	decimal(20,6)	COMMENT	'福利销售员提成'
) COMMENT '大客户提成-客户结算月回款时间系数明细_拆分业务提成'
