...
where (d.ywdl_cust is null and d.ng_cust is null and d.customer_code is not null) or e.customer_code is not null
;
...
select *,
	case 
		...
		when a.work_no in('81089289') and substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6) in('202311','202410') then 1
...
select 
	...
	if(a.service_falg in('服务费','销售员按服务费'),a.service_fee*if((a.sale_amt/a.sale_amt_real)>1,1,if((a.sale_amt/a.sale_amt_real)<-1,-1,(a.sale_amt/a.sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,
			...
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and coalesce(e1.account_period_name,'0')<>'预付货款' and b3.category_second is null,0,a.tc_sales_bbc_zy)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and coalesce(e1.account_period_name,'0')<>'预付货款' and b3.category_second is null,0,a.tc_sales_bbc_ly)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and coalesce(e1.account_period_name,'0')<>'预付货款' and b2.category_second is null,0,a.tc_sales_fl)
				))
