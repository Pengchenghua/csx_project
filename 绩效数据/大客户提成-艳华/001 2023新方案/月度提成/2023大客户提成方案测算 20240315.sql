...
select 
	...
	a.overdue_date,	-- 逾期开始日期	
	case 
	when (regexp_replace(substr(a.paid_date,1,10),'-','') between c.date_star and c.adjust_business_type='全业务') then c.paid_date_new
	when (regexp_replace(substr(a.paid_date,1,10),'-','') between c.date_star and c.adjust_business_type='日配' and a.business_type_name like '日配%') then c.paid_date_new
	when (regexp_replace(substr(a.paid_date,1,10),'-','') between c.date_star and c.adjust_business_type='BBC' and a.business_type_name like 'BBC%') then c.paid_date_new
	when (regexp_replace(substr(a.paid_date,1,10),'-','') between c.date_star and c.adjust_business_type='福利' and a.business_type_name like '福利%') then c.paid_date_new
...
select *,
	case 
		...
		when a.work_no in('81090419') and substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)<='202407' then profit_target_rate
		...
		when a.work_no in('81089289') and substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6) between '202311' and '202410' then profit_target_rate
...
select 
	...
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and coalesce(e1.account_period_name,'0')<>'预付货款' and b1.category_second is null and b3.category_second is null and b2.category_second is null,0,
	if(a.service_falg in('服务费','销售员按服务费'),a.service_fee*if((a.pay_amt/a.sale_amt_real)>1,1,if((a.pay_amt/a.sale_amt_real)<-1,-1,(a.pay_amt/a.sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and coalesce(e1.account_period_name,'0')<>'预付货款' and b1.category_second is null,0,a.tc_sales_rp)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and coalesce(e1.account_period_name,'0')<>'预付货款' and b2.category_second is null,0,a.tc_sales_bbc_zy)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and coalesce(e1.account_period_name,'0')<>'预付货款' and b2.category_second is null,0,a.tc_sales_bbc_ly)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and coalesce(e1.account_period_name,'0')<>'预付货款' and b3.category_second is null,0,a.tc_sales_fl)
				))
	)*if(d.category_second like'%提成减半%',0.5,1) as tc_sales,
	
	-- 取消打款
	-- if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and coalesce(e1.account_period_name,'0')<>'预付货款' and b1.category_second is null,0,	
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and coalesce(e1.account_period_name,'0')<>'预付货款' and b1.category_second is null,0,	
	...
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and coalesce(e1.account_period_name,'0')<>'预付货款' and b3.category_second is null,0,		
	...
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and coalesce(e1.account_period_name,'0')<>'预付货款' and b2.category_second is null,0,
