...
-- 202403签呈：每月处理 南京签呈刘连兵（工号：80980890）不计算毛利额达成系数，江苏盐城都按实际达成计算提成
...
select *,
	case 
		...
		when a.work_no in('80980890') and substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)>='202403' then if(profit_target_rate<1,profit_target_rate,1)
...
select 
	...
	if(a.service_falg in('服务费','销售员按服务费'),a.service_fee*if((a.rp_pay_amt/a.rp_sale_amt_real)>1,1,if((a.rp_pay_amt/a.rp_sale_amt_real)<-1,-1,(a.rp_pay_amt/a.rp_sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,a.tc_sales_rp))
	)*if(d.category_second like'%提成减半%',0.5,1) as tc_rp_sales,
	...
	if(a.service_falg in('服务费','销售员按服务费'),a.service_fee*if((a.fl_pay_amt/a.fl_sale_amt_real)>1,1,if((a.fl_pay_amt/a.fl_sale_amt_real)<-1,-1,(a.fl_pay_amt/a.fl_sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,a.tc_sales_fl))
	)*if(d.category_second like'%提成减半%',0.5,1) as tc_fl_sales,

	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and c.paid_date_new is null and coalesce(e1.account_period_name,'0')<>'预付货款' b2.category_second is null,0,
	if(((a.bbc_zy_pay_amt+a.bbc_ly_pay_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))>1,1,if(((a.bbc_zy_pay_amt+a.bbc_ly_pay_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))<-1,-1,((a.bbc_zy_pay_amt+a.bbc_ly_pay_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,(a.tc_sales_bbc_zy+a.tc_sales_bbc_ly)))
	)*if(d.category_second like'%提成减半%',0.5,1) as tc_bbc_sales,
	...
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and c.paid_date_new is null and coalesce(e1.account_period_name,'0')<>'预付货款' and b1.category_second is null,0,
	if(a.service_falg in('服务费','销售员按服务费'),a.service_fee*if((a.rp_pay_amt/a.rp_sale_amt_real)>1,1,if((a.rp_pay_amt/a.rp_sale_amt_real)<-1,-1,(a.rp_pay_amt/a.rp_sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,a.original_tc_sales_rp))
	)*if(d.category_second like'%提成减半%',0.5,1) as original_tc_rp_sales,

	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and c.paid_date_new is null and coalesce(e1.account_period_name,'0')<>'预付货款' and b3.category_second is null ,0,
	if(a.service_falg in('服务费','销售员按服务费'),a.service_fee*if((a.fl_pay_amt/a.fl_sale_amt_real)>1,1,if((a.fl_pay_amt/a.fl_sale_amt_real)<-1,-1,(a.fl_pay_amt/a.fl_sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,a.original_tc_sales_fl))
	)*if(d.category_second like'%提成减半%',0.5,1) as original_tc_fl_sales,

	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and c.paid_date_new is null and coalesce(e1.account_period_name,'0')<>'预付货款' b2.category_second is null,0,
	if(a.service_falg in('服务费','管家按服务费'),a.service_fee
		*if(((a.bbc_zy_pay_amt+a.bbc_ly_pay_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))>1,1,if(((a.bbc_zy_pay_amt+a.bbc_ly_pay_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))<-1,-1,((a.bbc_zy_pay_amt+a.bbc_ly_pay_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,(a.original_tc_sales_bbc_zy+a.original_tc_sales_bbc_ly)))
	)*if(d.category_second like'%提成减半%',0.5,1) as original_tc_bbc_sales,
...
select 
	...
	sum(tc_rp_sales) as tc_rp_sales,
	sum(tc_fl_sales) as tc_fl_sales,		
	sum(tc_bbc_sales) as tc_bbc_sales,	
	sum(original_tc_rp_sales) as original_tc_rp_sales,	
	sum(original_tc_fl_sales) as original_tc_fl_sales,	
	sum(original_tc_bbc_sales) as original_tc_bbc_sales,	
