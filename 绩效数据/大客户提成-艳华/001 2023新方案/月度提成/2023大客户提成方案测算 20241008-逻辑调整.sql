...
更新日志：
202410月 
1、更新 job_csx_analyse_fr_tc_customer_credit_order_detail-20241023 -调整北京央视、301系数.sql 根据签呈调整客户系数
2、更新浙江、安徽无销售系数按照原管家系数
3、广州销售系数有管家的销售员系数按照0.6


1、签呈任务需要重点下游，客户对应销售员与服务管家及提成系数_签呈版
 csx_analyse_customer_sale_service_info_rate_qc_mi  
2、涉及到福利&BBC系数*1.2表
	a.csx_analyse_fr_tc_customer_person_profit_real_mi
	b.csx_analyse.csx_analyse_tc_person_profit_target_rate
	c.csx_analyse_fr_tc_customer_bill_month_dff_rate_detail
*/
...
select 
	...
	--安徽(11)未挂销售的客户,由管家独立维护,该客户分配系数申请按40%进行核算提成。

	case when  a.province_code in ('23','24','32','1','26','19','14','20','906') then b1.level_sale_rate*2
		when a.province_code in ('13') and (a.work_no_new ='' or sales_name_new like '%B') then b1.level_sale_rate*2
	...
	-- when a.province_code='13' and sdt>='20240501' then 0 
		when a.province_code in ('23','24','32','1','26','19','14','20','906') then b2.level_sale_rate*2
		when a.province_code in ('13') and (a.work_no_new ='' or sales_name_new like '%B') then b2.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b2.level_sale_rate end as fl_service_user_fp_rate,
	case 
	-- when a.province_code='13' and sdt>='20240501' then 0 
		when  a.province_code in ('23','24','32','1','26','19','14','20','906') then b3.level_sale_rate*2
		when a.province_code in ('13') and (a.work_no_new ='' or sales_name_new like '%B') then b3.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
...
-- ******************************************************************** 
-- @功能描述：大客户提成-客户人员毛利完成值数据中心sheet4  csx_analyse_fr_tc_customer_person_profit_real_mi
-- @创建者： 饶艳华 
-- @创建者日期：2023-10-11 22:15:20 
-- @修改者日期：
-- @修改人：
-- @修改内容：20240814增加逻辑，取消签呈客户系数
-- ******************************************************************** 
...
select 
	...
	d.fl_sale_amt,
	-- 各类型定价毛利额
	-- 个人实际毛利额核算时福利及联营bbc业务按照1.2系数上浮 11 安徽取消1.2系数
	if(e.flag='1' or a.province_code ='11' ,(d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*adjust_rate+d.fl_profit*adjust_rate),(d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*1.2+d.fl_profit*1.2)) as profit,
	d.rp_profit,
	if(e.flag='1' or a.province_code ='11' ,d.bbc_profit_zy+d.bbc_profit_ly*adjust_rate,d.bbc_profit_zy+d.bbc_profit_ly*1.2) as bbc_profit,
	d.bbc_profit_zy as bbc_zy_profit,
	if(e.flag='1' or a.province_code ='11',d.bbc_profit_ly*adjust_rate,d.bbc_profit_ly*1.2 ) as bbc_ly_profit,
	if(e.flag='1' or a.province_code ='11' ,d.fl_profit*adjust_rate,d.fl_profit*1.2) as fl_profit,
	from_utc_timestamp(current_timestamp(),'GMT') update_time,
...
-- ******************************************************************** 
-- @功能描述：大客户提成-人员毛利目标达成情况csx_analyse.csx_analyse_tc_person_profit_target_rate
-- @创建者： 饶艳华 
-- @创建者日期：2023-11-10 18:55:17 
-- @修改者日期：
-- @修改人：
-- @修改内容：20240814新增取消1.2系数调整
-- ******************************************************************** 
...
select 
	...
	d.fl_sale_amt,
	-- 各类型定价毛利额
	-- 个人实际毛利额核算时福利及联营bbc业务按照1.2系数上浮20240814新增取消1.2系数调整。
	if(e.flag='1' ,(d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*adjust_rate+d.fl_profit*adjust_rate),(d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*1.2+d.fl_profit*1.2)) as profit,
	d.rp_profit,
	if(e.flag='1' ,d.bbc_profit_ly*adjust_rate+d.bbc_profit_zy,(d.bbc_profit_ly*1.2+d.bbc_profit_zy)) as bbc_profit,
	if(e.flag='1' ,d.bbc_profit_ly*adjust_rate,d.bbc_profit_ly*1.2) as bbc_ly_profit,
	d.bbc_profit_zy as bbc_zy_profit,
	if(e.flag='1' ,d.fl_profit*adjust_rate,d.fl_profit*1.2) as fl_profit
from
-- 客户对应的销售员、客服经理	
	(  
	select *
...
-- ******************************************************************** 
-- @功能描述：大客户提成-客户结算月回款时间系数明细  csx_analyse_fr_tc_customer_bill_month_dff_rate_detail
-- @创建者： 饶艳华 
...
-- 	if(e.flag='1',d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*e.adjust_rate+d.fl_profit*e.adjust_rate,d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*1.2+d.fl_profit*1.2) as profit,
-- 	d.rp_profit,
-- 	if(e.flag='1',d.bbc_profit_ly*e.adjust_rate+d.bbc_profit_zy,d.bbc_profit_ly*1.2+d.bbc_profit_zy) as bbc_profit,
-- 	if(e.flag='1',d.bbc_profit_ly*e.adjust_rate,d.bbc_profit_ly*1.2) as bbc_ly_profit,
-- 	d.bbc_profit_zy as bbc_zy_profit,
-- 	if(e.flag='1',d.fl_profit*e.adjust_rate,d.fl_profit*1.2) as fl_profit
-- from
-- -- 客户对应的销售员、客服经理	
-- 	(  
-- 	select *
...
(select 
	...
	case when e.flag='1' then d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*e.adjust_rate+d.fl_profit*e.adjust_rate	
		when a.province_code='11' then (d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly+d.fl_profit)
...
select *,
	case 
		...
		when a.work_no in('80716714') and substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6) between '202311' and '202410' then if(profit_target_rate<1,profit_target_rate,1)
		when a.work_no in('81029025') and substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6) between '202309' and '202408' then if(profit_target_rate<1,profit_target_rate,1)
		when a.work_no in('80980890') and substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)>='202403' then 1
...
select a.*,
	...
	case 
		 ...
		 when f2.category_second is not null 
		  and (substr(f3.adjust_business_type,1,2)=substr(a.business_type_name,1,2) or f3.adjust_business_type='全业务') then f3.hk_dff_rate 		 
...
left join 
	...
	select customer_code,smt_date as smonth,
		...
		concat(customer_code,'-',
			cast(REGEXP_EXTRACT('调整回款逾期天数系数：原系数：40%','(\\d+(\\.\\d+)?)',1) as decimal(20,2))) as link_xs ,
		cast(hk_date_dff_rate as decimal(20,6)) hk_dff_rate
	from csx_analyse.csx_analyse_tc_customer_special_rules_2023_1mf 
	where smt=substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)
	and smt_date=substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)
	and category_second like '%调整回款逾期天数系数%'		
	)f3 on a.customer_code=f3.customer_code	and concat(a.customer_code,'-',a.dff_rate)=f3.link_xs
;	
...
(
	select
		...
		case when customer_code='126275' and dff_rate_new>1 then 1 
	...
	group by 	region_code,
		...
		case when customer_code='126275' and dff_rate_new>1 then 1 else dff_rate_new end
)a	
...
create temporary table csx_analyse_tmp.tmp_tc_business_billmonth_profit_rate_tc_1
as
select
	a.region_code,
	a.region_name,
...
(
  select customer_code,
	smt_date as smonth,
	split(remark,'：')[1] as happen_month_pay_amt,  --扣减回款金额：销售月_打款日期_回款金额20246月出现销售月与金额多个，其他正常
...
-- 客户+结算月+回款时间系数：各业务类型毛利率提成比例 处理扣减回款金额后	
...
-- ******************************************************************** 
-- @功能描述：大客户提成_业绩毛利 csx_analyse_fr_tc_customer_sale_fwf_business
-- @创建者： 饶艳华 
-- @创建者日期：2023-09-25 05:31:39 
-- @修改者日期：
-- @修改人：
-- @修改内容：
-- ******************************************************************** 
