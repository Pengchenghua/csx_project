...
select 
	...
	case when  e.flag='1'  then (d.bbc_profit_zy+d.bbc_profit_ly*adjust_rate)
	    when a.province_code ='11'  then (d.bbc_profit_zy+d.bbc_profit_ly)
...
-- ******************************************************************** 
-- @功能描述：csx_analyse_fr_tc_customer_bill_month_dff_rate_detail
-- @创建者： 饶艳华 
...
(
	select
		...
		case when a.customer_code='126275' and dff_rate_new>1 then 1
			-- when b.adju_flag=1 and dff_rate_new>1 then hk_dff_rate 
...
-- 增加对最高系数限制
	...
	(
		...
		and smt_date=substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)
		and category_second like'%调整回款最高系数%'
	) c on a.customer_code=c.customer_code and a.business_type_name=c.adjust_business_type
;
...
select 
	...
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and c.paid_date_new is null 
		and coalesce(e1.account_period_name,'0')<>'预付货款' and b1.category_second is null 
		and b3.category_second is null and b2.category_second is null,0,
	...
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and a.yufu_flag='否' and c.paid_date_new is null 
		and coalesce(e1.account_period_name,'0')<>'预付货款' and b3.category_second is null,0,		
