...
3、相关表剔除永辉生活、永辉线下购   csx_dws_sale_detail_di		and order_channel_detail_code not in ('24','28')
csx_analyse_fr_tc_customer_credit_order_unpay_mi
csx_analyse_fr_tc_customer_sale_fwf_business 
csx_analyse_fr_tc_customer_credit_order_detail
job_csx_analyse_fr_tc_customer_bill_month_dff_rate_detail
...
-- 		 when a.province_code='906' and  a.work_no_new <>'' then 1
    ...
    -- 北京延迟一个月按照原方案执行
	case when  a.province_code in ('23','24','32','14','20','906','1') then b1.level_sale_rate*2
	...
	-- when a.province_code='13' and sdt>='20240501' then 0 
		when a.province_code in ('23','24','32','14','20','906','1') then b2.level_sale_rate*2
	...
	case 
	-- when a.province_code='13' and sdt>='20240501' then 0 
		when  a.province_code in ('23','24','32','14','20','906','1') then b3.level_sale_rate*2
...
select 
	...
	d.fl_sale_amt,
	-- 各类型定价毛利额
    -- 个人实际毛利额核算时福利及联营bbc业务按照1.2系数上浮 11 安徽取消1.2系数 902 江苏苏州取消1.2
	case when e.flag='1'  then   (d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*adjust_rate+d.fl_profit*adjust_rate)
	    ...
	    else (d.rp_profit+d.bbc_profit_zy+d.bbc_profit_ly*1.2+d.fl_profit*1.2) end  as profit,
	d.rp_profit,
	case when  e.flag='1'  then (d.bbc_profit_zy+d.bbc_profit_ly*adjust_rate)
	    when a.province_code in ('11','901')   then (d.bbc_profit_zy+d.bbc_profit_ly)
	    when  a.city_group_code in ('7') then (d.bbc_profit_zy+d.bbc_profit_ly)
	    else (d.bbc_profit_zy+d.bbc_profit_ly*1.2) end  as bbc_profit,
	d.bbc_profit_zy as bbc_zy_profit,
	case when e.flag='1'  then d.bbc_profit_ly*adjust_rate
	...
	case when e.flag='1' then d.fl_profit*adjust_rate
...
select a.*,
	...
	(rp_pay_amt*cust_rp_profit_rate_tc*dff_rate*rp_sales_fp_rate)*if(f.category_second<>'',f.adjust_rate,coalesce(case when f.category_second<>'' then f.adjust_rate 
	                                                                    when j.renew_flag='1' and a.happen_month>='202504' then j.renew_cust_rate  -- 续签80%从4月生效
		                                                                when e.new_cust_flag='1'and a.happen_month>='202504' then e.new_cust_rate
		                                                                else  1	end ,1))*coalesce(d1.profit_target_rate_tc,1) as tc_sales_rp,
	...
	(rp_pay_amt*cust_rp_profit_rate_tc*dff_rate*rp_sales_fp_rate)*if(f.category_second<>'',f.adjust_rate,coalesce(case when f.category_second<>'' then f.adjust_rate 
	                                                                    when j.renew_flag='1' and a.happen_month>='202504' then j.renew_cust_rate  -- 续签80%从4月生效
		                                                                when e.new_cust_flag='1' and a.happen_month>='202504' then e.new_cust_rate
		                                                                else  1	end ,1)) as original_tc_sales_rp,
	...
	case when f.category_second<>'' then f.adjust_rate 
	    when j.renew_flag='1' and a.happen_month>='202504' then j.renew_cust_rate  -- 续签80%从4月生效
		when e.new_cust_flag='1' and a.happen_month>='202504' then e.new_cust_rate
		else  1
	end  new_cust_rate
...
left join 
(
select distinct customer_code,1.2 as new_cust_rate,'1' new_cust_flag
	from 
	(
...
left join 
-- 老客续签系数0.8 取上月有销售的客户，20254月开始，所有的老客续签，合同终止大于当前月份，小于则为失效
(select   
	customer_code ,
...
-- ******************************************************************** 
-- @功能描述：csx_analyse_fr_tc_customer_sale_fwf_business
-- @创建者： 饶艳华 
...
from 
	...
	where 
		sdt>=regexp_replace(add_months(trunc('${sdt_yes_date}','MM'),-1),'-','') 
		and sdt<=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
		and channel_code in('1','7','9')
...
-- ******************************************************************** 
-- @功能描述：csx_analyse_fr_tc_customer_credit_order_detail
-- @创建者： 饶艳华 
...
select
	...
	case when (regexp_replace(substr(a.happen_date,1,10),'-','') between f.date_star and f.date_end) and f.category_second is not null 
		 ...
		 when a.customer_code in ('252183','252191','252193','252181','250767','151497','252182','252185','252186','252189','252195','106287') and a.dff between 60 and 90 then 1
