...
select
	...
	case when (regexp_replace(substr(a.happen_date,1,10),'-','') between f.date_star and f.date_end) and f.category_second is not null 
		 ...
		 when a.customer_code in ('252183','252191','252193','252181','250767','151497','252182','252185','252186','252189','252195','106287') and a.dff betweem 60 and 90 then 1
		 when a.customer_code in ('252183','252191','252193','252181','250767','151497','252182','252185','252186','252189','252195','106287') and a.dff betweem 91 and 120 then 0.8
		 when a.customer_code in ('252183','252191','252193','252181','250767','151497','252182','252185','252186','252189','252195','106287') and a.dff betweem 121 and 150 then 0.6
...
left join 
	...
	select customer_code,smt_date as smonth,
		...
		split(remark,'：')[1] happen_bill_paid_date,
		cast(hk_date_dff_rate as decimal(20,6)) hk_dff_rate
	from csx_analyse.csx_analyse_tc_customer_special_rules_2023_1mf 
	where smt=substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)
	and smt_date=substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)
	and category_second like '%调整回款时间系数：按照销售月_结算日_打款日%'		
	)f1 on a.customer_code=f.customer_code 
		and concat_ws('_',a.happen_date,regexp_replace(bill_date,'-',''),regexp_replace(paid_date,'-',''))=f1.happen_bill_paid_date	
