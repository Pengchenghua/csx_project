...
as
select *,
	case when b.performance_province_name in('湖北省') and regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')<='202308' then if(profit_target_rate<1,profit_target_rate,1)
		when b.performance_province_name not in('湖北省') and regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')<='202310' then if(profit_target_rate<1,profit_target_rate,1)
