...
select 
	...
	sum(if(c.unpay_amt<>0 and b.pay_amt_bill<>0 and abs(b.pay_amt_bill/c.unpay_amt)>1,b.pay_amt/abs(b.pay_amt_bill/c.unpay_amt),b.pay_amt)) pay_amt  -- 核销金额
from 
(
	select 
	...
	from 
		(-- 核销流水明细表:本月核销金额
			...
			(
				...
				and date_format(happen_date,'yyyy-MM-dd')>='2022-06-01'
				and delete_flag ='0'
				group by close_bill_code,claim_bill_code,close_account_code,
...
(
	...
	(
		select close_bill_code,
			...
			sum(pay_amt) pay_amt,
			max(bill_amt)-sum(pay_amt) unpay_amt
		-- 单据核销流水明细月快照表
		from csx_ads.csx_ads_sss_close_bill_account_record_snapshot_mf
...
select 
	...
	d.profit,
	d.rp_profit,
	(d.bbc_profit_ly+d.bbc_profit_zy)bbc_profit,
...
(
	select
		...
		case when customer_code='126275' and dff_rate>1 then 1 else dff_rate end as dff_rate,  -- 回款时间系数
...
select 
	...
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b1.category_second is null and b3.category_second is null and b2.category_second is null,0,
	if(a.service_falg in('服务费','销售员按服务费'),a.service_fee*if((a.sale_amt/a.sale_amt_real)>1,1,if((a.sale_amt/a.sale_amt_real)<-1,-1,(a.sale_amt/a.sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','销售员不算提成'),0,
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b1.category_second is not null,a.tc_sales_rp/if(a.dff_rate>1,a.dff_rate,1),a.tc_sales_rp)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b3.category_second is not null,a.tc_sales_bbc_zy/if(a.dff_rate>1,a.dff_rate,1),a.tc_sales_bbc_zy)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b3.category_second is not null,a.tc_sales_bbc_ly/if(a.dff_rate>1,a.dff_rate,1),a.tc_sales_bbc_ly)+
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b2.category_second is not null,a.tc_sales_fl/if(a.dff_rate>1,a.dff_rate,1),a.tc_sales_fl)
				))
	)*if(d.category_second like'%提成减半%',0.5,1) as tc_sales,
	
	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b1.category_second is null,0,	
	if(a.service_falg in('服务费','管家按服务费'),a.service_fee*if((a.rp_sale_amt/a.rp_sale_amt_real)>1,1,if((a.rp_sale_amt/a.rp_sale_amt_real)<-1,-1,(a.rp_sale_amt/a.rp_sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','管家不算提成'),0,
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b1.category_second is not null,a.tc_rp_service/if(a.dff_rate>1,a.dff_rate,1)
				,a.tc_rp_service)))
	)*if(d.category_second like'%提成减半%',0.5,1) as tc_rp_service,		

	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b3.category_second is null,0,		
	if(a.service_falg in('服务费','管家按服务费'),a.service_fee*if((a.fl_sale_amt/a.fl_sale_amt_real)>1,1,if((a.fl_sale_amt/a.fl_sale_amt_real)<-1,-1,(a.fl_sale_amt/a.fl_sale_amt_real)))*dff_rate,
		if(d.category_second in('不算提成','管家不算提成'),0,
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b3.category_second is not null,a.tc_fl_service/if(a.dff_rate>1,a.dff_rate,1)
				,a.tc_fl_service)))
		)*if(d.category_second like'%提成减半%',0.5,1) as tc_fl_service,	

	if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b2.category_second is null,0,
	if(a.service_falg in('服务费','管家按服务费'),a.service_fee
		*if(((a.bbc_zy_sale_amt+a.bbc_ly_sale_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))>1,1,if(((a.bbc_zy_sale_amt+a.bbc_ly_sale_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))<-1,-1,((a.bbc_zy_sale_amt+a.bbc_ly_sale_amt)/(a.bbc_sale_amt_zy_real+a.bbc_sale_amt_ly_real))))*dff_rate,
		if(d.category_second in('不算提成','管家不算提成'),0,
			if(a.paid_date<=last_day(add_months('${sdt_yes_date}',-3)) and b2.category_second is not null,a.tc_bbc_service/if(a.dff_rate>1,a.dff_rate,1)
				,a.tc_bbc_service)))
