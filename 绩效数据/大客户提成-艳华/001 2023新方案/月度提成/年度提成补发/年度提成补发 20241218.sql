...
-- create table csx_analyse_tmp.
with tmp_tc_person_month_dff_b
as
(select 
'销售员' as person_flag,
smt,
...
if(sum(sales_profit_finish)over(partition by work_no,city_group_name)/sum(sales_profit_basic)over(partition by work_no,city_group_name) >=1,
    1,sum(sales_profit_finish)over(partition by work_no,city_group_name)/sum(sales_profit_basic)over(partition by work_no,city_group_name) ) sales_profit_target_rate_tc_new,
tc_sales_new ,
...
work_no,
sales_name,
sales_profit_basic,     -- 销售基准毛利额
sales_profit_finish,    -- 实际毛利额
sales_target_rate,      -- 销售员毛利额_达成率

sales_target_rate_tc,   -- 销售员毛利额_达成系数
sum(pay_amt) pay_amt,   -- 回款金额
sum(tc_sales) tc_sales, -- 销售员提成额
...
sum(original_tc_sales) tc_sales_new,    -- 原始提成额
...
where smt between '202401' and '202412'
    and work_no<>''
group by 
smt,
region_name,
...
sales_profit_new,
sales_profit_new/sales_profit_basic_new  sales_profit_target_rate_new,
if(sales_profit_new/sales_profit_basic_new >=1,1,0) sales_profit_target_rate_tc_new,
tc_sales_new ,
if(sales_profit_new/sales_profit_basic_new >=1,(tc_sales_new)-(tc_sales),0) as tc_sales_dff_bu  -- 需补发提成
from tmp_tc_person_month_dff_b 
where 1=1
;
