-- 大客户提成 签呈表需查询的对应表

select 
concat_ws('-',substr(a.sdt,1,6),a.customer_no) as biz_id,
a.customer_id,
a.customer_no,
a.customer_name,
a.channel_code,
a.channel_name,
a.region_code,
a.region_name,
a.province_code,
a.province_name,
a.city_group_code,
a.city_group_name,
a.sales_id_new,
a.work_no_new,
a.sales_name_new,
rp_sales_user_work_no_new as rp_sales_number	,
rp_sales_user_name_new as rp_sales_name	,
-- rp_sales_user_position_new ,
fl_sales_user_work_no_new as fl_sales_number,
fl_sales_user_name_new as fl_sales_name,
bbc_sales_user_work_no_new as bbc_sales_number,
bbc_sales_user_name_new as bbc_sales_name,
a.rp_service_user_id_new,
a.rp_service_user_work_no_new,
a.rp_service_user_name_new,
a.fl_service_user_id_new,
a.fl_service_user_work_no_new,
a.fl_service_user_name_new,
a.bbc_service_user_id_new,
a.bbc_service_user_work_no_new,
a.bbc_service_user_name_new,
case	when a.region_code='2' and a.province_code !='906' and a.sales_id_new <>'' then 0.6
		when length(rp_service_user_id_new)<>0 and length(sales_id_new)>0 then 0.6
		when length(sales_id_new)>0 then 1
		end as  rp_sales_sale_rate,
null rp_sales_profit_rate,
	
case 
	 when a.region_code='2'  and a.province_code !='906' and a.work_no_new <>'' then 0.6
	 when length(fl_service_user_id_new)<>0 and length(work_no_new)>0 then 0.6
	 when length(work_no_new)>0 then 1
	 end as  fl_sales_sale_rate,
null fl_sales_profit_rate,	

case 
	 when a.region_code='2'  and a.province_code !='906' and a.work_no_new <>'' then 0.6
	 when length(bbc_service_user_id_new)<>0 and length(work_no_new)>0 then 0.6
	 when length(work_no_new)>0 then 1
	end as bbc_sales_sale_rate,
null bbc_sales_profit_rate,	
0 as rp_service_user_fp_rate,
null rp_service_user_profit_rate,
0 fl_service_user_fp_rate,
null fl_service_user_profit_rate,
0 as bbc_service_user_fp_rate,
null bbc_service_user_profit_rate,
substr(a.sdt,1,6) smt_date
from 
(
select *
from csx_analyse.csx_analyse_report_crm_customer_sale_service_manager_info_df 
where sdt='20251031'
and customer_no in 
(
-- '119990','127367','237170','131186','243793','178714','119206','171871','238753','263797','118427',
-- '127017','259584','202962','252324','118474','193339','117758','118289','246442','114255','114188',
-- '211672','216369','220857','253150','107123','232192','126201','130033','123559','114444','104493',
-- '253222','281546','258144','259930','255465','255678','254652','255611','259925','255640','255147',
-- '246412','249799','252404','246926','265050','252115','265890','265898','246159','247793','247826',
-- '235864','272828','291623','246424','252687','129794','220242','209968','124848','233646','203282',
-- '124321','196704','185117','187280','251712','191446','235128','235130','235131','235132','235133',
-- '235134','128371','255826','254028','126766','126157','127944','128951','129525','129668','230041',
-- '231077','231574','251454','258134','259846','124524','125748','128745','207975','245242','249615',
-- '251692','253106','254013','255101','255114','257384','259801','259823','282820',

'206048','257822','256261','250219','243932','265721','259604','253623','244177','236744','179272',
'224684','130504','130222','129939','129575','129482','127398','126127','125753','124697','121874',
'121862','120953','120952','120946','116383','108734','105628'
)
 
)a 

;

;

-- 确认需对哪些客服经理补充等级比例 计划1--3号提供
select 
	a.performance_region_name,
	a.performance_province_name,
	a.performance_city_name,
	b.flag,
	b.user_work_no,
	b.user_name,
	d.s_level,
	d.level_sale_rate,
	d.level_profit_rate,
	sum(sale_amt) sale_amt,
	sum(profit) profit
from
	(
		select 
			performance_region_name,
			performance_province_code,
			performance_province_name,
			performance_city_name,
			customer_code,
			substr(sdt,1,6) smonth,
			sum(sale_amt) sale_amt,
			sum(profit) profit
		from csx_dws.csx_dws_sale_detail_di
		where sdt>=regexp_replace(add_months(trunc('${sdt_yes_date}','MM'),-1),'-','')
			and sdt<=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
			and channel_code in('1','7')
		group by performance_region_name,
			performance_province_code,
			performance_province_name,
			performance_city_name,
			customer_code,
			substr(sdt,1,6)
	)a	
left join 
(
	select 
		'客服经理' flag,
		customer_no,
		rp_service_user_work_no_new as user_work_no,
		rp_service_user_name_new as user_name,
		rp_service_user_id_new as user_id
	from csx_analyse.csx_analyse_report_crm_customer_sale_service_manager_info_df
	where sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
	and rp_service_user_id_new is not null
	union all
	select 
		'客服经理' flag,
		customer_no,
		fl_service_user_work_no_new as user_work_no,
		fl_service_user_name_new as user_name,
		fl_service_user_id_new as user_id
	from csx_analyse.csx_analyse_report_crm_customer_sale_service_manager_info_df
	where sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
	and fl_service_user_id_new is not null
	union all	
	select 
		'客服经理' flag,
		customer_no,
		bbc_service_user_work_no_new as user_work_no,
		bbc_service_user_name_new as user_name,
		bbc_service_user_id_new as user_id
	from csx_analyse.csx_analyse_report_crm_customer_sale_service_manager_info_df
	where sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
	and bbc_service_user_id_new is not null
) b on b.customer_no=a.customer_code
left join 
(
	select *
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-2),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-2)),'-','')
	-- where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	-- and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
	-- 不算提成 此类人员需再次确认
	and salary_level<>'不算提成'
)d on b.user_work_no=d.service_user_work_no	
where d.s_level is null
group by a.performance_region_name,
	a.performance_province_name,
	a.performance_city_name,
	b.flag,
	b.user_work_no,
	b.user_name,
	d.s_level,
	d.level_sale_rate,
	d.level_profit_rate
;	


-- 客户销售员管家对应关系 ★★★★先维护好管家等级表 X月客户销售员对照表  
select 
a.customer_no,
a.region_name,
a.province_name,
a.city_group_name,
a.channel_name,
a.customer_no as customer_no_code,
a.customer_name,
a.sales_id_new,
a.work_no_new,
a.sales_name_new,
a.rp_service_user_id_new,
a.rp_service_user_work_no_new,
a.rp_service_user_name_new,
a.fl_service_user_id_new,
a.fl_service_user_work_no_new,
a.fl_service_user_name_new,
a.bbc_service_user_id_new,
a.bbc_service_user_work_no_new,
a.bbc_service_user_name_new,
-- 202405签呈 每月 浙江全部福利BBC业务调整提成比例，销售员按100%，管家0%
case 
	when a.region_name='华南大区' and a.work_no_new <>'' then 0.6
	when length(a.rp_service_user_id_new)<>0 and length(a.work_no_new)>0 then 0.6
	when length(a.work_no_new)>0 then 1	
	end as rp_sales_sale_rate,
case 
	when a.region_name='华南大区' and a.work_no_new <>'' then 0.6
	when a.province_name='浙江省' and sdt>='20240501' and a.work_no_new <>'' then 1
	when length(a.fl_service_user_id_new)<>0 and length(a.work_no_new)>0 then 0.6
	when length(a.work_no_new)>0 then 1	
	end as fl_sales_sale_rate,
case 
	when a.region_name='华南大区' and a.work_no_new <>'' then 0.6
	when a.province_name='浙江省' and sdt>='20240501' and a.work_no_new <>'' then 1
	when length(a.bbc_service_user_id_new)<>0 and length(a.work_no_new)>0 then 0.6
	when length(a.work_no_new)>0 then 1	
	end as bbc_sales_sale_rate,
-- b1.level_sale_rate as rp_service_user_fp_rate,
-- case when a.province_name='浙江省' and sdt>='20240501' then 0 else b2.level_sale_rate end as fl_service_user_fp_rate,
-- case when a.province_name='浙江省' and sdt>='20240501' then 0 else b3.level_sale_rate end as bbc_service_user_fp_rate ,
-- 华西延迟一个月202412 '23','24','32',
	case when  a.province_code in ('23','24','32') and b1.s_level='高级' then 0.3
	    when  a.province_code in ('23','24','32') and b1.s_level='中级' then 0.2
	    when  a.province_code in ('23','24','32') and b1.s_level='初级' then 0.15
		when a.province_code in ('13','23','24','32') and (a.work_no_new ='' or sales_name_new like '%B') then b1.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b1.level_sale_rate  end as rp_service_user_fp_rate,
	case 
	-- when a.province_code='13' and sdt>='20240501' then 0 
		when  a.province_code in ('23','24','32') and b2.s_level='高级' then 0.3
	    when  a.province_code in ('23','24','32') and b2.s_level='中级' then 0.2
	    when  a.province_code in ('23','24','32') and b2.s_level='初级' then 0.15
		when a.province_code in ('13','23','24','32') and (a.work_no_new ='' or sales_name_new like '%B') then b2.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b2.level_sale_rate end as fl_service_user_fp_rate,
	case 
	-- when a.province_code='13' and sdt>='20240501' then 0 
		when  a.province_code in ('23','24','32') and b3.s_level='高级' then 0.3
	    when  a.province_code in ('23','24','32') and b3.s_level='中级' then 0.2
	    when  a.province_code in ('23','24','32') and b3.s_level='初级' then 0.15
		when a.province_code in ('13','23','24','32') and (a.work_no_new ='' or sales_name_new like '%B') then b3.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b3.level_sale_rate end as bbc_service_user_fp_rate,
		a.SDT
from 
(
select *
from csx_analyse.csx_analyse_report_crm_customer_sale_service_manager_info_df
where sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
	
)a 
-- 关联管家等级	
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b1 on a.rp_service_user_work_no_new=b1.service_user_work_no	
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b2 on a.fl_service_user_work_no_new=b2.service_user_work_no
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b3 on a.bbc_service_user_work_no_new=b3.service_user_work_no
;

--用户信息
select user_id,user_number,user_name,province_name,city_name from csx_dim.csx_dim_uc_user where sdt='current' and user_name='杨权'

-- 调整对应人员比例查询
-- 那延长的只有华西和北京哈，华西2个月 
select 
concat_ws('-',substr(a.sdt,1,6),a.customer_no) as biz_id,
a.customer_id,
a.customer_no,
a.customer_name,
a.channel_code,
a.channel_name,
a.region_code,
a.region_name,
a.province_code,
a.province_name,
a.city_group_code,
a.city_group_name,
a.sales_id_new,
a.work_no_new,
a.sales_name_new,
a.rp_service_user_id_new,
a.rp_service_user_work_no_new,
a.rp_service_user_name_new,
a.fl_service_user_id_new,
a.fl_service_user_work_no_new,
a.fl_service_user_name_new,
a.bbc_service_user_id_new,
a.bbc_service_user_work_no_new,
a.bbc_service_user_name_new,
case	when a.region_code='2' and a.province_code !='906' and a.sales_id_new <>'' then 0.6
		when length(rp_service_user_id_new)<>0 and length(sales_id_new)>0 then 0.6
		when length(sales_id_new)>0 then 1
		end as  rp_sales_sale_rate,
null rp_sales_profit_rate,
	
case 
	 when a.region_code='2'  and a.province_code !='906' and a.work_no_new <>'' then 0.6
	 when length(fl_service_user_id_new)<>0 and length(work_no_new)>0 then 0.6
	 when length(work_no_new)>0 then 1
	 end as  fl_sales_sale_rate,
null fl_sales_profit_rate,	

case 
	 when a.region_code='2'  and a.province_code !='906' and a.work_no_new <>'' then 0.6
	 when length(bbc_service_user_id_new)<>0 and length(work_no_new)>0 then 0.6
	 when length(work_no_new)>0 then 1
	end as bbc_sales_sale_rate,
null bbc_sales_profit_rate,	
case when  a.province_code in ('23','24','32') and b1.s_level='高级' then 0.3
	    when  a.province_code in ('23','24','32') and b1.s_level='中级' then 0.2
	    when  a.province_code in ('23','24','32') and b1.s_level='初级' then 0.15
		when a.province_code in ('13','23','24','32') and (a.work_no_new ='' or sales_name_new like '%B') then b1.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b1.level_sale_rate  end as rp_service_user_fp_rate,
null rp_service_user_profit_rate,
case 
	-- when a.province_code='13' and sdt>='20240501' then 0 
		when  a.province_code in ('23','24','32') and b2.s_level='高级' then 0.3
	    when  a.province_code in ('23','24','32') and b2.s_level='中级' then 0.2
	    when  a.province_code in ('23','24','32') and b2.s_level='初级' then 0.15
		when a.province_code in ('13','23','24','32') and (a.work_no_new ='' or sales_name_new like '%B') then b2.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b2.level_sale_rate end as fl_service_user_fp_rate,
null fl_service_user_profit_rate,
case 
	-- when a.province_code='13' and sdt>='20240501' then 0 
		when  a.province_code in ('23','24','32') and b3.s_level='高级' then 0.3
	    when  a.province_code in ('23','24','32') and b3.s_level='中级' then 0.2
	    when  a.province_code in ('23','24','32') and b3.s_level='初级' then 0.15
		when a.province_code in ('13','23','24','32') and (a.work_no_new ='' or sales_name_new like '%B') then b3.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b3.level_sale_rate END as bbc_service_user_fp_rate,
null bbc_service_user_profit_rate,
substr(a.sdt,1,6) smt_date
from 
(
select *
from csx_analyse.csx_analyse_report_crm_customer_sale_service_manager_info_df 
where sdt='20250831'
and customer_no in 
('118850',
'172394',
'126983',
'128485',
'120250',
'279945',
'115613',
'116992',
'112605',
'103649',
'105022',
'248341',
'226740',
'105984',
'125263',
'235994',
'254879',
'275371',
'259378',
'259382')
)a 
-- 关联管家等级	
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b1 on a.rp_service_user_work_no_new=b1.service_user_work_no	
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b2 on a.fl_service_user_work_no_new=b2.service_user_work_no
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b3 on a.bbc_service_user_work_no_new=b3.service_user_work_no
order by a.province_name
;


-- 扣减回款金额 查看金额


select happen_month,
	
	 concat(happen_month,'_',cast(pay_amt as decimal(26,2))) as happen_month_pay_amt
	-- sum(cast(pay_amt as decimal(26,2))) as pay_amt
	from 
	(select substr(regexp_replace(happen_date,'-',''),1,6)happen_month,
	
	-- concat(happen_month,'_',cast(pay_amt as decimal(26,2))) as happen_month_pay_amt,
	sum(cast(pay_amt as decimal(26,2))) as pay_amt
	from csx_analyse.csx_analyse_fr_tc_customer_credit_order_detail
	where smt=substr(regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-',''),1,6)
	and status=1
	and customer_code ='248539'
	
	group by  substr(regexp_replace(happen_date,'-',''),1,6)
	)a

-- 销售人员对照表


-- 客户销售员管家对应关系 ★★★★先维护好管家等级表 X月客户销售员对照表
select 
a.customer_no,
a.region_name,
a.province_name,
a.city_group_name,
a.channel_name,
a.customer_no,
a.customer_name,
a.sales_id_new,
a.work_no_new,
a.sales_name_new,
a.rp_service_user_id_new,
a.rp_service_user_work_no_new,
a.rp_service_user_name_new,
a.fl_service_user_id_new,
a.fl_service_user_work_no_new,
a.fl_service_user_name_new,
a.bbc_service_user_id_new,
a.bbc_service_user_work_no_new,
a.bbc_service_user_name_new,
-- 202405签呈 每月 浙江全部福利BBC业务调整提成比例，销售员按100%，管家0%
case	when a.region_code='2' and a.province_code !='906' and a.sales_id_new <>'' then 0.6
		when length(rp_service_user_id_new)<>0 and length(sales_id_new)>0 then 0.6
		when length(sales_id_new)>0 then 1
		end as  rp_sales_sale_rate,
case 
	 when a.region_code='2'  and a.province_code !='906' and a.work_no_new <>'' then 0.6
	 when length(fl_service_user_id_new)<>0 and length(work_no_new)>0 then 0.6
	 when length(work_no_new)>0 then 1
	 end as  fl_sales_sale_rate,
case 
	 when a.region_code='2'  and a.province_code !='906' and a.work_no_new <>'' then 0.6
	 when length(bbc_service_user_id_new)<>0 and length(work_no_new)>0 then 0.6
	 when length(work_no_new)>0 then 1
	end as  bbc_sales_sale_rate,
case when  a.province_code in ('23','24','32','14','20','906','1') then b1.level_sale_rate*2
		when a.province_code in ('13') and (a.work_no_new ='' or sales_name_new like '%B') then b1.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b1.level_sale_rate  end as  rp_service_user_fp_rate,
	case 
		when a.province_code in ('23','24','32','14','20','906','1') then b2.level_sale_rate*2
		when a.province_code in ('13') and (a.work_no_new ='' or sales_name_new like '%B') then b2.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b2.level_sale_rate end as  fl_service_user_fp_rate,
	case 
		when  a.province_code in ('23','24','32','14','20','906','1') then b3.level_sale_rate*2
		when a.province_code in ('13') and (a.work_no_new ='' or sales_name_new like '%B') then b3.level_sale_rate*2
		when a.province_code in ('11') and a.work_no_new ='' then 0.4
		else b3.level_sale_rate end as bbc_service_user_fp_rate    
from 
(
select *
from csx_analyse.csx_analyse_report_crm_customer_sale_service_manager_info_df
where sdt='20241031'
)a 
-- 关联管家等级	
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b1 on a.rp_service_user_work_no_new=b1.service_user_work_no	
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b2 on a.fl_service_user_work_no_new=b2.service_user_work_no
left join
(
	select service_user_work_no,level_sale_rate
	from csx_analyse.csx_analyse_tc_service_level_mf 
	where smt=substr(regexp_replace(add_months('${sdt_yes_date}',-1),'-',''), 1, 6)
	and tc_sdt=regexp_replace(last_day(add_months('${sdt_yes_date}',-1)),'-','')
)b3 on a.bbc_service_user_work_no_new=b3.service_user_work_no
;



'265290',
'265294',
'265294',
'265303',
'265303',
'265305',
'265305',
'265307',
'265313',
'265313',
'265314',
'274904',
'274904'
