...
-- 客诉省区汇总  SKU 更改为省区整体SKU，DC更改为城市整体SKU，任静
with aa as (
select
  a.smonth,
...
(
  ...
  from   csx_analyse.csx_analyse_fr_oms_complaint_detail_di a
  where  sdt>='20240401' and sdt<='20240630'
...
group by  a.smonth,
  ...
  -- a.inventory_dc_name,
   ...
   case when a.classify_large_code in ('B04','B05','B06','B07','B08','B09') THEN '食百' 
        when a.classify_large_code='B01' then classify_large_name
        else classify_middle_name end,respon_dept
) 
select
  a.smonth,
  -- a.performance_region_code,
...
--  a.classify_middle_name,
  kesu,
  sum(sku)over(partition by performance_province_name) as all_sku,  --更改为省区整体SKU
 kesu/ sum(sku)over(partition by performance_province_name) kesu_lv
from  aa a;
...
select
  ...
  a.inventory_dc_code,
  -- a.inventory_dc_name,
  if(a.classify_large_code in ('B01', 'B02', 'B03'),'生鲜','食百') as  classify_name,
  respon_dept,
  a.classify_middle_name,
  sum(nvl(b.kesu,0)) kesu,
  sum(nvl(a.sku,0)) sku,
...
(
  select
	...
	performance_city_name,
    inventory_dc_code,
    concat(first_level_department_name,'-', second_level_department_name) respon_dept,
    if(second_level_department_name in ('交易支持','非食用品'),second_level_department_name,classify_middle_code) classify_middle_code,
    count(distinct complaint_code)  kesu --- 客诉单量
  ...
  group by  substr(sdt,1,6),        -- 客诉日期
          inventory_dc_code,
          concat(first_level_department_name,'-', second_level_department_name),
          performance_city_name,
         if(second_level_department_name in ('交易支持','非食用品'),second_level_department_name,classify_middle_code),
         performance_province_name
  ) b on a.inventory_dc_code=b.inventory_dc_code and a.smonth=b.smonth and a.classify_middle_code=b.classify_middle_code 
       and a.performance_province_name=b.performance_province_name and a.performance_city_name=b.performance_city_name
...
select
  ...
  a.classify_middle_name,
  kesu,
  sum(sku)over(partition by performance_province_name,performance_city_name) as all_sku,  --更改为省区整体SKU
  kesu/ sum(sku)over(partition by performance_province_name,performance_city_name) kesu_lv
from  aa a;
