...
select  c.performance_province_name,
        ...
        c.supervisor_user_number,
        c.supervisor_user_name,
        sum(if(b.htbh is not null and coalesce(b.htjey ,0) <> 0 ,b.htjey/10000 ,a.estimate_contract_amount)/if(if(b.htbh is not null and coalesce(b.htjey ,0) <> 0 ,yue ,regexp_extract(a.contract_cycle_desc ,'[0-9]+' ,0)) > 12 ,if(b.htbh is not null and coalesce(b.htjey ,0) <> 0 ,yue ,regexp_extract(a.contract_cycle_desc ,'[0-9]+' ,0))/12,1) ) contract_amount
from
(
	select  if(to_date(business_sign_time) = to_date(first_business_sign_time) ,'新商机' ,'老商机') cust_flag,
	        business_sign_time,
	...
	and status = 1,-- 是否有效 0.无效 1.有效 (status = 0 
   '停止跟进'
) and business_stage = 5 and to_date(business_sign_time) >= '2024-01-01' and to_date(business_sign_time) < '2024-04-01' and performance_province_name != '平台-B' )a
left join 
-- 可以取最新日期关联合同号 
(
	select  t1.htbh,-- 合同编码 
...
on b.customer_no = a.customer_code and b.htbh = a.contract_number
left join
(
	select  *
	from csx_dim.csx_dim_crm_customer_info
...
from (
      ...
      business_type_code,
		...
		where 
			...
			and to_date(business_sign_time) >= '2024-01-01'  and to_date(business_sign_time) < '2024-04-01' 
...
(
  ...
  (
    ...
    row_number() over (partition by province_name,city_group_name order by entry_ratio desc) rno	  
      ...
      from   csx_dws.csx_dws_sale_detail_di
            -- 取上一季度客户A类
        where sdt >='20231001'  and sdt <= '20231231'
...
from (select
	...
	from(
     select 
            ...
            where a.sdt >='20231001'  and a.sdt <= '20231231'
...
where smonth in ('202401','202402','202403')
union all
...
,a.business_type_code
-- ,a.business_type_name
,count(distinct a.customer_no) as cust_cn
from aa a 
group by  province_name
,a.city_group_name
,a.fourth_supervisor_work_no
...
select
  ...
  DATEDIFF('2024-04-01','2024-01-01')as accounting_cnt,
  ...
  if(avg(receivable_amount)=0 or coalesce(max(sale_amt),0)=0,0,DATEDIFF('2024-04-01','2024-01-01')/(coalesce(max(sale_amt),0)/avg(receivable_amount))) as turnover_days
from 
(
  select
    a.sdt,
  ...
  from 
   ( 
       ...
       where (sdt='20231231'  or sdt='20240331')  
         and channel_code  in ('1','7','9')
		--  and customer_code not in ('120459','121206')
       group by sdt,
         channel_name,
...
select
  ...
  c.sales_user_number,
	...
	c.customer_code,
	c.customer_name,
  DATEDIFF('2024-04-01','2023-12-31')as accounting_cnt,
  coalesce(max(sale_amt),0) sale_amt,
  coalesce(max(excluding_tax_sales),0) excluding_tax_sales,
  avg(receivable_amount) receivable_amount
from 
...
-- 取SAP应收表 输出表：csx_analyse.csx_analyse_fr_sap_subject_customer_credit_account_analyse_df
 ...
 select
       ...
       from 
		...
		-- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20231231'  or sdt='20240331')  
         and channel_name  in ('大客户','项目供应商','虚拟销售员','业务代理','') -- and province_name='福建省'
       group by sdt,
         channel_name,
         province_name,
         city_group_name,
