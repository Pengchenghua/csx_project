...
(
    ...
    and to_date(business_sign_time) >= '2024-07-01' 
    and to_date(business_sign_time) < '2024-10-01' 
...
from (
      ...
      business_type_code,
		...
		where 
			...
			and status=1  --  是否有效 0.无效 1.有效 (status=0,'停止跟进')
			and business_stage=5
			and to_date(business_sign_time) >= '2024-07-01'  and to_date(business_sign_time) < '2024-10-01' 
			and performance_province_name !='平台-B'
		)a
left join 
-- 可以取最新日期关联合同号
...
where smonth in ('202407','202408','202409')
union all
...
select
  ...
  DATEDIFF('2024-10-01','2024-07-01')as accounting_cnt,
  ...
  from 
   ( 
	 ...
	 -- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20240630'  or sdt='20240930')  
         and channel_code  in ('1','7','9')
	 --  and customer_code not in ('120459','121206')
       group by sdt,
         channel_name,
...
select
  ...
  c.sales_user_number,
	...
	c.customer_code,
	c.customer_name,
  DATEDIFF('2024-07-01','2024-04-01')as accounting_cnt,
  coalesce(max(sale_amt),0) sale_amt,
  coalesce(max(excluding_tax_sales),0) excluding_tax_sales,
  avg(receivable_amount) receivable_amount
from 
(
  select
    ...
    sum(b.excluding_tax_sales) excluding_tax_sales,
    sum(a.receivable_amount) receivable_amount,
    if(avg(receivable_amount)=0 or coalesce(max(sale_amt),0)=0,0,DATEDIFF('2024-07-01','2024-04-01')/(coalesce(max(sale_amt),0)/avg(receivable_amount))) as turnover_days
  from ( 
  	 select
         sdt,
         channel_name,
...
-- 中台核销 输出表：csx_dws.csx_dws_sss_customer_credit_invoice_bill_settle_stat_di
 select
      ...
      --  csx_analyse.csx_analyse_fr_sap_subject_customer_credit_account_analyse_df
		  -- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20240331'  or sdt='20240930')  
