...
from
	...
	where
		sdt in ('20241115','20241215','20250115')
...
from
	...
	where
	-- 次月19日 因年底提前，原是20号
		sdt in ('20241119','20241219','20250119')
	) a 
	join
		(
		select distinct 
...
-- 注意日期每个月最后一天
where sdt in ('20241031','20241231','20241130')
...
from (select
	...
	from(
     select 
            ...
            where a.sdt >='20240701'  and a.sdt <= '20240930'
...
left join 
    (select 
        customer_code,
        max(sdt)max_sdt,
        datediff('2024-12-31',from_unixtime(unix_timestamp(max(sdt),'yyyyMMdd'),'yyyy-MM-dd')) as diff_days,
