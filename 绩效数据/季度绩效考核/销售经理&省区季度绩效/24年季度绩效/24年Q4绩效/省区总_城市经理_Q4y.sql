...
from (
      ...
      business_type_code,
		...
		where 
			...
			and to_date(business_sign_time) >= '2024-10-01'  and to_date(business_sign_time) <= '2024-12-31' 
...
where smonth in ('202501','202502','202503')
union all
...
(
select * from    csx_analyse.csx_analyse_sale_d_customer_sign_new_about_di 
where smonth in ('202410','202411','202412')
union all
select * from    csx_analyse.csx_analyse_sale_d_customer_new_about_di
where smonth in  ('202410','202411','202412')
 )a
  ...
  from 
   ( 
	 ...
	 -- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20240930'  or sdt='20241231')  
         and channel_code  in ('1','7','9')
	 --  and customer_code not in ('120459','121206')
       group by sdt,
         channel_name,
...
select
  ...
  c.sales_user_number,
	...
	c.customer_code,
	c.customer_name,
  DATEDIFF('2024-12-31','2024-09-39')as accounting_cnt,
  coalesce(max(sale_amt),0) sale_amt,
  coalesce(max(excluding_tax_sales),0) excluding_tax_sales,
  avg(receivable_amount) receivable_amount
from 
