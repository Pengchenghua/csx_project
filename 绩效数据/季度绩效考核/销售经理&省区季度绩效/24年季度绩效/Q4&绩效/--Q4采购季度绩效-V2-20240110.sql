...
with aa as (
  select
    ...
    -- a.inventory_dc_name,
    if(
      a.classify_large_code in ('B01', 'B02', 'B03'),
      '生鲜',
      '食百'
    ) as dept_name,
    case
      when a.classify_large_code in ('B04', 'B05', 'B06', 'B07', 'B08', 'B09') THEN '食百'
  ...
  from
    ...
    left join (
      ...
      from
        csx_analyse.csx_analyse_fr_oms_complaint_detail_di a
      where
        sdt >= '20240101'
        and sdt <= '20240331'
        ...
        and first_level_department_name = '采购' --	    and second_level_department_name not like '%地采%'
      group by
        concat(
          first_level_department_name,
          '-',
...
select
  ...
  --  a.classify_middle_name,
  kesu,
  sum(sku) over(partition by performance_province_name) as all_sku,
  --更改为省区整体SKU
...
select
  ...
  kesu,
  sum(sku) over(
    partition by performance_province_name,
    performance_city_name
  ) as all_sku,
  --更改为省区整体SKU
  kesu / sum(sku) over(
