...
from
	...
	where
		sdt in ('20250215','20250315','20250415')
...
from
	...
	where
	-- 次月19日 因年底提前，原是20号
		sdt in ('20250220','20250320','20250420')
	) a 
	join
		(
		select distinct 
...
select
	...
	sum(a.invoice_amount),-- 开票金额
	sum(a.sale_amt),-- 财务含税销售额

	sum(a.invoice_amount)/sum(a.sale_amt) kp_ratio -- 开票率	
from
    tmp_total a 
group by province_name,
	rp_leader_user_number,
...
-- 注意日期每个月最后一天
where sdt in ('20250131','20250228','20250331')
...
from csx_analyse.csx_analyse_fr_sap_customer_credit_forecast_collection_report_df  --  承华  预测回款金额-帆软
-- 注意日期每个月最后一天
where sdt in ('20250131','20250331','20250228')
group by 	company_code,		--  公司代码
	company_name,		--  公司名称
	customer_code,		--  客户编码
	customer_name,substr(sdt,1,6)
...
from (select
	...
	from(
     select 
            ...
            where a.sdt >='20241001'  and a.sdt <= '20241231'
...
left join 
    (select 
        customer_code,
        max(sdt)max_sdt,
        datediff('2025-03-31',from_unixtime(unix_timestamp(max(sdt),'yyyyMMdd'),'yyyy-MM-dd')) as diff_days,
...
(SELECT
    ...
    coalesce(bbc_leader_city_name,'') as bbc_leader_city_name,
    typeder  
from  csx_analyse_tmp.csx_analyse_tmp_xiaoshou_ratio a
join
		(
		select distinct 
			sdt,customer_no,customer_name,region_name,province_name,city_group_name,
