...
group by customer_code,
  ...
  DATEDIFF('2025-03-31','2024-12-31')as accounting_cnt,
  ...
  if(avg(receivable_amount)=0 or coalesce(max(sale_amt),0)=0,0,DATEDIFF('2025-03-31','2024-12-31')/(coalesce(max(sale_amt),0)/avg(receivable_amount))) as turnover_days
from 
(
  select
    a.sdt,
  ...
  (
    select
      ...
      if(
        ...
        0,
        DATEDIFF('2025-03-31','2024-12-31') / (coalesce(max(sale_amt), 0) / avg(receivable_amount))
      ) as turnover_days
    from
      (
        select
