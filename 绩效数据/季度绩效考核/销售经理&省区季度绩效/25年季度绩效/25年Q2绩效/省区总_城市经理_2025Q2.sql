...
--B端销售汇总
select a.province_name sales_province_name,
  a.city_group_name,
...
from (
    ...
    where sdt >= '20250401'
      and sdt <= '20250630' -- and customer_code not in ('120459','121206')
      and (
  ...
  LEFT join (
    ...
    where sdt = '20250630' --  and channel_code  in ('1','7','9')
...
(select customer_code,
        ...
        sale_amt ,
        profit,
        row_number()over(partition by customer_code,business_type_name order by sale_amt desc ) sale_rn
...
(select distinct
    ...
    first_sale_data
from
    csx_analyse.csx_analyse_sales_new_customer_info_mf a
where
  smt >= '202504'
  and smt<='202506'
  and business_type_name='日配业务'
  )a 
  left join (select * from tmp_sale_detail where sale_rn=1)  b on a.customer_code=b.customer_code and a.business_type_name=b.business_type_name 
  )
  
--   select * from tmp_rp_new_customer_sale where customer_code='163887'
  
  ,
 tmp_fl_new_customer_sale as (
select
    performance_region_name,
    performance_province_name,
    performance_city_name,
    ...
    sum(b.profit) profit,
    min(business_sign_date) business_sign_date,
    break_sale_sdt,
    is_break_3month,
    new_flag,
    concat_ws('、',collect_set(first_sale_data)) first_sale_data
from
(select s_month,
    performance_region_name,
    performance_province_name,
...
where
  ...
  left join (select * from tmp_sale_detail )  b on a.customer_code=b.customer_code and a.business_type_name=b.business_type_name and a.s_month=b.s_month
...
(select
    ...
    business_sign_date,
    first_sale_data
from tmp_rp_new_customer_sale a 
union all 
select
    performance_region_name,
    performance_province_name,
    ...
    business_sign_date,
    first_sale_data
from tmp_fl_new_customer_sale a
)a 
left join 
(
            select *
...
from
    csx_analyse.csx_analyse_sales_new_customer_info_mf
where
  smt >= '202504'
  and smt<='202506'
  )a 
...
--   
    ...
    -- sales_user_number,
    -- sales_user_name,
    count(distinct customer_code) new_cust_cnt,
    sum(sale_amt) sale_amt ,
    sum(profit) profit
    from tmp_new_cust_detail 
    where rn=1 
    group by 
    performance_region_name,
    performance_province_name,
  ...
  from 
   ( 
	 ...
	 -- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20250331'  or sdt='20250630')  
         and channel_code  !=2
...
-- 取SAP应收表 输出表：csx_analyse.csx_analyse_fr_sap_subject_customer_credit_account_analyse_df
  select
      ...
      --  csx_analyse.csx_analyse_fr_sap_subject_customer_credit_account_analyse_df
		  -- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20240630'  or sdt='20250630')  
