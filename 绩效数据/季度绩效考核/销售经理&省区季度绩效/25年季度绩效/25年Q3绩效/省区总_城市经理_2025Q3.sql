...
from csx_analyse_tmp.csx_analyse_tmp_sales_sale_detail  a
   
group by a.sales_province_name,
  a.city_group_name,
    a.fourth_supervisor_work_no,
  a.fourth_supervisor_name,
  a.third_supervisor_work_no,
...
from   (
    ...
    where sdt >= '20250701'
      and sdt <= '20250930' -- and customer_code not in ('120459','121206')
      and (
...
LEFT join
        ...
        --   and channel_code  in ('1','7','9')
          ) c on a.customer_code=c.customer_code and a.smonth=c.s_month
group by 
      a.province_name,
      a.city_group_name,
...
select 
    *
    ,
    if(rn=1,'是','否') as is_max_sale_month
    from tmp_new_cust_detail 
...
select
  ...
  DATEDIFF('2025-09-30','2025-06-30')as accounting_cnt,
  ...
  if(avg(receivable_amount)=0 or coalesce(max(sale_amt),0)=0,0,DATEDIFF('2025-09-30','2025-06-30')/(coalesce(max(sale_amt),0)/avg(receivable_amount))) as turnover_days
from 
(
  select
    a.sdt,
  ...
  from 
   ( 
	 ...
	 -- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20250630'  or sdt='20250930')  
         and channel_code  !=2
  ...
  from ( 
       ...
       from 
        ...
        -- 取SAP 
          csx_dws.csx_dws_sap_customer_credit_settle_detail_di 
	 -- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20250930'  or sdt='20250630')  
         and channel_code !=2
	 --  and customer_code not in ('120459','121206')
       group by sdt,
         channel_name,
...
-- 取SAP应收表 输出表：csx_analyse.csx_analyse_fr_sap_subject_customer_credit_account_analyse_df
  select
      ...
      --  csx_analyse.csx_analyse_fr_sap_subject_customer_credit_account_analyse_df
		  -- csx_dws_sss_customer_invoice_bill_settle_stat_di
       where (sdt='20240630'  or sdt='20250930')  
...
from (select
	...
	from(
     select 
            ...
            where a.sdt >='20250401'  and a.sdt <= '20250630'
...
from (
      ...
      business_type_code,
		...
		where 
			...
			and to_date(business_sign_time) >= '2025-07-01'  and to_date(business_sign_time) <= '2025-09-30' 
...
where smonth in ('202507','202508','202509')
union all
