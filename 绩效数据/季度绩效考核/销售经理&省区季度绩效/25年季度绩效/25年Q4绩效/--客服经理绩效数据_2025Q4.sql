...
(select
    ...
    sum(partner_excluding_tax_profit) partner_excluding_tax_profit
from  csx_analyse_tmp.csx_analyse_tmp_quarterly_service_sale_detail a
group by 
      a.province_name,
      a.city_group_name,
	rp_leader_user_number,
 ...
 ------------------------------- 客服经理、高级客服经理 销售明细
--  销售明细
drop table  csx_analyse_tmp.csx_analyse_tmp_quarterly_service_sale_detail;
create table csx_analyse_tmp.csx_analyse_tmp_quarterly_service_sale_detail as
select
  performance_region_name,
  a.province_name,
  a.city_group_name,
  ...
  sum(
    if(partner_type_code in (1, 3), excluding_tax_sales, 0)
  ) partner_excluding_tax_sales,
  sum(
    if(partner_type_code in (1, 3), excluding_tax_profit, 0)
...
from
  ...
  join (
    select
      ...
      bbc_leader_city_name
    from
      csx_analyse_tmp.customer_sale_service_manager_leader01
    where
      sdt in ('20251231')
...
from  csx_analyse_tmp.csx_analyse_tmp_quarterly_service_receivable_detail
...
select * from  csx_analyse_tmp.csx_analyse_tmp_quarterly_service_receivable_detail

-- -- - 回款率明细
drop table csx_analyse_tmp.csx_analyse_tmp_quarterly_service_receivable_detail;
create table csx_analyse_tmp.csx_analyse_tmp_quarterly_service_receivable_detail as 
select
    a.smonth,
	b.region_name,
...
-- 注意日期每个月最后一天
where sdt in ('20251031','20251130','20251231')
	and channel_name not in ('项目供应商','前置仓','商超') 
...
with   tmp_customer_level as (select
  ...
  last_order_date,			-- 最后一次下单时间 需要注意这个是全业务的，下季度要调整20251031
  work_no,
  sales_name,
  sales_value,
  profit,
...
from
  csx_analyse_tmp.csx_analyse_tmp_service_terminate_info a
group by
   region_name,
  province_name,
  rp_leader_user_number,
