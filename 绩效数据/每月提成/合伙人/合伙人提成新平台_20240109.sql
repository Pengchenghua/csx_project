drop table csx_analyse_tmp.csx_analyse_tmp_cq_xm_sale;
create table csx_analyse_tmp.csx_analyse_tmp_cq_xm_sale as 
...
select a.smonth,
	...
	a.business_type_name,
	a.sales_user_number,
	a.sales_user_name,  -- b.dev_source_name
	sale_amt,
	profit,
	prorate,
	salery_cal,
	...
	coalesce(payment_amount_by,0)*0.0006 as salery_cal_1
...
( 
  ...
  or (sdt='19990101' and regexp_replace(substr(posting_time,1,10),'-','')>='20200601'
   and regexp_replace(substr(posting_time,1,10),'-','')<=regexp_replace(last_day(add_months(date_sub(current_date,1),-1)),'-',''))
  group by customer_code
) e on a.customer_code = e.customer_code   
;

select * from csx_analyse_tmp.csx_analyse_tmp_cq_xm_sale



CREATE EXTERNAL TABLE `csx_analyse_tmp`.`csx_analyse_tmp_cq_xm_sale`(
	id bigint not null auto_increment,
  `smonth` varchar(64), 
  `performance_province_name` varchar(64), 
  `customer_code` varchar(64), 
  `customer_name` varchar(64), 
  `business_type_name` varchar(64), 
  `sales_user_number` varchar(64), 
  `sales_user_name` varchar(64), 
  `sale_amt` decimal(35,6), 
  `profit` decimal(35,6), 
  `prorate` decimal(35,6), 
  `salery_cal` decimal(35,6), 
  `unpaid_amt_by` decimal(35,6), 
  `unpaid_amt_sy` decimal(35,6), 
  `payment_amount_by` decimal(35,6), 
  `payment_amount_sy` decimal(35,6), 
  `paid_amt` decimal(35,6), 
  `residue_amt` decimal(35,6), 
  `residue_amt_all` decimal(35,6), 
  `salery_cal_1` decimal(35,6)
 PRIMARY key (id),
 key(smonth,performance_province_name)
)COMMENT='重庆合伙人绩效明细'
ROW FORMAT SERDE 
;
