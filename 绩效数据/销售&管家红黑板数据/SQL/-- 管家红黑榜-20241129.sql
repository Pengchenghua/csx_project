...
select * from csx_analyse_tmp.csx_analyse_tmp_hr_service_sale
-- 管家销售额毛利额
...
with tmp_sale_detail as 
    ...
    (select  sale_month,
        ...
        service_user_name,
        service_manager_user_position,
        case when coalesce(b.service_manager_user_position,'') !='' and  then b.service_user_work_no
            ...
            -- when a.sales_user_position='CUSTOMER_SERVICE_MANAGER' and    then a.sales_user_number
            else '' end new_service_user_work_no,
        case when coalesce(b.service_manager_user_position,'') !='' then b.service_user_name
            when a.sales_user_position='CUSTOMER_SERVICE_MANAGER' and  coalesce(b.service_manager_user_position,'') =''  then a.sales_user_name
            else '' end new_service_user_name,
        case when coalesce(b.service_manager_user_position,'') !='' then b.service_manager_user_position
            when a.sales_user_position='CUSTOMER_SERVICE_MANAGER' and coalesce(b.service_manager_user_position,'') ='' then a.sales_user_position
    ...
    left join 
    csx_analyse_tmp.csx_analyse_tmp_hr_service_info  b on a.customer_code=b.customer_no and a.new_business_type_code=b.business_type_code
    )
    select a.*,
    ...
    (select 
        ...
        count(distinct new_service_user_work_no) cnt
...
-- 逾期系数 取中台逾期
 select * from  csx_analyse_tmp.csx_analyse_tmp_hr_service_over a
 -- drop table csx_analyse_tmp.csx_analyse_tmp_hr_service_over;
...
from 
   -- csx_dws.csx_dws_sss_customer_credit_invoice_bill_settle_stat_di
    ...
    and ( channel_name in ('大客户','业务代理') 
     ...
     and sales_employee_code not in ('81208614','81206921')
    and a.customer_code not in ('234036','224656','247525','243799','244172','237768')
...
select  sale_month,
    ...
    case when coalesce(b.service_manager_user_position,'') !='' then b.service_user_work_no
            when c.user_position='CUSTOMER_SERVICE_MANAGER' and  coalesce(b.service_manager_user_position,'') =''  then a.sales_employee_code 
            else '' end new_service_user_work_no,
        case when coalesce(b.service_manager_user_position,'') !='' then b.service_user_name
            when c.user_position='CUSTOMER_SERVICE_MANAGER' and  coalesce(b.service_manager_user_position,'') =''  then a.sales_employee_name
            else '' end new_service_user_name,
        case when coalesce(b.service_manager_user_position,'') !='' then b.service_manager_user_position
            when c.user_position='CUSTOMER_SERVICE_MANAGER' and coalesce(b.service_manager_user_position,'') ='' then c.user_position
            else '' end new_service_manager_user_position,
    overdue_amount,
    receivable_amount
from over_rate a 
left join 
 csx_analyse_tmp.csx_analyse_tmp_hr_service_info b on a.customer_code=b.customer_no and a.credit_business_attribute_code=b.attribute_code
left join 
 csx_analyse_tmp.csx_analyse_tmp_hr_sale_info c on a.sales_employee_code=c.user_number
;


select * from  csx_analyse_tmp.csx_analyse_tmp_hr_service_over;
...
with tmp_study_fm_user_form_data as (
  ...
  WHERE
    b.is_delete = 0
    and a.create_time<'2025-01-13 00:00:00'   -- 次月需要限制创建日期
...
(  
    ...
    from     csx_analyse_tmp.csx_analyse_tmp_hr_service_info  a 
    join 
    (select customer_code
...
from
  ...
  left join 
   (  
    ...
    (select customer_code
    from  csx_dws.csx_dws_sale_detail_di 
        where sdt>='20240901' and sdt<='20241231'
...
tmp_study_fm_user_form_data as (
  ...
  WHERE
    b.is_delete = 0
    and a.create_time<'2025-01-13 00:00:00'
    and a.create_time>='2025-01-01 00:00:00'
   -- AND a.form_key = 'Gznp3ieG'
),
tmp_study_fm_user_form_data_clean as (
...
(  
    ...
    (select customer_code
    from  csx_dws.csx_dws_sale_detail_di 
        where sdt>='20240601' and sdt<='20241231'
    group by customer_code ) b on a.customer_code=b.customer_code
    where sdt = '20241231'  
        and business_attribute_code in ('1','2','5')
...
(  
    ...
    (select customer_code
    from  csx_dws.csx_dws_sale_detail_di 
        where sdt>='20241101' and sdt<='20241130'
    group by customer_code ) b on a.customer_code=b.customer_code
    where sdt = '20241130'  
        and business_attribute_code in ('1','2','5')
        and service_manager_user_number !=''
...
tmp_study_fm_user_form_data as (
  ...
  WHERE
    b.is_delete = 0
    and a.create_time<'2024-11-22 00:00:00'   -- 限制日期
    and a.create_time >='2024-11-01 00:00:00'
    -- AND a.form_key = 'Gznp3ieG'
),
tmp_study_fm_user_form_data_clean as (
  select
