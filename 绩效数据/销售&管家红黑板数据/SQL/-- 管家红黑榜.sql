...
with sales_info as (
  select distinct substr(sdt, 1, 6) as smt,
...
with tmp_sale_detail as 
    ...
    left join 
    csx_analyse_tmp.csx_analyse_tmp_hr_service_info  b on a.customer_code=b.customer_no and a.business_type_code=b.business_type_code
    left join 
    (select distinct
        customer_no,
        business_type_code,
        cnt 
    from csx_analyse_tmp.csx_analyse_tmp_hr_service_info ) c on a.customer_code=c.customer_no and a.business_type_code=c.business_type_code
      where 
...
with tmp_study_fm_user_form_data as (
  ...
  WHERE
    b.is_delete = 0
    and a.create_time<'2024-11-22 00:00:00'   -- 次月需要限制创建日期
...
(  
    ...
    from  csx_dws.csx_dws_sale_detail_di 
        where sdt>='20241001' and sdt<='20241031'
...
where smt='202410' 
    and sale_month='202410'
    union all 
select  '202410'sale_month,
        performance_region_name,
        service_user_work_no,
        0 plan_sales_amt,
        0 plan_profit,
...
from
  ...
  left join 
   (  
    ...
    (select customer_code
    from  csx_dws.csx_dws_sale_detail_di 
        where sdt>='20240801' and sdt<='20241031'
...
tmp_study_fm_user_form_data as (
  ...
  WHERE
    b.is_delete = 0
    and a.create_time<'2024-11-22 00:00:00'
    and a.create_time>='2024-11-01 00:00:00'
   -- AND a.form_key = 'Gznp3ieG'
),
tmp_study_fm_user_form_data_clean as (
...
(  
    ...
    (select customer_code
    from  csx_dws.csx_dws_sale_detail_di 
        where sdt>='20240601' and sdt<='20241031'
    group by customer_code ) b on a.customer_code=b.customer_code
    where sdt = '20241031'  
        and business_attribute_code in ('1','2','5')
