...
-- 记得保证金要先处理
-- csx_analyse.csx_analyse_fr_hr_sales_red_black_sale_detail 销售经理&销售员销售明细表
...
with 
    ...
    from sale a 
    left join 
    csx_analyse_tmp.csx_analyse_tmp_hr_sale_info b on a.new_sales_user_number=b.user_number
 ;
...
-- 逾期系数 取SAP逾期
select * from  csx_analyse_tmp.csx_analyse_tmp_hr_sales_over a
...
select * from  csx_analyse_tmp.csx_analyse_tmp_business_01 
where 
-- 日配断约满足3个月，以90天计算未来最近日期
(after_date<'20241201' and business_type_code=1) 
    or business_type_code !=1 
    or after_date is null 
 ...
 ( select business_number,
  ...
  from     csx_dim.csx_dim_crm_business_info a 
    where sdt='current'
     ...
     and substr(create_time, 1, 10) >=  to_date(trunc(add_months ('${i_sdate}',-5),'MM'))  -- 近半年区间
...
(		select 
		...
		where 
			sdt between '20220101' and regexp_replace('${i_sdate}','-','')
...
from  csx_analyse_tmp.csx_analyse_tmp_business_01
where rn=1
   and ((after_date<'20241201' and business_type_code=1) or business_type_code!=1 or after_date is null )
group by 
    performance_region_name,
    owner_user_number	
)a 
...
(		select 
		...
		from 
			csx_dws.csx_dws_sale_detail_di 
		where 
			sdt between '20220101' and '20241130'
	   ...
	   where 1=1
	    and after_date>='20241101'  -- 大于当月的正在履约
			group by performance_province_name, 
			business_type_code,
			after_date, 
			a.customer_code,
...
( select customer_code,customer_name
	...
	where after_date<'20241101' or after_date is null    
;