...
(select    performance_province_name,
	...
	sale_cost_no_tax,
	profit_rate_no_tax,
	if((sign_company_code='2121' and profit_rate_no_tax<0.0795) or profit_rate_no_tax<0.0695,0,sale_amt_no_tax) as    result_sale_amt_no_tax, 
	if((sign_company_code='2121' and profit_rate_no_tax<0.0795) or profit_rate_no_tax<0.0695,0,profit_no_tax) as      result_profit_no_tax, 
	if((sign_company_code='2121' and profit_rate_no_tax<0.0795) or profit_rate_no_tax<0.0695,0,profit_rate_no_tax) as result_profit_rate
from 
(select
	performance_province_name,customer_code,sign_company_code,
	...
	sum(profit_no_tax)/sum(sale_amt_no_tax) profit_rate_no_tax
from
	csx_dws.csx_dws_sale_detail_di a 
	join 
	(select shop_code,performance_city_code,performance_city_name,basic_performance_city_name,city_name,town_name      
	from    csx_dim.csx_dim_shop where sdt='current') b on a.inventory_dc_code=b.shop_code
where
	sdt>=regexp_replace(add_months(trunc('${yesterdate}','MM'),-1),'-','') 
	and sdt<=regexp_replace(last_day(add_months('${yesterdate}',-1)),'-','') 
...
select substr(sdt,1,6) s_month,
	...
	b.sales_person_cityname performance_city_name,
  ...
  b.profit_no_tax,  
	b.sale_cost_no_tax,
	b.profit_rate_no_tax,
	b.result_sale_amt_no_tax, 
  b.result_profit_no_tax, 
  b.result_profit_rate,
	...
	a.overdue_rate  `逾期率`,
    overdue_amount_15_day 	`逾期1-15天金额`,
    overdue_amount_30_day 	`逾期15-30天金额`,
    overdue_amount_60_day 	`逾期30-60天金额`,
    overdue_amount_90_day 	`逾期60-90天金额`,
    overdue_amount_120_day 	`逾期90-120天金额`,
    overdue_amount_180_day 	`逾期120-180天金额`,
    overdue_amount_1_year 	`逾期180天-1年金额`,
    overdue_amount_2_year 	`逾期1年-2年金额`,
    overdue_amount_3_year 	`逾期2年-3年金额`,
    overdue_amount_more_3_year 	`逾期3年以上金额`,
    max_overdue_day     `最大逾期天数`,
...
from
	...
	-- 	csx_dws.csx_dws_sss_customer_invoice_bill_settle_stat_di
	      desc  csx_dws.csx_dws_sss_customer_credit_invoice_bill_settle_stat_di
	where 
		sdt =regexp_replace(last_day(add_months('${yesterdate}',-1)),'-','')  
...
from
	...
	from 
		csx_dws.csx_dws_sss_customer_invoice_bill_settle_stat_di
	where 
		sdt = '20240131' --and channel_name = '大客户' 
	)a
	join		 
		(
		select 
...
from
	...
	from    csx_dim.csx_dim_shop 
		where sdt='current') b on a.inventory_dc_code=b.shop_code
where
	sdt>='20231001' 
	and sdt<='20240131'
	and channel_code in ('1', '7', '9')
