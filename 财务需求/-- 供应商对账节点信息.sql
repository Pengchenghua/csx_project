...
create temporary table csx_analyse_tmp.csx_analyse_tmp_temp_pss_01 as 
...
drop table if exists csx_analyse_tmp.temp_pss_00 ;
create temporary table csx_analyse_tmp.temp_pss_00 as 
...
(select 
 ...
 group by  to_date(receive_time) 	,
    ...
    original_order_code,
    to_date(sign_date )
    ) as c on a.source_bill_no=c.original_order_code
left join  
--  采购订单表头
...
from csx_dwd.csx_dwd_scm_order_header_di
    where   sdt >= '${ysdate}'
group by  order_code,to_date(create_time),supplier_code,received_order_code) b on c.original_order_code=b.order_code   
;
...
--  insert overwrite table csx_analyse_tmp.ads_fr_r_d_po_reconciliation_report partition(sdt)
drop table if exists csx_analyse_tmp.temp_pss_03;
create temporary table csx_analyse_tmp.temp_pss_03 as 
select purchase_no,
    entry_order_no,
    batch_co,          -- 批次单号
    company_code,
   ...
   current_timestamp() as update_time,
    case when check_ticket_status_code=0 then '发票未录入' 
        when check_ticket_status_code=1 then '发票已录入未审核'
        when check_ticket_status_code=2 then '票核通过'
        when check_ticket_status_code=3 then '票核拒绝'
        when check_ticket_status_code=4 then '票核取消' 
    end check_ticket_status,    -- 票核状态
   regexp_replace(business_date,'-','')  as sdt
from csx_analyse_tmp.temp_pss_02
    where 1=1
    
;

-- 插入数据
insert overwrite table csx_analyse.csx_analyse_fr_po_reconciliation_di partition(sdt)
select *  from csx_analyse_tmp.temp_pss_03
distribute by sdt
;