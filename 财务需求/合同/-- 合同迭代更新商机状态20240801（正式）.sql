...
(select distinct l1.contract_code,
    ...
    (
        case
            ...
            and substr(l4.NODENAME, 1, 2) not in ('归档', '10') then '归档后退回'
            else l4.NODENAME
        end
    ) as last_node_name,
    -- 合同最新节点名称
...
from 
    ...
    from 
        (select 
            ...
            (case when contract_must=0 and d.contract_code is null  then '不签' 
                  when (c.if_effective='是' and c.if_htzz='否') or (substr(d.last_node_name,1,2) not in ('10','归档'))  then '已签' 
                  when contract_type=2 and (c.if_effective='否' or c.if_htzz='是') and c.is_gd='是' then '失效'  
                --  when contract_must=1 and contract_type=1 and substr(last_node_name,1,2) not in ('10','归档')  then '流程中' 
                  when contract_type=1 and d.contract_code is not null  then concat('流程中-',d.last_node_name) 
...
(select 
 ...
 from    csx_analyse_tmp_customer_contract l5  
 left join csx_analyse_tmp_customer_business m1 on l5.customer_code=m1.customer_code and l5.contract_code=m1.contract_number
 where l5.if_htzz='否' 
    and l5.if_effective in ('是','') 
    and l5.is_gd='是'
    and m1.contract_number is null
 group by l5.customer_code 
) l 
on a.customer_code=l.customer_code 
left join 
csx_analyse_tmp_customer_business m 
