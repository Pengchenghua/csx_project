...
-- /*业务盘点*/
-- 盘点未过账
drop table if exists csx_analyse_tmp.csx_analyse_tmp_pandian_post_no_2022; 
create table csx_analyse_tmp.csx_analyse_tmp_pandian_post_no_2022
as
select
	a.credential_no,
	b.province_name,   --省区
	...
	(case when a.direction_flag='+' then -1*a.qty else a.qty end) as qty,   --盘点数量
	(case when a.direction_flag='+' then -1*a.amt_no_tax else a.amt_no_tax end) as amt_no_tax,   --盘点不含税金额
	(case when a.direction_flag='+' then -1*a.amt else a.amt end) as amt   --盘点含税金额
from
	(
	select 
		* 
	...
	where 
		-- sdt='19990101' 
		sdt between '20220101' and '20220630'
		-- and to_date(credential_item_posting_time) >= add_months(trunc(date_sub(current_date,0),'MM'),-1)
		-- and to_date(credential_item_posting_time) < trunc(date_sub(current_date,0),'MM')
		and substr(move_type_code,1,3) in ('110','111') 
...
select * from csx_analyse_tmp.csx_analyse_tmp_pandian_post_no_2022;

-- /*业务盘点*/
-- 盘点未过账
drop table if exists csx_analyse_tmp.csx_analyse_tmp_pandian_post_no_2022_02; 
create table csx_analyse_tmp.csx_analyse_tmp_pandian_post_no_2022_02
as
select
	a.credential_no,
	b.province_name,   --省区
...
from
	...
	where 
		-- sdt='19990101' 
		sdt between '20220701' and '20221231'
		-- and to_date(credential_item_posting_time) >= add_months(trunc(date_sub(current_date,0),'MM'),-1)
		-- and to_date(credential_item_posting_time) < trunc(date_sub(current_date,0),'MM')
		and substr(move_type_code,1,3) in ('110','111') 
		and substr(reservoir_area_code,1,2)='PD' 
...
select * from csx_analyse_tmp.csx_analyse_tmp_pandian_post_no_2022_02;
	
-- /*财务盘点*/	
-- 盘点过账	+省区字段
drop table if exists csx_analyse_tmp.csx_analyse_tmp_pandian_post_yes_2022; 
create table csx_analyse_tmp.csx_analyse_tmp_pandian_post_yes_2022
as
select
	a.credential_no,
	b.province_name,   --省区
...
from
	...
	from 
		csx_dwd.csx_dwd_cas_credential_item_di
	where 
		sdt between '20220101' and '20221231'
		and substr(move_type_code,1,3) in ('115','116') 
		and amt_no_tax<>0
		and substr(company_code,1,1)<>'C' -- 20221101新加 公司代码 限制不为城市服务商
	) a
	left join 
...
select * from csx_analyse_tmp.csx_analyse_tmp_pandian_post_yes_2022
...
from 
	csx_analyse_tmp.csx_analyse_tmp_pandian_post_no_2022
where 
...
select 
	'业务盘点' pandian_group,*,amt amount
from 
	csx_analyse_tmp.csx_analyse_tmp_pandian_post_no_2022_02
where 
	reservoir_area_code='PD01'
	and division_name in('食百','易耗品')
union all
...
from 
	csx_analyse_tmp.csx_analyse_tmp_pandian_post_yes_2022
; 


--报损
drop table if exists csx_analyse_tmp.csx_analyse_tmp_baosun_2022; 
create table csx_analyse_tmp.csx_analyse_tmp_baosun_2022
as
 select
	e.province_name,   --省区
	e.city_name,   --城市 
...
from
	...
	left join 
		...
		from 
			...
			csx_ods.csx_ods_csx_b2b_accounting_accounting_credential_header_di
		where 
			sdt='19990101'
	...
	left join 
		...
		from
			(
			select 
				distinct order_code,frmloss_type_code,frmloss_type_name,cost_center_code 
			from 
				-- csx_ods.source_wms_r_d_frmloss_order_header_v1 
				csx_ods.csx_ods_csx_b2b_wms_wms_frmloss_order_header_df
			where 
				sdt=regexp_replace(last_day(add_months(current_date,-1)),'-','')
			)t
			left join
...
;
select * from csx_analyse_tmp.csx_analyse_tmp_baosun_2022
...
--	where sdt='19990101'
--	and to_date(credential_item_posting_time) >= ${hiveconf:i_sdate_12}
--	and to_date(credential_item_posting_time) < ${hiveconf:i_sdate_11}
--	and substr(move_type_code,1,3) in ('117')
--	and credential_no='PZ20200615001449'
--	and wms_order_no='BS200615000010'
--	and goods_code='1163327';
...
----------------------------------------------------------------------------------------------------------------------------------------------------------	
-- 查数
-- select credential_no,goods_code,qty,amt_no_tax,
-- case when substr(move_type_code,1,3) in ('110','111') and substr(reservoir_area_code,1,2)='PD' then '未过账'
--  when substr(move_type_code,1,3) in ('115','116') and amt_no_tax<>'0' then '已过账'
--  end leibie,
-- case when substr(move_type_code,1,3) in ('110','111') and substr(reservoir_area_code,1,2)='PD' then '未过账'
--  --when substr(move_type_code,1,3) in ('115','116') and amt_no_tax<>'0' then '已过账'
--  when substr(move_type_code,1,3) in ('115','116')  then '已过账'  
--  end leibie1,* 
-- from csx_ods.source_cas_r_d_accounting_credential_item 
-- where sdt='19990101' 
-- and to_date(credential_item_posting_time) >= '2021-05-01'
-- and to_date(credential_item_posting_time) <'2021-06-01'
...
select
	...
	a.move_type_code,   --移动类型编码
	a.move_type_name,   --移动类型名称
	sum(case when a.direction_flag='-' then -1*a.amt_no_tax else a.amt_no_tax end) as amt_no_tax,   --盘点不含税金额
	sum(case when a.direction_flag='-' then -1*a.amt else a.amt end) as amt   --盘点含税金额
from 
	csx_dwd.csx_dwd_cas_credential_item_di a 
where 
	sdt between '20220101' and '20221231'
	and substr(move_type_code,1,3) in ('115','116') 
	and amt_no_tax<>0
	and substr(company_code,1,1)<>'C' -- 20221101新加 公司代码 限制不为城市服务商
	and credential_no in ('PZ20221001001585','PZ20221001001749','PZ20221001001375','PZ20221001001393','PZ20221001001353')
group by 
	credential_no,
	if(substr(a.purchase_group_code,1,1) in('U','H'),'生鲜',if(substr(a.purchase_group_code,1,1)='A','食百','易耗品')),   --部类	
	-- a.company_code,   --公司代码
	-- a.company_name,   --公司名称
	-- a.location_code,   --DC编码
	-- a.location_name,   --DC名称
